...
Structure _s_WIDGET Extends _s_COORDINATE
   ...
   *areabar._s_WIDGET ; Виджет-контейнер для скроллинга контента
   ...
   List *__Items._s_ROWS( )          ; Развернутый рулон (указатели)
                                     ; List __rows._s_rows( )
   List __tabs._s_TABS()              ; Заголовки вкладок
...
Macro Clip(_widget_) : ClipOutput(_widget_\clip_x, _widget_\clip_y, _widget_\clip_w, _widget_\clip_h) 
  ; Debug  " "+_widget_\class +" "+ _widget_\clip_x +" "+ _widget_\clip_y +" "+ _widget_\clip_w +" "+ _widget_\clip_h
: EndMacro
...
Procedure update_visible_rows(*this._s_WIDGET)
   ...
   Protected view_bottom = *this\scroll_v\pos + *this\height - *this\columnheight
   ...
   *this\visible\last = 0
   ...
   ForEach *this\__rows( )
      ...
      Protected row_bottom = row_top + *this\__rows( )\height 
...
Procedure update_columns(*this._s_WIDGET)
   ...
   *this\scroll_h\max = cur_x - *this\Width
...
Procedure update_rows(*this._s_WIDGET)
   ...
   Protected view_top = *this\scroll_v\pos
   Protected view_bottom = view_top + *this\height - *this\columnheight
   ...
   *this\scroll_v\max = cur_y - *this\height 
...
Procedure swap_column(*this._s_WIDGET, *HoverColumn._s_columns, mx.i)
   ...
   If *this\root\DragColumn And *HoverColumn And *HoverColumn <> *this\root\DragColumn
      ...
      Protected col_middle_x = col_left_x + (*HoverColumn\width / 2)
...
Procedure swap_row(*this._s_WIDGET, *HoverRow._s_rows, my.i)
   ...
   If *this\root\DragItem And *HoverRow And *HoverRow <> *this\root\DragItem
      ...
      Protected row_middle_y = row_top_y + (*HoverRow\height / 2)
...
Procedure draw_columns(*this._s_WIDGET)
   ...
   Protected dy = *this\y
   Protected color
   Protected *column._s_COLUMNS
   
   ; 1. Рисуем общий фон шапки высота columnheight
   Box(*this\x, dy, *this\Width, *this\columnheight, $F5F5F5)
   ...
   ForEach *this\__columns( )
      *column = *this\__columns( )
      ...
      Box(col_x, dy, *column\Width, *this\columnheight, color)
   ...
   Line(*this\x, dy + *this\columnheight, *this\Width, 1, $AAAAAA)
EndProcedure
...
Procedure draw_rows(*this._s_WIDGET, List *rows._s_rows( ))
   ...
   Repeat 
      ...
      Box(*this\x + 1, dy, *this\width - 2, *row\height - 1, color)
      ...
      ForEach *this\__columns( )
         ...
         If col_x + col_w > *this\x And col_x < *this\x + *this\Width
            ...
            Protected clip_w = Min(col_x + col_w, *this\x + *this\Width) - clip_x
            
            ; Если ширина клипа получилась отрицательной (вне видимости) — пропускаем
            If clip_w > 0
               ClipOutput(clip_x, dy, clip_w, *row\height )
               ...
               If data_idx <= ArraySize(*row\Str( ))
                  ...
                  If *this\Type = #__type_Editor And (*row\mask & #__mask_active)
      ...
      Line(*this\x, dy + *row\height - 1, *this\Width, 1, #COLOR_LINE)
   ...
   If StartDrawing(CanvasOutput(*r\Canvas\gadget))
      ...
      If StartEnum(*r)
         ...
         Else
            ...
            DrawingMode(#PB_2DDrawing_Outlined)
            Box(*g\x, *g\y, *g\Width, *g\height , $CCCCCC) ; Цвет рамки
         EndIf
         
         UnClip( )
         
         ; Сбрасываем флаг изменения после завершения
         If *g\mask & #__mask_edit
            If *g\mask & #__mask_drag
               *g\mask &~ #__mask_edit
            EndIf
         EndIf
         
         ; Сбрасываем флаг перерисовки после завершения
         *g\mask &~ #__mask_redraw
...
Procedure.i hover_row(*this._s_WIDGET, List *rows._s_rows( ), my.i)
   ...
   If *this\rowheight
      ...
      ForEach *rows( )
         If my_rel >= *rows( )\y And my_rel < *rows( )\y + *rows( )\height 
            *res = *rows( )
            Break
         EndIf
      Next
...
Procedure.i hover_tab(*this._s_WIDGET, List *tabs._s_TABS( ), mx.l, my.l)
...
Procedure column_events(*this._s_WIDGET, *column._s_columns, event, is_edge.b = #False)
   Static *leave._s_columns
   
   Select event
      Case #PB_EventType_MouseLeave
         If *leave
            *leave\mask &~ #__mask_hover
            *this\mask | #__mask_redraw
            *leave = 0
         EndIf
         ...
         If Not *this\mask & #__mask_edit
            ...
            If *this\mask & #__mask_active
               If *this\root\DragColumn
                  If *this\mask & #__mask_drag ; Режим Resize
   ...
   Select event
      ...
      Case #PB_EventType_MouseMove
         ...
         If *this\root\DragItem
            If *this\root\DragItem\mask & #__mask_edit
...
Procedure tab_events(*this._s_WIDGET, *tab._s_TABS, event)
   Select event
      Case #PB_EventType_MouseMove
      ...
      Case #PB_Shortcut_Down
         If *current
            ...
            If NextElement(*this\__rows( ))
               ...
               *this\__rows( )\mask | #__mask_active
               ; Автоскролл вниз
               If *this\__rows( )\y + *this\__rows( )\height > *this\scroll_v\pos + *this\height 
                  *this\scroll_v\pos = (*this\__rows( )\y + *this\__rows( )\Height ) - *this\height 
...
Procedure do_events(*this._s_WIDGET, event)
   ...
   tab_events(*this, hover_tab(*this, *this\__tabs( ), mouse( )\x, mouse( )\y),  event)
...
CompilerIf #PB_Compiler_IsMainFile
  ...
  ; *g = ListIcon(10, 10, 280, 480, "Имя", 120)
  AddItem(*p, -1, "Вкладка B") 
   *t = Tree(300, 10, 280, 480)
   AddItem(*p, -1, "Вкладка C") 
   *e = Editor(590, 10, 265, 480)
   CloseList()
