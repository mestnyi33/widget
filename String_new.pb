...
DeclareModule Structures
  ...
  Structure Widget_S Extends Coordinate_S
    Type.i
    handle.i[3]    ; Adress of new list element
    index.i[3]  ; Index[0] of new list element ; inex[1]-entered ; index[2]-selected
...
;               EndIf
      ...
      With *This
        ; Draw caret
        If \Focus And \Text\Editable And ListSize(\Items()) : DrawingMode(#PB_2DDrawing_XOr)             
...
;           RoundBox(\X[0],\Y[0]+\Height[0]-\bs,\Width[0],\bs,\Radius,\Radius,\Color\Frame[Bool(\Focus)*2])
  ...
  Procedure ToInput(*This.Widget_S)
    ...
    With *This
      If \Canvas\Input
        Chr.s = Make(*This, Chr(\Canvas\Input)) ; Str(Random(500))) ; 
  ...
  Procedure.i Resize(*This.Widget_S, X.i,Y.i,Width.i,Height.i)
    ...
    If *This > 0
      With *This
        ...
        If X<>#PB_Ignore : If \p : \x[3] = X : X+\p\x+\p\bs : EndIf : If \X[1] <> X : Change_x = x-\x[1] : \X = X-\bs+\fs : \x[2] = x+\fs : \x[1] = x : \Resize | 1<<1 : EndIf : EndIf  
        If Y<>#PB_Ignore : If \p : \y[3] = Y : Y+\p\y+\p\bs+\p\TabHeight : EndIf : If \Y[1] <> Y : Change_y = y-\y[1] : \Y = Y-\bs+\fs : \y[2] = y+\fs+\TabHeight : \y[1] = y : \Resize | 1<<2 : EndIf : EndIf  
        If Width<>#PB_Ignore : If \Width[1] <> Width : Change_width = width-\width[1] : \Width = Width+\bs*2-\fs*2 : \width[2] = width-\fs*2 : \width[1] = width : \Resize | 1<<3 : EndIf : EndIf  
        If Height<>#PB_Ignore : If \Height[1] <> Height : Change_height = height-\height[1] : \Height = Height+\bs*2-\fs*2 : \height[2] = height-\fs*2-\TabHeight : \height[1] = height : \Resize | 1<<4 : EndIf : EndIf 
        
;         If X<>#PB_Ignore : If \p : \x[3] = X : X+\p\x+\p\bs : EndIf : If \X <> X : Change_x = x-\x : \X = X : \x[2] = \x+\bs : \x[1] = \x[2]-\fs : \Resize | 1<<1 : EndIf : EndIf  
;         If Y<>#PB_Ignore : If \p : \y[3] = Y : Y+\p\y+\p\bs+\p\TabHeight : EndIf : If \Y <> Y : Change_y = y-\y : \Y = Y : \y[2] = \y+\bs+\TabHeight : \y[1] = \y[2]-\fs : \Resize | 1<<2 : EndIf : EndIf  
...
;         If Height<>#PB_Ignore : If \Height <> Height : Change_height = height-\height : \Height = Height : \height[2] = \height-\bs*2-\TabHeight : \height[1] = \height[2]+\fs*2 : \Resize | 1<<4 : EndIf : EndIf 
        ...
        If \p And \x+\width>\p\clip\x+\p\clip\width-\p\bs : \clip\width = \p\clip\width-\p\bs-(\clip\x-\p\clip\x) : Else : \clip\width = \width-(\clip\x-\x) : EndIf
        If \p And \y+\height>=\p\clip\y+\p\clip\height-\p\bs : \clip\height = \p\clip\height-\p\bs-(\clip\y-\p\clip\y) : Else : \clip\height = \height-(\clip\y-\y) : EndIf
          ...
          Select EventType 
            ...
            Case #PB_EventType_LeftButtonDown
              If (*Last = *This)
                PushListPosition(*List())
                ForEach *List()
                  If *List()\Focus = *List() And *List() <> *This 
                    
                    *List()\Focus = 0
                    *Last = *List()
                    Events(*List(), #PB_EventType_LostFocus, *List()\Canvas\Gadget, 0)
                    *Last = *Widget 
                    
                    ; 
                    If Not *List()\Repaint : *List()\Repaint = 1
                      PostEvent(#PB_Event_Gadget, *List()\Canvas\Window, *List()\Canvas\Gadget, #PB_EventType_Repaint)
                    EndIf
                    Break 
                  EndIf
                Next
          ...
          Select EventType
            ...
            Case #PB_EventType_MouseLeave
              If CanvasModifiers 
                ...
                PushListPosition(*List())
                ForEach *List()
                  If *List()\Canvas\Gadget = Canvas And *List()\Focus <> *List() And *List() <> *This
                    *List()\Canvas\mouse\at = From(*List())
                    
                    If *List()\Canvas\mouse\at
                      If *Last
                        Events(*Last, #PB_EventType_MouseLeave, Canvas, 0)
                      EndIf     
...
;             ForEach *List()
;               If *List() = *List()\Focus
;                 Result | CallBack(*List(), #PB_EventType_LostFocus);, Canvas) 
;                 NextElement(*List())
;                 ;Debug *List()
;                 Result | CallBack(*List(), #PB_EventType_Focus);, Canvas) 
;                 Break
;               EndIf
;             Next
;         EndSelect
;     EndSelect
    ...
    Select EventType()
      ...
      Default
        ...
        ForEach *List()
          If Canvas = *List()\Canvas\Gadget
            Result | CallBack(*List(), EventType()) 
          SetWindowTitle(0, "SizeOf(Widget_S) - "+Str(SizeOf(Widget_S))+" "+Str(MemorySize(*List())))
    ...
    *S_8\Focus = 1
    ...
    Repeat 
      ...
      Select Event
        Case #PB_Event_Timer
;           ; 
;             SetText(*S_8, getText(*S_8)+Str(Random(500)))
;             *S_8\Text\Change = 1
;             *S_8\Text\Caret + 5
;             *S_8\Text\Caret[1] = *S_8\Text\Caret 
;             ReDraw(10, winBackColor)
       PostEvent(#PB_Event_Gadget, *S_8\Canvas\Window, *S_8\Canvas\Gadget, #PB_EventType_Input)
