...
Procedure resize_row(*this._s_WIDGET, *row._s_ITEMS, new_h.i)
   If *row
   ...
   Repeat 
      ...
      *row = *this\__items( )\row
      If *row 
                  ...
                  If data_idx <= ArraySize(*row\Str( ))
                     ...
                     If *this\row\indent > 0 And data_idx = 0
                        ...
                        If (*row\mask & #__mask_node)
                           ...
                           Protected ty = dy + (*item\height / 2) - 4
                     ...
                     If *row\sel_start <> *row\sel_end 
                        ...
                        Box(sel_x, dy + 2, sel_w, *item\height - 4, #COLOR_BACK_SELECTED)
                     EndIf
                     ...
                     Else
                        ...
                        If *this\mask & #__mask_press
                           If *row\mask & #__mask_hover And *row\mask & #__mask_active 
                              ...
                              Protected mid_y = dy + (*item\Height / 2)
                              Protected c_blue = $FF0000 ; Синий в BGR
         ...
         Line(rx, dy + *item\height - 1, *this\Width, 1, #COLOR_LINE)
      ...
      If StartEnum(*r)
         ...
         If *this\mask & #__mask_edit
            ...
            If *start_r And *end_r
               ...
               ForEach *this\__rows()
                  Protected *rw._s_ITEMS = *this\__rows()
                  
                  ; 1. ПЕРВЫМ ДЕЛОМ проверяем, не одна ли это строка
                  If *rw = *start_r And *rw = *end_r
                     *rw\row\sel_start = Min(*this\text\caret[1], *this\text\caret[0])
                     *rw\row\sel_end   = Max(*this\text\caret[1], *this\text\caret[0])
                  ...
                  ElseIf *rw\y > min_y And *rw\y < max_y
                     *rw\row\sel_start = 0 
                     *rw\row\sel_end = Len(*rw\row\Str(0))
                     
                     ; 4. И в конце — границы (старт и конец)
                  ElseIf *rw = *start_r
                     ; Начальная строка
                     If is_down 
                        *rw\row\sel_start = *this\text\caret[1] 
                        *rw\row\sel_end = Len(*rw\row\Str(0))
                     Else       
                        *rw\row\sel_start = 0 
                        *rw\row\sel_end = *this\text\caret[1] 
                     EndIf
                  ElseIf *rw = *end_r
                     ; Конечная строка
                     If is_down 
                        *rw\row\sel_start = 0 
                        *rw\row\sel_end = *this\text\caret[0]
                     Else       
                        *rw\row\sel_start = *this\text\caret[0] 
                        *rw\row\sel_end = Len(*rw\row\Str(0)) 
                     EndIf
                  EndIf
               Next
...
Procedure swap_row(*this._s_WIDGET, *pressed_row._s_ITEMS, *hover_row._s_ITEMS, my.i)
   Protected mode
   Protected._s_BAR *v = *this\scroll\v
   Protected._s_BAR *h = *this\scroll\h
