...
Module Editor
  ...
  Procedure.i Caret(*This._S_widget, Line.i = 0)
    ...
    With *This
      If Line < 0 And FirstElement(*This\rows())
        ...
        Position = 0
      ElseIf Line < ListSize(*This\rows()) And SelectElement(*This\rows(), Line)
  ...
  Procedure.l Draw(*this._S_widget)
    ...
    With *this
      If Not \hide
        ...
        ; Draw caret
        If \row\count ;And (\Text\Editable Or \Items()\Text\Editable) And \Focus = *This 
          DrawingMode(#PB_2DDrawing_XOr)             
          Line((\rows()\Text\X-\scroll\h\page\pos)+\rows()\Text\caret[2], \rows()\Y-\scroll\v\page\pos, 1, \rows()\Height, $FFFFFFFF)
  ...
  Procedure.l SetState(*this._S_widget, State.l)
    ...
    With *this
      If State >= 0 And State < \row\count
        *Result = SelectElement(\rows(), State) 
      EndIf
  ...
  Procedure.l SetItemColor(*this._S_widget, Item.l, ColorType.l, Color.l, Column.l=0)
    ...
    With *this
        ...
        ForEach \rows()
          Select ColorType
            Case #PB_Gadget_BackColor
              \rows()\color\back[Column] = Color
              
            Case #PB_Gadget_FrontColor
              \rows()\color\front[Column] = Color
              
            Case #PB_Gadget_FrameColor
              \rows()\color\frame[Column] = Color
              
            Case #PB_Gadget_LineColor
              \rows()\color\line[Column] = Color
      ...
      Else
        If Item >= 0 And Item < *this\row\count And SelectElement(*this\rows(), Item)
          Select ColorType
            Case #PB_Gadget_BackColor
              \rows()\color\back[Column] = Color
  ...
  Procedure.i SetItemFont(*this._S_widget, Item.l, Font.i)
    ...
    If Item >= 0 And Item < *this\row\count And 
       SelectElement(*this\rows(), Item) And 
       *this\rows()\text\fontID <> FontID
      *this\rows()\text\fontID = FontID
      ;       *this\rows()\text\change = 1
      ;       *this\change = 1
      Result = #True
    EndIf
  ...
  Procedure.l SetItemState(*this._S_widget, Item.l, State.b)
    ...
    Result = SelectElement(*this\rows(), Item) 
    ...
    If Result 
      ...
      If State & #PB_Tree_Inbetween Or State & #PB_Tree_Checked
        _set_state_(*this, *this\rows(), State)
      ...
      ElseIf State & #PB_Tree_Expanded
        *this\rows()\box[0]\checked = 0
        collapsed = 1
      EndIf
      
      If collapsed And *this\rows()\childrens
        ; 
  ...
  Procedure.i SetItemImage(*this._S_widget, Item.l, Image.i) ; Ok
    If Item >= 0 And Item < *this\row\count And SelectElement(*this\rows(), Item)
      If *this\rows()\image\index <> Image
        _set_item_image_(*this, *this\rows(), Image)
        _repaint_(*this)
      EndIf
    EndIf
  EndProcedure
  ...
  Procedure.l GetItemColor(*this._S_widget, Item.l, ColorType.l, Column.l=0)
    ...
    With *this
      If Item >= 0 And Item < *this\row\count And SelectElement(*this\rows(), Item)
        Select ColorType
          Case #PB_Gadget_BackColor
            Result = \rows()\color\back[Column]
            
          Case #PB_Gadget_FrontColor
            Result = \rows()\color\front[Column]
            
          Case #PB_Gadget_FrameColor
            Result = \rows()\color\frame[Column]
            
          Case #PB_Gadget_LineColor
            Result = \rows()\color\line[Column]
  ...
  Procedure.l GetItemState(*this._S_widget, Item.l)
    ...
    If Item >= 0 And Item < *this\row\count And SelectElement(*this\rows(), Item) 
      If *this\rows()\color\state
        Result | #PB_Tree_Selected
      EndIf
      
      If *this\rows()\box[1]\checked
        If *this\flag\threestate And *this\rows()\box[1]\checked = 2
  ...
  Procedure.i GetItemAttribute(*this._S_widget, Item.l, Attribute.i, Column.l=0)
    ...
    If SelectElement(*this\rows(), Item)
      Select Attribute
        Case #PB_Tree_SubLevel
          Result = *this\rows()\sublevel
      EndSelect
    EndIf
  ...
  Procedure RemoveItem(*this._S_widget, Item.l) 
    ...
    If Item >= 0 And 
       ...
       SelectElement(*this\rows(), Item)
      ;       Debug ""+*this\rows()\index +" "+ Item
      ; *this\row\sublevel = 0
      
      If *this\rows()\childrens
        sublevel = *this\rows()\sublevel
        
        PushListPosition(*this\rows())
        While NextElement(*this\rows())
          If *this\rows()\sublevel > sublevel 
            ;Debug *this\rows()\text\string
      ...
      If (*this\row\draw And (*this\row\count % *this\row\draw) = 0) Or 
        ...
        PushListPosition(*this\rows())
        ForEach *this\rows()
          ; *this\row\sublevel = *this\image\padding\left + *this\rows()\image\width
          *this\rows()\index = ListIndex(*this\rows())
        Next
        PopListPosition(*this\rows())
      EndIf 
      ...
      If *this\row\selected And *this\row\selected\index >= Item 
        *this\row\selected\color\state = 0
        ...
        If *this\row\selected\index <> Item 
          SelectElement(*this\rows(), *this\row\selected\index)
        EndIf
  ...
  Procedure tt_CallBack()
    ...
    ;     EndIf
    ;     
    ;     *event\widget\row\selected = *event\widget\rows()
    ;     *event\widget\rows()\color\State = 2
    ...
    With *this
      If *this
        *event\widget = *this
        ...
        \row\tt\visible = 1
        \row\tt\x = x+\rows()\x+\rows()\width-1
        \row\tt\y = y+\rows()\y-\scroll\v\page\pos
        \row\tt\width = \rows()\len - \rows()\width + 5
        \row\tt\height = \rows()\height
        Protected flag
        ...
        \row\tt\color = \rows()\color
        \row\tt\text = \rows()\text
        \row\tt\text\fontID = \rows()\fontID
        \row\tt\text\x =- (\width[2]-(\rows()\text\x-\rows()\x)) + 1
        \row\tt\text\y = (\rows()\text\y-\rows()\y)+\scroll\v\page\pos
  ...
  Procedure Events(*this._S_widget, eventtype.l, mouse_x.l=-1, mouse_y.l=-1, position.l=0)
    ...
    Select eventtype
      ...
      Case #PB_EventType_LeftButtonDown
        ...
        *this\rows()\text\len = Len(*this\rows()\text\string)
        Debug Caret(*this, *this\row\index)
        Result = 1
        ...
        If position And *this\row\index >= 0
          ...
          If position > 0
            If down And *this\flag\multiselect 
              ...
              If *this\rows()\color\state = 0
                *this\rows()\color\state = 1
              EndIf
            EndIf
            
            ; event mouse leave line
          Else
            If (*this\rows()\color\state = 1)
  ...
  Procedure.l CallBack(*this._S_widget, EventType.l, mouse_x.l=-1, mouse_y.l=-1)
    ...
    With *this
      ...
      If leave
        If *event\enter\row\count And *event\enter\row\index >= 0 ;And SelectElement(*event\enter\rows(), *event\enter\row\index)
  ...
  Procedure.i Widget(X.l, Y.l, Width.l, Height.l, Flag.i=0, Radius.l=0)
    ...
    If *this
      With *this
        ...
        \flag\multiSelect = 1;Bool(flag&#PB_Flag_MultiSelect)
...
;         EndIf
      ...
      \scroll\h = Bar::Scroll(0, 0, 0, Bool(Not flag&#PB_Editor_WordWrap)*16, 0,0,0, 0, 7)
...
CompilerIf #PB_Compiler_IsMainFile
  ...
  If OpenWindow(0, 0, 0, 1110, 650, "editorgadget", #PB_Window_SystemMenu | #PB_Window_ScreenCentered)
    Define i,a,g = 1
    EditorGadget(g, 10, 10, 210, 210, #PB_Editor_ReadOnly)                                         
    ; 1_example
    ...
    g = 2
    EditorGadget(g, 230, 10, 210, 210, #PB_Editor_WordWrap)                                         
    ; 3_example
    ...
    If widget
      *g = Widget(10, 100, 210, 210, #PB_Editor_ReadOnly)                                         
      *g\canvas\Gadget = g_Canvas
      AddElement(*List()) : *List() = *g
    Else
      *g = GetGadgetData(Gadget(g, 10, 100, 210, 210, #PB_Editor_ReadOnly))
    EndIf
    ...
    g = 11
    If widget
      *g = Widget(230, 100, 210, 210, #PB_Editor_WordWrap)                                         
      *g\canvas\Gadget = g_Canvas
      AddElement(*List()) : *List() = *g
    Else
      *g = GetGadgetData(Gadget(g, 230, 100, 210, 210, #PB_Editor_WordWrap))
    EndIf
