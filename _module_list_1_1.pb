  ...
  Macro _multi_select_(_this_)
    ...
    ForEach _this_\items()
      If Not _this_\items()\hide
        _this_\items()\color\state =  Bool((_this_\columns()\row\selected\index >= _this_\items()\index And _this_\from =< _this_\items()\index) Or ; верх
                                           (_this_\columns()\row\selected\index =< _this_\items()\index And _this_\from >= _this_\items()\index)) * 2  ; вниз
          ...
          Else
            ...
            If (_this_\change Or _this_\scroll\v\change Or _this_\scroll\h\change)
              ...
              If _this_\flag\buttons Or _this_\flag\lines 
                _this_\Columns()\items()\box[0]\x = _this_\Columns()\x + _this_\Columns()\items()\sublength - _this_\row\sublength + Bool(_this_\flag\buttons And Not _this_\flag\gridlines) * 4 + Bool(Not _this_\flag\buttons And _this_\flag\lines) * 8 - _this_\scroll\h\page\pos - _this_\row\sublevel
              ...
              _this_\Columns()\items()\image\x = _this_\Columns()\x + _this_\image\padding\left + _this_\Columns()\items()\sublength -_this_\scroll\h\page\pos - _this_\row\sublevel
              _this_\Columns()\items()\text\x = _this_\Columns()\x + _this_\text\padding\left + _this_\Columns()\items()\sublength + _this_\Columns()\sublevel  -_this_\scroll\h\page\pos
      ...
      If *this ;And SelectElement(\Columns(), 0)
        ForEach \Columns()
          ...
          If handle
            ...
            If \row\sublength ;And Not \Columns()\index
              If Not item
            ...
            If Not \Columns()\index
              ...
              If \row\sublength
                \Columns()\items()\sublength = \Columns()\items()\sublevel * \row\sublength + Bool(\flag\buttons) * 20 ; + Bool(\flag\checkBoxes) * 18 
              EndIf
            EndIf
        ...
        Case #PB_EventType_LeftButtonUp ; : Debug ""+#PB_Compiler_Line +" отпустили " + _this_ +" "+ _this_\from
           ...
           If Not _this_\flag\multiselect
            If Not _this_\flag\clickselect
             ...
             Debug _this_\columns()\items()\text\string
            _this_\columns()\row\selected = _this_\columns()\items()
              _this_\columns()\items()\color\state = 2
        ...
        Case #PB_EventType_LeftButtonDown
          ...
          If from >= 0 ; And SelectElement(\items(), from)
            ...
            ElseIf (\flag\buttons And \columns()\items()\childrens) And
              ...
              ;ForEach \columns()
              While NextElement(\columns()\items())
                If \columns()\index = 0 And \columns()\items()\parent
                  hide = Bool(\columns()\items()\parent\box[0]\checked | \columns()\items()\parent\hide)
                EndIf
                 ...
                 If \columns()\items()\sublevel = sublevel
                  ...
                  \columns()\items()\hide = hide
                   ;Debug ""+\columns()\text\string +" "+ \columns()\items()\text\string
                  EndIf
                Wend
              ;Next
              PopListPosition(\columns())
