...
;               EndIf
              ...
              If Drawing
                ...
                ; Draw selections
                If ( \index=\Line Or \index=\focus) ; \index=*This\Line ; с этим остается последное виделеное слово
...
;       \Items()\Hide = Bool(Not Bool(\Items()\y>=\y[2] And (\Items()\y-\y[2])+\Items()\height=<\height[2]))
  ...
  Procedure.i list_AddItem(*This._S_widget, Item.i,Text.s,Image.i=-1,Flag.i=0)
        ...
        ForEach \Items()
          ...
          If \Items()\index = Item
            ...
            If State&#PB_Attribute_Collapsed Or State&#PB_Attribute_Expanded
              \Items()\box[0]\checked = Bool(State&#PB_Attribute_Collapsed)
      ...
      If Not Bool(MouseX=-1 And MouseY=-1)
        ...
        ForEach \Items()
          ...
          If (MouseY > (\Items()\Y) And MouseY =< ((\Items()\Y+\Items()\Height))) And 
            ...
            If focus
              If (MouseY > (\Items()\box[1]\y) And MouseY =< ((\Items()\box[1]\y+\Items()\box[1]\height))) And 
                 ((MouseX > \Items()\box[1]\x) And (MouseX =< (\Items()\box[1]\x+\Items()\box[1]\width)))
              ...
              If (\Flag\buttons And \Items()\childrens) And
                 (MouseY > (\Items()\box[0]\y) And MouseY =< ((\Items()\box[0]\y+\Items()\box[0]\height))) And 
                 ((MouseX > \Items()\box[0]\x) And (MouseX =< (\Items()\box[0]\x+\Items()\box[0]\width)))
                ...
                While NextElement(\Items())
                  ...
                  ElseIf sublevel < \Items()\sublevel 
                    If \Items()\parent
                      PushListPosition(\Items())
                      ChangeCurrentElement(\Items(), \Items()\parent)
                      collapsed = \Items()\box[0]\checked
                      If \Items()\hide
                        collapsed = 1
                      EndIf
                      PopListPosition(\Items())
...
;         \Caret[1] =- 1
  ...
  Procedure Canvas_CallBack()
    ...
    If *This
      With *This
        If Not \hide
          AutoHide.b = 0; Bool(\scroll\v\from=0 And \scroll\h\from=0)
          
          If \scroll\v
            Repaint = Bar::CallBack(\scroll\v, Event, MouseX, MouseY, WheelDelta, AutoHide, \scroll\h, Window, Canvas)
            If Repaint
              ReDraw(*This)
            EndIf
          EndIf
          ...
          If \scroll\h
            Repaint = Bar::CallBack(\scroll\h, Event, MouseX, MouseY, WheelDelta, AutoHide, \scroll\v, Window, Canvas)
            If Repaint
              ReDraw(*This)
            EndIf
          EndIf
            ...
            Select Event
              Case #PB_EventType_MouseWheel
                If Not \scroll\v\Hide
                  Select -WheelDelta
                    Case-1 : Repaint = Bar::SetState(\scroll\v, \scroll\v\Page\Pos - (\scroll\v\Max-\scroll\v\Min)/30)
                    Case 1 : Repaint = Bar::SetState(\scroll\v, \scroll\v\Page\Pos + (\scroll\v\Max-\scroll\v\Min)/30)
                  EndSelect
                EndIf
