...
;       *scroll\v\Page\Pos =- \v\Page\Pos
;       *scroll\h\Page\Pos =- \h\Page\Pos
;       *Scroll\width = \v\Max
;       *Scroll\height = \h\Max
          ...
          For IT = 1 To \Text\Count
            ...
            If SelectElement(\Items(), IT-1)
             ...
             EndIf
              ...
              ; Resize item
              If (Left And Not  Bool(\scroll\h\Page\Pos = Left))
                _set_content_X_(*This)
              EndIf
...
;               Case #PB_EventType_LeftDoubleClick : PostEvent(#PB_Event_Widget, \Canvas\Window, \Canvas\Gadget, #PB_EventType_LeftDoubleClick)
                ...
                ForEach \Items()
                  If Not \Items()\hide 
                    If \Line = \Items()\Item 
                      ...
                      If (\Canvas\Mouse\Y > (\Items()\Y) And \Canvas\Mouse\Y =< ((\Items()\Y+\Items()\Height))) And 
                         ((\Canvas\Mouse\X > \Items()\X) And (\Canvas\Mouse\X =< (\Items()\X+\Items()\Width)))
              ...
              Case #PB_EventType_MouseMove  
;                 If Not (\Canvas\Mouse\Y > (\Y[2]) And \Canvas\Mouse\Y =< ((\Y[2]+\Height[2]-Bar::Height(\scroll\h)))) And 
;                          ((\Canvas\Mouse\X > \X[2]) And (\Canvas\Mouse\X =< (\X[2]+\Width[2]-Bar::Width(\scroll\v))))
