...
Module Tree
  Macro _from_point_(_mouse_x_, _mouse_y_, _type_, _width_=width, _height_=height, _mode_=)
    Bool (_mouse_x_ > _type_\x#_mode_ And _mouse_x_ =< (_type_\x#_mode_+_type_\_width_#_mode_) And 
          _mouse_y_ > _type_\y#_mode_ And _mouse_y_ =< (_type_\y#_mode_+_type_\_height_#_mode_))
  EndMacro
        ...
        If Not _this_\items()\hide
          ...
          If (_this_\change Or _this_\scroll\v\change Or _this_\scroll\h\change)
            ...
            If _this_\flag\checkBoxes
              ...
              _this_\items()\box[1]\y = (_this_\items()\y+_this_\items()\height)-(_this_\items()\height+_this_\items()\box[1]\len)/2-_this_\scroll\v\page\pos
            EndIf
            ...
            If _this_\flag\buttons Or _this_\flag\lines 
              ...
              _this_\items()\box[0]\y = (_this_\items()\y+_this_\items()\height)-(_this_\items()\height+_this_\items()\box[0]\len)/2-_this_\scroll\v\page\pos
            EndIf
        ...
        If \row\count
          ...
          ForEach \draws()
              ...
              If \draws()\childrens And \flag\buttons
                DrawingMode(#PB_2DDrawing_Default)
                Bar::Arrow(\draws()\box[0]\x+(\draws()\box[0]\len-6)/2,\draws()\box[0]\y+(\draws()\box[0]\len-6)/2, 6, Bool(Not \draws()\box[0]\checked)+2, $FF7E7E7E, 0,0) 
              EndIf
              
              ; Draw checkbox
              If \flag\checkboxes
                DrawingMode(#PB_2DDrawing_Default)
                CheckBox(\draws()\box[1]\x,\draws()\box[1]\y,\draws()\box[1]\len,\draws()\box[1]\len, 3, \draws()\box[1]\checked, $FFFFFFFF, $FF7E7E7E, 2, 255)
              EndIf
            ...
            If \flag\lines And \sub_len
              ; DrawingMode(#PB_2DDrawing_XOr)
              Protected h_point
              Protected x_point=\draws()\box[0]\x+\draws()\box[0]\len/2
              Protected y_point=\draws()\box[0]\y+\draws()\box[0]\len/2
          ...
          ForEach \items()
            ...
            If \items()\draw
              ...
              If \items()\childrens And \flag\buttons
                DrawingMode(#PB_2DDrawing_Default)
                Bar::Arrow(\items()\box[0]\x+(\items()\box[0]\len-6)/2,\items()\box[0]\y+(\items()\box[0]\len-6)/2, 6, Bool(Not \items()\box[0]\checked)+2, $FF7E7E7E, 0,0) 
              EndIf
              
              ; Draw checkbox
              If \flag\checkboxes
                DrawingMode(#PB_2DDrawing_Default)
                CheckBox(\items()\box[1]\x,\items()\box[1]\y,\items()\box[1]\len,\items()\box[1]\len, 3, \items()\box[1]\checked, $FFFFFFFF, $FF7E7E7E, 2, 255)
              EndIf
            ...
            If \flag\lines And \sub_len
              ; DrawingMode(#PB_2DDrawing_XOr)
              Protected h_point
              Protected x_point=\items()\box[0]\x+\items()\box[0]\len/2
              Protected y_point=\items()\box[0]\y+\items()\box[0]\len/2
    ...
    With *this
      If *this
        ...
        If handle
          ...
          \items()\box[0]\len = \flag\buttons
          \items()\box[1]\len = \flag\checkBoxes
  ...
  Procedure.l CallBack(*this._S_widget, EventType.l, mouse_x.l=-1, mouse_y.l=-1)
    ...
    With *this
      ...
      If Not \hide And \scroll\v\from =- 1 And \scroll\h\from =- 1 And
         ((_from_point_(mouse_x, mouse_y, *this, width,height, [2]) And Not Down) Or Down = *this)
      ...
      Select EventType
        ...
        Case #PB_EventType_LeftButtonUp 
          If \from >= 0 And Down = *this ; SelectElement(\items(), \from)
            ...
            If Not ((\flag\buttons And \items()\childrens And 
                     _from_point_(\canvas\mouse\x, \canvas\mouse\y, \items()\box[0],len,len)) Or
                    _from_point_(\canvas\mouse\x, \canvas\mouse\y, \items()\box[1],len,len))
        ...
        Case #PB_EventType_LeftButtonDown
          ...
          If from >= 0 ; And SelectElement(\items(), from)
            ...
            If _from_point_(\canvas\mouse\x, \canvas\mouse\y, \items()\box[1], len, len)
              
              \items()\box[1]\checked ! 1
              
              Result = #True
            ElseIf (\flag\buttons And \items()\childrens) And
                   _from_point_(\canvas\mouse\x, \canvas\mouse\y, \items()\box[0], len, len)
