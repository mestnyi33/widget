...
Module Tree
  ...
  Macro _set_item_image_(_this_, _image_)
    ...
    If _this_\Items()\image\change
      ...
      _this_\row\sublevel = _this_\image\padding\left + _this_\items()\image\width
    EndIf
  EndMacro
      ...
      ForEach _this_\items()
        If Not _this_\items()\hide
          ...
          If (_this_\change Or _this_\scroll\v\change Or _this_\scroll\h\change)
              ...
              If _this_\items()\parent 
                ...
                If _this_\items()\draw
                  If _this_\items()\parent\last
                    _this_\items()\parent\last\vl\height = 0
                ...
                Else
                  ...
                  If _this_\items()\parent\last
                    _this_\items()\parent\last\vl\height = (_this_\y[2] + _this_\height[2]) -  _this_\items()\vl\y ;(_this_\items()\y-_this_\y[2]) ; _this_\row\first\last\vl\y;+
              ...
              Else
                If _this_\items()\draw
                  If _this_\row\first\last And
                    ...
                    If _this_\row\first\last\first
                      _this_\row\first\last\vl\height = 0
                ...
                Else
                  If _this_\row\first\last And
                    ...
                    _this_\items()\vl\y = (_this_\row\first\last\first\y+_this_\row\first\last\first\height- Bool(_this_\row\first\last\first\sublevel = _this_\row\first\last\sublevel) * _this_\row\first\last\first\height/2) - _this_\scroll\v\page\pos
                    If _this_\items()\vl\y < _this_\y[2] : _this_\items()\vl\y = _this_\y[2] : EndIf
                    
                    _this_\row\first\last\vl\height = (_this_\items()\y+_this_\items()\height/2)-_this_\items()\vl\y - _this_\scroll\v\page\pos
                    If _this_\row\first\last\vl\height < 0 : _this_\row\first\last\vl\height = 0 : EndIf
                    If _this_\items()\vl\y + _this_\row\first\last\vl\height > _this_\y[2]+_this_\height[2] 
                      If _this_\items()\vl\y > _this_\y[2]+_this_\height[2] 
                        _this_\row\first\last\vl\height = 0
                      Else
                        _this_\row\first\last\vl\height = (_this_\y[2] + _this_\height[2]) -  _this_\items()\vl\y 
                      EndIf
                    EndIf
                    
                    
                    ;_this_\row\first\last\vl\height = (_this_\y[2] + _this_\height[2]) -  _this_\items()\vl\y ;(_this_\items()\y-_this_\y[2]) ; _this_\row\first\last\vl\y;+
            ...
            If _this_\flag\lines And _this_\row\sublevellen
            _this_\items()\hl\y = _this_\items()\box[0]\y+_this_\items()\box[0]\height/2
            _this_\items()\vl\x = _this_\items()\box[0]\x+_this_\items()\box[0]\width/2
            
            If (_this_\x[2]-_this_\items()\vl\x) < _this_\flag\lines
              If _this_\items()\vl\x<_this_\x[2]
                _this_\items()\hl\width =  (_this_\flag\lines - (_this_\x[2]-_this_\items()\vl\x))
              Else
                _this_\items()\hl\width = _this_\flag\lines
              EndIf
              
              If _this_\items()\draw And _this_\items()\hl\y > _this_\y[2] And _this_\items()\hl\y < _this_\y[2]+_this_\height[2]
                _this_\items()\hl\x = _this_\items()\vl\x + (_this_\flag\lines-_this_\items()\hl\width)
                _this_\items()\hl\height = 1
              Else
                _this_\items()\hl\height = 0
              EndIf
              
              ; And _this_\items()\parent\last = _this_\items()
              ; Vertical plot
              If _this_\items()\first And _this_\x[2]<_this_\items()\vl\x
                _this_\items()\vl\y = (_this_\items()\first\y+_this_\items()\first\height- Bool(_this_\items()\first\sublevel = _this_\items()\sublevel) * _this_\items()\first\height/2) - _this_\scroll\v\page\pos
                If _this_\items()\vl\y < _this_\y[2] : _this_\items()\vl\y = _this_\y[2] : EndIf
                
                _this_\items()\vl\height = (_this_\items()\y+_this_\items()\height/2)-_this_\items()\vl\y - _this_\scroll\v\page\pos
                If _this_\items()\vl\height < 0 : _this_\items()\vl\height = 0 : EndIf
                If _this_\items()\vl\y + _this_\items()\vl\height > _this_\y[2]+_this_\height[2] 
                  If _this_\items()\vl\y > _this_\y[2]+_this_\height[2] 
                    _this_\items()\vl\height = 0
                  Else
                    _this_\items()\vl\height = (_this_\y[2] + _this_\height[2]) -  _this_\items()\vl\y 
                  EndIf
                EndIf
                
                If _this_\items()\vl\height
                  _this_\items()\vl\width = 1
                Else
                  _this_\items()\vl\width = 0
                EndIf
              EndIf 
      ...
      If Not \hide
        ...
        If \row\count
          ...
          ForEach \items()
            ...
            If \flag\lines And \row\sublevellen
                If \items()\vl\width
                  Line(\items()\vl\x+Random(5), \items()\vl\y,\items()\vl\width,\items()\vl\height, $FF7E7E7E)
                EndIf
                If \items()\hl\height
                  Line(\items()\hl\x , \items()\hl\y,\items()\hl\width,\items()\hl\height, $FF7E7E7E)
                EndIf
...
; ;                 EndIf
; ;                 
; ;                 If \items()\draw And y_point > \y[2] And y_point < \y[2]+\height[2]
; ;                   ; DrawingMode(#PB_2DDrawing_CustomFilter) : CustomFilterCallback(@PlotX())
; ;                   Line(x_point + (\flag\lines-line_size), y_point,line_size+2,1, $FF7E7E7E)
; ;                 EndIf
; ;                 
; ;                 ; Vertical plot
; ;                 If \items()\first And \x[2]<x_point
; ;                   y_point = (\items()\first\y+\items()\first\height- Bool(\items()\first\sublevel = \items()\sublevel) * \items()\first\height/2) - \scroll\v\page\pos
; ;                   If y_point < \y[2] : y_point = \y[2] : EndIf
; ;                   
...
; ;                   If h_point
; ;                     ; DrawingMode(#PB_2DDrawing_CustomFilter) : CustomFilterCallback(@PlotY())
; ;                     Line(x_point+Random(5),y_point,1,h_point, $FF7E7E7E)
; ;                   EndIf
; ;                 EndIf
; ;               EndIf
            EndIf
    ...
    With *this
      If *this
        ...
        If handle
          ...
          \items()\sublevellen = \items()\sublevel * \row\sublevellen + Bool(\flag\buttons) * 20 + Bool(\flag\checkBoxes) * 18 
