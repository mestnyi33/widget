; 16 сентябрь 
; единственное отличие позиция
; канваса сохраняем в перемение
...
DeclareModule Structures
  ...
  ;- - _S_canvas
  Structure _S_canvas Extends _S_point
    window.i
    gadget.i
   ; widget.i
      ...
      ForEach _items_
        ...
        Else
          If _this_\change
            ...
            _items_\height = _items_\text\height + 2 ; + Bool(_this_\flag\GridLines) + Bool(#PB_Compiler_OS = #PB_OS_Windows) * 2
...
;                 If Not Window
        ...
        Window = OpenWindow(#PB_Any, x+\items()\x,y+\items()\y-\scroll\v\page\pos,\items()\len+5,\items()\height, "", #PB_Window_BorderLess|#PB_Window_NoActivate|#PB_Window_Tool) ;|#PB_Window_NoGadgets
        Gadget = CanvasGadget(#PB_Any,0,0,\items()\len+5,\items()\height)
        If StartDrawing(CanvasOutput(Gadget))
          If \items()\text\fontID 
            DrawingFont(\items()\text\fontID) 
          EndIf 
          DrawingMode(#PB_2DDrawing_Default)
          Box(1,1,\items()\len+5-2,\items()\height-2, *color\back[1])
          DrawingMode(#PB_2DDrawing_Transparent)
          DrawText(\items()\text\x-\items()\x, (\items()\text\y-\items()\y)+\scroll\v\page\pos, \items()\text\string, *color\front[1])
          DrawingMode(#PB_2DDrawing_Outlined)
          Box(0,0,\items()\len+5,\items()\height, *color\frame[1])
          StopDrawing()
        EndIf
    ...
    If mouse_x =- 1 And mouse_y =- 1
      Select EventType
        ...
        Case #PB_EventType_Resize
          *this\canvas\x = GadgetX(*this\canvas\gadget, #PB_Gadget_ScreenCoordinate)
          *this\canvas\y = GadgetY(*this\canvas\gadget, #PB_Gadget_ScreenCoordinate)
          ...
          If _this_\row\from >= 0
            ...
            If Bool((_this_\flag\buttons=0 And _this_\flag\lines=0))
              If _this_\items()\len > _this_\width[2]
                ...
                ;_this_\items()\width = _this_\width[2]
               ToolTip(_this_, _this_\canvas\x, _this_\canvas\y, _this_\items()\color)
