...
Module Tree
  ...
  ;-
  Procedure tt_draw(*this._S_tt, *color._S_color)
    With *this
    ...
    With *this
      If *this
        *event\widget = *this
        ...
        Window = OpenWindow(#PB_Any, \row\tt\x, \row\tt\y, \row\tt\width, \row\tt\height, "", #PB_Window_BorderLess|#PB_Window_NoActivate, WindowID(\canvas\window))
        ...
        Case #PB_EventType_LeftButtonDown ; : Debug ""+#PB_Compiler_Line +" нажали " + _this_ +" "+ _this_\from
          ...
          If Bool((_this_\flag\buttons=0 And _this_\flag\lines=0))
            If _this_\items()\len > _this_\width[2]
              ...
              Redraw(_this_)
              tt_draw(_this_\row\tt, _this_\items()\color)
