...
Module Tree
  Macro _box_(_x_,_y_, _width_, _height_, _checked_, _type_, _color_=$FFFFFFFF, _radius_=2, _alpha_=255) 
    ...
    If _checked_
      ...
      If _type_ = 1
        Circle(_x_+(_width_)/2, _y_+(_height_)/2, 2,_color_&$FFFFFF|_alpha_<<24)
      ...
      ForEach _items_
        If _items_\draw
          ...
          If _this_\flag\OptionBoxes And _items_\parent
            DrawingMode(#PB_2DDrawing_Default)
            _box_(_items_\box[1]\x, _items_\box[1]\y, _items_\box[1]\width+1, _items_\box[1]\height+1, _items_\box[1]\checked, 1, $FFFFFFFF, 7, 255)
  ...
  ;-
  Procedure tt_draw(*parent._S_widget)
    If IsGadget(*parent\row\tt)
      Protected width
      
      With *parent
        If StartDrawing(CanvasOutput(*parent\row\tt))
          width = OutputWidth()
          ...
          DrawingMode(#PB_2DDrawing_Transparent)
          DrawText(-(\width[2]-(\items()\text\x-\items()\x)) + 1, (\items()\text\y-\items()\y)+\scroll\v\page\pos, \items()\text\string, \items()\color\front[\items()\color\state])
          DrawingMode(#PB_2DDrawing_Outlined)
  ...
  Procedure tt_CallBack()
    Select Event()
      Case #PB_Event_ActivateWindow
        UnbindEvent(#PB_Event_ActivateWindow, @tt_CallBack(), EventWindow())
  ...
  Procedure tt_creare(*this._S_widget, x,y)
    Static Window
    Protected width, height, ParentID
    
    With *this
      If *this
        *event\widget = *this
        x+\items()\x+\items()\width-1
        y+\items()\y-\scroll\v\page\pos
        width = \items()\len - \items()\width + 5
        height = \items()\height
        ParentID = WindowID(\canvas\window)
        
        Window = OpenWindow(#PB_Any, x, y, width, height, "", #PB_Window_BorderLess|#PB_Window_NoActivate, ParentID)
        BindEvent(#PB_Event_ActivateWindow, @tt_CallBack(), Window)
        \row\tt = CanvasGadget(#PB_Any,0,0, width, height)
        tt_draw(*this)
  ...
  Procedure tt_close()
    ProcedureReturn tt_creare(0,0,0)
  EndProcedure
    ...
    Macro _callback_(_this_, _type_)
      Select _type_
        ...
        Case #PB_EventType_LeftButtonDown ; : Debug ""+#PB_Compiler_Line +" нажали " + _this_ +" "+ _this_\from
          ...
          If Bool((_this_\flag\buttons=0 And _this_\flag\lines=0))
            If _this_\items()\len > _this_\width[2] And _this_\row\tt
...
CompilerIf #PB_Compiler_IsMainFile
  ...
  If OpenWindow(0, 0, 0, 1110, 650, "TreeGadget", #PB_Window_SystemMenu | #PB_Window_ScreenCentered)
    ...
    *g\canvas\Gadget = g_Canvas
    ...
    g = 13
    *g = Widget(670, 100, 210, 210, #PB_Flag_OptionBoxes|#PB_Flag_NoLines|#PB_Flag_NoButtons|#PB_Flag_ClickSelect)                                         
    *g\canvas\Gadget = g_Canvas
    AddElement(*List()) : *List() = *g
    ;  4_example
