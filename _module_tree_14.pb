...
Module Tree
  ...
  ;-
  Procedure Events(*this._S_widget, eventtype.l, position.l, mouse_x.l=-1, mouse_y.l=-1)
    Protected Result
  ...
  Procedure.l CallBack(*this._S_widget, EventType.l, mouse_x.l=-1, mouse_y.l=-1)
    Protected Result, from =- 1
    Static cursor_change, LastX, LastY, Last, Down, buttons
...
;         Protected enter = Bool(*event\leave <> *this And Not (*event\leave And *event\leave\index > *this\index) And _from_point_(mouse_x, mouse_y, *this))
;         Protected leave = Bool(*event\leave And (enter Or (*event\leave = *this And Not _from_point_(mouse_x, mouse_y, *this))))
;         
;         If (*event\leave And *event\leave\mouse\Buttons)
;           If (enter Or Not _from_point_(mouse_x, mouse_y, *event\leave))
;             If EventType = #PB_EventType_LeftButtonUp
;              Debug 2222
;              Events(*event\leave, #PB_EventType_LeftButtonUp, *event\leave\from)
;              Events(*event\leave, #PB_EventType_MouseLeave, *event\leave\from)
;              Events(*this, #PB_EventType_MouseEnter, *this\from)
;              Debug 3333
;              
...
;         EndIf
        ...
        If buttons = 0
          Protected enter = Bool(*event\leave <> *this And Not (*event\leave And *event\leave\index > *this\index) And _from_point_(mouse_x, mouse_y, *this))
          Protected leave = Bool(*event\leave And (enter Or (*event\leave = *this And Not _from_point_(mouse_x, mouse_y, *event\leave))))
        
;           If EventType = #PB_EventType_LeftButtonUp
;             If *event\leave And *event\leave\index > *this\index
;               If *event\leave\mouse\Buttons
;                 *event\leave\mouse\Buttons = 1
; ;                 Events(*event\leave, #PB_EventType_LeftButtonUp, *event\leave\from)
; ;                 Events(*event\leave, #PB_EventType_MouseLeave, *event\leave\from)
;               EndIf
;               
;               If _from_point_(mouse_x, mouse_y, *this)
;                 
;                 Events(*this, #PB_EventType_MouseEnter, *this\from)
;                 *event\leave = *this
;               EndIf
;               
;               Debug ""+*this +" "+ _from_point_(mouse_x, mouse_y, *this); Bool(_from_point_(mouse_x, mouse_y, *this))
;             EndIf
;             
;           EndIf
          
         If (leave); And Not *event\leave\mouse\Buttons)
            If *event\leave\from >= 0
              _callback_(*event\leave, #PB_EventType_MouseLeave)
            EndIf
            
            Events(*event\leave, #PB_EventType_MouseLeave, *event\leave\from)
            
            If *event\leave And *this\canvas\gadget <> *event\leave\canvas\gadget
              PostEvent(#PB_Event_Gadget, *event\leave\canvas\window, *event\leave\canvas\gadget, #PB_EventType_Repaint, *event\leave)
            EndIf
          ...
          If (enter); And Not (*event\leave And *event\leave\mouse\Buttons))
            Events(*this, #PB_EventType_MouseEnter, *this\from)
      ...
      Select *this
        Case *event\leave;, *event\active
          ...
          ; 
          If *this = *event\leave And Not \mouse\buttons
          ...
          Select EventType
            Case #PB_EventType_MouseMove
              If \mouse\buttons And \row\drag = 0 And
                 (Abs((mouse_x-\delta\x)+(mouse_y-\delta\y)) >= 6)
                \row\drag = 1
                
                If \row\selected <> \items()
                  \row\selected = \items()
            ...
            Case #PB_EventType_LeftButtonUp 
              ;Debug "left up "+\mouse\buttons
              If \mouse\buttons
                Events(*this, #PB_EventType_LeftButtonUp, *this\from)
;                 
              EndIf
              
              If \from >= 0 And \mouse\buttons
...
CompilerIf #PB_Compiler_IsMainFile
  ...
  If OpenWindow(0, 0, 0, 1110, 650, "TreeGadget", #PB_Window_SystemMenu | #PB_Window_ScreenCentered)
    ...
    *g\canvas\Gadget = g_Canvas
    ...
    g = 14
    *g = Widget(750, 140, 103, 210, #PB_Tree_NoButtons)                                         
    *g\canvas\Gadget = g_Canvas
    AddElement(*List()) : *List() = *g
    ;  5_example
        AddItem(*g, 0, "Tree_0", -1 )
