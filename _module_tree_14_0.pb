...
Module Tree
        ...
        Else
          ...
          If (_this_\change Or _this_\scroll\v\change Or _this_\scroll\h\change)
            ...
            If _items_\draw And 
              ...
              _this_\row\lastvisible = _items_
  ...
  ;-
  Procedure Events(*this._S_widget, eventtype.l,  mouse_x.l, mouse_y.l)
    Protected Result, from =- 1
    ...
    Select EventType
      Case #PB_EventType_MouseMove
        If Down And Down = *this And *this\row\drag = 0 And
           (Abs((mouse_x-*this\delta\x)+(mouse_y-*this\delta\y)) >= 6)
          *this\row\drag = 1
          
          If *this\row\from <> *this\from
            Post(#PB_EventType_Change, *this, *this\from)
          EndIf
          Post(#PB_EventType_DragStart, *this, *this\from)
      ...
      Case #PB_EventType_LeftDoubleClick
        If *this\from >= 0
          Post(#PB_EventType_LeftDoubleClick, *this, *this\from)
        EndIf
        
      Case #PB_EventType_RightClick
        If *this\from >= 0
          Post(#PB_EventType_RightClick, *this, *this\from)
        EndIf
        
      Case #PB_EventType_LeftButtonUp 
        If *this\from >= 0 And Down = *this ; SelectElement(\items(), \from)
          Down = 0 : LastX = 0 : LastY = 0 
          
          If *this\row\from < 0
            If *this\row\from =- 1
              Post(#PB_EventType_Up, *this, *this\from, *this\items()\box[0]\checked)
            EndIf
            
            If *this\row\from =- 2
              Post(#PB_EventType_StatusChange, *this, *this\from, *this\items()\box[1]\checked)
            EndIf
            
            If *this\row\selected
              *this\row\from = *this\row\selected\index
            Else
              *this\row\from = 0
            EndIf
          EndIf
          
          ; Debug " "+*this\row\from +" "+*this\from
          
          If Not ((*this\flag\buttons And *this\items()\childrens And 
                   _from_point_(mouse_x, mouse_y, *this\items()\box[0])) Or
                  _from_point_(mouse_x, mouse_y, *this\items()\box[1]))
            
            _callback_(*this, #PB_EventType_LeftButtonUp)
            
            If (*this\row\from >= 0 And *this\row\from <> *this\from) Or Not *this\row\from : *this\row\from = *this\from
              If Not *this\row\drag
                Post(#PB_EventType_Change, *this, *this\from)
              EndIf
            EndIf
          EndIf
          ...
          If *this\row\drag 
            *this\row\drag = 0
          Else
            Post(#PB_EventType_LeftClick, *this, *this\from)
          EndIf
      ...
      Case #PB_EventType_LeftButtonDown
        If *this\from >= 0 ; And SelectElement(\items(), from)
          ...
          *this\delta\y = mouse_y
          ...
          If _from_point_(*this\canvas\mouse\x, *this\canvas\mouse\y, *this\items()\box[1])
            *this\row\from =- 2
            
            If *this\flag\OptionBoxes
              
              ;Debug ""+*this\items()\optiongroup\text\string +" "+ *this\items()\optiongroup\box[1]\checked
  ...
  Procedure.l CallBack(*this._S_widget, EventType.l, mouse_x.l=-1, mouse_y.l=-1)
    Protected Result, from =- 1
    Static cursor_change, LastX, LastY, Last, Down, *leave._S_widget
    ...
    With *this
      ...
      If leave
        ...
        If Not *this\canvas\mouse\buttons
          Events(*leave, #PB_EventType_MouseLeave, mouse_x, mouse_y)
      ...
      If enter
        *leave = *this
        ...
        If *this\canvas\mouse\buttons = 0
          Events(*this, #PB_EventType_MouseEnter, mouse_x, mouse_y)
          *event\leave = *this
        EndIf
      EndIf
      ...
      If *this = *event\leave 
        If EventType = #PB_EventType_LeftButtonDown
          If *event\active <> *event\leave
            _set_active_(*event\leave)
          EndIf
        EndIf
        
        If *event\leave\mouse\buttons And EventType = #PB_EventType_LeftButtonUp
          *event\leave\mouse\buttons = 0
          Result | Events(*event\leave, #PB_EventType_LeftButtonUp, mouse_x, mouse_y)
      ...
      Select *this
        Case *event\leave, *event\active
          If EventType = #PB_EventType_MouseEnter
            Result | Events(*this, #PB_EventType_MouseMove, mouse_x, mouse_y)
          Else
