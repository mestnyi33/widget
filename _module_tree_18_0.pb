...
DeclareModule Structures
  ...
  ;- - _S_scroll
  Structure _S_scroll Extends _S_point
    height.l
    width.l
...
;             If (_items_\y-_this_\y-_this_\scroll\v\page\pos) > (_this_\height[2]-_items_\height) And 
;               (_items_\y-_this_\y-_this_\scroll\v\page\pos) < (_this_\height[2])
;               _this_\scroll\y = (_items_\y-_this_\scroll\v\y) - (_this_\height[2]-_items_\height)
;               Debug ""+_items_\text\string +" "+ _this_\scroll\y
;             EndIf
            
            If (_items_\y-_this_\scroll\v\page\pos) > (_this_\y[2]-_items_\height) And 
              (_items_\y-_this_\scroll\v\page\pos) < (_this_\y[2])
              
              _this_\scroll\y = (_items_\y-_this_\scroll\v\y) ;(_items_\y-_this_\scroll\v\y) - (_this_\height[2]-_items_\height)
              Debug ""+_items_\text\string +" "+ _this_\scroll\y
            EndIf
          EndIf
  ...
  Procedure.i AddItem(*this._S_widget, Item.l, Text.s, Image.i=-1, subLevel.i=0)
    ...
    With *this
      If *this
        ...
        If Item < 0 Or Item > ListSize(\rows()) - 1
          LastElement(\rows())
          handle = AddElement(\rows()) 
          Item = ListIndex(\rows())
        Else
          handle = SelectElement(\rows(), Item)
  ...
  Procedure Events(*this._S_widget, eventtype.l, mouse_x.l=-1, mouse_y.l=-1, position.l=0)
    ...
    If Not position And *this\scroll
      ...
      If (*this\scroll\v\change Or *this\scroll\h\change)
        ...
        *this\change = Bar::SetState(*this\scroll\v, *this\scroll\y)
  ...
  Procedure.l CallBack(*this._S_widget, EventType.l, mouse_x.l=-1, mouse_y.l=-1)
    ...
    With *this
      ...
      If *this\row\count And *this = *event\enter And 
        *this\scroll\v\from =- 1 And *this\scroll\h\from =- 1 ;And Not *this\canvas\key; And Not *this\mouse\buttons
