...
Module Tree
        ...
        If ListSize(\items())
          ...
          ForEach \items()
            ...
            If \items()\draw
              ...
              ; Draw selections
              If (\items()\index = \items()\focus Or \items()\index = \items()\entered) ; \color\state;
              ...
              If \items()\childrens And \flag\buttons
                DrawingMode(#PB_2DDrawing_Default)
                Bar::Arrow(\items()\box[0]\x+(\items()\box[0]\width-6)/2,\items()\box[0]\y+(\items()\box[0]\height-6)/2, 6, Bool(Not \items()\box[0]\checked)+2, \items()\color\front[\items()\color\state], 0,0) 
              EndIf
  ...
  Procedure.i AddItem(*This._S_widget, Item.i,Text.s,Image.i=-1,Flag.i=0)
    Static *i_parent._S_items, first.i
...
;           \items()\image\y = \items()\y
  ...
  Procedure.i SetState(*This._S_widget, State.i)
    With *This
      ...
      PushListPosition(\items())
      SelectElement(\items(), State) : \items()\focus = State : \items()\entered = \items()\index : \items()\color\state = 2
      ...
      Select _type_
        ...
        Case #PB_EventType_MouseEnter  ;: Debug ""+#PB_Compiler_Line +" Мышь находится внутри итема " + _this_ +" "+ _this_\from
          ...
          If down And _this_\flag\multiselect 
            ...
            ForEach _this_\items()
              If  Not _this_\items()\hide
                If ((_this_\i_selected\index >= _this_\items()\index And _this_\from =< _this_\items()\index) Or ; верх
                    (_this_\i_selected\index =< _this_\items()\index And _this_\from >= _this_\items()\index))   ; вниз
                  
                  If _this_\items()\entered <> _this_\items()\index
                    _this_\items()\entered = _this_\items()\index
          ...
          ElseIf _this_\items()\color\state <> 2
            _this_\items()\entered = _this_\items()\index
            _this_\items()\focus = _this_\items()\index
        ...
        Case #PB_EventType_LeftButtonDown ; : Debug ""+#PB_Compiler_Line +" нажали " + _this_ +" "+ _this_\from
          ...
          If Not _this_\flag\clickselect And _this_\i_selected
            _this_\i_selected\entered =- 1
            _this_\i_selected\color\state = 0
            _this_\i_selected\focus =- 1
          EndIf
          ...
          _this_\i_selected = _this_\items()
