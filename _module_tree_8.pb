        ...
        If \text\count
          ...
          ForEach \items()
            ...
            If \flag\lines And \sublevel
              ...
              ; Vertical plot
              If \items()\first 
                start = (\items()\first\y+\items()\first\height) - \scroll\v\page\pos
...
CompilerIf #PB_Compiler_IsMainFile
  ...
  Procedure __ReDraw(Canvas)
    If StartDrawing(CanvasOutput(Canvas))
      FillMemory( DrawingBuffer(), DrawingBufferPitch() * OutputHeight(), $F6)
      
      ; PushListPosition(*List())
      ForEach *List()
        If Not *List()\hide And *List()\from =- 1
          Draw(*List())
        EndIf
      Next
      ; PopListPosition(*List())
    ...
    If StartDrawing(CanvasOutput(Canvas))
      ...
      ForEach *List()
        ...
        *List()\canvas\window = EventWindow()
        ...
        If CallBack(*List(), EventType);, MouseX, MouseY)
          Draw(*List())
        EndIf
      Next
