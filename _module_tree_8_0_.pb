        ...
        If Not _this_\items()\hide
          ...
          If _this_\change <> 0
            ...
            If _this_\items()\parent ;And Not _this_\items()\first ;parent\sublevel
              If _this_\items()\parent\first[1] 
                _this_\items()\parent\first[1]\first = 0
              EndIf
              
              _this_\items()\first = _this_\items()\parent
                _this_\items()\parent\first[1] = _this_\items()
            
            ElseIf _this_\items()\parent
              ; Debug _this_\items()\text\string
              If _this_\first[1] 
                _this_\first[1]\first = 0
              EndIf
              _this_\items()\first = _this_\first
              _this_\first[1] = _this_\items()
            EndIf
...
;                 EndIf
              ...
              ; Vertical plot
              If \items()\first 
                y_point = (\items()\first\y+\items()\first\height) - \scroll\v\page\pos
