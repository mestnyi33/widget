          ...
          ForEach \items()
            ...
            If \flag\lines And \sublevel
              ...
              If \items()\lines And Not \items()\lines\hide
                y_point = (\items()\lines\y+\items()\lines\height/2) - \scroll\v\page\pos
                start = (\items()\y+\items()\height/2) - \scroll\v\page\pos 
                
                If start < \y[1] : start = \y[1] : EndIf
                If y_point < \y[1] : y_point = \y[1] : EndIf
...
;                 EndIf
                ...
                ;If y_point >\y[2] And y_point < \y[2]+\height[2]
                  Line(Random(5) + x_point,y_point,1,start-y_point, $FF7E7E7E)
                ;EndIf
              EndIf
              
              If \items()\lines[1] 
                start = (\items()\lines[1]\y+\items()\lines[1]\height/2) - \scroll\v\page\pos
                ...
                ; DrawingMode(#PB_2DDrawing_CustomFilter) : CustomFilterCallback(@PlotY())
                Line(Random(5) + x_point+\sublevel,start,1,y_point-start, $FF7E7E7E)
              EndIf
