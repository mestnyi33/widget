; Линии здесь рисуются правильно то есть один раз
; 
      ...
      ForEach _this_\items()
        If Not _this_\items()\hide
          ...
          If _this_\change <> 0
            ...
            If _this_\flag\lines And _this_\items()\parent
              _this_\items()\parent\lines[1] = _this_\items()
            EndIf
          EndIf
        EndIf
      Next
        ...
        If \text\count
          ...
          ForEach \items()
            ...
            If \flag\lines And \sublevel
              ...
              ; Vertical plot
              If \items()\lines And Not \items()\lines\hide
                y_point = (\items()\y+\items()\height/2) - \scroll\v\page\pos
                If y_point < \y[2] : y_point = \y[2] : EndIf
                
                h_point = (\items()\lines\y+\items()\lines\height/2)-y_point - \scroll\v\page\pos
              ...
              If \items()\lines[1] And Not \items()\lines[1]\hide 
                y_point = (\items()\y+\items()\height) - \scroll\v\page\pos
                If y_point < \y[2] : y_point = \y[2] : EndIf
                
                h_point = (\items()\lines[1]\y+\items()\lines[1]\height/2)-y_point - \scroll\v\page\pos
                If h_point < 0 : h_point = 0 : EndIf
                If y_point + h_point > \y[2]+\height[2] 
                  If y_point > \y[2]+\height[2] 
                    h_point = 0
                ...
                If h_point
                  ; DrawingMode(#PB_2DDrawing_CustomFilter) : CustomFilterCallback(@PlotY())
                  Line(x_point+\sublevel,y_point,1,h_point, $FF7E7E7E)
                EndIf
              EndIf
            EndIf
              ...
              If state 
                ...
                Else
                  DrawingMode(#PB_2DDrawing_Default|#PB_2DDrawing_AlphaBlend)
                  RoundBox(\x[2],Y,\width[2],\items()\height,\items()\radius,\items()\radius,\items()\color\back[state]&$FFFFFF|140<<24)
                EndIf
