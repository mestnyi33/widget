...
DeclareModule Structures
  ...
  Structure _S_row 
    ...
    box.b ; checkbox; collapsebutton; image; text
...
DeclareModule Tree
  ...
  Declare.b Post(eventtype.l, *this._S_widget, item.l=#PB_All, *data=0)
EndDeclareModule
        ...
        If _items_\draw
          ...
          If _this_\row\sublength
            ...
            If _this_\flag\buttons
              ...
              ElseIf _this_\row\checked And _this_\row\checked\childrens
                Bar::Arrow(_this_\row\checked\box[0]\x+(_this_\row\checked\box[0]\width-6)/2,_this_\row\checked\box[0]\y+(_this_\row\checked\box[0]\height-6)/2, 6, Bool(Not _this_\row\checked\box[0]\checked)+2, _this_\row\checked\color\front[_this_\row\checked\color\state], 0,0) 
              EndIf
            EndIf
          EndIf
    ...
    Macro _callback_(_this_, _type_)
      Select _type_
        ...
        Case #PB_EventType_LeftButtonDown ; : Debug ""+#PB_Compiler_Line +" нажали " + _this_ +" "+ _this_\from
          ...
          _this_\row\checked = _this_\items()
        ...
        Case #PB_EventType_LeftButtonUp 
          If \from >= 0 And Down = *this ; SelectElement(\items(), \from)
            ...
            Else
              ...
              If Not (\items()\color\state <> 2 And \flag\buttons And \items()\childrens And
                      _from_point_(\canvas\mouse\x, \canvas\mouse\y, \items()\box[0]))
                
                If \row\box = 2
                  If Not \flag\clickselect And \row\selected
                    \row\selected\color\state = 0
                  EndIf
        ...
        Case #PB_EventType_LeftButtonDown
          ...
          If from >= 0 ; And SelectElement(\items(), from)
            ...
            If (\flag\buttons And \items()\childrens) And
              ...
              Post(#PB_EventType_Down, *this, \from, \items()\box[0]\checked)
              Result = #True
            Else
              _callback_(*this, #PB_EventType_LeftButtonDown)
            EndIf
