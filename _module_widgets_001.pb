...
;         If ScrollPos > (\max-\page\len) : ScrollPos = (\max-\page\len) : EndIf
      ...
      With *this
        ...
        If \Resize
          ...
          CompilerIf Defined(Bar, #PB_Module)
            ...
            \width[2] = *this\Scroll\h\Page\len ; x(*this\Scroll\v)-*this\Scroll\h\x ; 
            \height[2] = *this\Scroll\v\Page\len; y(*this\Scroll\h)-*this\Scroll\v\y ;
          CompilerElse
            \width[2] = \width[2]
            \height[2] = \height[2]
          CompilerEndIf
            ...
            CompilerIf Defined(Bar, #PB_Module)
              If \Scroll\v And \Scroll\v\max <> \Scroll\Height And 
                ...
                If \Text\editable And (\Items()\y >= (\height[2]-\Items()\height))
                  ...
                  SetState(\Scroll\v, (\Items()\y-((\height[2]+\Text\y)-\Items()\height)))
                EndIf
              ...
              If \Scroll\h And \Scroll\h\Max<>\Scroll\Width And 
                ...
                \width[2] = *this\Scroll\h\Page\len ; x(*this\Scroll\v)-*this\Scroll\h\x ; 
                \height[2] = *this\Scroll\v\Page\len; y(*this\Scroll\h)-*this\Scroll\v\y ;
                                                           ;  \width[2] = \width[2] - Bool(Not \Scroll\v\Hide)*\Scroll\v\Width : iwidth = \width[2]
              EndIf
  ...
  Procedure.i Draw_Scroll(*this._S_widget)
    ...
    With *this 
      ...
      ;       Debug ""+Str(\Area\Pos+\Area\len) +" "+ \button\x[2]
      ;       Debug ""+Str(\Area\Pos+\Area\len) +" "+ \button\y[2]
      ;Debug \width
      ...
      If \Thumb\len 
        ...
        If \Color[3]\back[State_3]<>-1
          If \Color[3]\Fore[State_3]
            DrawingMode( #PB_2DDrawing_Gradient|#PB_2DDrawing_AlphaBlend)
          EndIf
          boxGradient( \Vertical, \button\x[3], \button\y[3], \button\Width[3], \button\Height[3], \Color[3]\Fore[State_3], \Color[3]\Back[State_3], \Radius, \color\alpha)
        EndIf
        
        ; Draw thumb frame
        If \Color[3]\Frame[State_3]<>-1
          DrawingMode(#PB_2DDrawing_Outlined|#PB_2DDrawing_AlphaBlend)
          RoundBox( \button\x[3], \button\y[3], \button\Width[3], \button\Height[3], \Radius, \Radius, \Color[3]\Frame[State_3]&$FFFFFF|Alpha)
        EndIf
      EndIf
      ...
      If \button\Size[1]
        ...
        If \Color[1]\back[State_1]<>-1
          If \Color[1]\Fore[State_1]
            DrawingMode( #PB_2DDrawing_Gradient|#PB_2DDrawing_AlphaBlend)
          EndIf
          boxGradient( \Vertical, \button\x[1], \button\y[1], \button\Width[1], \button\Height[1], \Color[1]\Fore[State_1], \Color[1]\Back[State_1], \Radius, \color\alpha)
        EndIf
        ...
        If \Color[1]\Frame[State_1]
          RoundBox( \button\x[1], \button\y[1], \button\Width[1], \button\Height[1], \Radius, \Radius, \Color[1]\Frame[State_1]&$FFFFFF|Alpha)
        EndIf
        
        ; Draw arrows
        Arrow( \button\x[1]+( \button\Width[1]-\button\arrow_size[1])/2, \button\y[1]+( \button\Height[1]-\button\arrow_size[1])/2, \button\arrow_size[1], Bool( \Vertical),
               (Bool(Not _scroll_in_start_(*this)) * \Color[1]\Front[State_1] + _scroll_in_start_(*this) * \Color[1]\Frame[0])&$FFFFFF|Alpha, \button\arrow_type[1])
      EndIf
      ...
      If \button\Size[2]
        ...
        If \Color[2]\back[State_2]<>-1
          If \Color[2]\Fore[State_2]
            DrawingMode( #PB_2DDrawing_Gradient|#PB_2DDrawing_AlphaBlend)
          EndIf
          boxGradient( \Vertical, \button\x[2], \button\y[2], \button\Width[2], \button\Height[2], \Color[2]\Fore[State_2], \Color[2]\Back[State_2], \Radius, \color\alpha)
        EndIf
        ...
        If \Color[2]\Frame[State_2]
          RoundBox( \button\x[2], \button\y[2], \button\Width[2], \button\Height[2], \Radius, \Radius, \Color[2]\Frame[State_2]&$FFFFFF|Alpha)
        EndIf
        
        ; Draw arrows
        Arrow( \button\x[2]+( \button\Width[2]-\button\arrow_size[2])/2, \button\y[2]+( \button\Height[2]-\button\arrow_size[2])/2, \button\arrow_size[2], Bool( \Vertical)+2, 
               (Bool(Not _scroll_in_stop_(*this)) * \Color[2]\Front[State_2] + _scroll_in_stop_(*this) * \Color[2]\Frame[0])&$FFFFFF|Alpha, \button\arrow_type[2])
      EndIf
      ...
      If \Thumb\len And \Color[3]\Fore[State_3]<>-1  ; Draw thumb lines
        ...
        DrawingMode(#PB_2DDrawing_Default|#PB_2DDrawing_AlphaBlend)
        Protected size = \button\arrow_size[2]+3
        
        If \Vertical
          Line( \button\x[3]+(\button\Width[3]-(size-1))/2, \button\y[3]+\button\Height[3]/2-3,size,1, LinesColor)
          Line( \button\x[3]+(\button\Width[3]-(size-1))/2, \button\y[3]+\button\Height[3]/2,size,1, LinesColor)
          Line( \button\x[3]+(\button\Width[3]-(size-1))/2, \button\y[3]+\button\Height[3]/2+3,size,1, LinesColor)
        Else
          Line( \button\x[3]+\button\Width[3]/2-3, \button\y[3]+(\button\Height[3]-(size-1))/2,1,size, LinesColor)
          Line( \button\x[3]+\button\Width[3]/2, \button\y[3]+(\button\Height[3]-(size-1))/2,1,size, LinesColor)
          Line( \button\x[3]+\button\Width[3]/2+3, \button\y[3]+(\button\Height[3]-(size-1))/2,1,size, LinesColor)
        EndIf
      EndIf
  ...
  Procedure.i Draw_Spin(*this._S_widget)
    ...
    With *this 
      ...
      If \box\Size[2]
        ...
        ; Draw arrows
        Arrow( \box\x[1]+( \box\Width[1]-\button\arrow_size[1])/2, \box\y[1]+( \box\Height[1]-\button\arrow_size[1])/2, \button\arrow_size[1], Bool(\Vertical)*3,
               (Bool(Not _scroll_in_start_(*this)) * \Color[1]\Front[State_1] + _scroll_in_start_(*this) * \Color[1]\Frame[0])&$FFFFFF|Alpha, \button\arrow_type[1])
        
        ; Draw arrows
        Arrow( \box\x[2]+( \box\Width[2]-\button\arrow_size[2])/2, \box\y[2]+( \box\Height[2]-\button\arrow_size[2])/2, \button\arrow_size[2], Bool(Not \Vertical)+1, 
               (Bool(Not _scroll_in_stop_(*this)) * \Color[2]\Front[State_2] + _scroll_in_stop_(*this) * \Color[2]\Frame[0])&$FFFFFF|Alpha, \button\arrow_type[2])
...
;           EndIf
  ...
  Procedure.i Bar(Type.i, Size.i, Min.i, Max.i, PageLength.i, Flag.i=0, Radius.i=7, SliderLen.i=7, Parent.i=0)
    ...
    With *this
      ...
      \button\arrow_size[1] = 4
      \button\arrow_size[2] = 4
      \button\arrow_type[1] =- 1 ; -1 0 1
      \button\arrow_type[2] =- 1 ; -1 0 1
