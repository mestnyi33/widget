...
;             EndIf
  ...
  Procedure.l CallBack(*this._S_widget, EventType.l, mouse_x.l=-1, mouse_y.l=-1)
    ...
    If *this <> *this\root And Not *this\handle
      ProcedureReturn 0
    EndIf
    
    With *this
      If *this And ListSize(\childrens()) ; \countItems ; Not Root()\mouse\buttons
          Protected ParentItem = Bool(\type = #PB_GadgetType_Panel) * \index[2]
          
          PushListPosition(\childrens())    ;
          LastElement(\childrens())         ; Что бы начать с последнего элемента
          Repeat                            ; Перебираем с низу верх
            ;If Not \childrens()\hide And \childrens()\parentItem = ParentItem 
              
       If ListSize(\childrens()\childrens())
                Root()\mouse\x = 0
                Root()\mouse\y = 0
                Protected *Result = CallBack(\childrens(), EventType, mouse_x, mouse_y)
      ...
      Protected enter = Bool(*value\event\enter <> *this And Not (*value\event\enter And *value\event\enter\index > *this\index) And _from_point_(mouse_x, mouse_y, *this, [#_c_4]))
      Protected leave = Bool(*value\event\enter And (enter Or (*value\event\enter = *this And Not _from_point_(mouse_x, mouse_y, *value\event\enter, [#_c_4]))))
...
;     EndIf
    ...
    With *this
      ...
      Repaint = CallBack(*this, EventType, *this\canvas\mouse\x, *this\canvas\mouse\y)
