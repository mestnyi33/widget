...
;             EndIf
  ...
  Procedure.l CallBack(*this._S_widget, EventType.l, mouse_x.l=-1, mouse_y.l=-1)
    ...
    If *this <> *this\root And Not *this\handle ; Or *this = *this\root
      ProcedureReturn 0
    EndIf
...
;       enter = Bool(enter And *value\event\enter <> *this\parent) ; And (Not *Value\event\enter Or (*Value\event\enter And *Value\event\enter\Parent <> *this)))
;       leave = Bool(leave And *value\event\enter <> *this\parent)
      Protected Parent = \parent
      Protected Window = \window
      
     
      If *value\event\enter = *this\parent
        enter = Bool(*value\event\leave <> *this And Not (*value\event\leave And *value\event\leave\index > *this\index) And _from_point_(mouse_x, mouse_y, *this, [4]))
        leave = Bool(*value\event\leave And (enter Or (*value\event\leave = *this And Not _from_point_(mouse_x, mouse_y, *value\event\leave, [4]))))
        
        If leave And Not enter
          Debug "  leave - " + *value\event\leave;+ enter +" "+ leave
        ...
        If enter ;And Not leave
          ...
          If *value\event\leave <> *this
            
            If Not *this\root\canvas\mouse\buttons
              Result | Events(*this, #PB_EventType_MouseEnter, mouse_x, mouse_y)
            EndIf
            
            Debug "  enter - "+*this
            *value\event\leave = *this
          EndIf
        EndIf
      ...
      Select EventType 
        Case #PB_EventType_LeftButtonDown
          If *this = *value\event\leave;*value\event\enter 
...
;       EndIf
      ...
      If *this And ListSize(\childrens()) ; \countItems ; Not Root()\mouse\buttons
        Protected x,y,Width,Height,ParentItem = Bool(\type = #PB_GadgetType_Panel) * \index[2]
        
        PushListPosition(\childrens())    ;
        LastElement(\childrens())         ; Что бы начать с последнего элемента
        Repeat                            ; Перебираем с низу верх
          
            Repaint | CallBack(\childrens(), EventType, *this\root\canvas\mouse\x, *this\root\canvas\mouse\y)
            
            If ListSize(\childrens()\childrens())
              LastElement(\childrens()\childrens())         ; Что бы начать с последнего элемента
              Repeat                                        ; Перебираем с низу верх
                
                Repaint | CallBack(\childrens()\childrens(), EventType, *this\root\canvas\mouse\x, *this\root\canvas\mouse\y)
                
              Until PreviousElement(\childrens()\childrens()) = #False 
            EndIf
