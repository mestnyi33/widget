        ...
        If enter
          *value\event\enter = *this
          ...
          If Not *this\root\canvas\mouse\buttons And Not (*value\event\leave And *value\event\leave\parent = *this)
            Debug ""+*value\event\leave +" "+ *this +" "+ *value\event\enter
...
;                 EndIf
  ...
  Procedure.l ReCallback(*this._S_widget, eventtype.l, mouse_x.l, mouse_y.l)
    ...
    With *this
      ...
      If ListSize(\childrens()) ; \countItems ; Not Root()\mouse\buttons
        ;PushListPosition(\childrens())    ;
        LastElement(\childrens())         ; Что бы начать с последнего элемента
        Repeat                            ; Перебираем с низу верх
        ;ForEach \childrens()
          result | CallBack(\childrens(), eventtype, mouse_x, mouse_y)
          If ListSize(\childrens()\childrens()) And 
             ReCallback(\childrens()\childrens(), eventtype, mouse_x, mouse_y)
            Break
          EndIf
        ; Debug ""
;          If result
...
;           Next
         ; PopListPosition(\childrens())
        result | CallBack(*this, eventtype, mouse_x, mouse_y)
         EndIf
...
CompilerIf #PB_Compiler_IsMainFile
  ...
  If OpenWindow(0, 100, 100, 220, 220, "Window_0", #PB_Window_SystemMenu);, WindowID(100))
    ...
    SetData(Button(0, 20, 180, 50, "butt_1"), 200)
