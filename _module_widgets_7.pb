...
;     *second._S_widget
  ...
  Declare.l CallBack(*this, EventType.l, mouse_x.l, mouse_y.l)
  ...
  Procedure.l _draw_(*this._S_widget)
    ...
    Macro _update_(_this_, _items_)
      ...
      If _this_\scroll\v\page\len And _this_\scroll\v\max<>_this_\scroll\height-Bool(_this_\flag\gridlines) And
         SetAttribute_Bar(_this_\scroll\v, #PB_ScrollBar_Maximum, _this_\scroll\height-Bool(_this_\flag\gridlines))
        
        Resizes_Bar(_this_\scroll, #PB_Ignore, #PB_Ignore, #PB_Ignore, #PB_Ignore)
      EndIf
      ...
      If _this_\scroll\h\page\len And _this_\scroll\h\max<>_this_\scroll\width And
         SetAttribute_Bar(_this_\scroll\h, #PB_ScrollBar_Maximum, _this_\scroll\width)
      ...
      If _this_\change <> 0
        ...
        If _this_\row\selected And _this_\row\scrolled
          SetState_Bar(_this_\scroll\v, ((_this_\row\selected\y-_this_\scroll\v\y) - (Bool(_this_\row\scrolled>0) * (_this_\height[2]-_this_\row\selected\height))) ) 
    ...
    With *this
      If *this > 0
        ...
        If ListSize(\items())
          ...
          If \scroll\v And \scroll\v\page\len And \scroll\v\max<>\scroll\height And 
             SetAttribute_Bar(\scroll\v, #PB_Bar_Maximum, \scroll\height) : \scroll\v\scrollstep = \text\height
            Resizes_Bar(\scroll, 0,0, #PB_Ignore, #PB_Ignore)
          EndIf
      ...
      If *this > 0
        Select \type
          ...
          Case #PB_GadgetType_Image
            ...
            If Result
              If \scroll
                SetAttribute_Bar(\scroll\v, #PB_Bar_Maximum, \image\height)
                SetAttribute_Bar(\scroll\h, #PB_Bar_Maximum, \image\width)
  ...
  Procedure.i Area(*parent, X.i,Y.i,Width.i,Height.i, Size, mode.i=0)
    ...
    *this\v._S_widget = AllocateStructure(_S_widget)
    ...
    With *this\v
      ...
      If Height
        SetAttribute_Bar(*this\v, #PB_ScrollBar_PageLength, Height)
      EndIf
    ...
    *this\h._S_widget = AllocateStructure(_S_widget)
    ...
    With *this\h
      ...
      If Width
        SetAttribute_Bar(*this\h, #PB_ScrollBar_PageLength, Width)
      EndIf
...
;       \scroll\h = Bar(#PB_GadgetType_ScrollBar,Size,0,0,Width, 0, 7, 7, *this)
      \scroll = Area(*this, 0,0,Width,Height, size, 1)
;       SetAttribute_Bar(\scroll\v, #PB_ScrollBar_Maximum, ScrollAreaHeight)
;       SetAttribute_Bar(\scroll\h, #PB_ScrollBar_Maximum, ScrollAreaWidth)
;       SetAttribute_Bar(\scroll\v, #PB_ScrollBar_PageLength, Height)
;       SetAttribute_Bar(\scroll\h, #PB_ScrollBar_PageLength, Width)
;       
...
;       \scroll\v = Bar(#PB_GadgetType_ScrollBar,Size,0,0,Height, #PB_Vertical, 7, 7, *this)
;       \scroll\h = Bar(#PB_GadgetType_ScrollBar,Size,0,0,Width, 0, 7, 7, *this)
      \scroll = Area(*this, 0,0,0,0, size, 1)
...
;       SetAttribute_Bar(\scroll\h, #PB_ScrollBar_PageLength, Width)
  ...
  EndProcedure
   ...
   Procedure.l Callback(*this._S_widget, eventtype.l, mouse_x.l, mouse_y.l)
    Static repaint
    Protected enter, leave, NewList parentlist.i()
    Protected *parent_enter._S_widget, *parent._S_widget
      
    With *this
      enter = Bool(*value\event\enter <> *this And Not (*value\event\enter And *value\event\enter\index_ > *this\index_) And _from_point_(mouse_x, mouse_y, *this, [4]))
      leave = Bool(*value\event\enter And ((*value\event\enter = *this And Not _from_point_(mouse_x, mouse_y, *value\event\enter, [4]))))
      
      If leave
        If *value\event\enter\State 
          *value\event\enter\State = 0
      ...
      If enter
        *value\event\enter = *this
        ...
        If *value\event\enter\State = 0
          *value\event\enter\State = 1
          repaint | Events(*value\event\enter, #PB_EventType_MouseEnter, mouse_x, mouse_y)
        EndIf
      EndIf
      
      If ListSize(\childrens()) ;  \CountItems ; 
        ForEach \childrens()    ; LastElement(\childrens()) : Repeat                            ; Перебираем с низу верх
          
          enter = Bool(*value\event\enter <> \childrens() And Not (*value\event\enter And *value\event\enter\index_ > \childrens()\index_) And _from_point_(mouse_x, mouse_y, \childrens(), [4]))
          leave = Bool(*value\event\enter And *value\event\enter = \childrens() And Not _from_point_(mouse_x, mouse_y, *value\event\enter, [4]))
          
          If leave
            *parent = *value\event\enter
            ClearList(parentlist())
            While *parent And *parent\State And Not _from_point_(mouse_x, mouse_y, *parent, [4])
              *parent\State = 0
              ;Debug "  - leave"+*parent\index
              AddElement(parentlist()) 
              parentlist() = *parent
              *parent = *parent\parent
            Wend
            
            If ListSize(parentlist())
              ForEach parentlist()
                repaint | Events(parentlist(), #PB_EventType_MouseLeave, mouse_x, mouse_y)
              Next
            EndIf
          ...
          If enter
            *parent_enter = \childrens()\parent
            
            If *value\event\enter <> \root And Not (*value\event\enter And *value\event\enter\parent\index_ < *parent_enter\index_) ;Or  
               ; Not (*value\event\enter And *value\event\enter\index_ > \childrens()\index_)
             *parent = *value\event\enter
              ClearList(parentlist())
              While (*parent And *parent\State And *parent <> *parent_enter)
                *parent\State = 0
                ;Debug "  -- leave "+*parent\index
                AddElement(parentlist()) 
                parentlist() = *parent
            ...
            *parent = *value\event\enter
            ClearList(parentlist())
            While *parent And *parent\State = 0
              *parent\State = 1
              ;Debug "  -- enter "+*parent\index
              AddElement(parentlist()) 
              parentlist() = *parent
              *parent = *parent\parent
            Wend
            ...
            If ListSize(parentlist())
              LastElement(parentlist())
              Repeat
                repaint | Events(parentlist(), #PB_EventType_MouseEnter, mouse_x, mouse_y)
              Until Not PreviousElement(parentlist())
            EndIf
  ...
  EndProcedure
   ...
   Procedure.l _____Callback(*this._S_widget, eventtype.l, mouse_x.l, mouse_y.l)
    Static repaint
    Protected enter, leave, NewList parentlist.i()
    Protected *parent_enter._S_widget, *parent._S_widget
    ...
    With *this
      enter = Bool(*value\event\enter <> *this And Not (*value\event\enter And *value\event\enter\index_ > *this\index_) And _from_point_(mouse_x, mouse_y, *this, [4]))
          ...
          If enter
            *parent_enter = *this\parent
            
            If *value\event\enter <> \root And Not (*value\event\enter And *value\event\enter\parent\index_ < *parent_enter\index_) ;Or  
               ; Not (*value\event\enter And *value\event\enter\index_ > *this\index_)
             *parent = *value\event\enter
              ClearList(parentlist())
              While (*parent And *parent\State And *parent <> *parent_enter)
                *parent\State = 0
  ...
  Procedure.l ____Callback(*this._S_widget, eventtype.l, mouse_x.l, mouse_y.l)
    Static repaint
    
    With *this
     ; Debug \index
      
;       If _from_point_(mouse_x, mouse_y, *this, [4])
;         If *value\event\enter <> *this 
;           If Not *this\state : *this\state = 1
;             repaint = Events(*this, #PB_EventType_MouseEnter, mouse_x, mouse_y)
;             If repaint
;               Events(*this, #PB_EventType_Repaint, mouse_x, mouse_y)
;             EndIf
;           EndIf
;           *value\event\enter = *this
;         EndIf
...
;       EndIf
      ...
      ;If _from_point_(mouse_x, mouse_y, *this, [4])
      If Bool (mouse_x > *this\x[4] And mouse_x < (*this\x[4]+*this\width[4] - Bool(*this\width=*this\width[4])**this\bs) And 
                   mouse_y > *this\y[4] And mouse_y < (*this\y[4]+*this\height[4] - Bool(*this\height=*this\height[4])**this\bs))
                
            If *value\event\enter <> *this And Not (*value\event\enter And *value\event\enter\index_ > *this\index_) 
              
              If *this\state = 0 : *this\state = 1
                If *value\event\enter And *value\event\enter\State And 
                   *value\event\enter\parent = *this\parent
                  *value\event\enter\State = 0
                  repaint = Events(*value\event\enter, #PB_EventType_MouseLeave, mouse_x, mouse_y)
                EndIf
          ...
          ;If Not _from_point_(mouse_x, mouse_y, *this, [4]) 
            If *this\state = 1 : *this\state = 0
              If Not (*value\event\enter And *value\event\enter\index_ > *this\index_) 
                repaint = Events(*this, #PB_EventType_MouseLeave, mouse_x, mouse_y)
              EndIf
              *value\event\enter = 0
            EndIf
  ...
  Procedure.l ___Callback(*this._S_widget, eventtype.l, mouse_x.l, mouse_y.l)
    Static repaint
...
;               EndIf
      ...
      If ListSize(\childrens()) ;  \CountItems ; 
        ...
        Repeat                            ; Перебираем с низу верх
                                          ;ForEach \childrens()
          If _from_point_(mouse_x, mouse_y, \childrens(), [4]) ;And Not (*value\event\enter And _from_point_(mouse_x, mouse_y, *value\event\enter, [4]))
          ...
          Else
            If \childrens()\state : \childrens()\state = 0
              If *value\event\enter ;And *value\event\enter\color\state = 0 : *value\event\enter\color\state = 1
                repaint = Events(*value\event\enter, #PB_EventType_MouseEnter, mouse_x, mouse_y)
;               Debug " "+\childrens()\index
;               Debug  Bool(*value\event\enter And *value\event\enter\index > \childrens()\index) 
;               
;                 
                ;*value\event\leave = *value\event\enter
              EndIf
  ...
  Procedure.l __Callback(*this._S_widget, eventtype.l, mouse_x.l, mouse_y.l)
    Static repaint
      ...
      If ListSize(\childrens()) ;  \CountItems ; 
        ...
        ForEach \childrens()
          ...
          Else
            ...
            If _from_point_(mouse_x, mouse_y, \childrens(), [4]) And Not (*value\event\enter And *value\event\enter\index > \childrens()\index) 
              If *value\event\enter <> \childrens()
                If Not \childrens()\state : \childrens()\state = 1
                  If (*value\event\enter And *value\event\enter\index < \childrens()\index And \childrens()\parent = *value\event\enter\parent)
                    repaint = Events(*value\event\enter, #PB_EventType_MouseLeave, mouse_x, mouse_y)
                  EndIf
                  repaint = Events(\childrens(), #PB_EventType_MouseEnter, mouse_x, mouse_y)
                  
                  If repaint
                    Events(\childrens(), #PB_EventType_Repaint, mouse_x, mouse_y)
                  EndIf
                EndIf
                *value\event\enter = \childrens()
              EndIf
              
            ElseIf \childrens()\state : \childrens()\state = 0
              If (*value\event\enter And *value\event\enter\index < \childrens()\index And \childrens()\parent = *value\event\enter\parent)
                repaint = Events(*value\event\enter, #PB_EventType_MouseEnter, mouse_x, mouse_y)
              EndIf
              repaint = Events(\childrens(), #PB_EventType_MouseLeave, mouse_x, mouse_y)
  ...
  Procedure.l _Callback(*this._S_widget, eventtype.l, mouse_x.l, mouse_y.l)
    Static repaint
    ...
    With *this
      ...
      If ListSize(\childrens()) ; \CountItems ; 
        ForEach \childrens()
          If _from_point_(mouse_x, mouse_y, \childrens(), [4])
            If *value\event\enter <> \childrens() And Not (*value\event\enter And *value\event\enter\index > \childrens()\index)
              If Not \childrens()\state : \childrens()\state = 1
                repaint = Events(\childrens(), #PB_EventType_MouseEnter, mouse_x, mouse_y)
          ...
          ; if children have children's
          If ListSize(\childrens()\childrens()) ; \childrens()\CountItems ; 
            repaint = Callback(\childrens(), eventtype, mouse_x, mouse_y)
            
            If repaint
              Debug " repaint2 - "+repaint 
              repaint = 0
              ProcedureReturn 1
             EndIf
          EndIf
...
CompilerIf #PB_Compiler_IsMainFile ;= 100
  ...
  If OpenWindow(3, 0, 0, 995, 605, "Position de la souris sur la fenêtre", #PB_Window_SystemMenu | #PB_Window_ScreenCentered)
    If Open(3, 0, 0, 995, 605)
      ...
      Widgets(Hex(103)) = Option(10, 50, 110,20, "Option_widget" )  
      Widgets(Hex(104)) = Option(10, 60, 110,20, "Option_widget" )  
      CloseList()
      ...
      Widgets(Hex(#PB_GadgetType_ScrollArea)) = ScrollArea(335, 305, 160,95,180,90,1, #PB_ScrollArea_Flat ) 
      Widgets(Hex(201)) = Button(0, 0, 150,20, "ScrollArea_"+Hex(#PB_GadgetType_ScrollArea) ) 
      Widgets(Hex(203)) = Button(0, 10, 150,20, "ScrollArea_"+Hex(#PB_GadgetType_ScrollArea) ) 
      Widgets(Hex(204)) = Button(0, 20, 150,20, "ScrollArea_"+Hex(#PB_GadgetType_ScrollArea) ) 
      Widgets(Hex(202)) = Button(180-150, 90-20, 150,20, "Button_"+Hex(202) ) 
      CloseList()
