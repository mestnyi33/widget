    ...
    With *this
      If *this > 0
        ...
        EndIf
          ...
          If Not \vertical
          DrawingMode(#PB_2DDrawing_Transparent|#PB_2DDrawing_XOr)
          DrawRotatedText(\button[3]\x-4, \y+(\height+TextWidth("v-"+Str(\Page\Pos)+"-"+Str(\thumb\Pos-\x)))/2, "v-"+Str(\Page\Pos)+"-"+Str(\thumb\Pos-\x), Bool(Not \Vertical) * 90, \Color[3]\frame)
          ; DrawRotatedText(\x+(\width+TextHeight("A")+Bool(#PB_Compiler_OS = #PB_OS_MacOS)*2)/2, \y+(\height-TextWidth("%"+Str(\Page\Pos)))/2, "%"+Str(\Page\Pos), Bool(\Vertical) * 270, \Color[3]\frame)
        Else
          DrawingMode(#PB_2DDrawing_Transparent|#PB_2DDrawing_XOr)
          DrawText(\x+(\width-TextWidth("h-"+Str(\Page\Pos)+"-"+Str(\thumb\Pos-\y)))/2, \button[3]\y-4, "h-"+Str(\Page\Pos)+"-"+Str(\thumb\Pos-\y),\Color[3]\frame)
        EndIf
...
;         EndIf
        ...
        If \splitter And \splitter\fixed
          If \splitter\fixed = #_b_1
            \splitter\fixed[\splitter\fixed] = \thumb\pos - \area\pos - Bool(\thumb\len<>\button\len) * (\button\len - 1)
            \page\pos = 0
          EndIf
          If \splitter\fixed = #_b_2
            \splitter\fixed[\splitter\fixed] = \area\len - ((\thumb\pos+\thumb\len)-\area\pos) - Bool(\thumb\len<>\button\len) * (\button\len/2 + 1)
            \page\pos = \max
          EndIf
        EndIf
...
;         SetGadgetAttribute(Splitter_1, #PB_Splitter_SecondMinimumSize, 40)
    ...
    Splitter_4 = SplitterGadget(#PB_Any, 10, 10, 410, 210, Splitter_0, Splitter_3, #PB_Splitter_Vertical)
