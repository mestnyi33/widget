...
Module Editor
  ...
  Procedure _stop_drawing_(*this._s_widget)
    StopDrawing()    
  EndProcedure
  ...
  Macro _text_is_sel_line_(_this_)
    Bool(_this_\items()\text[2]\width And 
         _this_\root\mouse\x > _this_\items()\text[2]\x-*this\scroll\h\page\pos And
         _this_\root\mouse\y > _this_\items()\text\y-*this\scroll\v\page\pos And 
         _this_\root\mouse\y < (_this_\items()\text\y + _this_\items()\text\height)-*this\scroll\v\page\pos And
         _this_\root\mouse\x < (_this_\items()\text[2]\x + _this_\items()\text[2]\width)-*this\scroll\h\page\pos)
  EndMacro
  ...
  Procedure.l _text_caret_(*this._s_widget)
    ...
    MouseX = *this\root\mouse\x - (*this\items()\text\x-*this\scroll\h\page\pos)
    
    ; Get caret pos
    For i = 0 To *this\items()\text\len
      X = TextWidth(Left(*this\items()\text\string, i))
  ...
  Procedure _text_sel_(*this._s_widget, _pos_, _len_)
    ...
    *this\items()\text[3]\len = *this\items()\text\len-*this\items()\text[3]\pos
    ...
    If *this\items()\text[2]\len > 0
      ...
      *this\items()\text[2]\width = TextWidth(*this\items()\text[2]\string) + _caret_last_len_
    Else
      *this\items()\text[2]\string = ""
      *this\items()\text[2]\width = _caret_last_len_
    EndIf
    ...
    CompilerIf #PB_Compiler_OS = #PB_OS_MacOS
      ...
      If *this\items()\text[2]\width And Not (*this\index[1] = *this\index[2] And *this\text\caret\stop > *this\text\caret\start) And
         *this\items()\text[2]\width <> *this\items()\text\width - (*this\items()\text[1]\width+*this\items()\text[3]\width) + _caret_last_len_
        *this\items()\text[2]\width = *this\items()\text\width - (*this\items()\text[1]\width+*this\items()\text[3]\width) + _caret_last_len_
      EndIf
    CompilerEndIf
    ...
    *this\items()\text[3]\x = (*this\items()\text[2]\x+*this\items()\text[2]\width)
    ...
    ; set cursor pos
    *this\text\caret\y = *this\items()\text\y
    *this\text\caret\height = *this\items()\text\height - 1
    
    If *this\index[1] > *this\index[2] Or
       (*this\index[1] = *this\index[2] And *this\text\caret\stop > *this\text\caret\start)
      *this\text\caret\x = *this\items()\text[3]\x
    Else
      *this\text\caret\x = *this\items()\text[2]\x
    EndIf
  ...
  Procedure _text_sel_set_(*this._s_widget, _line_, _caret_) ; Ok
    Protected Repaint.l, Pos.l, Len.l, _last_line_
    
    If *this\text\caret\stop <> _caret_
      *this\text\caret\stop = _caret_
      Repaint =- 1
    EndIf
    ...
    If *this\index[1] <> _line_ 
      ...
      If *this\index[2] = *this\index[1]
        ...
        If *this\index[2] > _line_
          ; to top
          Pos = 0
          Len = *this\text\caret\start    
        Else
          ; to bottom
          Pos = *this\text\caret\start
    ...
    If Repaint
      If *this\index[2] = _line_
        If *this\text\caret\start = *this\text\caret\stop 
          Pos = *this\text\caret\start
          ; Если выделяем справо на лево
        ElseIf *this\text\caret\start > *this\text\caret\stop 
          ; |<<<<<< to left
          Pos = *this\text\caret\stop 
          Len = (*this\text\caret\start-Pos)
        Else 
          ; >>>>>>| to right
          Pos = *this\text\caret\start
          Len = (*this\text\caret\stop-Pos)
        EndIf
      ...
      If Repaint > 0
        ...
        *this\change = _bar_update_(*this\scroll\v, *this\items()\y, *this\items()\height)
        ...
        ForEach *this\items()
          ...
          ElseIf (*this\items()\text[2]\width <> 0 And *this\index[2] <> *this\items()\index And _line_ <> *this\items()\index)
            
            ; Сброс выделения целых строк
            _text_sel_reset_(*this\items())
  ...
  Procedure _text_set_selector_(*this._s_widget, _line_, _caret_=-1)
    Protected Repaint.b
      ...
      If _start_drawing_(*this)
        ...
        If *this\text\caret\line
          ...
          If _text_is_sel_line_(*this)
            If *this\text\caret\start <> *this\items()\text[1]\len
              *this\text\caret\start = *this\items()\text[1]\len
              *this\text\caret\stop = *this\items()\text[1]\len+*this\items()\text[2]\len
              
              If _caret_ < *this\items()\text[1]\len+*this\items()\text[2]\len/2
                _caret_ = *this\items()\text[1]\len
              Else
                _caret_ = *this\items()\text[1]\len+*this\items()\text[2]\len
              EndIf
              
              Repaint =- 1
            EndIf
          Else
            If *this\text\caret\start <> _caret_
              *this\text\caret\start = _caret_
              *this\text\caret\stop = _caret_
              Repaint =- 1
            EndIf
          EndIf
          ...
          If Repaint 
            ...
            *this\text\caret\y = *this\items()\text\y
            *this\text\caret\height = *this\items()\text\height - 1
            *this\text\caret\x = *this\items()\text\x + TextWidth(Left(*this\items()\text\string, _caret_))
          EndIf
  ...
  Procedure.i _text_sel_start_(*this._s_widget)
    Protected result.i, i.i, char.i
    
    With *this
      ; |<<<<<< left edge of the word 
      If \text\caret\stop > 0 
        For i = \text\caret\stop - 1 To 1 Step - 1
  ...
  Procedure.i _text_sel_stop_(*this._s_widget)
    Protected result.i, i.i, char.i
    
    With *this
      ; >>>>>>| right edge of the word
      For i = \text\caret\stop + 2 To \items()\text\len
        char = Asc(Mid(\items()\text\string.s, i, 1))
        If _text_sel_end_(char)
          Break
        EndIf
        ...
        For i = 1 To Len 
          ...
          If Input
            If \type = #PB_GadgetType_IPAddress
              left.s = Left(\text\string, \text\caret\stop )
              ...
              EndSelect                                           
              count = Len(left.s+Trim(StringField(Mid(\text\string, \text\caret\stop +1), 1, "."), #LF$))
              If count < 3 And (Val(left.s) > 25 Or Val(left.s+Chr.s) > 255)
                Continue
                ;               ElseIf Mid(\text\string, \text\caret\stop + 1, 1) = "."
                ;                 \text\caret\stop + 1 : \text\caret\start=\text\caret\stop 
              EndIf
            EndIf
            
            If Not Dot And Input = '.' And Mid(\text\string, \text\caret\stop + 1, 1) <> "."
              Dot = 1
            ...
            If Not Minus And Input = '-' And Mid(\text\string, \text\caret\stop + 1, 1) <> "-"
              Minus = 1
            ElseIf Input <> '-'
              Minus = 0
            Else
  ...
  Procedure.l text_scroll(*this._s_widget, Width.l)
    ...
    With *this
      ...
      If SelectElement(\items(), \text\_scroll_line_index) ;And \items()\text\x+\items()\text\width > \items()\x+\items()\width
        ...
        Protected mouse_x.i = \root\mouse\x-(\items()\text\x-\scroll\h\page\pos)
            ...
            If ExamineRegularExpression(0, string_out)
              While NextRegularExpressionMatch(0)  ; 239
                ...
                If AddElement(\items())
                  ...
                  ;                       \items()\text[1]\string.s = Left(\items()\text\string.s, \text\caret\stop ) : \items()\text[1]\change = #True
                  ;                       \items()\text[3]\string.s = Right(\items()\text\string.s, \items()\text\len-(\text\caret\stop + \items()\text[2]\len)) : \items()\text[3]\change = #True
                  ;                     EndIf
                EndIf
  ...
  Procedure.b text_paste(*this._s_widget, Chr.s, Count.l=0)
    ...
    With *this
      If \index[1] <> \index[2] ; Это значить строки выделени
        ...
        If Count
          \index[2] + Count
          \text\caret\stop = Len(StringField(Chr.s, 1 + Count, #LF$))
        ElseIf Chr.s = #LF$ ; to return
          \index[2] + 1
          \text\caret\stop = 0
        Else
          SelectElement(\items(), \index[2])
          ;Debug " sss "+\index[2]+" "+\items()\text\string
          \text\caret\stop = \items()\text[1]\len + Len(Chr.s)
        EndIf
        
        ; reset items selection
        PushListPosition(*this\items())
        ForEach *this\items()
          If *this\items()\text[2]\width <> 0 
            _text_sel_reset_(*this\items())
          EndIf
        Next
        PopListPosition(*this\items())
        
        \text\caret\start = \text\caret\stop 
        \index[1] = \index[2]
    ...
    With *this
      ...
      If Chr.s
        ...
        If Not text_paste(*this, Chr.s, Count)
          If \items()\text[2]\len 
            If \text\caret\stop > \text\caret\start : \text\caret\stop = \text\caret\start : EndIf
      ...
      With *this
        ...
        If (\text\change Or \resize)
          ...
          If \text\change And \index[2] >= 0 And \index[2] < ListSize(\items())
            ...
            If (\items()\y+\items()\height >= \height[2])
              ; This is for the editor widget when you enter the key - (enter & backspace)
              Bar::SetState(\scroll\v, ((\items()\y+\items()\height)-(\height[2]+\text\y)))
            EndIf
        ...
        If \text\editable And \countitems
          If \text\change =- 1
            ...
            _text_sel_(*this, \text\caret\stop , 0)
        ...
        If \countitems
          ...
          ForEach \items()
            ; Is visible lines ---
            Drawing = Bool(Not \items()\hide And \items()\y+\items()\height-*this\scroll\v\page\pos>*this\y[2] And (\items()\y-*this\y[2])-*this\scroll\v\page\pos<iheight)
            
            Height = \items()\height
            Y = \items()\y-*this\scroll\v\page\pos
            Text_X = \items()\text\x-*this\scroll\h\page\pos
            Text_Y = \items()\text\y-*this\scroll\v\page\pos
            ...
            If Drawing 
              ...
              If \items()\text[2]\width And *this\color\front <> *this\row\color\front[2]
                ...
                CompilerIf #PB_Compiler_OS = #PB_OS_MacOS
                  ...
                  If (*this\index[1] > *this\index[2] Or 
                    ...
                    DrawRotatedText(Text_X, Text_Y, \items()\text\string.s, angle, *this\row\color\front[*this\row\color\state])
                    
                    If *this\row\color\fore[2]
                      DrawingMode(#PB_2DDrawing_Gradient|#PB_2DDrawing_AlphaBlend)
                      _box_gradient_(\items()\Vertical,\items()\text[2]\x-*this\scroll\h\page\pos, Y, \items()\text[2]\width, Height, *this\row\color\fore[2], *this\row\color\back[2], \items()\round)
                    Else
                      DrawingMode(#PB_2DDrawing_Default|#PB_2DDrawing_AlphaBlend)
                    ...
                    If \items()\text[2]\string.s
                      DrawingMode(#PB_2DDrawing_Transparent|#PB_2DDrawing_AlphaBlend)
                      DrawRotatedText(\items()\text[2]\x-*this\scroll\h\page\pos, Text_Y, \items()\text[2]\string.s, angle, *this\row\color\front[2])
                    EndIf
                  ...
                  Else
                    ...
                    If \items()\text[3]\string.s
                      DrawingMode(#PB_2DDrawing_Transparent|#PB_2DDrawing_AlphaBlend)
                      DrawRotatedText(\items()\text[3]\x-*this\scroll\h\page\pos, Text_Y, \items()\text[3]\string.s, angle, *this\row\color\front[*this\row\color\state])
                    EndIf
                    
                    If \items()\text[2]\string.s
                      DrawingMode(#PB_2DDrawing_Transparent|#PB_2DDrawing_AlphaBlend)
                      DrawRotatedText(Text_X, Text_Y, \items()\text[1]\string.s+\items()\text[2]\string.s, angle, *this\row\color\front[2])
                    EndIf
                    
                    If \items()\text[1]\string.s
                      DrawingMode(#PB_2DDrawing_Transparent|#PB_2DDrawing_AlphaBlend)
                      DrawRotatedText(Text_X, Text_Y, \items()\text[1]\string.s, angle, *this\row\color\front[*this\row\color\state])
                    EndIf
                  EndIf
                ...
                CompilerElse
                  ...
                  If \items()\text[1]\string.s
                    DrawRotatedText(\items()\text[1]\x-*this\scroll\h\page\pos, \items()\text\y-*this\scroll\v\page\pos, \items()\text[1]\string.s, angle, *this\row\color\front[*this\row\color\state])
                  EndIf
                  If \items()\text[2]\string.s
                    DrawRotatedText(\items()\text[2]\x-*this\scroll\h\page\pos, \items()\text\y-*this\scroll\v\page\pos, \items()\text[2]\string.s, angle, *this\row\color\front[2])
                  EndIf
                  If \items()\text[3]\string.s
                    DrawRotatedText(\items()\text[3]\x-*this\scroll\h\page\pos, \items()\text\y-*this\scroll\v\page\pos, \items()\text[3]\string.s, angle, *this\row\color\front[*this\row\color\state])
                  EndIf
                CompilerEndIf
              ...
              Else
                ...
                If \color\state = 2
                  DrawingMode(#PB_2DDrawing_Transparent)
                  DrawRotatedText(Text_X, Text_Y, \items()\text[0]\string.s, angle, *this\row\color\front[2])
                Else
                  DrawingMode(#PB_2DDrawing_Transparent)
                  DrawRotatedText(Text_X, Text_Y, \items()\text[0]\string.s, angle, *this\row\color\front[*this\row\color\state])
                EndIf
              EndIf
              ...
              If *this\sci\margin\width
                DrawingMode(#PB_2DDrawing_Transparent)
                DrawText(*this\sci\margin\width-TextWidth(Str(\items()\index))-3, \items()\y-*this\scroll\v\page\pos, Str(\items()\index), *this\sci\margin\color\front)
              EndIf
            EndIf
          Next
          PopListPosition(*this\items()) ; 
        ...
        If *this\text\editable ; And *this\color\state
          ...
          Line(*this\text\caret\x-*this\scroll\h\page\pos, *this\text\caret\y-*this\scroll\v\page\pos, 1, *this\text\caret\height, $FFFFFFFF)
        EndIf
  ...
  Procedure.i SetState(*this._s_widget, State.i)
    ...
    With *this
      ...
      If State <> #PB_Ignore
        ...
        \items()\text[1]\string = Left(\items()\text\string, \text\caret\stop )
        \items()\text[1]\change = 1
        \text\caret\start = \text\caret\stop 
    ...
    With *this
      ...
      ForEach \items()
        If \items()\index[1] = \items()\index
          Result = \items()\text\pos + \text\caret\stop 
        EndIf
      Next
      PopListPosition(\items())
  ...
  Procedure.i events_key_editor(*this._S_widget, eventtype.l, mouse_x.l, mouse_y.l) ; Editable(*this._s_widget, EventType.i)
    ...
    Protected _line_, _step_ = 1, _caret_min_ = 0, _caret_max_ = *this\items()\text\len, _line_first_ = 0, _line_last_ = *this\countitems - 1
    ...
    With *this
      ...
      Select EventType
        ...
        Case #PB_EventType_KeyDown
          Select *this\root\keyboard\key
            Case #PB_Shortcut_Home : *this\text\caret\start = 0
              If _key_control_
                *this\index[2] = 0 
              EndIf
              Repaint = _text_set_selector_(*this, *this\index[2], *this\text\caret\start)
              
            Case #PB_Shortcut_End : *this\text\caret\start = *this\text\len
              If _key_control_
            ...
            Case #PB_Shortcut_Up     ; Ok
              If *this\index[1] > _line_first_
                If _caret_last_pos_
                  If Not *this\root\keyboard\key[1] & #PB_Canvas_Alt 
                    *this\text\caret\stop = _caret_last_pos_
                    *this\text\caret\start = _caret_last_pos_
                  EndIf
                  _caret_last_pos_ = 0
                EndIf
                ...
                If _key_shift_
                  ...
                  Else
                    Repaint = _text_set_selector_(*this, *this\index[1] - _step_, *this\text\caret\stop)  
                  EndIf
                ElseIf *this\root\keyboard\key[1] & #PB_Canvas_Alt 
                  If *this\text\caret\stop <> _caret_min_ 
                    *this\text\caret\start = _caret_min_
                  Else
                    *this\index[2] - _step_ 
                  EndIf
              ...
              ElseIf *this\index[1] = _line_first_
                ...
                If *this\text\caret\stop <> _caret_min_ : *this\text\caret\start = _caret_min_ : _caret_last_pos_ = *this\text\caret\stop
                  Repaint = _text_set_selector_(*this, _line_first_, *this\text\caret\start)  
                EndIf
            ...
            Case #PB_Shortcut_Down   ; Ok
              If *this\index[1] < _line_last_
                ...
                ElseIf *this\root\keyboard\key[1] & #PB_Canvas_Alt 
                  If *this\text\caret\stop <> _caret_max_ 
                    *this\text\caret\start = _caret_max_
                  Else
                    *this\index[2] + _step_ 
                    ...
                    If SelectElement(*this\items(), *this\index[2]) 
                      ...
                      If *this\text\caret\stop <> _caret_max_
                        *this\text\caret\start = _caret_max_
                        ...
                        Debug ""+#PB_Compiler_Procedure + "*this\text\caret\stop <> _caret_max_"
                      EndIf
                    EndIf
                  EndIf
                ...
                Else
                  If _key_control_
                    ...
                    *this\text\caret\start = *this\text\len
                  Else
                    *this\index[2] + _step_
                  EndIf
              ...
              ElseIf *this\index[1] = _line_last_
                ...
                If *this\text\caret\stop <> _caret_max_ : *this\text\caret\start = _caret_max_ : _caret_last_pos_ = *this\text\caret\stop
            ...
            Case #PB_Shortcut_Left   ; Ok
              If _key_shift_        
                ...
                Else
                  _line_ = *this\index[1] - Bool(*this\index[1] > _line_first_ And *this\text\caret\stop = _caret_min_) * _step_
                  
                  ; коректируем позицию коректора
                  If *this\items()\index <> _line_ And
                     SelectElement(*this\items(), _line_) 
                  EndIf
                  If *this\text\caret\stop > *this\items()\text\len
                    *this\text\caret\stop = *this\items()\text\len
                  EndIf
                  
                  If *this\index[1] <> _line_
                    Repaint = _text_set_selector_(*this, _line_, *this\items()\text\len)  
                  ElseIf *this\text\caret\stop > _caret_min_
                    Repaint = _text_set_selector_(*this, _line_, *this\text\caret\stop - _step_)  
                  EndIf
                EndIf
              ...
              ElseIf *this\index[1] > _line_first_
                ...
                Else
                  ...
                  Else
                    ...
                    Else
                      *this\text\caret\start = *this\text\caret\stop - _step_ 
                    EndIf
                    ...
                    If *this\text\caret\stop = _caret_min_
                      *this\index[2] - _step_
                      ...
                      If SelectElement(*this\items(), *this\index[2]) 
                        *this\text\caret\stop = *this\items()\text\len
                        *this\text\caret\start = *this\items()\text\len
                      EndIf
                    EndIf
                  EndIf
            ...
            Case #PB_Shortcut_Right  ; Ok
              If _key_shift_       
                ...
                Else
                  ...
                  If *this\text\caret\stop > _caret_max_
                    *this\text\caret\stop = _caret_max_
                  EndIf
                  
                  _line_ = *this\index[1] + Bool(*this\index[1] < _line_last_ And *this\text\caret\stop = _caret_max_) * _step_
                  
                  ; если дошли в конец строки,
                  ; то переходим в начало
                  If *this\index[1] <> _line_ 
                    Repaint = _text_set_selector_(*this, _line_, 0)  
                  ElseIf *this\text\caret\stop < _caret_max_
                    Repaint = _text_set_selector_(*this, _line_, *this\text\caret\stop + _step_)  
                  EndIf
                EndIf
              ...
              ElseIf *this\index[1] < _line_last_
                If *this\root\keyboard\key[1] & #PB_Canvas_Alt 
                  *this\text\caret\start = _text_sel_stop_(*this)
            ...
            Case #PB_Shortcut_Back   
              ...
              *this\root\keyboard\input = 65535
              ...
              If Not text_cut(*this)
                ...
                ElseIf \text\caret\start > 0 : \text\caret\stop - 1 
                  \items()\text[1]\string.s = Left(\items()\text\string.s, \text\caret\stop )
                  \items()\text[1]\len = Len(\items()\text[1]\string.s) : \items()\text[1]\change = 1
                  
                  \items()\text\string.s = \items()\text[1]\string.s + \items()\text[3]\string.s
                  \items()\text\len = \items()\text[1]\len + \items()\text[3]\len : \items()\text\change = 1
                  
                  \text\string.s = Left(\text\string.s, \items()\text\pos+\text\caret\stop ) + \text[3]\string
                  \text\change =- 1 ; - 1 post event change widget
                Else
                  ; Если дошли до начала строки то 
                  ; переходим в конец предыдущего итема
                  If \index[1] > _line_first_ 
                    \text\string.s = RemoveString(\text\string.s, #LF$, #PB_String_CaseSensitive, \items()\text\pos+\text\caret\stop, 1)
                    
                    ;to up
                    \index[1] - 1
                    \index[2] - 1
                    
                    If *this\items()\index <> \index[2] And
                       SelectElement(*this\items(), \index[2]) 
                    EndIf
            ...
            Case #PB_Shortcut_Delete 
              If Not text_cut(*this)
                ...
                ElseIf \text\caret\start < \items()\text\len 
                  \items()\text[3]\string.s = Right(\items()\text\string.s, \items()\text\len - \text\caret\stop - 1)
                  \items()\text[3]\len = Len(\items()\text[3]\string.s) : \items()\text[3]\change = 1
                  
                  \items()\text\string.s = \items()\text[1]\string.s + \items()\text[3]\string.s
                  \items()\text\len = \items()\text[1]\len + \items()\text[3]\len : \items()\text\change = 1
                  
                  \text[3]\string = Right(\text\string.s, \text\len - (\items()\text\pos + \text\caret\stop ) - 1)
      ...
      ;         
      ;         If \text\caret\stop < \text\caret\start
      ;           ; Debug \text\caret\start-\text\caret\stop 
      ;           _text_sel_(*this, \text\caret\stop , \text\caret\start-\text\caret\stop )
      ;         Else
      ;           ; Debug \text\caret\stop -\text\caret\start
      ;           _text_sel_(*this, \text\caret\start, \text\caret\stop-\text\caret\start)
      ;         EndIf
      ;         
      ;         _stop_drawing_(*this)
      ;       EndIf                                                  
            ...
            Select EventType 
              ...
              Case #PB_EventType_LeftButtonDown
                ...
                If _line_ >= 0 And 
                   ...
                   _line_ <> \items()\index And 
                   SelectElement(\items(), _line_) 
                EndIf
                
                If *this\text\caret\line And 
                   *this\text\caret\line = \items() And
                   (ElapsedMilliseconds() - *this\text\caret\time) < 500
                  *this\text\caret\line = #Null
                  *this\text\caret\start = 0
                  Repaint = _text_set_selector_(*this, _line_, \items()\text\len)
...
;                    
                  ...
                  If *this\text\caret\start = *this\text\caret\stop 
                    ...
                    If *this\index[2] = *this\index[1] And *this\text\caret\start > *this\text\caret\line\text[2]\pos + *this\text\caret\line\text[2]\len
                      *this\text\caret\start - *this\text\caret\line\text[2]\len
                    EndIf
                    ; Debug ""+*this\text\caret\start +" "+ *this\text\caret\line\text[2]\pos
                    
                    *this\text\caret\line\text\string = RemoveString(*this\text\caret\line\text\string, *this\text\caret\line\text[2]\string, #PB_String_CaseSensitive, *this\text\caret\line\text[2]\pos, 1)
                    *this\text\string = RemoveString(*this\text\string, *this\text\caret\line\text[2]\string, #PB_String_CaseSensitive, *this\text\caret\line\text\pos+*this\text\caret\line\text[2]\pos, 1)
                    
                    *this\items()\text\string = InsertString(*this\items()\text\string, *this\text\caret\line\text[2]\string, *this\text\caret\start+1)
                    *this\text\string = InsertString(*this\text\string, *this\text\caret\line\text[2]\string, *this\items()\text\pos+*this\text\caret\start+1)
                    
;                     text_insert(*this, *this\text\caret\line\text[2]\string)
                    
                    Debug *this\text\caret\line\index
;                     *this\index[1] = *this\text\caret\line\index
;                     *this\index[2] = *this\text\caret\line\index
;                     Protected len = *this\text\caret\line\text[2]\len
;                     ;
;                     _line_ = *this\text\caret\line\index
;                     If _line_ >= 0 And 
;                      _line_ < \countitems And 
;                      _line_ <> \items()\index And 
;                      SelectElement(\items(), _line_) 
;                   EndIf
;                           
                    Debug *this\text\caret\line\text\string
                    
                    If *this\index[2] <> *this\index[1]
                    ; *this\text\change =- 1
                      _text_sel_reset_(*this\text\caret\line)
...
;                     
                    ...
                    *this\text\caret\stop = *this\text\caret\line\text[2]\len
                    
                    ;Swap *this\text\caret\stop, *this\text\caret\start
                    *this\text\caret\line = 0
                    
                     Repaint = _text_sel_(*this, *this\text\caret\start, *this\text\caret\stop)
                    ;Repaint = _text_sel_set_(*this, *this\index[1], *this\text\caret\start); + *this\text\caret\line\text[2]\len)
                     
                    SetGadgetAttribute(*this\root\canvas, #PB_Canvas_Cursor, #PB_Cursor_IBeam)
                  Else
                    *this\text\caret\start = _text_caret_(*this)
                    *this\items()\text[2]\len = 0
                    *this\index[2] = _line_
                    Repaint = _text_sel_set_(*this, _line_, *this\text\caret\start + *this\text\caret\line\text[2]\len)
                  EndIf
          ...
          Select EventType()
            Case #PB_EventType_LeftClick
              ...
              If EventGadget() = 100
                ...
                ForEach *E\items()
                  If *E\items()\text[2]\width
                    Debug ""+*E\items()\index +" - "+ *E\items()\text[2]\string
                  EndIf
                Next
