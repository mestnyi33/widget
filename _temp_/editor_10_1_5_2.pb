...
Module Editor
  ...
  Macro _bar_update_(_this_, _pos_, _len_)
    Bool(Bool((_pos_-_this_\y-_this_\page\pos) < 0 And Scroll::SetState(_this_, (_pos_-_this_\y))) Or
         Bool((_pos_-_this_\y-_this_\page\pos) > (_this_\page\len-_len_) And
              Scroll::SetState(_this_, (_pos_-_this_\y) - (_this_\page\len-_len_)))) ; : _this_\change = 0
  EndMacro
  ...
  Procedure _text_sel_change_(*this._s_widget, _line_, _caret_) ; Ok
    ...
    If Repaint
      ...
      ; set cursor pos
      If *this\index[2] < _line_ Or 
         (*this\index[2] = _line_ And *this\text\caret\pos > *this\text\caret\end)
  ...
  Procedure.i ToLeft(*this._s_widget) ; Ok
    ...
    With *this
     If *this\index[1] > _end_scroll_
        If *this\root\keyboard\key[1] & #PB_Canvas_Shift
          ;Debug *this\text\caret\pos
          Repaint = _text_set_selector_(*this, *this\index[1] - Bool(*this\text\caret\pos = _end_caret_) * _step_, *this\text\caret\pos - _step_)  
  ...
  Procedure.i ToRight(*this._s_widget) ; Ok
    ...
    With *this
      If *this\index[1] < _end_scroll_
        If *this\root\keyboard\key[1] & #PB_Canvas_Shift
          If *this\text\caret\pos = _end_caret_ And *this\items()\index <> *this\index[1] + 1 And
             SelectElement(*this\items(), *this\index[1] + 1) 
             _end_caret_ = 0;*this\items()\text\len
            *this\text\caret\pos = 0
          EndIf
          
          Debug ""+*this\items()\text\len +" "+ *this\items()\text\string ; Bool(*this\text\caret\pos = _end_caret_)
          Repaint = _text_set_selector_(*this, *this\index[1] + Bool(*this\text\caret\pos = _end_caret_) * _step_, *this\text\caret\pos + _step_)  
    ...
    If Not *this\hide
      ...
      With *this
        If ListSize(\items())
          ...
          ForEach \items()
            ; Is visible lines ---
            Drawing = Bool(\items()\y+\items()\height+*this\scroll\y>*this\y[2] And (\items()\y-*this\y[2])+*this\scroll\y<iheight)
            ...
            If Drawing
              ...
              If \items()\image\handle
                ...
                DrawAlphaImage(\items()\image\handle, \items()\image\x+*this\scroll\x, \items()\image\y+*this\scroll\y, *this\row\color\alpha)
              EndIf
              ...
              If *this\sci\margin\width
                DrawingMode(#PB_2DDrawing_Transparent)
                DrawText(*this\sci\margin\width-TextWidth(Str(\items()\index))-3, \items()\y+*this\scroll\y, Str(\items()\index), *this\sci\margin\color\front)
              EndIf
            EndIf
          Next
          PopListPosition(*this\items()) ; 
          ...
          If Not \hide And Not \Disable And \interact
            ...
            If \root\mouse\buttons
              If \root\mouse\y < \y
                _line_ =- 1
              Else
                _line_ = ((\root\mouse\y-\y-\text\y-\scroll\y) / \text\height)
              EndIf
            EndIf
...
; Folding = ------------------------------------------04-G0v-rr4-0uP-8--28-+-474----------------------------------------
; EnableXP