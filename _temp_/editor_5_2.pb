...
;     ProcedureReturn 777
    ...
    With *this
      ...
      If \index[1] <> Line And Line =< ListSize(\items())
        If (\index[1] >= 0 And \index[1] < ListSize(\items())) ; isItem(\index[1], \items()) 
          ...
          If \index[1] > Line
            \text\caret\pos = 0
          Else
            \text\caret\pos = \items()\text\len ; - Bool(Not \items()\text\len) ; Выделение пустой строки
          EndIf
      ...
      If (Line >= 0 And Line < ListSize(\items())) ; isItem(Line, \items()) 
      ...
      ForEach \items()
        ...
        If Bool((_selected_index_ > \items()\index And _index_ < \items()\index) Or   ; верх
          ...
          text_setsel_change(*this, 0, (\items()\text\len - Bool(Not \items()\text\len)))
          Repaint = #True
          
        ElseIf (\items()\text[2]\len And _selected_index_ <> \items()\index And _index_ <> \items()\index)
          
          text_setsel_change(*this, 0, 0)
