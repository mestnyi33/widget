        ...
        If ListSize(*this\items())
          ...
          ForEach *this\items()
            ...
            If Drawing
              ...
              ; Draw string
              If \text[2]\width ; And *this\color\front <> *this\row\color\front[2]
      ...
      If StartDrawing(CanvasOutput(\root\canvas)) 
        ...
        ForEach \items()
          If Bool((_selected_index_ >= \items()\index And _index_ =< \items()\index) Or ; верх
            ...
            If \items()\text[2]\change : \items()\text[2]\change = #False 
              \items()\text[2]\x = \items()\text\x+\items()\text[1]\width
              \items()\text[2]\width = TextWidth(\items()\text[2]\string.s) + (Bool(\items()\text\len = \items()\text[2]\len) * *this\flag\fullSelection) ; TextWidth() - bug in mac os
              \items()\text[3]\x = \items()\text[2]\x+\items()\text[2]\width
            EndIf 
            
            
          ElseIf \items()\text[2]\width
            \items()\text[2]\len = 0   
