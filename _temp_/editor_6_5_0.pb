...
Module Editor
  ...
  Procedure.i text_setsel_change(*this._s_widget, Pos.i, Len.i)
    With *this
      \items()\text[2]\pos = Pos
      \items()\text[2]\len = Len
      ...
      If (\text[1]\change Or \text[3]\change)
        If \text[1]\change
          \text[1]\len = (\items()\text\pos + \items()\text[1]\len)
          \text[1]\string.s = Left(\text\string.s, \text[1]\len) 
          \text[2]\pos = \text[1]\len
        EndIf
        
        If \text[3]\change
          \text[3]\pos = (\items()\text\pos + \items()\text[3]\pos)
  ...
  Procedure text_setsel(*this._s_widget) ; Ok
    ...
    With *this
      ...
      If (Caret <> \text\caret\pos Or Line <> \index[1] Or (\text\caret\end >= 0 And Caret1 <> \text\caret\end))
        ...
        text_setsel_change(*this, Pos, Len)
      ...
      With *this
        ...
        If \text\editable And ListSize(\items())
          If \text\change =- 1
            ...
            \text[3]\change = 1
            \text\len = Len(\text\string.s)
            text_setsel_change(*this, \text\caret\pos , 0)
          ...
          ForEach *this\items()
            ...
            If Drawing
              ...
              If \text[2]\len ; And *this\color\front <> *this\row\color\front[2]
                ...
                DrawingMode(#PB_2DDrawing_Default|#PB_2DDrawing_AlphaBlend)
                Box(\text[2]\x+*this\scroll\x, Y, \text[2]\width, Height, RowBackColor(*this, 2))
    ...
    With *this
      If Shift
        ...
        If \index[1] = \index[2]
          ...
          text_setsel_change(*this, \text\caret\end, \items()\text\len-\text\caret\end)
        Else
          SelectElement(\items(), \index[2]) 
          text_setsel_change(*this, 0, \items()\text\len)
        EndIf
        ; Debug \text\caret\end
        \index[2] + 1
        ...
        ;         \text\caret\end = \text\caret\pos 
        SelectElement(\items(), \index[2]) 
        text_setsel_change(*this, 0, \text\caret\end) 
        text_setsel(*this)
        Repaint = 1 
  ...
  Procedure editor_setsel(*this._s_widget, Line.i)
    ...
    With *this
      Static _line_=-1, _selected_line_=-1
      Protected _caret_, Pos, len, Selection
      Protected _index_ = Line
      Protected _selected_index_ = \index[2]
      
      If \drag[1] <> \drag
        \drag[1] = StartDrawing(CanvasOutput(\root\canvas)) 
      ; Debug "---"
      ...
      If \drag[1]
        CallCFunctionFast(\drag[1])
        ; *this\flag\fullSelection =- 1
        Protected Selectionlen = *this\flag\fullSelection
        ;               If *this\flag\fullSelection =- 1
        ;                 Selectionlen = \width[2] - 100
        ;               EndIf
        ...
        ForEach \items()
          If Bool((_selected_index_ >= \items()\index And _index_ =< \items()\index) Or ; верх
            ...
            If (_index_ = \items()\index)
              _caret_ = caret(*this) 
              
              If _line_ <> _index_ Or
                 \text\caret\pos <> _caret_
                \text\caret\pos = _caret_
                _line_ = _index_
                
                If _index_ = _selected_index_
                  ; Debug "line - "+\items()\index
                  _selected_line_ =- 1
                  ...
                  ; чтобы выделить конец строки
                  If \text\caret\pos And \text\caret\pos = \items()\text\len And 
                     \root\mouse\x > \text\caret\x+Selectionlen
                    Selection = 1
                    \text\caret\x + Selectionlen
                  EndIf
                  
                Else
                  ; Debug "enter - "+\items()\index
              ...
              If Repaint > 0 ;And len
                ...
                \items()\text[1]\x = \items()\text\x
                \items()\text[1]\width = TextWidth(\items()\text[1]\string.s) 
                
                ; \items()\text[2]\change = #True
                \items()\text[2]\pos = Pos
                \items()\text[2]\len = Len
                \items()\text[2]\string = Mid(\items()\text\string, 1 + \items()\text[2]\pos, \items()\text[2]\len) 
                
                \items()\text[2]\x = \items()\text[1]\x+\items()\text[1]\width
                
                If Selection
                  If Selectionlen < 0
                    \items()\text[2]\width = \items()\width-\items()\text[1]\width-\sci\margin\width+4
                  Else
                    \items()\text[2]\width = TextWidth(\items()\text[2]\string.s) + Selectionlen
                  EndIf
                Else
                ...
                ; \items()\text[3]\change = #True
                \items()\text[3]\pos = (pos + len)
                \items()\text[3]\len = (\items()\text\len - \items()\text[3]\pos)
                \items()\text[3]\string.s = Right(\items()\text\string.s, \items()\text[3]\len) 
                
                \items()\text[3]\x = \items()\text[2]\x + \items()\text[2]\width
                \items()\text[3]\width = TextWidth(\text[3]\string.s)
                
                
              EndIf
              
            ElseIf _selected_index_ = \items()\index
              ; Debug "select - "+\items()\index
              
              If _selected_line_ <> _selected_index_
                
                If _selected_index_ > _index_ ; верх
                  pos = 0
                  len = \text\caret\end 
                  Selection = 0
                Else
                  pos = \text\caret\end 
                  len = (\items()\text\len-pos) + 1
                  Selection = 1
                EndIf
                
                _selected_line_ = _selected_index_
            ...
            Else
              ...
              If \items()\text[2]\Len <> (\items()\text\len - Bool(Not \items()\text\len))
                
                ;Debug ""+pos+" "+\items()\text[2]\len
                
                ;\items()\text[1]\change = #True
                \items()\text[1]\len = 0
                \items()\text[1]\string.s = ""
                \items()\text[1]\x = \items()\text\x
                \items()\text[1]\width = 0
                ...
                ;\items()\text[3]\change = #True
                \items()\text[3]\len = 0
                \items()\text[3]\string.s = ""
                \items()\text[1]\x = \items()\text[2]\x+\items()\text[2]\width
                \items()\text[3]\width = 0
    ...
    With *this
      ...
      If Repaint =- 1
        If \text\caret\pos <\text\caret\end
          ...
          text_setsel_change(*this, \text\caret\pos , \text\caret\end-\text\caret\pos )
        Else
          ; Debug \text\caret\pos -\text\caret\end
          text_setsel_change(*this, \text\caret\end, \text\caret\pos -\text\caret\end)
        EndIf
      EndIf                                                  
    EndWith
