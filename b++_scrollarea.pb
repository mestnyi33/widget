...
CompilerIf Not Defined(Bar, #PB_Module)
  ...
  Module bar
    ...
    ;-
    ;- PUBLICs
    Macro get_parent_childrens()
      *this\parent\_childrens();\root
    EndMacro
    Macro Get_this_childrens()
      *this\_childrens();\root
    EndMacro
    ...
    Procedure.b Draw(*this._s_widget)
      With *this
        If \width[#__c_4] > 0 And \height[#__c_4] > 0
          ...
          Protected count
        ForEach Get_this_childrens()
          If Not Get_this_childrens()\hide And (Get_this_childrens()\width[#__c_4] ); Or Get_this_childrens()\height[#__c_4])
            Debug ""+count +" "+ Str(Get_this_childrens()\width[#__c_4])
            Draw(Get_this_childrens())
            count + 1
          EndIf
        Next
    ...
    Procedure   ReDraw(*this._s_widget)
      If StartDrawing(CanvasOutput(*this\root\canvas\gadget))
        ...
        ; PushListPosition(Get_this_childrens())
        ForEach Get_this_childrens()
          If Not Get_this_childrens()\hide
            Draw(Get_this_childrens())
          EndIf
        Next
        ; PopListPosition(Get_this_childrens())
              ...
              If *this\parent\scroll\v = *this
                ...
                *this\parent\scroll\y =- *this\bar\page\pos
                ...
                If *this\parent\container And *this\parent\count\childrens
                  ...
                  ; PushListPosition(get_parent_childrens())
                  ;ChangeCurrentElement(get_parent_childrens(), *this\parent\adress)
                  ; While NextElement(get_parent_childrens())
                    ForEach get_parent_childrens()
                    ;If get_parent_childrens()\parent = *this\parent 
                      Resize(get_parent_childrens(), #PB_Ignore, 
                             get_parent_childrens()\y[#__c_3] + *this\bar\thumb\change, #PB_Ignore, #PB_Ignore)
                    ;EndIf
                    Next
                  ; Wend
                  ; PopListPosition(get_parent_childrens())
                EndIf
              EndIf
            Else
                ...
                *this\parent\scroll\x =- *this\bar\page\pos
                ...
                If *this\parent\container And *this\parent\count\childrens
                  ...
                  ; While NextElement(get_parent_childrens())
                    ForEach get_parent_childrens()
                    ;If get_parent_childrens()\parent = *this\parent 
                      Resize(get_parent_childrens(), 
                             get_parent_childrens()\x[#__c_3] + *this\bar\thumb\change, #PB_Ignore, #PB_Ignore, #PB_Ignore)
                    ;EndIf
                    Next
                  ; Wend
                  ; PopListPosition(get_parent_childrens())
    ...
    Procedure.b Update(*this._s_widget)
      ...
      If Not *this\bar\area\len < 0
        ...
        Else
          ...
          If *this\bar\page\pos > *this\bar\page\end And *this\parent And *this\parent\scroll And *this\parent\scroll\v And *this\parent\scroll\h
          ...
          If \resize & #__resize_change
            ...
            If *this\container And *this\count\childrens
             ; _move_childrens_(*this, 0,0)
                ...
                ;PushListPosition(Get_this_childrens())
                ForEach Get_this_childrens()
                 ; If Get_this_childrens()\parent = *this 
                    ; Debug Get_this_childrens()\class +" - "+ Get_this_childrens()\parent\class +" - "+ *this\class +" - "+ *this\parent\class; Bool(Get_this_childrens()\type = #PB_GadgetType_Button)
                    
                    Resize(Get_this_childrens(), 
                           Get_this_childrens()\x[#__c_3],
                           Get_this_childrens()\y[#__c_3], 
                           #PB_Ignore, #PB_Ignore)
                 ; EndIf
                Next
                ;PopListPosition(Get_this_childrens())
      ...
      CompilerElse
        *event\widget = *this
        ...
        If *this\parent <> *Parent
          ...
          ;  ChangeCurrentElement(get_parent_childrens(), *this\adress)
          ;  DeleteElement(get_parent_childrens())  
            
;             MoveElement(get_parent_childrens(), #PB_List_After, *Parent\adress)
;             
;             While PreviousElement(get_parent_childrens()) 
;               If _is_child(get_parent_childrens(), *this)
;                 MoveElement(get_parent_childrens(), #PB_List_After, *this\adress)
;               EndIf
;             Wend
          EndIf
          ...
          *this\parent = *Parent
          ...
          If Not (*this\parent And *this\parent\type = #PB_GadgetType_Splitter)
            ...
            *this\level = *this\parent\level + 1
            LastElement(get_parent_childrens())
            *this\adress = AddElement(get_parent_childrens()) 
            get_parent_childrens() = *this
          EndIf
          
          ;  Debug ListIndex(get_parent_childrens())  
        EndIf
      CompilerEndIf
    EndProcedure
    ...
    If *this And *this\count\childrens
      ;Debug ListSize((Get_this_childrens()\_childrens()) );*this\count\childrens
      
      ;PushListPosition(Get_this_childrens())    ;
      LastElement(Get_this_childrens())         ; ??? ?? ?????? ? ?????????? ????????
      Repeat                                 ; ?????????? ? ???? ????
        If Not Get_this_childrens()\hide And _from_point_(mouse_x,mouse_y, Get_this_childrens(), [#__c_4]) 
          ; Debug Get_this_childrens()\index
          If Get_this_childrens()\count\childrens
            ; result = From(Get_this_childrens(), mouse_x, mouse_y)
            
            LastElement(Get_this_childrens()\_childrens())  ; ??? ?? ?????? ? ?????????? ????????
            Repeat                                      ; ?????????? ? ???? ????
              If ListSize(Get_this_childrens()\_childrens()) And Not Get_this_childrens()\_childrens()\hide And _from_point_(mouse_x,mouse_y, Get_this_childrens()\_childrens(), [#__c_4]) 
                
                If Get_this_childrens()\_childrens()\count\childrens
                  result = From(Get_this_childrens()\_childrens(), mouse_x, mouse_y)
                  
                  If Not result
                    ; parent of children
                    result = Get_this_childrens()\_childrens()
                  EndIf
                Else
                  ; last child
                  result = Get_this_childrens()\_childrens()
            ...
            Until PreviousElement(Get_this_childrens()\_childrens()) = #False 
            
            If Not result
              ; parent of children
              result = Get_this_childrens()
            EndIf
          Else
            ; last child
            result = Get_this_childrens()
      ...
      Until PreviousElement(Get_this_childrens()) = #False 
      ;PopListPosition(Get_this_childrens())
    EndIf
  ...
  Procedure CallBack()
      ...
      If change
;         ; get at point
;         LastElement(get_root_()\_childrens()) 
;         Repeat                                 
;           If _from_point_(mouse_x, mouse_y, get_root_()\_childrens(), [#__c_4])
;             *this = get_root_()\_childrens()
;             
;             ; scrollbars events
...
;             Break
;           EndIf
;         Until PreviousElement(get_root_()\_childrens()) = #False 
...
CompilerIf #PB_Compiler_IsMainFile
  ...
  If OpenWindow(0, 0, 0, 380, 200, "ScrollBarGadget", #PB_Window_SystemMenu | #PB_Window_ScreenCentered)
