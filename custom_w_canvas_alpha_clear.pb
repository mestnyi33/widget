...
Procedure.i CustomCanvasProc(hWnd.i, uMsg.l, wParam.i, lParam.i)
  ...
  Select uMsg
    ...
    Case #WM_CLEAR_CANVAS
      ; --- РАДИКАЛЬНАЯ И 100% РАБОЧАЯ ОЧИСТКАПРОЗРАЧНОГО ОКНА ---
      If *myData
        ; 1. Мгновенно прячем прозрачный холст с экрана, чтобы проявилось родительское окно
        ShowWindow_(hWnd, #SW_HIDE)
        
        ; 2. Полностью очищаем внутренний буфер в оперативной памяти (заливаем черным)
        hdc = GetDC_(hWnd)
        hMemDC = CreateCompatibleDC_(hdc)
        hOldBitmap = SelectObject_(hMemDC, *myData\hLayeredImage)
        ...
        ; 3. Находим родительское окно и заставляем его полностью обновиться (стереть старые линии)
        parentHWND = GetParent_(hWnd)
        
        ; Флаги RDW_ERASE и RDW_INVALIDATE принудительно закрашивают старый след цветом формы
        RedrawWindow_(parentHWND, #Null, #Null, #RDW_ERASE | #RDW_INVALIDATE | #RDW_UPDATENOW)
        
        ; 4. Возвращаем уже чистый холст обратно на экран
