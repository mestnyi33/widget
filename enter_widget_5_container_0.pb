...
; ; ;             If *value\event\enter <> \childrens() And 
...
; ; ;                 If *value\event\enter\State : *value\event\enter\State = 0
; ; ;                   repaint = Events(*value\event\enter, #PB_EventType_MouseLeave, mouse_x, mouse_y)
; ; ;                 EndIf
; ; ;               EndIf
; ; ;               
; ; ;               If Not \childrens()\state : \childrens()\state = 1
; ; ;                 If (*value\event\enter And *value\event\enter\parent\index_ > \childrens()\parent\index_) 
; ; ;                   Debug 88888
; ; ;                   repaint = Events(*value\event\enter, #PB_EventType_MouseLeave, mouse_x, mouse_y)
; ; ;                 EndIf
; ; ;                 
; ; ;                 Debug Bool(Not (*value\event\enter And *value\event\enter\parent\index_ > \childrens()\parent\index_) )
...
; ; ;           
          ...
          Protected enter = Bool(*value\event\enter <> \childrens() And Not (*value\event\enter And *value\event\enter\index_ > \childrens()\index_) And _from_point_(mouse_x, mouse_y, \childrens(), [4]))
          Protected leave = Bool(*value\event\enter And (enter Or (*value\event\enter = \childrens() And Not _from_point_(mouse_x, mouse_y, *value\event\enter, [4]))))
          
          If leave
            ;If *value\event\enter <> \childrens()\parent ; Not \childrens()\canvas\mouse\buttons ; And 
            repaint | Events(*value\event\enter, #PB_EventType_MouseLeave, mouse_x, mouse_y)
            
            ;           If *value\event\enter And *value\event\enter\canvas\gadget <> \childrens()\canvas\gadget
            ;             ReDraw(*value\event\enter)
            ;             ; _repaint_(*value\event\enter)
            ;           EndIf
            
            *value\event\leave = *value\event\enter ; надо проверить нужен или нет (а так для них нужен *value\event\enter <> \childrens()\parent)
            ;EndIf
          ...
          If enter
           ...
           ; If Not (*value\event\leave And *value\event\leave\parent = \childrens()) ;  Not \childrens()\canvas\mouse\buttons ; And 
            
;             PushListPosition(\childrens())
;             ForEach \childrens()
;               If \childrens()\color\state And *value\event\enter\parent <> \childrens()
;                 repaint | Events(\childrens(), #PB_EventType_MouseLeave, mouse_x, mouse_y)
;               Else
;                 Break
;               EndIf
