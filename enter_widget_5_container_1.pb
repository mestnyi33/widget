      ...
      If *this > 0 
        ...
        If *Parent <> \parent Or \parent_item <> parent_item
          ...
          Else
            ...
            ;             PushListPosition(\root\childrens())
            ;             While NextElement(\root\childrens())
            ;               \root\childrens()\index = ListIndex(\root\childrens())
            ;             Wend
            ;             PopListPosition(\root\childrens())
          EndIf
  ...
  Procedure.l Callback(*this._S_widget, eventtype.l, mouse_x.l, mouse_y.l)
    ...
    With *this
      If ListSize(\childrens()) ;  \CountItems ; 
        LastElement(\childrens()) 
        Repeat                            ; Перебираем с низу верх
          
          ;Debug ""+ListIndex(\childrens()) +" "+ \childrens()\index_ +" "+ \childrens()\index
          ; If _from_point_(mouse_x, mouse_y, \childrens(), [4])
          If Bool (mouse_x > \childrens()\x[4] And mouse_x < (\childrens()\x[4]+\childrens()\width[4] - Bool(\childrens()\width=\childrens()\width[4])*\childrens()\bs) And 
                   mouse_y > \childrens()\y[4] And mouse_y < (\childrens()\y[4]+\childrens()\height[4] - Bool(\childrens()\height=\childrens()\height[4])*\childrens()\bs))
            
             If *value\event\enter <> \childrens() ;And Not (*value\event\enter And *value\event\enter\index_ > \childrens()\index_) 
              If \childrens()\state = 0 : \childrens()\state = 1
               ;Debug \childrens()\x[4]+\childrens()\width[4] - Bool(\childrens()\width=\childrens()\width[4])
...
;           Else
;             If \childrens()\state = 1 : \childrens()\state = 0
