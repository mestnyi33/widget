  ...
  Procedure.i Draw(*this._S_widget, Childrens=0)
    ...
    With *this
      ...
      If \height>0 And \width>0 And Not \hide And \color\alpha 
        ...
        DrawText(\x+2, \y, Str(\id), $FF000000)
    ...
    With *this
      If *this > 0 
        ...
        If *Parent <> \parent Or \parent_item <> parent_item
          ...
          ; Add new children 
          If (\id < 0 Or \id > ListSize(\root\childrens()) - 1)
            LastElement(\root\childrens())
            \adress = AddElement(\root\childrens()) 
            \id = ListIndex(\root\childrens())
          Else
            SelectElement(\root\childrens(), \id)
    ...
    With *this
      If ListSize(\childrens()) ;  \CountItems ; 
          ...
          If _from_point_(mouse_x, mouse_y, \childrens(), [4])
            ...
            If *value\event\enter <> \childrens() And Not (*value\event\enter And *value\event\enter\index > \childrens()\index) 
              ...
              If \childrens()\state = 0 : \childrens()\state = 1
                If *value\event\enter And *value\event\enter\State ;And *value\event\enter\parent = \childrens()\parent
        ...
        ForEach \childrens() 
          ...
          ;Debug ""+ListIndex(\childrens()) +" "+ \childrens()\index_ +" "+ \childrens()\index
          If Not _from_point_(mouse_x, mouse_y, \childrens(), [4]) 
            If \childrens()\state = 1 : \childrens()\state = 0
              If Not (*value\event\enter And *value\event\enter\index > \childrens()\index) 
