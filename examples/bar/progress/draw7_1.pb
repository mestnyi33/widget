...
Macro _draw_h_progress_2(_pos_, _len_, _x_, _y_, _width_ ,_height_, _round_, _color1_, _color2_)
  ...
  If (_pos_)
    FrontColor(_color1_)
  For y = _y_ To _y_ + _height_
    If Point((_x_ + (_pos_)), y) & $00FFFFFF = _color2_ & $00FFFFFF
      Line( (_x_ + (_pos_)), y, 1, _height_-(y-_y_)*2)
      Break 
    EndIf
  Next y
  
  FillArea(_x_ + (_pos_)/2, _y_ + _height_/2,  -1) 
...
;       EndIf
;     Next x
      If Point((_x_ + (_width_-_pos_)), y) & $00FFFFFF = _color1_ & $00FFFFFF
        Line( (_x_ + (_width_-_pos_)), y, 1, _height_-(y-_y_)*2)
      Break 
    EndIf
  
  Next y
     FillArea(_x_ + (_width_-_pos_)/2 + 1, _y_ + _height_/2,  -1) 
 EndIf
