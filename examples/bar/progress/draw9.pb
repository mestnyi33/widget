...
;   If (_pos_)
;     For i = 0 To (_len_)
;       If Point(_x_ + i, _y_ + (_pos_)) & $00FFFFFF = _color2_ & $00FFFFFF
;         Line(_x_ + i, _y_ + (_pos_), (_len_) - i*2, 1)
;         Break
;       EndIf
;     Next i
;     
;     FillArea(_x_ + (_len_)/2, _y_ + (_pos_)/2,  -1) 
;   EndIf
EndMacro

Macro _draw_h_progress_r(_pos_, _len_, _x_, _y_, _width_ ,_height_, _round_, _color1_, _color2_, _frame_size_)
  FrontColor(_color1_)
  DrawingMode(#PB_2DDrawing_Default|#PB_2DDrawing_AlphaBlend)
  RoundBox(_x_, _y_, _width_ ,_height_, _round_, _round_)
  ...
  If (_width_ - _pos_)
    For i = 0 To (_len_)
      If Point(_x_ + (_width_ - _pos_), _y_ + i) & $00FFFFFF = _color1_ & $00FFFFFF
        Line(_x_ + (_width_ - _pos_), _y_ + i, 1, (_len_) - i*2)
        Break
      EndIf
    Next i
