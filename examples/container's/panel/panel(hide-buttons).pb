...
Procedure events_widgets()
   If WidgetEvent() = #__event_LeftClick
      SetWidgetState(*container, GetWidgetData(EventWidget()))
   EndIf
EndProcedure

Procedure AddButtons( item, x,y,Text.s )
   ButtonWidget(x, y, 80, 24, Text)
   SetWidgetData(widget( ), item )
   BindWidgetEvent(widget(), @events_widgets() )
EndProcedure

;\\
If OpenRoot(0, 0, 0, 300, 200, "PanelGadget", #PB_Window_SystemMenu | #PB_Window_ScreenCentered)
