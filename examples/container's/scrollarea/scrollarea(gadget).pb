...
CompilerIf #PB_Compiler_IsMainFile
   ...
   Procedure events_widgets()
      ; Debug ""+Str(Index(EventWidget( )))+ " - widget event - " +WidgetEvent( )+ " bar - " +GetClass(this()\item)+ " direction - " +this()\data 
   ...
   Procedure OpenArea( X,Y,Width,Height, areawidth,areaheight,scrollstep, Flag.q=0)
      Protected._s_WIDGET *g 
      *g = ScrollArea(X,Y,Width,Height, areawidth,areaheight,scrollstep, Flag)
      *g\scroll\gadget[1] = ContainerGadget(#PB_Any, 0,0,0,0)
      *g\scroll\gadget[2] = ContainerGadget(#PB_Any, 0, 0, (areawidth), (areaheight))
      
      CompilerIf #PB_Compiler_OS = #PB_OS_Windows
         SetWindowLongPtr_( GadgetID(*g\scroll\gadget[1]), #GWL_STYLE, GetWindowLongPtr_( GadgetID(*g\scroll\gadget[1]), #GWL_STYLE ) | #WS_CLIPCHILDREN )
         SetWindowLongPtr_( GadgetID(*g\scroll\gadget[2]), #GWL_STYLE, GetWindowLongPtr_( GadgetID(*g\scroll\gadget[2]), #GWL_STYLE ) | #WS_CLIPCHILDREN )
      CompilerEndIf
      
      
      ;SetGadgetColor(*g\scroll\gadget[1], #PB_Gadget_BackColor, $00FFFF)
   ...
   If Open(0, 0, 0, 305+305, 500, "ScrollArea", #PB_Window_SystemMenu | #PB_Window_ScreenCentered)
      ...
      *g = OpenArea(0, 0, 100, 100, Sw, Sh, 30, #PB_ScrollArea_Flat)
      SetGadgetColor(*g\scroll\gadget[2], #PB_Gadget_BackColor, $00FFFF)
      ...
      *b = ButtonGadget  (#PB_Any, Sw-130, Sh-30, 130, 30,"Button")
      ...
      If count
         ...
         OpenGadgetList(*g\scroll\gadget[2])
         time = ElapsedMilliseconds()
         For i=0 To count
            If Bool(i>count-110)
               ButtonGadget  (#PB_Any, (count-i)*2, (count-i)*2, 130, 30,"Button"+Str(i))
