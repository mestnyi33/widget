...
CompilerIf #PB_Compiler_IsMainFile
         ...
         Case #__event_StatusChange
            ...
            If StartEnum( *area )
               If *g = widgets()
                  Continue
               EndIf
               If widgets()\type = #PB_GadgetType_Tree
   ...
   Procedure SetAttribute_( *this._s_WIDGET, attribute, value )
      ...
      Select attribute
         Case #PB_ScrollArea_InnerWidth
            ;Repaint( )
            If value < GadgetWidth(*this)
               value = GadgetWidth(*this)
            EndIf
            If SetGadgetAttribute( *this, attribute, value )
               ;If Not *this\flag & #__flag_AutoSize
                  ResizeGadget(first, #PB_Ignore, #PB_Ignore, value, #PB_Ignore)
               ;EndIf
            EndIf
            
         Case #PB_ScrollArea_InnerHeight
            ;Repaint( )
            If value < GadgetHeight(*this)
               value = GadgetHeight(*this)
            EndIf
            If SetGadgetAttribute( *this, attribute, value )
               ;If Not *this\flag & #__flag_AutoSize
                  ResizeGadget(first, #PB_Ignore, #PB_Ignore, #PB_Ignore, value)
              ; EndIf
            EndIf
   ...
   Procedure ListIcon_(X,Y,Width,Height,firstcolumntitle.s, firstcolumnwidth, flags.q=0 )
      Protected *parent = ScrollAreaGadget(#PB_Any, X,Y,Width,Height, Width,Height, 1 )
      Protected *g1 = TreeGadget(#PB_Any, 0,0,0,0, #PB_Tree_NoLines|flags);|#__flag_BorderLess)
     ; BindGadgetEvent(*g1, @listicon_tree_events())
; ;       Hide(*g1\scroll\v, 1)
; ;       Hide(*g1\scroll\h, 1)
      SetGadgetData(*g1, 1)
      
      Protected *this._s_WIDGET = SplitterGadget(#PB_Any, 0,0,Width,Height, *g1,ButtonGadget(-1,0,0,0,0,"1"), #PB_Splitter_Separator|#PB_Splitter_Vertical|#PB_Splitter_FirstFixed )
;       ;
...
;       *this\bar\button\round = 0;  DPIScaled(1)
   ...
   Procedure AddColumn_( *parent._s_WIDGET, position.l, Text.s, Width.l, Image.i = -1 )
      ;ProcedureReturn 
      Protected *this._s_WIDGET = first
      ...
      If *this
         ...
         EndIf
         ;
;          Define *Tree._s_WIDGET=GetGadgetAttribute(*this, #PB_Splitter_FirstGadget)
         *g2 = GetGadgetAttribute(parent, #PB_Splitter_SecondGadget)
         *g1 = TreeGadget(#PB_Any, 0,0,0,0, #PB_Tree_NoLines) ; 
         
         If GadgetType(*g2) = #PB_GadgetType_Button
            Define *g4 = TreeGadget(#PB_Any, 0,0,0,0, #PB_Tree_NoLines)
            SetGadgetAttribute( parent, #PB_Splitter_SecondGadget, *g4 )
            FreeGadget( *g2 )
            *g2 = *g4
         Else
         EndIf
...
;          Hide(*g1\scroll\h, 1)
         ...
         If position =- 1
            ...
            SetGadgetData(*g1, c)
         Else
            SetGadgetData(*g1, position+1)
         EndIf
          

           
;          If *g2 > 0
            *g = SplitterGadget(#PB_Any,  0,0,0,0, *g2, *g1, #PB_Splitter_Separator|#PB_Splitter_Vertical|#PB_Splitter_FirstFixed|#__flag_BorderLess )
            SetGadgetAttribute( parent, #PB_Splitter_SecondGadget, *g )
            SetGadgetData(*g, *parent)
            SetGadgetState(*g, Width)
            ...
            parent = *g
;          Else
;             SetGadgetAttribute( parent, #PB_Splitter_SecondGadget, *g1 )
...
;          EndIf
   ...
   Procedure AddItem_( *parent._s_WIDGET, Item.l, Text.s, Image.i = - 1, Flag.q = 0 )
      ...
      If *this
         Protected._s_WIDGET *g1 = GetGadgetAttribute(*this, #PB_Splitter_FirstGadget)
         Protected._s_WIDGET *g2 = GetGadgetAttribute(*this, #PB_Splitter_SecondGadget)
   ...
   If Open(0, 0, 0, 800, 450, "ListiconGadget", #PB_Window_SystemMenu | #PB_Window_ScreenCentered)
      ;{ - widget
      Define i,Count,t=ElapsedMilliseconds()
      Define g = 11
...
;       
; ;       Repaint()
;        Define value = 400;*g\FirstWidget( )\split_1( )\Scroll\v\bar\max
...
;       SetAttribute_( *g, #PB_ScrollArea_InnerHeight, value )
      ...
      Define g = 13
      Define *g = ListIcon_(350, 230, 430, 210, "Column_1",130, #__Flag_GridLines|#__Flag_CheckBoxes|#__flag_RowFullSelect);|: *g = GetGadgetData(g)                                          
