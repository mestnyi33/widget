...
;          If \p
;            x = \p\s\x ;+ Bool(ListSize(\p\Childrens()))*\p\x
;            y = \p\s\y ;+ Bool(ListSize(\p\Childrens()))*\p\y
;          EndIf
;          If \s
;            ClipOutput(\x+2,\y+2,\s\h\Page\len,\s\v\Page\len)
;          Else
    ...
    If *This > 0
      ...
      With *This
        ...
        If \Resize
          ...
          If \p And \x+\width>\p\clip\x+\p\clip\width
            \clip\width = \p\clip\width-(\clip\x-\p\clip\x)
          Else
            \clip\width = \width-(\clip\x-\x)
          EndIf
          
          If \p And \y+\height>\p\clip\y+\p\clip\height
            \clip\height = \p\clip\height-(\clip\y-\p\clip\y)
          Else
...
;     Repaint | CallBack(*Scroll_1, EventType, mouseX,mouseY)
  ...
  Procedure Window_0()
    If OpenWindow(0, 0, 0, 400, 400, "Demo inverted scrollbar direction", #PB_Window_SystemMenu | #PB_Window_ScreenCentered | #PB_Window_SizeGadget)
      ...
      *Scroll_1.Bar_S  = Image(0, 0, 0, 0, 0); : SetState(*Scroll_1, 1) 
      *Scroll_2.Bar_S  = ScrollArea(0, 0, 0, 0, 250,250) : SetState(*Scroll_2\s\h, 45)
      ...
      *child_1.Bar_S  = Progress(10, 40, 80, 20, 0,200,100) : SetState(*child_1, 50)
      *child_2.Bar_S  = Scroll(10, 70, 80, 20, 0,200,100) : SetState(*child_2, 80)
      
      *ScrollArea.Bar_S  = ScrollArea(50, 50, 150, 150, 250,250) : SetParent(*ScrollArea, *Scroll_2)
