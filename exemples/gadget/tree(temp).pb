...
CompilerIf Not Defined(widget, #PB_Module)
  ...
  DeclareModule widget
    ...
    Macro _bar_scrollarea_draw_(_this_)
      ; Draw scroll bars
      ...
      With *this
        If Not \hide
          ...
          ;_draws_(*this, \row\draws())
           ...
           PushListPosition(\row\draws())
        ForEach \row\draws()
          If \row\draws()\draw
            If \row\draws()\width <> *this\width[2]
              \row\draws()\width = *this\width[2]
            EndIf
            
            If \row\draws()\fontID And
               *this\row\fontID <> \row\draws()\fontID
              *this\row\fontID = \row\draws()\fontID
              DrawingFont(\row\draws()\fontID) 
              
              ;  Debug "    "+ \row\draws()\text\height +" "+ \row\draws()\text\string
            EndIf
            
            
            Y = \row\draws()\y - *this\scroll\v\bar\page\pos
            state = \row\draws()\color\state + Bool(*this\color\state<>2 And \row\draws()\color\state=2)
            
            ; Draw select back
            If \row\draws()\color\back[state]
              DrawingMode(#PB_2DDrawing_Default)
              RoundBox(*this\x[2],Y,*this\width[2],\row\draws()\height,\row\draws()\round,\row\draws()\round,\row\draws()\color\back[state])
            EndIf
            
            ; Draw select frame
            If \row\draws()\color\frame[state]
              DrawingMode(#PB_2DDrawing_Outlined)
              RoundBox(*this\x[2],Y,*this\width[2],\row\draws()\height,\row\draws()\round,\row\draws()\round, \row\draws()\color\frame[state])
            EndIf
            
            ; Draw checkbox
            ; Draw option
            If *this\flag\option_group And \row\draws()\parent
              DrawingMode(#PB_2DDrawing_Default)
              _tree_box_(\row\draws()\box[1]\x, \row\draws()\box[1]\y, \row\draws()\box[1]\width, \row\draws()\box[1]\height, \row\draws()\box[1]\checked, 1, $FFFFFFFF, 7, 255)
              
            ElseIf *this\flag\checkboxes
              DrawingMode(#PB_2DDrawing_Default)
              _tree_box_(\row\draws()\box[1]\x, \row\draws()\box[1]\y, \row\draws()\box[1]\width, \row\draws()\box[1]\height, \row\draws()\box[1]\checked, 3, $FFFFFFFF, 2, 255)
            EndIf
            
            ; Draw image
            If \row\draws()\image\handle
              DrawingMode(#PB_2DDrawing_Transparent|#PB_2DDrawing_AlphaBlend)
              DrawAlphaImage(\row\draws()\image\handle, \row\draws()\image\x, \row\draws()\image\y, \row\draws()\color\alpha)
            EndIf
            
            ; Draw text
            If \row\draws()\text\string.s
              DrawingMode(#PB_2DDrawing_Transparent)
              DrawRotatedText(\row\draws()\text\x, \row\draws()\text\y, \row\draws()\text\string.s, *this\text\rotate, \row\draws()\color\front[state])
            EndIf
            
            ; Horizontal line
            If *this\flag\GridLines And 
               \row\draws()\color\line <> \row\draws()\color\back
              DrawingMode(#PB_2DDrawing_Default|#PB_2DDrawing_AlphaBlend)
              Box(\row\draws()\x, (\row\draws()\y+\row\draws()\height+Bool(*this\flag\gridlines>1))-*this\scroll\v\bar\page\pos, *this\width[2], 1, *this\color\line)
            EndIf
          EndIf
        Next
        ...
        If *this\flag\lines
          ...
          ForEach \row\draws()
            If \row\draws()\draw 
              If \row\draws()\l\h\height
                ;  CustomFilterTree_CallBack(@Tree_PlotX())
                Line(\row\draws()\l\h\x, \row\draws()\l\h\y, \row\draws()\l\h\width, \row\draws()\l\h\height, \row\draws()\color\line)
              EndIf
              
              If \row\draws()\l\v\width
                ;  CustomFilterTree_CallBack(@Tree_PlotY())
                Line(\row\draws()\l\v\x, \row\draws()\l\v\y, \row\draws()\l\v\width, \row\draws()\l\v\height, \row\draws()\color\line)
              EndIf
            EndIf    
          Next
        EndIf
        ...
        If *this\flag\buttons
          ...
          ForEach \row\draws()
            If \row\draws()\draw And \row\draws()\childrens
              Arrow(\row\draws()\box[0]\x+(\row\draws()\box[0]\width-6)/2,\row\draws()\box[0]\y+(\row\draws()\box[0]\height-6)/2, 6, Bool(Not \row\draws()\box[0]\checked)+2, \row\draws()\color\front[\row\draws()\color\state], 0,0) 
              ;             DrawingMode(#PB_2DDrawing_Outlined|#PB_2DDrawing_AlphaBlend)
              ;             ;RoundBox(\row\draws()\box[0]\x-3, \row\draws()\box[0]\y-3, \row\draws()\box[0]\width+6, \row\draws()\box[0]\height+6, 7,7, \row\draws()\color\front[\row\draws()\color\state])
              ;             RoundBox(\row\draws()\box[0]\x-1, \row\draws()\box[0]\y-1, \row\draws()\box[0]\width+2, \row\draws()\box[0]\height+2, 7,7, \row\draws()\color\front[\row\draws()\color\state])
              ;             Arrow(\row\draws()\box[0]\x+(\row\draws()\box[0]\width-4)/2,\row\draws()\box[0]\y+(\row\draws()\box[0]\height-4)/2, 4, Bool(Not \row\draws()\box[0]\checked)+2, \row\draws()\color\front[\row\draws()\color\state], 0,0) 
            EndIf    
          Next
        EndIf
        
        PopListPosition(\row\draws()) ; 
          ;         UnclipOutput()
