...
If OpenWindow(0, 100, 50, 530, 700, "listiconGadget", #PB_Window_SystemMenu)
  ListIconGadget(0, 10, 10, 250, 680, "column_0", 200)
  listicon::Gadget(1, 270, 10, 250, 680, "column_0", 200, #PB_Flag_FullSelection) : *w=GetGadgetData(1)
  
  
  Define time = ElapsedMilliseconds()
  For a = 3 To LN
    listicon::AddItem (*w, -1, "Item "+Str(a));, 0,Random(5)+1)
    ...
    EndIf
  Next
  Debug Str(ElapsedMilliseconds()-time) + " - add widget items time count - " + listicon::CountItems(*w)
  listicon::Redraw(*w)
  ...
  Define time = ElapsedMilliseconds()
  count = listicon::CountItems(*w) : For a = 0 To count : listicon::RemoveItem(*w, a) : Next : Debug Str(ElapsedMilliseconds()-time) + " - remove widget items time count - " + listicon::CountItems(*w)
