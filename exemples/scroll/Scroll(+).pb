...
;     EndIf
  ...
  Procedure.b CallBack(*This.Bar_S, EventType.i, MouseX.i, MouseY.i);, WheelDelta.i=0, AutoHide.b=0, *Scrol1l.Scroll_S=#Null, Window=-1, Gadget=-1)
    Protected repaint
    Static Last, Down, *Scroll.Bar_S, *Last.Bar_S, Mousebuttons, *Mouseat.Bar_S
...
; ; ;           EndIf
  ...
  Procedure.b _CallBack(*This.Bar_S, EventType.i)
    Protected repaint
    ...
    If *This
      With *This
        If \Type = #PB_GadgetType_ScrollBar
          ...
          ; get at point buttons
          If \s\mouse\buttons 
          ElseIf (\s\mouse\x>=\X And \s\mouse\x<\X+\Width And \s\mouse\y>\Y And \s\mouse\y=<\Y+\Height) 
            If (\s\mouse\x>\X[1] And \s\mouse\x=<\X[1]+\Width[1] And  \s\mouse\y>\Y[1] And \s\mouse\y=<\Y[1]+\Height[1])
              \buttons = 1
            ElseIf (\s\mouse\x>\X[3] And \s\mouse\x=<\X[3]+\Width[3] And \s\mouse\y>\Y[3] And \s\mouse\y=<\Y[3]+\Height[3])
              \buttons = 3
            ElseIf (\s\mouse\x>\X[2] And \s\mouse\x=<\X[2]+\Width[2] And \s\mouse\y>\Y[2] And \s\mouse\y=<\Y[2]+\Height[2])
              \buttons = 2
            Else
              \buttons =- 1
            EndIf 
          ...
          Else
            ...
            Select EventType 
              Case #PB_EventType_MouseEnter, #PB_EventType_MouseLeave
                If \Vertical
                  If \s\h\buttons
                    If \s\h\buttons > 0
                      repaint | Events(\s\h, EventType, \s\mouse\x, \s\mouse\y, \s\h\buttons)
                    EndIf
                    repaint | Events(\s\h, EventType, \s\mouse\x, \s\mouse\y, - 1)
                    If EventType = #PB_EventType_MouseLeave
                      *Scroll = 0
                    EndIf
            ...
            If \Vertical
              If \s\h\buttons
                If \Color[2]\State
                  repaint | Events(*This, #PB_EventType_MouseLeave, \s\mouse\x, \s\mouse\y, \buttons)
;                   repaint | Events(*This, #PB_EventType_MouseLeave, \s\mouse\x, \s\mouse\y, - 1)
;                   repaint | Events(\s\h, #PB_EventType_MouseEnter, \s\mouse\x, \s\mouse\y, - 1)
                  repaint | Events(\s\h, #PB_EventType_MouseEnter, \s\mouse\x, \s\mouse\y, \s\h\buttons)
                  \Color[2]\State = 0
                EndIf
              Else
                \s\mouse\at = 0
            ...
            Else
              If \s\v\buttons
                If \Color[2]\State
                  repaint | Events(*This, #PB_EventType_MouseLeave, \s\mouse\x, \s\mouse\y, \buttons)
;                   repaint | Events(*This, #PB_EventType_MouseLeave, \s\mouse\x, \s\mouse\y, - 1)
;                   repaint | Events(\s\v, #PB_EventType_MouseEnter, \s\mouse\x, \s\mouse\y, - 1)
                  repaint | Events(\s\v, #PB_EventType_MouseEnter, \s\mouse\x, \s\mouse\y, \s\v\buttons)
                  \Color[2]\State = 0
                EndIf
              Else
                \s\mouse\at = 0
          ...
          If *Scroll = *This
            If Last <> \Buttons
              ;
               ; Debug ""+Last +" "+ *This\Buttons +" "+ *This +" "+ *Last
              If Last > 0 Or (Last = 2 And \Buttons =- 1 And *Last)
                repaint | Events(*This, #PB_EventType_MouseLeave, \s\mouse\x, \s\mouse\y, Last) : *Last = 0
              EndIf
              If Not \Buttons Or (Last = 2 And \Buttons =- 1 And *Last)
                repaint | Events(*This, #PB_EventType_MouseLeave, \s\mouse\x, \s\mouse\y, - 1) : *Last = 0
              EndIf
              
              If Not last ; Or (Last =- 1 And \Buttons = 2 And *Last)
                repaint | Events(*This, #PB_EventType_MouseEnter, \s\mouse\x, \s\mouse\y, - 1)
              EndIf
              If \Buttons > 0
                repaint | Events(*This, #PB_EventType_MouseEnter, \s\mouse\x, \s\mouse\y, \Buttons)
              EndIf
            ...
            Select EventType 
              Case #PB_EventType_LeftButtonDown
                If \Buttons
                  Down = \Buttons
                  repaint | Events(*This, EventType, \s\mouse\x, \s\mouse\y, \Buttons)
                EndIf
                
              Case #PB_EventType_LeftButtonUp 
                If Down
                  repaint | Events(*This, EventType, \s\mouse\x, \s\mouse\y, Down)
                EndIf
...
CompilerIf #PB_Compiler_IsMainFile
  ...
  Procedure Events(Canvas.i, EventType.i)
    ...
    *Scroll\mouse\Buttons = GetGadgetAttribute(EventGadget(), #PB_Canvas_Buttons)
    ...
    Repaint | Scroll::CallBack(*Scroll\v, EventType, *Scroll\mouse\X, *Scroll\mouse\y)
    Repaint | Scroll::CallBack(*Scroll\h, EventType, *Scroll\mouse\X, *Scroll\mouse\y)
