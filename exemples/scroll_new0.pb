...
Module Widget
  ...
  Procedure.i Resizes(*Scroll._S_scroll, X.i,Y.i,Width.i,Height.i)
    With *Scroll
      ...
      \v\page\len = Height - Bool(Not \h\hide And \h\height) * \h\height
      \h\page\len = Width - Bool(Not \v\hide And \v\width) * \v\width
...
Global Event.i, x.i, y.i, drag.i, hole.i, Width=300, Height=300
Global NewList Images.canvasitem()

Procedure.i Open(Window.i, X.i,Y.i,Width.i,Height.i, *Draw, *Event)
    Protected w.i=-1, Canvas.i=-1
    ...
    BindGadgetEvent(Canvas, *Event)
    BindGadgetEvent(Canvas, @Canvas_CallBack())
    SetGadgetData(canvas, *Draw)   
...
Procedure.b _Resize(*This._S_widget, X.l,Y.l,Width.l,Height.l, *Scroll._S_widget=#Null)
    ...
    With *This
      ...
      If ((\Max-\Min) >= \page\len)
        ...
        If \area\len
          ...
          If (\area\len > \box\size)
            If \box\size
              If (\thumb\len < \box\size)
                \area\len = Round(\area\len - (\box\size-\thumb\len), #PB_Round_Nearest)
                \thumb\len = \box\size 
              EndIf
            Else
              If (\thumb\len < 7)
          ...
          If \area\len > 0
            If (\Type <> #PB_GadgetType_TrackBar) And (\Thumb\Pos+\thumb\len) >= (\area\len+\box\size)
      ...
      If \Vertical
        ...
        \X[3] = X + Lines : \Width[3] = Width - Lines : \Y[3] = \Thumb\Pos : \Height[3] = \thumb\len           ; Thumb coordinate on scroll bar
      Else
        ...
        \Y[3] = Y + Lines : \Height[3] = Height - Lines : \X[3] = \Thumb\Pos : \Width[3] = \thumb\len          ; Thumb coordinate on scroll bar
      EndIf
  ...
  Procedure.b _Updates(*v._S_widget, *h._S_widget, ScrollArea_X, ScrollArea_Y, ScrollArea_Width, ScrollArea_Height)
    ...
    *v\Hide = _Resize(*v, #PB_Ignore, #PB_Ignore, #PB_Ignore, #PB_Ignore, *h) 
    *h\Hide = _Resize(*h, #PB_Ignore, #PB_Ignore, #PB_Ignore, #PB_Ignore, *v)
  ...
  Procedure.b _Resizes(*v._S_widget, *h._S_widget, X.l,Y.l,Width.l,Height.l )
    ...
    *v\Hide = _Resize(*v, Width-*v\Width, Y, #PB_Ignore, #PB_Ignore, *h) : iWidth = Width-Width(*v)
    *h\Hide = _Resize(*h, X, Height-*h\Height, #PB_Ignore, #PB_Ignore, *v) : iHeight = Height-Height(*h)
    
    If *v\width And *v\page\len<>iHeight : SetAttribute(*v, #PB_ScrollBar_PageLength, iHeight) : EndIf
    If *h\height And *h\page\len<>iWidth : SetAttribute(*h, #PB_ScrollBar_PageLength, iWidth) : EndIf
    
    If *v\width : *v\Hide = _Resize(*v, #PB_Ignore, #PB_Ignore, #PB_Ignore, #PB_Ignore, *h) : EndIf
    If *h\height : *h\Hide = _Resize(*h, #PB_Ignore, #PB_Ignore, #PB_Ignore, #PB_Ignore, *v) : EndIf
...
;     *h\Both = Bool(Not *v\Hide) 
    ...
    If *v\Hide : *v\Page\Pos = 0 : Else
      If *h\Radius : _Resize(*h, #PB_Ignore, #PB_Ignore, *v\x+*v\Radius/2-1, #PB_Ignore) : EndIf
    EndIf
    If *h\Hide : *h\Page\Pos = 0 : Else
      If *v\Radius : _Resize(*v, #PB_Ignore, #PB_Ignore, #PB_Ignore, *h\y+*v\Radius/2-1) : EndIf
    EndIf
...
Procedure _Draw ();canvas.i)
  ...
  Select EventType()
    Case #PB_EventType_Resize
       ;_Resizes(*Scroll\v, *Scroll\h, 0, 0, Width, Height)
  ...
  With *scroll
    ...
    DrawingMode(#PB_2DDrawing_Default)
    Box(0, 0, \h\page\len, \v\page\len, RGB(255,255,255))
...
Procedure ScrollUpdates(*v._S_widget, *h._S_widget, ScrollArea_X, ScrollArea_Y, ScrollArea_Width, ScrollArea_Height)
  Protected iWidth = X(*v), iHeight = Y(*h)
  Static hPos, vPos : vPos = *v\Page\Pos : hPos = *h\Page\Pos
...
;
canvas = Open(0, 10, 10, 400, 400, @_Draw(), @_CallBack()) : SetGadgetAttribute(Canvas, #PB_Canvas_Cursor, #PB_Cursor_Hand)
    
*Scroll\v = Scroll(0, 0,  20, 0, 0, Height, 0, #PB_ScrollBar_Vertical)
*Scroll\h = Scroll(0, 0, 0,  20, 0, Width, 0, 0)
