...
DeclareModule Scroll
  ...
  ; Draw gradient box
  Macro _box_(_type_, _x_,_y_,_width_,_height_,_color_1_,_color_2_, _radius_=0, _alpha_=255)
  ...
  Procedure.i ScrollPos(*this._S_widget, ThumbPos.i)
    ...
    With *this
      ...
      If ScrollPos <> ThumbPos 
        ...
        ThumbPos =   (\min + Round(((ThumbPos) - \area\pos) / (\area\len / (\max-\min)), #PB_Round_Down)) ; #PB_Round_Nearest
    ...
    With *this
      If Not \hide And \color\alpha
        ...
        If \thumb\len
          ...
          _box_(\Vertical,\button[#Thumb]\x,\button[#Thumb]\y,\button[#Thumb]\width,\button[#Thumb]\height,\Color[3]\fore[\color[3]\state],\Color[3]\Back[\color[3]\state], \Radius, \color\alpha)
        ...
        If \button\len
          ; Draw buttons
          DrawingMode(#PB_2DDrawing_Gradient|#PB_2DDrawing_AlphaBlend)
          _box_(\Vertical,\button[#Button1]\x,\button[#Button1]\y,\button[#Button1]\width,\button[#Button1]\height,\Color[1]\fore[\color[1]\state],\Color[1]\Back[\color[1]\state], \Radius, \color\alpha)
          _box_(\Vertical,\button[#Button2]\x,\button[#Button2]\y,\button[#Button2]\width,\button[#Button2]\height,\Color[2]\fore[\color[2]\state],\Color[2]\Back[\color[2]\state], \Radius, \color\alpha)
  ...
  Procedure ScrollUpdates(*scroll._S_scroll, ScrollArea_X, ScrollArea_Y, ScrollArea_Width, ScrollArea_Height)
    With *scroll
      ...
      If -\h\page\pos > ScrollArea_X
        SetState(\h, (ScrollArea_Width-ScrollArea_X)-ScrollArea_Width) 
      EndIf
      If -\v\page\pos > ScrollArea_Y
        SetState(\v, (ScrollArea_Height-ScrollArea_Y)-ScrollArea_Height) 
      EndIf
      
      \h\thumb\len = _thumb_len_(\h)
      \h\thumb\pos = _thumb_pos_(\h, _invert_(\h, \h\page\pos, \h\inverted))
            ...
            If Drag = #True
              If isCurrentItem
                If LastElement(Images())
                  ...
                  GetScrollCoordinate()
                  Repaint = ScrollUpdates(*scroll, ScrollX, ScrollY, ScrollWidth, ScrollHeight)
