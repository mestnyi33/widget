...
Module Scroll
  ...
  Macro _page_pos_(_this_, _thumb_pos_)
    (_this_\min + Round(((_thumb_pos_) - _this_\area\pos) / (_this_\area\len / (_this_\max-_this_\min)), #PB_Round_Down)) ; #PB_Round_Nearest
  EndMacro
  ...
  Procedure.i ScrollPos(*this._S_widget, ThumbPos.i)
    ...
    With *this
      ...
      If ScrollPos <> ThumbPos 
        ThumbPos = _page_pos_(*this, ThumbPos)
...
;       EndIf
      ...
      If \page\pos <> ScrollPos
        \thumb\pos = _thumb_pos_(*this, _invert_(*this, ScrollPos, \inverted))
  ...
  Procedure.b Updates(*scroll._S_scroll, ScrollArea_X.l, ScrollArea_Y.l, ScrollArea_Width.l, ScrollArea_Height.l)
    With *scroll
      ...
      If ScrollArea_Height <> #PB_Ignore And \v\max<>ScrollArea_Height : SetAttribute(\v, #PB_Bar_Maximum, ScrollArea_Height) : EndIf
      If ScrollArea_Width <> #PB_Ignore And \h\max<>ScrollArea_Width : SetAttribute(\h, #PB_Bar_Maximum, ScrollArea_Width) : EndIf
  ...
  Procedure.b Resizes(*scroll._S_scroll, X.l,Y.l,Width.l,Height.l )
    With *scroll
      ...
      If \v\width And \v\page\len<>iHeight : SetAttribute(\v, #PB_Bar_PageLength, iHeight) : EndIf
      If \h\height And \h\page\len<>iWidth : SetAttribute(\h, #PB_Bar_PageLength, iWidth) : EndIf
    ...
    Select EventType
      Case #PB_EventType_LeftButtonUp
        ...
        If (ScrollX<0 Or ScrollY<0)
          ...
          EndIf
;         Else
;           ;SetState(*scroll\h, ScrollX)
;           If *scroll\v\max<>ScrollWidth : SetAttribute(*scroll\v, #PB_Bar_Maximum, ScrollWidth) : EndIf
;           If *scroll\h\max<>ScrollHeight : SetAttribute(*scroll\h, #PB_Bar_Maximum, ScrollHeight) : EndIf
...
;   *Scroll\v = Gadget(0, 0,  20, 0, 0, 0, 1, #PB_Bar_Vertical, 9)
;   *Scroll\h = Gadget(0, 0, 0,  20, 0, 0, 1, 0, 9)
  *Scroll\v = Gadget(0, 0,  20, 0, 0, 0, 0, #PB_ScrollBar_Vertical, 9)
  *Scroll\h = Gadget(0, 0, 0,  20, 0, 0, 0, 0, 9)
