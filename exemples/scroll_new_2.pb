...
Module Widget
  ...
  Procedure.i Canvas_Events(Canvas.i, EventType.i)
    ...
    Protected *this._S_scroll = GetGadgetData(Canvas)
    ...
    If Repaint 
      If StartDrawing(CanvasOutput(Canvas))
        ...
        If *this\v\Function 
          Debug *this\h\page\len
          ClipOutput(*this\h\x, *this\v\y, *this\h\page\len, *this\v\page\len)
          CallCFunctionFast(*this\v\Function)
          UnclipOutput()
        EndIf
...
CompilerIf #PB_Compiler_IsMainFile
  ...
  Procedure.i Open(Window.i, X.i,Y.i,Width.i,Height.i, *CallBack)
    ...
    *scroll\v = Scroll( 0, 0, 20, 0 ,0, 0, 0, #PB_ScrollBar_Vertical, 9)
    *scroll\h = Scroll( 0, 0, 0, 20, 0, 0, 0, 0, 9)
    
    *scroll\v\Function = *CallBack
    *scroll\h\Function = *CallBack
    
    PostEvent(#PB_Event_Gadget, Window,Canvas, #PB_EventType_Resize)
    BindGadgetEvent(Canvas, @Canvas_CallBack())
    SetGadgetData(canvas, *scroll)   
  ...
  If OpenWindow(0, 0, 0, 325, 160, "Scroll on the canvas", #PB_Window_SystemMenu | #PB_Window_SizeGadget | #PB_Window_ScreenCentered)
    *scroll = Open(0, 10,10,305,140, @Draw_CallBack()) ; : SetGadgetAttribute(Canvas, #PB_Canvas_Cursor, #PB_Cursor_Hand)
    
    With *scroll
      SetAttribute(\v, #PB_ScrollBar_Maximum, ImageHeight(0))
      SetAttribute(\h, #PB_ScrollBar_Maximum, ImageWidth(0))
