...
;         EndIf
    ...
    Procedure.i Draw_Spin(*this._s_widget) 
      ...
      Box(*this\bar\button[#__b_1]\x-2,*this\y[#__c_1], *this\bar\button[#__b_3]\width, *this\height[#__c_1], *this\color\frame[*this\color\state])
            ...
            If \text\change Or *this\resize & #__resize_change
              ...
              *this\text\width = TextWidth(*this\text\string)
              ...
              If *this\type = #PB_GadgetType_Spin Or
                ...
                EndIf
                ;Debug ""+*this\x +" "+ *this\x[2]
                
                _text_change_(*this, *this\x[2], *this\y[2], *this\width[2], *this\height[2])
              EndIf
            EndIf
          EndIf
      ...
      If Not *this\bar\area\len < 0
        ...
        If *this\draw
          ...
          If *this\type = #PB_GadgetType_Spin
            ...
            *this\bar\button[#__b_3]\width    = *this\bar\button[#__b_3]\len ; *this\x[#__c_2]+*this\width[#__c_3] - *this\bar\button[#__b_1]\x+3
        ...
        *this\bar\inverted = Bool(Flag & #__bar_Inverted = Bool(*this\type <> #PB_GadgetType_TabBar) * #__bar_Inverted)
        ...
        If *this\Type = #PB_GadgetType_Spin
          ...
          *this\bar\button[#__b_2]\interact = #True
          ...
          If *this\bar\vertical
            *this\bar\button[#__b_3]\len = Size + 2+2
          Else
            *this\bar\button[#__b_3]\len = Size + 3 ;*2 ;+ 2
          EndIf
...
CompilerIf #PB_Compiler_IsMainFile
  ; Shows possible flags of SpinGadget in action...
  UseLib(Widget)
  EnableExplicit
  
  Global *B_0, *B_1, *B_2, *B_3, *B_4, *B_5
  Global *Spin_0._S_widget
  Global *Spin_1._S_widget
  ...
  If OpenWindow(0, 0, 0, 222+222, 205+70, "Spins on the canvas", #PB_Window_SystemMenu | #PB_Window_ScreenCentered)
    SpinGadget(0, 10, 10, 200, 20, 0,100)
    SpinGadget(1, 10, 40, 200, 20, 0,100, #PB_Spin_ReadOnly)
    SpinGadget(2, 10, 70, 200, 20, 0,100, #PB_Spin_Numeric)
    SpinGadget(3, 10,100, 200, 60, 0,100)
    SpinGadget(4, 10,170, 200, 60, 0,100)
    SpinGadget(5, 10,170+70, 200, 20, 0,100)
    
    Open(0,  222, 0, 222, 205+70)
    
    Spin(10, 10, 200, 20, 0,100, #__text_left|#__bar_vertical,-1,8)
    Spin(10, 40, 200, 20, 0,100, #__text_center|#__bar_vertical);, #__Spin_ReadOnly)
    Spin(10, 70, 200, 20, 0,100, #__text_right|#__bar_vertical);, #__Spin_Numeric)
    Spin(10,100, 200, 60, 0,100)
    Spin(10,170, 200, 60, 0,100, 0, 4)
    Spin(10,170+70, 200, 25, 0,100)
...
;     
;     Resize(*Spin_0, Width-85, #PB_Ignore, #PB_Ignore, Height-30)
;     Resize(*Spin_1, #PB_Ignore, #PB_Ignore, Width-100, #PB_Ignore)
;     ResizeGadget(c2, 10, 10, Width-20, Height-20)
;     SetWindowTitle(EventWindow(), Str(*Spin_1\width))
;   EndProcedure
;   
; ;   Procedure ResizeCallBack()
...
;   
;   If OpenWindow(11, 0, 0, 260, 160, "Spin on the canvas", #PB_Window_SystemMenu | #PB_Window_SizeGadget | #PB_Window_ScreenCentered)
;     Open(11, 10, 10 , 240, 160)
;     c2 = GetGadget(Root())
;     
;     *Spin_0 = Spin(270, 5,  60, 120, "Spin (Vertical)", #__Spin_multiline|#__Spin_vertical);|#__Spin_inverted)
;     SetColor(*Spin_0, #__Color_Front, $D56F1A)
;     SetFont(*Spin_0, FontID(0))
;     
;     *Spin_1 = Spin(5, 42, 250,  60, "Spin (Horisontal)", #__Spin_multiline,-1)
;     SetColor(*Spin_1, #__Color_fore, $0000FF)
;     SetColor(*Spin_1, #__Color_Back, $00FF00)
;     SetColor(*Spin_1, #__Color_Front, $FFFFFF) ; $4919D5)
;     SetFont(*Spin_1, FontID(0))
;     
;     
