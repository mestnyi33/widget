...
CompilerIf Not Defined(widget, #PB_Module)
  ...
  DeclareModule widget
    ...
    Declare.b Draw(*this)
    Declare   ReDraw(*this, canvas_backcolor=#Null)
  ...
  Module widget
          ...
          *this\text\height = TextHeight("A")
          ...
          ForEach \bar\_s()
            If \index[#__s_2] = \bar\_s()\index
              State_3 = 2
              ;               \bar\_s()\y = 2
              ;               \bar\_s()\height=\bar\button[#__b_3]\height-1
            Else
              State_3 = Bool(\index[#__s_1] = \bar\_s()\index); = \bar\_s()\color\state
                                                              ;               \bar\_s()\y = 4
                                                              ;               \bar\_s()\height=\bar\button[#__b_3]\height-4-1
            EndIf
    ...
    Procedure   Bar_Events(*this._s_widget, _event_type_.l, _mouse_x_.l, _mouse_y_.l, _wheel_x_.b=0, _wheel_y_.b=0)
      Protected Repaint
      ...
      If _event_type_ = #__Event_MouseMove Or
        ...
        If *this\bar\button[#__b_3]\interact And
           *this\bar\button[#__b_3]\state <> #__s_3 And
           _from_point_(_mouse_x_, _mouse_y_, *this, [#__c_2]) And
        ...
        If *this\Type = #PB_GadgetType_TabBar
          If *this\count\items
            ForEach *this\bar\_s()
              ; If *this\bar\_s()\draw
              If _from_point_((_mouse_x_-*this\x[#__c_1])+*this\bar\page\pos, _mouse_y_-*this\y[#__c_1], *this\bar\_s()) And *this\bar\from = #__b_3
                ;If _from_point_(_mouse_x_, _mouse_y_, *this\bar\_s()) And *this\bar\from = #__b_3
        ...
        If *this\Type = #PB_GadgetType_TabBar
          If *this\index[#__s_1] >= 0
            *this\index[#__s_2] = *this\index[#__s_1]
            If *this\Type = #PB_GadgetType_TabBar
              Repaint = Panel_SetState(*this\parent, *this\index[#__s_1])
            EndIf
          EndIf
        EndIf
      ...
      If _event_type_ = #__Event_MouseMove
        If *this\bar\button[#__b_3]\fixed And *this = *this\root\selected And *this\bar\state
          Repaint = Bar_SetPos(*this, (((Bool(Not *this\bar\vertical) * _mouse_x_) + (Bool(*this\bar\vertical) * _mouse_y_)) - *this\bar\button[#__b_3]\fixed))
    ...
    Procedure   Editor_Events(*this._s_widget, _event_type_.l, _mouse_x_.l, _mouse_y_.l)
        ...
        If *this And Not scroll
          If ListSize(*this\row\_s())
            ...
            If *this\text\editable And GetActive()\gadget = *this
              Repaint | Editor_Events_Key(*this, _event_type_, \root\mouse\x, \root\mouse\y)
            EndIf
          EndIf
        EndIf
        ;EndIf
    ...
    Declare _draw_update_(*this._s_widget, List _items_._s_rows())
    ...
    Procedure.l Tree_Draw(*this._s_widget)
      ...
      With *this
        If Not \hide
          ...
          ForEach \row\_s()
            If \row\_s()\draw
              If \row\_s()\width <> *this\width[2]
                \row\_s()\width = *this\width[2]
              EndIf
              
              If \row\_s()\fontID And
                 *this\row\fontID <> \row\_s()\fontID
                *this\row\fontID = \row\_s()\fontID
                DrawingFont(\row\_s()\fontID) 
                
                ;  Debug "    "+ \row\_s()\text\height +" "+ \row\_s()\text\string
              EndIf
              
              
              Y = \row\_s()\y - *this\scroll\v\bar\page\pos
              state = \row\_s()\color\state + Bool(*this\color\state<>2 And \row\_s()\color\state=2)
              
              ; Draw select back
              If \row\_s()\color\back[state]
                DrawingMode(#PB_2DDrawing_Default)
                RoundBox(*this\x[2],Y,*this\width[2],\row\_s()\height,\row\_s()\round,\row\_s()\round,\row\_s()\color\back[state])
              EndIf
              
              ; Draw select frame
              If \row\_s()\color\frame[state]
                DrawingMode(#PB_2DDrawing_Outlined)
                RoundBox(*this\x[2],Y,*this\width[2],\row\_s()\height,\row\_s()\round,\row\_s()\round, \row\_s()\color\frame[state])
              EndIf
              
              ; Draw checkbox
              ; Draw option
              If *this\flag\option_group And \row\_s()\parent
                DrawingMode(#PB_2DDrawing_Default)
                _tree_box_(\row\_s()\box[1]\x, \row\_s()\box[1]\y, \row\_s()\box[1]\width, \row\_s()\box[1]\height, \row\_s()\box[1]\checked, 1, $FFFFFFFF, 7, 255)
                
              ElseIf *this\flag\checkboxes
                DrawingMode(#PB_2DDrawing_Default)
                _tree_box_(\row\_s()\box[1]\x, \row\_s()\box[1]\y, \row\_s()\box[1]\width, \row\_s()\box[1]\height, \row\_s()\box[1]\checked, 3, $FFFFFFFF, 2, 255)
              EndIf
              
              ; Draw image
              If \row\_s()\image\handle
                DrawingMode(#PB_2DDrawing_Transparent|#PB_2DDrawing_AlphaBlend)
                DrawAlphaImage(\row\_s()\image\handle, \row\_s()\image\x, \row\_s()\image\y, \row\_s()\color\alpha)
              EndIf
              
              ; Draw text
              If \row\_s()\text\string.s
                DrawingMode(#PB_2DDrawing_Transparent)
                DrawRotatedText(\row\_s()\text\x, \row\_s()\text\y, \row\_s()\text\string.s, *this\text\rotate, \row\_s()\color\front[state])
              EndIf
              
              ; Horizontal line
              If *this\flag\GridLines And 
                 \row\_s()\color\line <> \row\_s()\color\back
                DrawingMode(#PB_2DDrawing_Default|#PB_2DDrawing_AlphaBlend)
                Box(\row\_s()\x, (\row\_s()\y+\row\_s()\height+Bool(*this\flag\gridlines>1))-*this\scroll\v\bar\page\pos, *this\width[2], 1, *this\color\line)
              EndIf
            EndIf
          Next
          ...
          If *this\flag\lines
            ...
            ForEach \row\_s()
              If \row\_s()\draw 
                If \row\_s()\l\h\height
                  ;  CustomFilterTree_CallBack(@Tree_PlotX())
                  Line(\row\_s()\l\h\x, \row\_s()\l\h\y, \row\_s()\l\h\width, \row\_s()\l\h\height, \row\_s()\color\line)
                EndIf
                
                If \row\_s()\l\v\width
                  ;  CustomFilterTree_CallBack(@Tree_PlotY())
                  Line(\row\_s()\l\v\x, \row\_s()\l\v\y, \row\_s()\l\v\width, \row\_s()\l\v\height, \row\_s()\color\line)
                EndIf
              EndIf    
            Next
          EndIf
          ...
          If *this\flag\buttons
            ...
            ForEach \row\_s()
              If \row\_s()\draw And \row\_s()\childrens
                Arrow(\row\_s()\box[0]\x+(\row\_s()\box[0]\width-6)/2,\row\_s()\box[0]\y+(\row\_s()\box[0]\height-6)/2, 6, Bool(Not \row\_s()\box[0]\checked)+2, \row\_s()\color\front[\row\_s()\color\state], 0,0) 
                ;             DrawingMode(#PB_2DDrawing_Outlined|#PB_2DDrawing_AlphaBlend)
                ;             ;RoundBox(\row\_s()\box[0]\x-3, \row\_s()\box[0]\y-3, \row\_s()\box[0]\width+6, \row\_s()\box[0]\height+6, 7,7, \row\_s()\color\front[\row\_s()\color\state])
                ;             RoundBox(\row\_s()\box[0]\x-1, \row\_s()\box[0]\y-1, \row\_s()\box[0]\width+2, \row\_s()\box[0]\height+2, 7,7, \row\_s()\color\front[\row\_s()\color\state])
                ;             Arrow(\row\_s()\box[0]\x+(\row\_s()\box[0]\width-4)/2,\row\_s()\box[0]\y+(\row\_s()\box[0]\height-4)/2, 4, Bool(Not \row\_s()\box[0]\checked)+2, \row\_s()\color\front[\row\_s()\color\state], 0,0) 
              EndIf    
            Next
          EndIf
    ...
    Procedure _draw_update_(*this._s_widget, List _items_._s_rows())
      If *this\change <> 0
        *this\scroll\width = 0
      ...
      ForEach _items_()
        ...
        ElseIf *this\text\fontID  
          If _items_()\fontID <> *this\text\fontID
            _items_()\fontID = *this\text\fontID
            
            DrawingFont(_items_()\fontID) 
            _items_()\text\height = *this\text\height
            ; Debug  " - "+_items_()\text\height +" "+ _items_()\text\string
          EndIf
        EndIf
        ...
        Else
          If *this\change
            ...
            _items_()\y = *this\y[2] + *this\scroll\height
            
            _items_()\width = *this\width[2] ; ???
          EndIf
          
          If (*this\change Or *this\scroll\v\change Or *this\scroll\h\change)
            ; check box
            If *this\flag\checkBoxes Or *this\flag\option_group
              _items_()\box[1]\x = _items_()\x + 3 - *this\scroll\h\bar\page\pos
              _items_()\box[1]\y = (_items_()\y+_items_()\height)-(_items_()\height+_items_()\box[1]\height)/2-*this\scroll\v\bar\page\pos
            EndIf
            
            ; expanded & collapsed box
            If *this\flag\buttons Or *this\flag\lines 
              _items_()\box[0]\x = _items_()\x + _items_()\sublevellen - *this\row\sublevellen + Bool(*this\flag\buttons) * 4 + Bool(Not *this\flag\buttons And *this\flag\lines) * 8 - *this\scroll\h\bar\page\pos 
              _items_()\box[0]\y = (_items_()\y+_items_()\height)-(_items_()\height+_items_()\box[0]\height)/2-*this\scroll\v\bar\page\pos
            EndIf
            
            ;
            _items_()\image\x = _items_()\x + *this\image\padding\left + _items_()\sublevellen - *this\scroll\h\bar\page\pos
            _items_()\image\y = _items_()\y + (_items_()\height-_items_()\image\height)/2-*this\scroll\v\bar\page\pos
            
            _items_()\text\x = _items_()\x + *this\text\padding\left + _items_()\sublevellen + *this\row\sublevel - *this\scroll\h\bar\page\pos
            _items_()\text\y = _items_()\y + (_items_()\height-_items_()\text\height)/2-*this\scroll\v\bar\page\pos
            
            _items_()\draw = Bool(_items_()\y+_items_()\height-*this\scroll\v\bar\page\pos>*this\y[2] And 
                                  (_items_()\y-*this\y[2])-*this\scroll\v\bar\page\pos<*this\height[2])
            
            ; lines for tree widget
            If *this\flag\lines And *this\row\sublevellen
              _tree_lines_(*this, _items_())
            EndIf
          ...
          If *this\change <> 0
            ; *this\scroll\height + _items_()\height + *this\flag\GridLines
            _tree_make_scroll_height_(*this)
            _tree_make_scroll_width_(*this)
            
            ;             If *this\scroll\width < ((_items_()\text\x + _items_()\text\width + *this\scroll\h\bar\page\pos) - *this\x[2])
            ;               *this\scroll\width = ((_items_()\text\x + _items_()\text\width + *this\scroll\h\bar\page\pos) - *this\x[2])
            ;             EndIf
    ...
    Procedure _tree_events_(*this._s_widget, eventtype.l, mouse_x.l=-1, mouse_y.l=-1, position.l=0)
      ...
      If (Not position And *this\scroll And *this\scroll\v And *this\scroll\h)
        ...
        If (*this\scroll\v\change Or *this\scroll\h\change)
          ...
          If StartDrawing(CanvasOutput(*this\root\canvas\gadget))
            _draw_update_(*this, *this\row\_s())
            StopDrawing()
          EndIf
    ...
    Procedure.l Tree_Events_Key(*this._s_widget, _event_type_.l, mouse_x.l=-1, mouse_y.l=-1)
    ...
    Procedure.l Tree_Events(*this._s_widget, _event_type_.l, mouse_x.l=-1, mouse_y.l=-1)
      Protected Result, from =- 1
      Static cursor_change, Down, *row_selected._s_rows
      ...
      With *this
        ...
        Select _event_type_ 
          Case #__Event_LeftButtonDown
            ...
            *this\root\mouse\delta\y = mouse_y
            ...
            Result | _tree_events_(*this, _event_type_, mouse_x, mouse_y)
        ...
        If _event_type_ = #__Event_MouseMove Or
           ...
           _event_type_ = #__Event_MouseLeave Or
           _event_type_ = #__Event_LeftButtonUp
          ; Debug ""+mouse_x +" - "+ _event_type_ +" "+ *this\root\mouse\delta\x;Bool(Abs((mouse_x-*this\root\mouse\delta\x)+(mouse_y-*this\root\mouse\delta\y)) >= 6)
          
          If *this = Root()\entered ;And *this\scroll\v\from =- 1 And *this\scroll\h\from =- 1 ;And Not *this\root\key; And Not *this\root\mouse\buttons
            
            If _from_point_(mouse_x, mouse_y, *this, [2]) 
              
              ; at item from points
              ForEach *this\row\_s()
                If (mouse_y >= *this\row\_s()\y-*this\scroll\v\bar\page\pos And
                    mouse_y < *this\row\_s()\y+*this\row\_s()\height-*this\scroll\v\bar\page\pos)
                  
                  If *this\row\index <> *this\row\_s()\index
                    If *this\row\index >= 0 ;And SelectElement(\row\_s(), \row\index)
                      Result | _tree_events_(*this, #__Event_MouseMove, mouse_x, mouse_y, -1)
                    EndIf
                    
                    *this\row\index = *this\row\_s()\index
        ...
        ; tree key events
        If GetActive()\gadget = *this
          Result | Tree_Events_Key(*this, _event_type_, mouse_x, mouse_y)
        EndIf
      EndWith
    ...
    Procedure   Window_Events(*this._s_widget, _event_type_.l, _mouse_x_.l, _mouse_y_.l)
      Protected Repaint
    ...
    Procedure   Panel_Events(*this._s_widget, _event_type_, _mouse_x_, _mouse_y_)
      Protected Repaint
      ...
      If _event_type_ = #__Event_MouseMove Or
        ...
        If *this\tab\bar\button[#__b_2]\len And 
           _from_point_(_mouse_x_, _mouse_y_, *this\tab\bar\button[#__b_2])
        ...
        ElseIf *this\tab\bar\button[#__b_1]\len And
               _from_point_(_mouse_x_, _mouse_y_, *this\tab\bar\button[#__b_1])
        ...
        Else
          ...
          If *this\count\items
            ForEach *this\bar\_s()
              If *this\bar\_s()\draw
                If _from_point_(_mouse_x_, _mouse_y_, *this\bar\_s()) And
                   _from_point_(_mouse_x_, _mouse_y_, *this\tab\bar\button[#__b_3])
      ...
      If *this\width[#__c_2] > 90
        ...
        If *this\tab\bar\button[#__b_2]\x > *this\tab\bar\button[#__b_3]\x 
          If Not _from_point_(_mouse_x_, _mouse_y_, *this\tab\bar\button[#__b_1])
            If *this\tab\bar\button[#__b_1]\color\state = #__s_3 Or
              ...
              If *this\tab\bar\button[#__b_1]\x > *this\tab\bar\button[#__b_2]\x-*this\tab\bar\button[#__b_1]\width
                If Not _from_point_(_mouse_x_, _mouse_y_, *this\tab\bar\button[#__b_2]) 
    ...
    Procedure   ReDraw(*this._s_widget, canvas_backcolor=#Null)
      If StartDrawing( CanvasOutput(*this\root\canvas\gadget) )
    ...
    Procedure.b edit2_Resizes(*scroll._s_scroll, X.l,Y.l,Width.l,Height.l)
      With *scroll
        ...
        *scroll\v\hide = Resize(*scroll\v, Width+x-*scroll\v\width, y, #PB_Ignore, _get_page_height_(*scroll, 1))
        
        Bar_SetAttribute(*scroll\h, #__bar_pagelength, _make_area_width_(*scroll, Width, Height))
        *scroll\h\hide = Resize(*scroll\h, x, Height+y-*scroll\h\height, _get_page_width_(*scroll, 1), #PB_Ignore)
        
        If Bar_SetAttribute(*scroll\v, #__bar_pagelength, _make_area_height_(*scroll, Width, Height))
          *scroll\v\hide = Resize(*scroll\v, #PB_Ignore, #PB_Ignore, #PB_Ignore, _get_page_height_(*scroll, 1))
        EndIf
        
        If Bar_SetAttribute(*scroll\h, #__bar_pagelength, _make_area_width_(*scroll, Width, Height))
          *scroll\h\hide = Resize(*scroll\h, #PB_Ignore, #PB_Ignore, _get_page_width_(*scroll, 1), #PB_Ignore)
        EndIf
    ...
    Procedure   AddItem(*this._s_widget, Item.i, Text.s, Image.i=-1, sublevel.i=0)
      With *this
        If *this\type = #PB_GadgetType_MDI
          ...
          OpenList(*this)
          *this\count\items + 1
          Window(pos_x+x, pos_y+y, Width, Height, Text, #__window_systemmenu|#__window_sizegadget|#__window_maximizegadget|#__window_minimizegadget, *this) : CloseList()
    ...
    Procedure Area(*parent._s_widget, Width, Height, AreaWidth, AreaHeight, ScrollStep, Mode = 1)
      *parent\scroll\v = Bar(#__Type_ScrollBar, *parent, 0,0,#__scroll_buttonsize,0, 0,AreaHeight, Height, #__scroll_buttonsize, #__bar_child|#__bar_vertical, 7, ScrollStep)
      *parent\scroll\h = Bar(#__Type_ScrollBar, *parent, 0,0,0,#__scroll_buttonsize, 0,AreaWidth, Width, Bool(Mode)*#__scroll_buttonsize, #__bar_child, 7, ScrollStep)
    EndProcedure
      ...
      With *this
        ...
        *this\bs = *this\fs
        ...
        If constants::_check_(flag, #__flag_noGadget, #False)
          OpenList(*this)
        EndIf
        Area(*this, width, height, ScrollAreaWidth, ScrollAreaHeight, ScrollStep)
        Resize(*this, X,Y,Width,Height)
      EndWith
    ...
    Procedure Events(*this._s_widget, _event_type_.l, _mouse_x_.l, _mouse_y_.l, _wheel_x_.b=0, _wheel_y_.b=0)
      ...
      If *this\type = #__Type_Window
        Repaint = Window_Events(*this, _event_type_, _mouse_x_, _mouse_y_)
      EndIf
      
      If *this\type = #PB_GadgetType_Panel
        Repaint = Panel_Events(*this, _event_type_, _mouse_x_, _mouse_y_)
      EndIf
      
      If *this\type = #PB_GadgetType_Tree
        Repaint = Tree_Events(*this, _event_type_, _mouse_x_, _mouse_y_)
      EndIf
      
      If *this\type = #PB_GadgetType_Editor
        Repaint = Editor_Events(*this, _event_type_, _mouse_x_, _mouse_y_)
      EndIf
      ...
      If *this\type = #PB_GadgetType_Spin Or
        ...
        Repaint = Bar_Events(*this, _event_type_, _mouse_x_, _mouse_y_, _wheel_x_, _wheel_y_)
      EndIf
            ...
            If _is_widget_(Root()\_childrens()) And
              ...
              *this = Root()\_childrens()
              ...
              If *this And *this\_tab 
                If Not *this\_tab\hide And 
                   *this\_tab\type And _from_point_(mouse_x,mouse_y, *this\_tab, [#__c_4])
                  *this = *this\_tab
                EndIf
              EndIf
...
CompilerIf #PB_Compiler_IsMainFile
  ...
  Global g_Canvas1, Button_0, Button_1, Button_2, Button_3, Button_4, Button_5, Splitter_0, Splitter_1, Splitter_2, Splitter_3, Splitter_4
  ...
  If OpenWindow(0, 0, 0, 605+30, 140+200+140+140, "ScrollBarGadget", #PB_Window_SystemMenu | #PB_Window_ScreenCentered)
    ...
    Open(0, 30,30,200,80)
;     g_Canvas1 = getgadget(root())
    Define g = Tree(0,0,200,180, #__flag_autosize)
    
    AddItem(g, -1, "index_0_level_0")
    AddItem(g, -1, "index_1_sublevel_1", 0, 1)
    AddItem(g, -1, "index_2_sublevel_2", 0, 2)
    AddItem(g, -1, "index_3_level_0")
    AddItem(g, -1, "index_4_sublevel_1", 0, 1)
    AddItem(g, -1, "index_5_sublevel_2", 0, 2)
    AddItem(g, -1, "Form_0")
    AddItem(g, -1, "Form_0")
    AddItem(g, -1, "Form_0")
    AddItem(g, -1, "Form_0")
    AddItem(g, -1, "Form_0")
    SetItemColor(g,  #PB_All, #__Color_Line,  $FF00f000)
    
    ;resize(g, #PB_Ignore, #PB_Ignore, 180, #PB_Ignore)
  ...
  Procedure ResizeCallBack()
    ...
    ResizeGadget(Splitter_0, #PB_Ignore, #PB_Ignore, WindowWidth(EventWindow(), #PB_Window_InnerCoordinate)-365, WindowHeight(EventWindow(), #PB_Window_InnerCoordinate)-16)
  ...
  g_Canvas1 = ButtonGadget(-1,0,0,0,0,"")
  g_Canvas = Gadget(#PB_GadgetType_Tree, -1, 270, 10, 250, 50);, "", #__flag_borderless)
  Define *g = GetGadgetData(g_Canvas)
  ;  3_example
...
;   root() = root
;   
;  ; Open(0, 30,30,200,380)
;     Splitter(8, 8, 306, 491,g_Canvas, g_Canvas1, #__flag_autosize)
; ;     Splitter_0 = getgadget(root())
; ;    
; ;    PostEvent(#PB_Event_SizeWindow, 0, #PB_Ignore) ; Bug no linux
; ;    BindEvent(#PB_Event_SizeWindow, @ResizeCallBack(), 0)
  
  Splitter_0 = SplitterGadget(-1,328, 8, 306, 291,g_Canvas, g_Canvas1)
  CompilerIf #PB_Compiler_Version =< 546
