...
DeclareModule Widget
  ...
  Declare.i CloseList()
  Declare.i OpenList(*this._S_widget, Item.l=0)
  ...
  Declare.i Post(eventtype.l, *this._S_widget, eventitem.l=#PB_All, *data=0)
  Declare.i Bind(*callback, *this._S_widget=#PB_All, eventtype.l=#PB_All)
    ...
    If *event\active <> _this_
      If *event\active 
        ...
        *event\active\mouse\buttons = 0
        ...
        If _this_\root\canvas\gadget <> *event\active\root\canvas\gadget 
          ; set lost focus canvas
          PostEvent(#PB_Event_Gadget, *event\active\root\canvas\window, *event\active\root\canvas\gadget, #PB_EventType_Repaint);, *event\active)
        EndIf
    ...
    With *this
      If *this > 0 
        ...
        If *Parent <> \parent Or \parent_item <> parent_item
          ...
          ;           If \root And ListSize(\root\childrens())
          ;             ChangeCurrentElement(\root\childrens(),*this\adress) 
          ...
          ; Add new children 
          If \index < 0 Or \index > ListSize(\root\childrens()) - 1
            LastElement(\root\childrens())
            \adress = AddElement(\root\childrens()) 
            \index = ListIndex(\root\childrens())
          Else
            SelectElement(\root\childrens(), \index)
            \adress = InsertElement(\root\childrens())
          EndIf
  ...
  Procedure.i SetPosition(*this._S_widget, Position.i, *Widget_2 =- 1) ; Ok SetStacking()
    ...
    With *this
      ...
      If \parent
        ...
        If *Widget_2 =- 1
          Select Position
            Case #PB_List_First  : MoveElement(\root\childrens(), #PB_List_First)
            Case #PB_List_Before : PreviousElement(\root\childrens()) : MoveElement(\root\childrens(), #PB_List_After, \root\childrens()\Adress)
            Case #PB_List_After  : NextElement(\root\childrens())     : MoveElement(\root\childrens(), #PB_List_Before, \root\childrens()\Adress)
            Case #PB_List_Last   : MoveElement(\root\childrens(), #PB_List_Last)
          EndSelect
          
        ElseIf *Widget_2
          Select Position
            Case #PB_List_Before : MoveElement(\root\childrens(), #PB_List_Before, *Widget_2)
            Case #PB_List_After  : MoveElement(\root\childrens(), #PB_List_After, *Widget_2)
          EndSelect
        EndIf
        
        ; \root\childrens()\adress = @\root\childrens()
  ...
  Procedure.i SetText(*this._S_widget, Text.s)
    ...
    With *this
      If \text And \text\string.s[1] <> Text.s
        \text\string.s[1] = Text.s ; Text_Make(*this, Text.s)
...
;         
;         If Root()\_active\_active : Root()\_active\_active\focus = 0
...
;     EndIf
    ...
    If Text
      *this = Form(-1, 0,0,Width,Height, Text.s, Flag, Root())
      Width = Width(*this)
      Height = Height(*this)
    Else
      SetActive(Root())
  ...
  Procedure.l Callback(*this._S_widget, eventtype.l, mouse_x.l, mouse_y.l)
    ...
    With *this
      ...
      enter = Bool(*event\enter <> *this And Not (*event\enter And *event\enter\index_ > *this\index_) And _from_point_(mouse_x, mouse_y, *this, [4]))
      leave = Bool(*event\enter And ((*event\enter = *this And Not _from_point_(mouse_x, mouse_y, *event\enter, [4]))))
      
      If leave
        If *event\enter\State 
          *event\enter\State = 0
          repaint | Events(*event\enter, #PB_EventType_MouseLeave, mouse_x, mouse_y)
        EndIf
      ...
      If enter
        *event\enter = *this
        ...
        If *event\enter\State = 0
          *event\enter\State = 1
          repaint | Events(*event\enter, #PB_EventType_MouseEnter, mouse_x, mouse_y)
        EndIf
      EndIf
      
      If ListSize(\childrens()) ;  \CountItems ; 
        ForEach \childrens()    ; LastElement(\childrens()) : Repeat                            ; Перебираем с низу верх
          enter = Bool(*event\enter <> \childrens() And Not (*event\enter And *event\enter\index_ > \childrens()\index_) And _from_point_(mouse_x, mouse_y, \childrens(), [4]))
          leave = Bool(*event\enter And *event\enter = \childrens() And Not _from_point_(mouse_x, mouse_y, *event\enter, [4]))
          ...
          If enter
            *parent_enter = \childrens()\parent
            ...
            If *event\enter <> \root And  Not (*event\enter And *event\enter\parent\index_ < *parent_enter\index_) ;Or  
               ; Not (*event\enter And *event\enter\index_ > \childrens()\index_)
             *parent = *event\enter
              ClearList(parentlist())
