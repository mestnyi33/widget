...
Module Widget
  ...
  Macro _set_last_parameters_(_this_, _type_, _flag_)
    *event\widget = _this_
    ...
    ; Set parent
    If LastElement(Root()\OpenedList())
      If _this_\type = #PB_GadgetType_Option
        If ListSize(Root()\OpenedList()\childrens()) 
          If Root()\OpenedList()\childrens()\type = #PB_GadgetType_Option
            _this_\option_group = Root()\OpenedList()\childrens()\option_group 
          Else
            _this_\option_group = Root()\OpenedList()\childrens() 
          EndIf
        Else
          _this_\option_group = Root()\OpenedList()
        EndIf
      EndIf
      
      SetParent(_this_, Root()\OpenedList(), Root()\OpenedList()\o_i)
    EndIf
  ...
  Procedure.i CloseList()
    If LastElement(Root()\OpenedList())
      If Root()\OpenedList()\type = #PB_GadgetType_Popup
        ReDraw(Root()\OpenedList())
      EndIf
      
      ProcedureReturn DeleteElement(Root()\OpenedList())
    EndIf
  EndProcedure
  ...
  Procedure.i OpenList(*this._S_widget, Item.l=0)
    ...
    With *this
      If *this > 0
        ...
        LastElement(Root()\OpenedList())
        result = AddElement(Root()\OpenedList())
        If result
          Root()\OpenedList() = *this 
          Root()\OpenedList()\o_i = Item
  ...
  Procedure.i Form(X.l,Y.l,Width.l,Height.l, Text.s, Flag.i=0, *Widget._S_widget=0)
    ...
    If *Widget 
      ...
      *this\class = #PB_Compiler_Procedure
      ...
      EndIf
    Else
      If ListSize(Root()\OpenedList()) 
        ChangeCurrentElement(Root()\OpenedList(), GetAdress(Root()))
        While NextElement(Root()\OpenedList())
          DeleteElement(Root()\OpenedList())
        Wend
      EndIf
