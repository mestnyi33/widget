...
Module Widget
      ...
      If _this_\type = #PB_GadgetType_Option
        If ListSize(Root()\opened\childrens()) 
          If Root()\opened\childrens()\type = #PB_GadgetType_Option
            _this_\option_group = Root()\opened\childrens()\option_group 
          Else
            _this_\option_group = Root()\opened\childrens() 
          EndIf
        Else
          _this_\option_group = Root()\opened
        EndIf
      EndIf
      
      SetParent(_this_, Root()\opened, Root()\opened\o_i)
    EndIf
  ...
  Procedure.i SetActive(*this._S_widget)
    ...
    Macro _set_active_state_(_state_)
      ...
      If Not(Active() = Active()\root And Active()\root\type =- 5)
        If _state_
        ...
        PostEvent(#PB_Event_Gadget, Active()\root\canvas\window, Active()\root\canvas\gadget, #PB_EventType_Repaint)
      EndIf
...
CompilerIf #PB_Compiler_IsMainFile
  ...
  Procedure Window_2()
    If OpenWindow(0, 0, 0, 830, 600, "Demo inverted scrollbar direction", #PB_Window_SystemMenu | #PB_Window_ScreenCentered | #PB_Window_SizeGadget)
      ...
      w_list(Hex(211)) = String(10, 10, 180, 85, "String_111") : SetData(w_list(Hex(211)), 211)
      w_list(Hex(212)) = String(10, 105, 180, 85, "String_112") : SetData(w_list(Hex(212)), 212) 
      
      Bind(@Widget_Handler())
      ReDraw(Root())
      
      w_list(Hex(220)) = Open(#PB_Any, 160, 120, 200, 200, "Window_120", #PB_Window_SystemMenu) : SetData(w_list(Hex(220)), 220)
      ;       Open(OpenWindow(-1, 160, 120, 200, 200, "", #PB_Window_BorderLess), 0, 0, 200, 200, "")
      ;       w_list(Hex(220)) = Form(0, 0, 200, 200, "Window_120", #PB_Window_SystemMenu) : SetData(w_list(Hex(220)), 120)
      w_list(Hex(221)) = String(10, 10, 180, 85, "String_121") : SetData(w_list(Hex(221)), 221)
      w_list(Hex(222)) = String(10, 105, 180, 85, "String_122") : SetData(w_list(Hex(222)), 222)
      
      Bind(@Widget_Handler())
      ReDraw(Root())
      
      w_list(Hex(230)) = Open(#PB_Any, 220, 140, 200, 200, "Window_130") : SetData(w_list(Hex(230)), 230)
      ;       Open(OpenWindow(-1, 220, 140, 200, 200, "", #PB_Window_BorderLess), 0, 0, 200, 200, "")
      ;       w_list(Hex(230)) = Form(0, 0, 200, 200, "Window_130", #PB_Window_SystemMenu) : SetData(w_list(Hex(230)), 130)
      w_list(Hex(231)) = String(10, 10, 180, 85, "String_131") : SetData(w_list(Hex(231)), 231)
      w_list(Hex(232)) = String(10, 105, 180, 85, "String_132") : SetData(w_list(Hex(232)), 232)
