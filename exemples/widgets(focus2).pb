...
DeclareModule Widget
  ...
  Macro GetFocus() ; active gadget 
    *event\active_window\active_gadget
  EndMacro
...
Module Widget
      ...
      If _this_\type = #PB_GadgetType_Option
        If ListSize(*value\OpenedList()\childrens()) 
          If *value\OpenedList()\childrens()\type = #PB_GadgetType_Option
            _this_\option_group = *value\OpenedList()\childrens()\option_group 
          Else
            _this_\option_group = *value\OpenedList()\childrens() 
          EndIf
        Else
          _this_\option_group = *value\OpenedList()
        EndIf
      EndIf
    ...
    With *this
      ...
      If \height>0 And \width>0 And Not \hide And \color\alpha 
        ...
        If \width[#_c_4] > 0 And \height[#_c_4] > 0
          ...
          If *this\focus And (*this = *event\active_window\active_gadget Or *this = *event\active_window)  ; Demo default coordinate
            DrawingMode(#PB_2DDrawing_Outlined)
  ...
  Procedure.i SetActive(*this._S_widget)
    ...
    With *this
      ...
      ;         w_Events(*this, #PB_EventType_Change, \root\anchor)
      ;       EndIf
      ...
      If \type > 0
        If *event\active_window\active_gadget <> *this
          
          If *event\active_window <> \window
            If *event\active_window
              *event\active_window\focus = 0
              Events(*event\active_window, *event\active_window\from, #PB_EventType_LostFocus, -1, -1)
              
              If *event\active_window\active_gadget
                *event\active_window\active_gadget\focus = 0
                Events(*event\active_window\active_gadget, *event\active_window\active_gadget\from, #PB_EventType_LostFocus, -1, -1)
              EndIf
            EndIf
            
            *event\active_window = \window
            *event\active_window\focus = 1
            Events(*event\active_window, *event\active_window\from, #PB_EventType_Focus, -1, -1)
          ...
          *event\active_window\active_gadget = *this
          *event\active_window\active_gadget\focus = 1
          Events(*event\active_window\active_gadget, *event\active_window\active_gadget\from, #PB_EventType_Focus, -1, -1)
        EndIf
        
      Else
        If *event\active_window <> *this
          If *event\active_window
            *event\active_window\focus = 0
            Events(*event\active_window, *event\active_window\from, #PB_EventType_LostFocus, -1, -1)
      ...
      SetPosition(*event\active_window, #PB_List_Last)
      
      If \window
        If \window\root
          PostEvent(#PB_Event_Gadget, \window\root\canvas\window, \window\root\canvas\gadget, #PB_EventType_Repaint)
        EndIf
      EndIf
    EndWith
  ...
  Procedure.i CloseList()
    If LastElement(*value\OpenedList())
      ...
      ProcedureReturn DeleteElement(*value\OpenedList())
    EndIf
  EndProcedure
  ...
  Procedure.i OpenList(*this._S_widget, Item.l=0)
    ...
    With *this
      If *this > 0
        ...
        result = AddElement(*Value\OpenedList())
        If result
  ...
  Procedure.i Open(Window.i, X.l,Y.l,Width.l,Height.l, Text.s="", Flag.i=0, WindowID.i=0)
    ...
    With *this
      If Not IsWindow(Window) And Window >- 2
        w = OpenWindow(Window, X,Y,Width,Height, Text.s, Flag, WindowID) : If Window =- 1 : Window = w : EndIf
        X = 0 
        Y = 0
      EndIf
  ...
  Procedure.i Free(*this._S_widget)
    ...
    With *this
      If *this
        ...
        *event\active_window = 0
        *event\active_window\active_gadget = 0
  ...
  Procedure.i Post(eventtype.l, *this._S_widget, eventitem.l=#PB_All, *data=0)
    ...
    *event\data = *data
    ...
    If *this And Not *this\event
      *this\event = AllocateStructure(_S_event)
      
      If Not ListSize(*event\list())
        AddElement(*event\list())
      EndIf
    EndIf
    
    If ListSize(*event\list())
      AddElement(*this\event\list())
      *this\event\list() = eventtype
    EndIf
  ...
  Procedure.i Bind(*callback, *this._S_widget=#PB_All, eventtype.l=#PB_All)
    ...
    *this\event\callback = *callback
    ...
    If ListSize(*event\list())
      Debug 5555
      ClearList(*event\list())
    EndIf
    
    If ListSize(*this\childrens()) ;
      
;       ForEach *this\event\list()
;         Post(*this\event\list(), *this, *this\from)
;         DeleteElement(*this\event\list())
;       Next
      
      ForEach *this\childrens()
        If *this\childrens()\event And ListSize(*this\childrens()\event\list()) 
          
          ForEach *this\childrens()\event\list()
            Post(*this\childrens()\event\list(), *this\childrens(), *this\childrens()\from)
            DeleteElement(*this\childrens()\event\list())
          Next
          
;           ClearStructure(*this\childrens()\event, _S_event)
;           *this\childrens()\event = 0
          
          Bind(*callback, *this\childrens(), eventtype)
        EndIf
      Next
    EndIf
      ...
      If *value\last <> *this
        ...
        If *value\last And *value\last <> Parent And *value\last <> Window And *value\last <> Root() 
          If *value\last\mouse\buttons
            ;             Debug "selected out"
          Else
            Events(*value\last, *value\last\from, #PB_EventType_MouseLeave, mouse_x, mouse_y)
          EndIf
        EndIf
        ...
        If *this
          If (Not *value\last Or (*value\last And *value\last\parent <> *this))
            ...
            ;             EndIf
            ;             
            ;             If (*value\last And *value\last\parent <> *this)
            ;               Debug "leave parent"
            ;             EndIf
      ...
      Select EventType 
        Case #PB_EventType_MouseMove,
          ...
          If Root()\mouse\buttons
            ...
            If *event\active_window\active_gadget 
              repaint | Events(*event\active_window\active_gadget, *event\active_window\active_gadget\from, #PB_EventType_MouseMove, mouse_x, mouse_y)
            EndIf  
        ...
        Case #PB_EventType_LeftButtonUp, 
          ...
          If EventType <> #PB_EventType_MiddleButtonDown
            If *event\active_window\active_gadget And
               *event\active_window\active_gadget\state = 2 
              *event\active_window\active_gadget\state = 1 
              
              repaint | Events(*event\active_window\active_gadget, *event\active_window\active_gadget\from, EventType, mouse_x, mouse_y)
              
              If _from_point_(mouse_x, mouse_y, *event\active_window\active_gadget, [#_c_4])
                
                If *event\active_window\active_gadget = *this       
                  If EventType = #PB_EventType_LeftButtonUp
                    repaint | Events(*event\active_window\active_gadget, *event\active_window\active_gadget\from, #PB_EventType_LeftClick, mouse_x, mouse_y)
                  EndIf
                  If EventType = #PB_EventType_RightClick
                    repaint | Events(*event\active_window\active_gadget, *event\active_window\active_gadget\from, #PB_EventType_RightClick, mouse_x, mouse_y)
                  EndIf
                EndIf
                
              Else
                *event\active_window\active_gadget\state = 0
                repaint | Events(*event\active_window\active_gadget, *event\active_window\active_gadget\from, #PB_EventType_MouseLeave, mouse_x, mouse_y)
              EndIf
              
              *event\active_window\active_gadget\mouse\buttons = 0   
              If *event\active_window\active_gadget\mouse\delta
                FreeStructure(*event\active_window\active_gadget\mouse\delta)
                *event\active_window\active_gadget\mouse\delta = 0
                *event\active_window\active_gadget\drag = 0
              EndIf
        ...
        Case #PB_EventType_Input, 
          ...
          If *this And (*event\active_window\active_gadget = *this Or *this = *event\active_window)
  ...
  Procedure Window_0()
    If OpenWindow(0, 0, 0, 900, 600, "Demo inverted scrollbar direction", #PB_Window_SystemMenu | #PB_Window_ScreenCentered | #PB_Window_SizeGadget)
      ...
      *window = Open(0, 10,10, 880, 550, "")
      ...
      If *window
        w_list(Hex(100)) = Form(520, 140, 200+2, 260+26+2, "Window_100", #PB_Window_ScreenCentered) : SetData(w_list(Hex(100)), 100)
        ...
        w_list(Hex(0)) = Form(100, 100, 200, 200, "Window_0", #PB_Window_SystemMenu) : SetData(w_list(Hex(0)), 0)
        ...
        w_list(Hex(10)) = Form(160, 120, 200, 200, "Window_10", #PB_Window_SystemMenu) : SetData(w_list(Hex(10)), 10)
        ...
        w_list(Hex(20)) = Form(220, 140, 200, 200, "Window_20", #PB_Window_SystemMenu) : SetData(w_list(Hex(20)), 20)
