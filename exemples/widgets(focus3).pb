...
Module Widget
    ...
    With *this
      ...
      If \height>0 And \width>0 And Not \hide And \color\alpha 
        ...
        If \width[#_c_4] > 0 And \height[#_c_4] > 0
          ...
          If *this\focus And (*this = *event\_active\active_gadget Or *this = *event\_active)  ; Demo default coordinate
            DrawingMode(#PB_2DDrawing_Outlined)
  ...
  Procedure.i SetActive(*this._S_widget)
    ...
    With *this
      If \type > 0
        If *event\_active\active_gadget <> *this
          ...
          If *event\_active <> \window
            If *event\_active
              *event\_active\focus = 0
              ...
              If *event\_active\active_gadget
                *event\_active\active_gadget\focus = 0
                Events(*event\_active\active_gadget, *event\_active\active_gadget\from, #PB_EventType_LostFocus, -1, -1)
              EndIf
            EndIf
          ...
          *event\_active\active_gadget = *this
          *event\_active\active_gadget\focus = 1
          Events(*event\_active\active_gadget, *event\_active\active_gadget\from, #PB_EventType_Focus, -1, -1)
        EndIf
    ...
    *this\event\callback = *callback
    ...
    If ListSize(*event\list()) 
      *event\bind = 1
      ...
      Next
      ClearStructure(*event\list(), _S_bind)
      ...
      Select EventType 
        Case #PB_EventType_MouseMove,
          ...
          If Root()\mouse\buttons
            ...
            If *event\_active\active_gadget 
              repaint | Events(*event\_active\active_gadget, *event\_active\active_gadget\from, #PB_EventType_MouseMove, mouse_x, mouse_y)
            EndIf  
        ...
        Case #PB_EventType_LeftButtonUp, 
          ...
          If EventType <> #PB_EventType_MiddleButtonDown
            If *event\_active\active_gadget And
               *event\_active\active_gadget\state = 2 
              *event\_active\active_gadget\state = 1 
              
              repaint | Events(*event\_active\active_gadget, *event\_active\active_gadget\from, EventType, mouse_x, mouse_y)
              
              If _from_point_(mouse_x, mouse_y, *event\_active\active_gadget, [#_c_4])
                
                If *event\_active\active_gadget = *this       
                  If EventType = #PB_EventType_LeftButtonUp
                    repaint | Events(*event\_active\active_gadget, *event\_active\active_gadget\from, #PB_EventType_LeftClick, mouse_x, mouse_y)
                  EndIf
                  If EventType = #PB_EventType_RightClick
                    repaint | Events(*event\_active\active_gadget, *event\_active\active_gadget\from, #PB_EventType_RightClick, mouse_x, mouse_y)
                  EndIf
                EndIf
                
              Else
                *event\_active\active_gadget\state = 0
                repaint | Events(*event\_active\active_gadget, *event\_active\active_gadget\from, #PB_EventType_MouseLeave, mouse_x, mouse_y)
              EndIf
              
              *event\_active\active_gadget\mouse\buttons = 0   
              If *event\_active\active_gadget\mouse\delta
                FreeStructure(*event\_active\active_gadget\mouse\delta)
                *event\_active\active_gadget\mouse\delta = 0
                *event\_active\active_gadget\drag = 0
              EndIf
        ...
        Case #PB_EventType_Input, 
          ...
          If *this And (*event\_active\active_gadget = *this Or *this = *event\_active)
