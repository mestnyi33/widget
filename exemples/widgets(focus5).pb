...
DeclareModule Widget
  ...
  ;- - _S_event
  Structure _S_event Extends _S_bind
    ;*leave._S_widget  
    *active._S_widget
...
Module Widget
    ...
    With *this
      ...
      If \height>0 And \width>0 And Not \hide And \color\alpha 
        ...
        If \width[#_c_4] > 0 And \height[#_c_4] > 0
          ...
          If *this\focus And (*this = *event\active\active_gadget Or *this = *event\active)  ; Demo default coordinate
            DrawingMode(#PB_2DDrawing_Outlined)
  ...
  Procedure.i SetActive(*this._S_widget)
    ...
    With *this
      Protected *win1._S_widget = *event\active
      Protected *win2._S_widget = *event\active
      ...
      If \type > 0
        ...
        If *event\active\active_gadget <> *this
          
          If *event\active <> \window
            If *event\active
              *event\active\focus = 0
              Events(*event\active, *event\active\from, #PB_EventType_LostFocus, -1, -1)
              
              If *event\active\active_gadget
                *event\active\active_gadget\focus = 0
                Events(*event\active\active_gadget, *event\active\active_gadget\from, #PB_EventType_LostFocus, -1, -1)
              EndIf
            EndIf
            
            *event\active = \window
            *event\active\focus = 1
            Events(*event\active, *event\active\from, #PB_EventType_Focus, -1, -1)
          Else
            If *event\active\active_gadget
              *event\active\active_gadget\focus = 0
              Events(*event\active\active_gadget, *event\active\active_gadget\from, #PB_EventType_LostFocus, -1, -1)
          ...
          *event\active\active_gadget = *this
          *event\active\active_gadget\focus = 1
          Events(*event\active\active_gadget, *event\active\active_gadget\from, #PB_EventType_Focus, -1, -1)
        EndIf
      ...
      SetPosition(*event\active, #PB_List_Last)
      ...
      If *win1
        *win2 = *event\active
        ...
        If *win1 <> *win2
          Select Root()
            Case *win1\root : ReDraw(*win1)
            Case *win2\root : ReDraw(*win2)
          EndSelect
        EndIf
      EndIf
    EndWith
  ...
  Procedure.i Post(eventtype.l, *this._S_widget, eventitem.l=#PB_All, *data=0)
    ...
    *event\type = eventtype
    ...
    ;       *event\list() = AllocateStructure(_S_bind)
    ;       *event\list()\widget = *this
    ;       *event\list()\type = eventtype
    ;       *event\list()\item = eventitem
    ;       *event\list()\data = *data
    ;     EndIf
  ...
  Procedure.i Bind(*callback, *this._S_widget=#PB_All, eventtype.l=#PB_All)
    ...
    *this\event\callback = *callback
    ...
    If ListSize(*event\list()) 
      *event\bind = 1
      ForEach *event\list()
        Post(*event\list()\type, *event\list()\widget, *event\list()\item, *event\list()\data)
      Next
      FreeStructure(*event\list())
    EndIf
      ...
      Select EventType 
        Case #PB_EventType_MouseMove,
          ...
          If Root()\mouse\buttons
            ...
            If *event\active\active_gadget 
              repaint | Events(*event\active\active_gadget, *event\active\active_gadget\from, #PB_EventType_MouseMove, mouse_x, mouse_y)
            EndIf  
        ...
        Case #PB_EventType_LeftButtonUp, 
          ...
          If EventType <> #PB_EventType_MiddleButtonDown
            If *event\active\active_gadget And
               *event\active\active_gadget\state = 2 
              *event\active\active_gadget\state = 1 
              
              repaint | Events(*event\active\active_gadget, *event\active\active_gadget\from, EventType, mouse_x, mouse_y)
              
              If _from_point_(mouse_x, mouse_y, *event\active\active_gadget, [#_c_4])
                
                If *event\active\active_gadget = *this       
                  If EventType = #PB_EventType_LeftButtonUp
                    repaint | Events(*event\active\active_gadget, *event\active\active_gadget\from, #PB_EventType_LeftClick, mouse_x, mouse_y)
                  EndIf
                  If EventType = #PB_EventType_RightClick
                    repaint | Events(*event\active\active_gadget, *event\active\active_gadget\from, #PB_EventType_RightClick, mouse_x, mouse_y)
                  EndIf
                EndIf
                
              Else
                *event\active\active_gadget\state = 0
                repaint | Events(*event\active\active_gadget, *event\active\active_gadget\from, #PB_EventType_MouseLeave, mouse_x, mouse_y)
              EndIf
              
              *event\active\active_gadget\mouse\buttons = 0   
              If *event\active\active_gadget\mouse\delta
                FreeStructure(*event\active\active_gadget\mouse\delta)
                *event\active\active_gadget\mouse\delta = 0
                *event\active\active_gadget\drag = 0
              EndIf
        ...
        Case #PB_EventType_Input, 
          ...
          If *this And (*event\active\active_gadget = *this Or *this = *event\active)
