...
DeclareModule Widget
  ...
  *value\height_tab = 25
  ...
  Macro GetFocus() ; active gadget 
    Root()\active_gadget\active_gadget
  EndMacro
...
Module Widget
    ...
    With *this
      ...
      If \height>0 And \width>0 And Not \hide And \color\alpha 
        ...
        If \width[#_c_4] > 0 And \height[#_c_4] > 0
          ...
          If *this\focus And (*this = Root()\active_gadget\active_gadget Or *this = Root()\active_gadget)  ; Demo default coordinate
            DrawingMode(#PB_2DDrawing_Outlined)
  ...
  Procedure.i SetActive(*this._S_widget)
    ...
    With *this
      If Root()
        Protected *win1._S_widget = Root()\active_gadget
        Protected *win2._S_widget = Root()\active_gadget
...
;         EndIf
        ...
        If \type > 0
;           If Not Root()\active_gadget
;             Root()\active_gadget = Root()
;           EndIf
          
          If Root()\active_gadget\active_gadget <> *this
            
            If Root()\active_gadget <> \window
              If Root()\active_gadget
                Root()\active_gadget\focus = 0
                Events(Root()\active_gadget, Root()\active_gadget\from, #PB_EventType_LostFocus, -1, -1)
                
                If Root()\active_gadget\active_gadget
                  Root()\active_gadget\active_gadget\focus = 0
                  Events(Root()\active_gadget\active_gadget, Root()\active_gadget\active_gadget\from, #PB_EventType_LostFocus, -1, -1)
                EndIf
              EndIf
              
              Root()\active_gadget = \window
              Root()\active_gadget\focus = 1
              Events(Root()\active_gadget, Root()\active_gadget\from, #PB_EventType_Focus, -1, -1)
            Else
              If Root()\active_gadget\active_gadget
                Root()\active_gadget\active_gadget\focus = 0
                Events(Root()\active_gadget\active_gadget, Root()\active_gadget\active_gadget\from, #PB_EventType_LostFocus, -1, -1)
            ...
            Root()\active_gadget\active_gadget = *this
            Root()\active_gadget\active_gadget\focus = 1
            Events(Root()\active_gadget\active_gadget, Root()\active_gadget\active_gadget\from, #PB_EventType_Focus, -1, -1)
          EndIf
          
        Else
          If Root()\active_gadget <> *this
            If Root()\active_gadget
              Root()\active_gadget\focus = 0
              Events(Root()\active_gadget, Root()\active_gadget\from, #PB_EventType_LostFocus, -1, -1)
        ...
        SetPosition(Root()\active_gadget, #PB_List_Last)
        
        ; redraw 
        If *win1
          *win2 = Root()\active_gadget
...
;         Root()\active_gadget\active_gadget = 0
;         Root()\active_gadget = 0
      ...
      Select EventType 
        Case #PB_EventType_MouseMove,
          ...
          If Root()\mouse\buttons
            ...
            If Root()\active_gadget\active_gadget 
              repaint | Events(Root()\active_gadget\active_gadget, Root()\active_gadget\active_gadget\from, #PB_EventType_MouseMove, mouse_x, mouse_y)
            EndIf  
        ...
        Case #PB_EventType_LeftButtonUp, 
          ...
          If EventType <> #PB_EventType_MiddleButtonDown
            If Root()\active_gadget\active_gadget And
               Root()\active_gadget\active_gadget\state = 2 
              Root()\active_gadget\active_gadget\state = 1 
              
              repaint | Events(Root()\active_gadget\active_gadget, Root()\active_gadget\active_gadget\from, EventType, mouse_x, mouse_y)
              
              If _from_point_(mouse_x, mouse_y, Root()\active_gadget\active_gadget, [#_c_4])
                
                If Root()\active_gadget\active_gadget = *this       
                  If EventType = #PB_EventType_LeftButtonUp
                    repaint | Events(Root()\active_gadget\active_gadget, Root()\active_gadget\active_gadget\from, #PB_EventType_LeftClick, mouse_x, mouse_y)
                  EndIf
                  If EventType = #PB_EventType_RightClick
                    repaint | Events(Root()\active_gadget\active_gadget, Root()\active_gadget\active_gadget\from, #PB_EventType_RightClick, mouse_x, mouse_y)
                  EndIf
                EndIf
                
              Else
                Root()\active_gadget\active_gadget\state = 0
                repaint | Events(Root()\active_gadget\active_gadget, Root()\active_gadget\active_gadget\from, #PB_EventType_MouseLeave, mouse_x, mouse_y)
              EndIf
              
              Root()\active_gadget\active_gadget\mouse\buttons = 0   
              If Root()\active_gadget\active_gadget\mouse\delta
                FreeStructure(Root()\active_gadget\active_gadget\mouse\delta)
                Root()\active_gadget\active_gadget\mouse\delta = 0
                Root()\active_gadget\active_gadget\drag = 0
              EndIf
        ...
        Case #PB_EventType_Input, 
          ...
          If *this And (Root()\active_gadget\active_gadget = *this Or *this = Root()\active_gadget)
