...
Module Widget
    ...
    With *this
      ...
      If \height>0 And \width>0 And Not \hide And \color\alpha 
        ...
        If \width[#_c_4] > 0 And \height[#_c_4] > 0
          ...
          If *this\focus And (*this = Root()\_active\_active Or *this = Root()\_active)  ; Demo default coordinate
            DrawingMode(#PB_2DDrawing_Outlined)
  ...
  Procedure.i SetActive(*this._S_widget)
    ...
    With *this
      If Root()
        Protected *win1._S_widget = Root()\_active
        Protected *win2._S_widget = Root()\_active
        ...
        If \type > 0
          ...
          If Root()\_active\_active <> *this 
            ;Debug " "+\root+" "+\window+" "+\root\window+" "+Root()+" "+Root()\window+" "+Root()\_active+" "+Root()\_active\_active
           
            If Root()\_active\_active = \root
              Root()\_active = *this
            EndIf
            
            If Root()\_active <> \window And \window <> \root
              If Root()\_active
                Root()\_active\focus = 0
                
                ;If Root()\_active <> Root()\_active\root
                  Events(Root()\_active, Root()\_active\from, #PB_EventType_LostFocus, -1, -1)
                  
                  If Root()\_active\_active
                    Root()\_active\_active\focus = 0
                    Events(Root()\_active\_active, Root()\_active\_active\from, #PB_EventType_LostFocus, -1, -1)
                  EndIf
                ;EndIf
              EndIf
            
              Root()\_active = \window
              Root()\_active\focus = 1
              ;If Root()\_active <> Root()\_active\root
                Events(Root()\_active, Root()\_active\from, #PB_EventType_Focus, -1, -1)
              ;EndIf
            Else
              If Root()\_active\_active
                Root()\_active\_active\focus = 0
            ...
            Root()\_active\_active = *this
            Root()\_active\_active\focus = 1
            Events(Root()\_active\_active, Root()\_active\_active\from, #PB_EventType_Focus, -1, -1)
          EndIf
          
        Else 
;           If Not (Root()\_active And Root()\_active = Root()\_active\root)
;             Root()\_active = *this
;           Else
            If Root()\_active <> *this 
            If Root()\_active
              Root()\_active\focus = 0
        ...
        ;If Root()\_active
          SetPosition(Root()\_active, #PB_List_Last)
        ;EndIf
  ...
  Procedure.i Form(X.l,Y.l,Width.l,Height.l, Text.s, Flag.i=0, *Widget._S_widget=0)
    ...
    *this\class = #PB_Compiler_Procedure
    ...
    With *this
      ...
      If Bool((flag&#PB_Window_TitleBar Or 
               flag&#PB_Window_SystemMenu Or
               flag&#PB_Window_MinimizeGadget Or
               flag&#PB_Window_MaximizeGadget) And
              Not flag&#PB_Window_BorderLess)
        \tabHeight = 25
      EndIf
      ...
      Select EventType 
        Case #PB_EventType_MouseMove,
          ...
          If Root()\mouse\buttons
            ...
            If Root()\_active\_active 
              repaint | Events(Root()\_active\_active, Root()\_active\_active\from, #PB_EventType_MouseMove, mouse_x, mouse_y)
            EndIf  
        ...
        Case #PB_EventType_LeftButtonUp, 
          ...
          If EventType <> #PB_EventType_MiddleButtonDown
            If Root()\_active\_active And
               Root()\_active\_active\state = 2 
              Root()\_active\_active\state = 1 
              
              repaint | Events(Root()\_active\_active, Root()\_active\_active\from, EventType, mouse_x, mouse_y)
              
              If _from_point_(mouse_x, mouse_y, Root()\_active\_active, [#_c_4])
                
                If Root()\_active\_active = *this       
                  If EventType = #PB_EventType_LeftButtonUp
                    repaint | Events(Root()\_active\_active, Root()\_active\_active\from, #PB_EventType_LeftClick, mouse_x, mouse_y)
                  EndIf
                  If EventType = #PB_EventType_RightClick
                    repaint | Events(Root()\_active\_active, Root()\_active\_active\from, #PB_EventType_RightClick, mouse_x, mouse_y)
                  EndIf
                EndIf
                
              Else
                Root()\_active\_active\state = 0
                repaint | Events(Root()\_active\_active, Root()\_active\_active\from, #PB_EventType_MouseLeave, mouse_x, mouse_y)
              EndIf
              
              Root()\_active\_active\mouse\buttons = 0   
              If Root()\_active\_active\mouse\delta
                FreeStructure(Root()\_active\_active\mouse\delta)
                Root()\_active\_active\mouse\delta = 0
                Root()\_active\_active\drag = 0
              EndIf
        ...
        Case #PB_EventType_Input, 
          ...
          If *this And (Root()\_active\_active = *this Or *this = Root()\_active)
