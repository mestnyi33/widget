...
DeclareModule Widget
  ...
  Macro GetFocus() ; _active gadget 
    *event\_active\_gadget
  EndMacro
  
  Macro GetActive() ; _active window
    *event\_active
  EndMacro
...
Module Widget
    ...
    With *this
      ...
      If \height>0 And \width>0 And Not \hide And \color\alpha 
        ...
        If \width[#_c_4] > 0 And \height[#_c_4] > 0
          ...
          If *this\focus And (*this = *event\_active\_gadget Or *this = *event\_active)  ; Demo default coordinate
            DrawingMode(#PB_2DDrawing_Outlined)
  ...
  Procedure.i SetActive(*this._S_widget)
    ...
    With *this
      ...
      If \type > 0
        If *event\_active\_gadget <> *this
          
          If *event\_active <> \window
            If *event\_active
              *event\_active\focus = 0
              Events(*event\_active, *event\_active\from, #PB_EventType_LostFocus, -1, -1)
              
              If *event\_active\_gadget
                *event\_active\_gadget\focus = 0
                Events(*event\_active\_gadget, *event\_active\_gadget\from, #PB_EventType_LostFocus, -1, -1)
              EndIf
            EndIf
            
            *event\_active = \window
            *event\_active\focus = 1
            Events(*event\_active, *event\_active\from, #PB_EventType_Focus, -1, -1)
          Else
            If *event\_active\_gadget
              *event\_active\_gadget\focus = 0
              Events(*event\_active\_gadget, *event\_active\_gadget\from, #PB_EventType_LostFocus, -1, -1)
          ...
          *event\_active\_gadget = *this
          *event\_active\_gadget\focus = 1
          Events(*event\_active\_gadget, *event\_active\_gadget\from, #PB_EventType_Focus, -1, -1)
        EndIf
        
      Else
        If *event\_active <> *this
          If *event\_active
            *event\_active\focus = 0
            Events(*event\_active, *event\_active\from, #PB_EventType_LostFocus, -1, -1)
      ...
      SetPosition(*event\_active, #PB_List_Last)
      ...
      Select EventType 
        Case #PB_EventType_MouseMove,
          ...
          If Root()\mouse\buttons
            ...
            If *event\_active\_gadget 
              repaint | Events(*event\_active\_gadget, *event\_active\_gadget\from, #PB_EventType_MouseMove, mouse_x, mouse_y)
            EndIf  
        ...
        Case #PB_EventType_LeftButtonUp, 
          ...
          If EventType <> #PB_EventType_MiddleButtonDown
            If *event\_active\_gadget And
               *event\_active\_gadget\state = 2 
              *event\_active\_gadget\state = 1 
              
              repaint | Events(*event\_active\_gadget, *event\_active\_gadget\from, EventType, mouse_x, mouse_y)
              
              If _from_point_(mouse_x, mouse_y, *event\_active\_gadget, [#_c_4])
                
                If *event\_active\_gadget = *this       
                  If EventType = #PB_EventType_LeftButtonUp
                    repaint | Events(*event\_active\_gadget, *event\_active\_gadget\from, #PB_EventType_LeftClick, mouse_x, mouse_y)
                  EndIf
                  If EventType = #PB_EventType_RightClick
                    repaint | Events(*event\_active\_gadget, *event\_active\_gadget\from, #PB_EventType_RightClick, mouse_x, mouse_y)
                  EndIf
                EndIf
                
              Else
                *event\_active\_gadget\state = 0
                repaint | Events(*event\_active\_gadget, *event\_active\_gadget\from, #PB_EventType_MouseLeave, mouse_x, mouse_y)
              EndIf
              
              *event\_active\_gadget\mouse\buttons = 0   
              If *event\_active\_gadget\mouse\delta
                FreeStructure(*event\_active\_gadget\mouse\delta)
                *event\_active\_gadget\mouse\delta = 0
                *event\_active\_gadget\drag = 0
              EndIf
          ...
          ; _active widget key state
        Case #PB_EventType_Input, 
             #PB_EventType_KeyDown, 
             #PB_EventType_KeyUp
          
          If *this And (*event\_active = *this Or *this = *event\_active\_gadget)
...
CompilerIf #PB_Compiler_IsMainFile
  ...
  Procedure Widget_Handler()
    ...
    Select EventType
      Case #PB_EventType_Focus   
        ...
        Else
          Debug "_active "+ GetData(EventWidget)
        EndIf
      ...
      Case #PB_EventType_LostFocus 
        ...
        Else
          Debug " De_active "+ GetData(EventWidget) ;Val(StringField(GetText(EventWidget), 2, "_")) ; +\Type +" "+ at +" "+ *This
        EndIf
  ...
  Procedure Window_1()
    w_list(Hex(110)) = Open(#PB_Any, 100, 100, 200, 200, "", #PB_Window_SystemMenu) : SetData(w_list(Hex(110)), 110)
      ...
      ReDraw(Root())
      ;       
      w_list(Hex(120)) = Open(#PB_Any, 160, 120, 200, 200, "Window_120", #PB_Window_SystemMenu) : SetData(w_list(Hex(120)), 120)
      ...
      w_list(Hex(130)) = Open(#PB_Any, 220, 140, 200, 200, "Window_130") : SetData(w_list(Hex(130)), 130)
  ...
  Procedure Window_2()
    If OpenWindow(0, 0, 0, 830, 600, "Demo inverted scrollbar direction", #PB_Window_SystemMenu | #PB_Window_ScreenCentered | #PB_Window_SizeGadget)
      ...
      If w_list(Hex(10))
        ...
        w_list(Hex(120)) = Form(160, 120, 200, 200, "Window_120", #PB_Window_SystemMenu) : SetData(w_list(Hex(120)), 120)
        w_list(Hex(121)) = String(10, 10, 180, 85, "String_121") : SetData(w_list(Hex(121)), 121)
        w_list(Hex(122)) = String(10, 105, 180, 85, "String_122") : SetData(w_list(Hex(122)), 122)
        
        w_list(Hex(130)) = Form(220, 140, 200, 200, "Window_130", #PB_Window_SystemMenu) : SetData(w_list(Hex(130)), 130)
        w_list(Hex(131)) = String(10, 10, 180, 85, "String_131") : SetData(w_list(Hex(131)), 131)
        w_list(Hex(132)) = String(10, 105, 180, 85, "String_132") : SetData(w_list(Hex(132)), 132)
      ...
      If w_list(Hex(20))
        ...
        w_list(Hex(150)) = Form(160, 120, 200, 200, "Window_150", #PB_Window_SystemMenu) : SetData(w_list(Hex(150)), 150)
        ...
        w_list(Hex(160)) = Form(220, 140, 200, 200, "Window_160", #PB_Window_SystemMenu) : SetData(w_list(Hex(160)), 160)
  ...
  Procedure Window_3()
    If OpenWindow(0, 0, 0, 900, 600, "Demo inverted scrollbar direction", #PB_Window_SystemMenu | #PB_Window_ScreenCentered | #PB_Window_SizeGadget)
      ...
      If w_list(Hex(-1))
        ...
        w_list(Hex(101)) = Button(10, 10, 180, 20, "101 _active window - 0") : SetData(w_list(Hex(101)), 101)
        ...
        w_list(Hex(104)) = Button(10, 85+10, 180, 20, "104 _active window - 10") : SetData(w_list(Hex(104)), 104)
        ...
        w_list(Hex(107)) = Button(10, 85+85+10, 180, 20, "107 _active window - 20") : SetData(w_list(Hex(107)), 107)
        ...
        Bind(@Widget_Handler());, w_list(Hex(22)))
        ReDraw(Root())
      EndIf
