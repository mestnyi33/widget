...
;                 EndIf
  ...
  Procedure.i SetActive(*this._S_widget)
    ...
    With *this
      ...
      If \window And *event\active\widget <> \window                                     And \window<>Root() And Not \root\anchor[#Anchor_moved]
        If *event\active\widget                                                          And *event\active\widget<>Root()
          \window\deactive = *event\active\widget 
          *event\active\widget\focus = 0
        EndIf
      ...
      If \window
        ...
        If \window\deactive And \window<>\window\deactive
          PostEvent(#PB_Event_Gadget, \window\deactive\root\canvas\window, \window\deactive\root\canvas\gadget, #PB_EventType_Repaint)
        EndIf
      EndIf
    EndWith
        ...
        If *Lastat = *This
          ...
          Select EventType 
            Case #PB_EventType_MouseWheel
              ...
              ElseIf *event\active\widget
                repaint | Events(*event\active\widget, - 1, EventType, MouseScreenX, MouseScreenY, WheelDelta)
              EndIf
              
            Case #PB_EventType_LeftButtonDown : mouseB = 1
              If \from
                If *event\active\widget <> *This
                  If *event\active\widget
                    repaint | Events(*event\active\widget, \from, #PB_EventType_LostFocus, MouseScreenX, MouseScreenY)
                  EndIf
                  repaint | Events(*This, \from, #PB_EventType_Focus, MouseScreenX, MouseScreenY)
                  *event\active\widget = *This
                EndIf
