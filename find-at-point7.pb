...
Procedure GetAtPointWidget( List this.widget( ), mouse_x, mouse_y )
  ...
  If ListSize( this( ) )
    LastElement( this( ) )
    Repeat
      If Not ( this( )\parent And this( )\draw ) And 
         Atbox( this( )\x, this( )\y, this( )\width, this( )\height, mouse_x, mouse_y )
        
        If this( )\childrens
          result = GetAtPointWidget( this( )\child( ), mouse_x, mouse_y )
         ; Debug ""+Str(this( )) +" "+ result
        EndIf
        If result = 0
          result = this( )
        EndIf
...
OpenWidget(-1,-1, 190,190,500,500)
...
For i=0 To count ; Step 5
  ...
  Else
    s + 5
  EndIf
  AddWidget(widget(), widget(), i+s,i+s, 50,50)
