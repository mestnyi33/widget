...
Procedure OpenWidget( window, gadget, x,y,width,height, flag = 0 )
  Protected g, w, result = AddElement( widget( ) )
...
Procedure GetAtPointWidget( List this.widget( ), mouse_x, mouse_y )
  ...
  If ListSize( this( ) )
    LastElement( this( ) )
    Repeat ; Not ( this( )\parent And this( )\draw ) And 
      If this( )\draw And Atbox( this( )\x, this( )\y, this( )\width, this( )\height, mouse_x, mouse_y )
 ...
 ;; Debug result
;   If result = 0
;     If Root( ) And Root( )\draw And Atbox( Root( )\x, Root( )\y, Root( )\width, Root( )\height, mouse_x, mouse_y )
;       result = Root( )
;     EndIf
;   EndIf
...
*root1.widget = OpenWidget(0,0, 0,0,300,300)

*root2.widget = OpenWidget(0,0, 300,300,600,600)
...
Define *add_widget.widget = *root2
...
For i=0 To count ; Step 5
  ...
  Else
    s + 5
  EndIf
  AddWidget(*root1, *root1\child( ), i,i+s, 50,50)
  AddWidget(*add_widget, *add_widget\child( ), i+s,i+s, 50,50)
  ...
  If ListIndex(*root1\child()) = 0
    For ii=0 To count ; Step 5
      ...
      Else
        ss + 10
      EndIf
      AddWidget(*root1\child(), *root1\child()\child(), ii+ss,ii, 50,50)
      AddWidget(*root2\child(), *root2\child()\child(), ii+ss,ii, 50,50)
...
; ;
; ReDrawWidget( *root2 )
StartDrawing(CanvasOutput(widget()\canvas\gadget))
FillMemory( DrawingBuffer( ), DrawingBufferPitch( ) * OutputHeight( ), $f0 )
ForEach widget()
...
Repeat 
  ...
  If event = #PB_Event_Gadget
    ...
    If EventType() = #PB_EventType_MouseMove
      If GetGadgetData( EventGadget( ) ) = 0
        Root( ) = GetAtPointWidget( widget( ), GetGadgetAttribute( EventGadget( ), #PB_Canvas_MouseX ), GetGadgetAttribute( EventGadget( ), #PB_Canvas_MouseY ) )
      EndIf  
      
      If Root( )
        ;Define time = ElapsedMilliseconds()
        *widget.widget = GetAtPointWidget( Root( )\child( ), GetGadgetAttribute( EventGadget( ), #PB_Canvas_MouseX ), GetGadgetAttribute( EventGadget( ), #PB_Canvas_MouseY ) )
        ; Debug ElapsedMilliseconds() - time
