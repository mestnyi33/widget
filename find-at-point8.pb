...
Procedure OpenWidget( window, gadget, x,y,width,height, flag = 0 )
  ...
  If *this
    *this\canvas = AllocateStructure( canvas )
...
; Procedure _GetAtPointWidget( List this.s_widget( ), mouse_x, mouse_y )
;   Protected result
;   
;   If ListSize( this( ) )
;     LastElement( this( ) )
...
Procedure GetAtPointWidget( List *this.s_widget( ), mouse_x, mouse_y )
  Protected result
...
*root1.s_widget = OpenWidget(0,-1, 0,0,300,300)

*root2.s_widget = OpenWidget(0,-1, 300,300,600,600)
  ...
  If ListIndex(*root1\child()) = 0
    For ii=0 To count ; Step 5
      ...
      Else
        ss + 10
      EndIf
      AddWidget(*root1\child(), ii+ss,ii, 50,50)
...
*first.s_widget = FirstElement(*root1\child())
...
Repeat 
  ...
  If event = #PB_Event_Gadget
    ...
    If EventType() = #PB_EventType_MouseMove
      If Root( )
        ;Define time = ElapsedMilliseconds()
        *widget.s_widget = GetAtPointWidget( Root( )\child( ), GetGadgetAttribute( EventGadget( ), #PB_Canvas_MouseX ), GetGadgetAttribute( EventGadget( ), #PB_Canvas_MouseY ) )
        ; Debug ElapsedMilliseconds() - time
