      ...
      If *bar\widget\type = #__type_Splitter 
        If *bar\thumb\len 
          If *bar\widget\vertical
            *bar\button[#__split_3]\x = *bar\widget\x[#__c_frame]           + 1 ; white line size 
            *bar\button[#__split_3]\width = *bar\widget\width[#__c_frame]   - 1 ; white line size 
            *bar\button[#__split_3]\y = *bar\widget\y[#__c_inner_b] + *bar\thumb\pos
            *bar\button[#__split_3]\height = *bar\thumb\len                              
          Else
            *bar\button[#__split_3]\y = *bar\widget\y[#__c_frame]           + 1 ; white line size
            *bar\button[#__split_3]\height = *bar\widget\height[#__c_frame] - 1 ; white line size
            *bar\button[#__split_3]\x = *bar\widget\x[#__c_inner_b] + *bar\thumb\pos 
            *bar\button[#__split_3]\width = *bar\thumb\len                                  
          EndIf
        EndIf
        
        If *bar\widget\vertical
          *bar\button[#__split_1]\width    = *bar\widget\width[#__c_frame]
          *bar\button[#__split_1]\height   = *bar\thumb\pos
          
          *bar\button[#__split_1]\x        = *bar\widget\x[#__c_frame]
          *bar\button[#__split_2]\x        = *bar\widget\x[#__c_frame]
          
          If Not (( #PB_Compiler_OS = #PB_OS_MacOS ) And splitter_is_first_gadget_( *bar\widget ) And Not *bar\widget\parent( ) )
            *bar\button[#__split_1]\y      = *bar\widget\y[#__c_frame] 
            *bar\button[#__split_2]\y      = ( *bar\thumb\pos + *bar\thumb\len ) + *bar\widget\y[#__c_frame] 
          Else
            *bar\button[#__split_1]\y      = *bar\widget\height[#__c_frame] - *bar\button[#__split_1]\height
          EndIf
          
          *bar\button[#__split_2]\height   = *bar\widget\height[#__c_frame] - ( *bar\button[#__split_1]\height + *bar\thumb\len )
          *bar\button[#__split_2]\width    = *bar\widget\width[#__c_frame]
          
        Else
          *bar\button[#__split_1]\width    = *bar\thumb\pos
          *bar\button[#__split_1]\height   = *bar\widget\height[#__c_frame]
          
          *bar\button[#__split_1]\y        = *bar\widget\y[#__c_frame]
          *bar\button[#__split_2]\y        = *bar\widget\y[#__c_frame]
          *bar\button[#__split_1]\x        = *bar\widget\x[#__c_frame]
          *bar\button[#__split_2]\x        = ( *bar\thumb\pos + *bar\thumb\len ) + *bar\widget\x[#__c_frame]
          
          *bar\button[#__split_2]\width    = *bar\widget\width[#__c_frame] - ( *bar\button[#__split_1]\width + *bar\thumb\len )
          *bar\button[#__split_2]\height   = *bar\widget\height[#__c_frame]
        ...
        If splitter_first_gadget_( *bar\widget )
          If splitter_is_first_gadget_( *bar\widget )
            ...
            Else
              PB(ResizeGadget)( splitter_first_gadget_( *bar\widget ),
                                *bar\button[#__split_1]\x + GadgetX( *bar\widget\root\canvas\gadget ), 
                                *bar\button[#__split_1]\y + GadgetY( *bar\widget\root\canvas\gadget ),
                                *bar\button[#__split_1]\width, *bar\button[#__split_1]\height )
            EndIf
          Else
            If splitter_first_gadget_( *bar\widget )\x <> *bar\button[#__split_1]\x Or
               splitter_first_gadget_( *bar\widget )\y <> *bar\button[#__split_1]\y Or
               splitter_first_gadget_( *bar\widget )\width <> *bar\button[#__split_1]\width Or
               splitter_first_gadget_( *bar\widget )\height <> *bar\button[#__split_1]\height
        ...
        If splitter_second_gadget_( *bar\widget )
          If splitter_is_second_gadget_( *bar\widget )
            ...
            Else
              PB(ResizeGadget)( splitter_second_gadget_( *bar\widget ), 
                                *bar\button[#__split_2]\x + GadgetX( *bar\widget\root\canvas\gadget ),
                                *bar\button[#__split_2]\y + GadgetY( *bar\widget\root\canvas\gadget ),
                                *bar\button[#__split_2]\width, *bar\button[#__split_2]\height )
            EndIf
          Else
            If splitter_second_gadget_( *bar\widget )\x <> *bar\button[#__split_2]\x Or 
               splitter_second_gadget_( *bar\widget )\y <> *bar\button[#__split_2]\y Or
               splitter_second_gadget_( *bar\widget )\width <> *bar\button[#__split_2]\width Or
               splitter_second_gadget_( *bar\widget )\height <> *bar\button[#__split_2]\height 
      ...
      *bar\thumb\end = *bar\area\len - ( BBB1( )\size + BBB2( )\size ) - bordersize*2
      ...
      If Not *bar\max And
        ...
        *bar\thumb\len = *bar\button[#__split_3]\size
        ...
        Else
          ; get page end
          If ( *bar\page\change Or *bar\fixed = #__split_1 )
            *bar\page\end = *bar\area\len - *bar\thumb\len 
          EndIf
        EndIf
      EndIf
      ...
      With *this
        ...
        If *bar\page\pos <> ScrollPos 
          ...
          *bar\page\pos = ScrollPos
          ...
          DoEvents( *bar\widget, #PB_EventType_Change, EnteredButton( ), *bar\page\change )
        ...
        ElseIf tabindex
          If *parent\type = #__type_Splitter
            If tabindex%2
              ...
              bar_Update( *parent\bar )
              bar_Resize( *parent\bar )  
