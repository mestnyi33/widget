...
CompilerIf Not Defined( Widget, #PB_Module )
  ...
  DeclareModule Widget
    ...
    Macro BB1( ) : BB( *this\bar, [#__split_1]): EndMacro
    Macro BB2( ) : BB( *this\bar, [#__split_2]): EndMacro
    Macro BB3( ) : BB( *this\bar, [#__split_3]): EndMacro
    
    
    Macro EnteredButton( ) : BB( *this\bar, [0] ): EndMacro
...
;     EndMacro
  ...
  Module Widget
    ...
    Procedure.b Update( *this._S_widget, mode.b = 1 )
      Protected fixed.l, result.b, ScrollPos.f, ThumbPos.i =- 1, ThumbChange
...
;           If *this\vertical
;             If SetThumbPos( *this, ( Mouse( )\y - Mouse( )\delta\y ))
;               Repaint = 1
;             EndIf
;           Else
;             If SetThumbPos( *this, ( Mouse( )\x - Mouse( )\delta\x ))
;               Repaint = 1
;             EndIf
;           EndIf
        ...
        *this\color = _get_colors_( )
        ...
        If *this\type = #__type_Splitter
          ...
          *this\bar.allocate( BAR )
          BB( *this\bar ).allocate( BUTTONS, [#__split_1] )
          BB( *this\bar ).allocate( BUTTONS, [#__split_2] )
          BB( *this\bar ).allocate( BUTTONS, [#__split_3] )
      ...
      If *this\bar
        ...
        If eventtype = #PB_EventType_LeftButtonDown
          ...
          If PressedButton( ) And
            ...
            If PressedButton( ) = BB3( )
              DoEvents( *this, #PB_EventType_StatusChange, PressedButton( ), 2 )
            EndIf
        ...
        If eventtype = #PB_EventType_LeftButtonUp
          If PressedButton( )
            If PressedButton( )\state\press = #True
              ...
              If PressedButton( )\state\enter = 0
                 DoEvents( *this, #PB_EventType_StatusChange, PressedButton( ), - 1 )
              EndIf
            EndIf
          EndIf
          
          ; drop
          If EnteredButton( ) And EnteredButton( ) <> PressedButton( ) And EnteredButton( )\state\enter
            Debug 7778888
            DoEvents( EnteredWidget( ), #PB_EventType_StatusChange, EnteredButton( ), 1 )
          EndIf
        EndIf
          ...
          If EnteredButton( )
            If EnteredButton( )\state\enter = #True
              ...
              If Not mouse( )\buttons
               ; DoEvents( *this, #PB_EventType_StatusChange, EnteredButton( ), - 1 )
              EndIf
            EndIf
          EndIf
          ...
          If EnteredWidget( )
            ...
            If ( eventtype = #PB_EventType_LeftButtonDown Or
              ...
              If Not EnteredWidget( )\_a_\transform 
                If EnteredWidget( )\bar And EnteredWidget( )\bar\button > 0
                  ; bar mouse delta pos
                  ;; Debug "   bar delta pos >> "+ #PB_Compiler_Procedure +" ( "+#PB_Compiler_Line +" )"
                  ;If EnteredButton( EnteredWidget( ) ) = BB( EnteredWidget( )\bar, #__b_3 ) ; EnteredButton( EnteredWidget( ) )\index = #__b_3
