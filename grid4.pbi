...
Structure _s_ROWS
  Text._s_TEXT : Level.i : Height.i : Y.i : Hide.b : IsSelected.b : IsGroup.b : IsFolded.b
EndStructure
...
Structure _s_WIDGET
  ...
  List *__Items._s_ROWS()          ; Визуальный рулон (указатели)
...
Procedure Row_UpdateLayout(*this._s_WIDGET)
  ...
  ForEach *this\__Items()
    ...
    ; Если высота не задана (0), вычисляем по шрифту (пример для 1 строки)
    If *this\RowHeight = 0 And *row\Height = 0
       *row\Height = 24 ; Тут можно добавить замер через TextHeight()
    ElseIf *this\RowHeight > 0
...
Procedure Column_HandleSwap(*this._s_WIDGET, mx, my)
  Protected *root.struct_ROOT = *this\root
    ...
    ForEach *this\__columns()
      ...
      If *target = *root\DragColumn : Continue : EndIf ; Не сравниваем с собой
      
      ; Координата X текущей перебираемой колонки
      Protected cx = *this\X + *target\X - *this\scroll_h_pos
      
      ; Если мышь внутри границ этой колонки
      If mx > cx And mx < cx + *target\Width
        PushListPosition(*this\__columns())
        
        ; Определяем, куда вставить: До или После середины *target
        ChangeCurrentElement(*this\__columns(), *root\DragColumn)
        
        If mx > cx + (*target\Width / 2)
          MoveElement(*this\__columns(), #PB_List_After, *target)
        Else
          MoveElement(*this\__columns(), #PB_List_Before, *target)
        EndIf
        
        PopListPosition(*this\__columns())
        Column_UpdateLayout(*this) ; Пересчитываем X всех колонок
        Break ; Выход, так как перемещение совершено
      EndIf
    Next
  ...
  ElseIf *root\DragItem And *this\HoverItem
    If *this\HoverItem <> *root\DragItem
      ...
      Protected *dragNode = 0, *hoverNode = 0, hover_y = *this\Y + *this\HeaderHeight
      
      ; Ищем узлы и вычисляем Y наведенного элемента
      ForEach *this\__Items()
        If *this\__Items() = *this\HoverItem : *hoverNode = @*this\__Items() : Break : EndIf
        hover_y + *this\__Items()\Height
      Next
      ForEach *this\__Items()
        If *this\__Items() = *root\DragItem  : *dragNode = @*this\__Items() : Break : EndIf
      Next
      
      If *dragNode And *hoverNode
        ChangeCurrentElement(*this\__Items(), *dragNode)
        ; Интеллектуальный Swap: До или После середины строки
        If my > hover_y + (*this\HoverItem\Height / 2)
          MoveElement(*this\__Items(), #PB_List_After, *hoverNode)
        Else
    ...
    *this\HoverColumn = 0 : *this\HoverItem = 0
    ...
    If mx >= *this\X And mx <= *this\X + *this\Width And my >= *this\Y And my <= *this\Y + *this\Height
      *root\ActiveWidget = *this
      ...
      ; Твое исправление Hit-Test: разделение Header и Body
      ForEach *this\__columns()
        If mx >= (*this\X + *this\__columns()\X) - *this\scroll_h_pos And 
           mx <= (*this\X + *this\__columns()\X + *this\__columns()\Width) - *this\scroll_h_pos And
           my >= *this\Y And my <= *this\Y + *this\HeaderHeight
           *this\HoverColumn = @*this\__columns() : Break
        EndIf
      Next
      
      If *this\visible\firstElement And my > *this\Y + *this\HeaderHeight
  ...
  Select et
    ...
    Case #PB_EventType_MouseMove
      ...
      ElseIf *root\DragColumn Or *root\DragItem 
        ...
        ; Auto-Scroll Logic
        If *root\DragItem
           If my < *this\Y + *this\HeaderHeight + 20 : *this\scroll_v_pos - 5 : ElseIf my > *this\Y + *this\Height - 25 : *this\scroll_v_pos + 5 : EndIf
...
AddElement(*w\__columns()) : *w\__columns()\Text\Str(0) = "Файл" : *w\__columns()\Width = 200
AddElement(*w\__columns()) : *w\__columns()\Text\Str(0) = "Тип" : *w\__columns()\Width = 100
AddElement(*w\__columns()) : *w\__columns()\Text\Str(0) = "Дата" : *w\__columns()\Width = 150
Column_UpdateLayout(*w)

Column_AddItem(*w, "Мои проекты" + #LF$ + "Папка" + #LF$ + "2024", 0, #True)
Define i : For i = 1 To 30 : Column_AddItem(*w, "Элемент_" + Str(i) + #LF$ + "Файл" + #LF$ + "Январь", 1) : Next
