...
#SystemEOL = #LF$ 
#SystemHelpExt = ""
#SystemBatchExt = ".sh";unsure of this one... but it seems to work in terminal
...
#Asm_Format = "";OS X doesn't use a format header. (uses nasm instead of fasm)
#Asm_Formatx64 = ""; non existant atm, however it needs to be defined :)
#Asm_TextSection = "section .text"
#Asm_MOV_EAX_EXTERNAL = "MOV    eax,"

#MAXLONG = 2147483647
#MAX_PATH = 260
#CCS_NODIVIDER = 0;not used on OS X, so it doesn't matter what it's set to
...
Procedure ProgressBarSetIndeterminate(gadget, state)
  SetControlData(GadgetID(gadget), #kControlEntireControl, #kControlProgressBarIndeterminateTag, SizeOf(state), @state)
EndProcedure
...
Procedure.s CompileAsm(file.s, out.s)
  ProcedureReturn ExecuteProgram("nasm", "-f macho -O1 -o "+#DQUOTE$+out+#DQUOTE$+" "+#DQUOTE$+file+#DQUOTE$, TBDestFolder$, TBDestFolder$)
EndProcedure

Procedure HandleASMCompileFailure(asmCompilerOutput.s, FileString$)
  If Len(asmCompilerOutput) > 0;nasm outputs nothing if compiled correctly
    TBError("nasm: "+FileString$+NL$+NL$+asmCompilerOutput, 1, TBTempPath$)
    End
  EndIf
EndProcedure
