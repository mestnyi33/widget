...
Procedure SplitFunctions(FileStart, FileEnd, DestFolder$, FinalDestFolder$, mID, decoration.s="")
  ...
  ;MP Jan 2015
  If PBnbVersion>=524 ;And PBVersionX64

    ;- float Problem x64
  ;  *delSeeker = FileStart
  ;  *delSeeker = FindNextString("FSTP   dword [rsp-8]", *delSeeker, FileEnd)
  ;  While *delSeeker<FileEnd 
  ;    PokeS(*delSeeker,";")
  ;    *delSeeker = FindNextString("FSTP   dword [rsp-8]", *delSeeker+4, FileEnd)
  ;  Wend
  
    *delSeeker = FileStart
    *delSeeker = FindNextString("MOVSS  xmm0,[rsp-8]", *delSeeker, FileEnd)
    While *delSeeker<FileEnd 
      PokeS(*delSeeker,";")
      *delSeeker = FindNextString("MOVSS  xmm0,[rsp-8]", *delSeeker+4, FileEnd)
    Wend
    ;-
    ;- double Problem x64
  ;  *delSeeker = FileStart
  ...
  ;    PokeS(*delSeeker,";")
  ;    *delSeeker = FindNextString("FSTP   qword [rsp-8]", *delSeeker+4, FileEnd)
  ;  Wend
    ...
    *delSeeker = FindNextString("MOVSD  xmm0,[rsp-8]", *delSeeker, FileEnd)
    While *delSeeker<FileEnd 
      PokeS(*delSeeker,";")
      *delSeeker = FindNextString("MOVSD  xmm0,[rsp-8]", *delSeeker+4, FileEnd)
    Wend  ;-
  EndIf
  ;MP Jan 2015  
  ...
  While PeekS(FileSeeker, Len(HeaderEnd$))<>HeaderEnd$ And FileSeeker<FileEnd
    ...
    Else
      If FunctionType=1
        PBLibName$ = UCase(PBLibName$) ; Changing MPZ PB 4.50 Beta1
        If Right(PBLibName$,4) = ".LIB"
           PBLibName$ = Left(PBLibName$,Len(PBLibName$)-4)
        EndIf
