...
;     CGEventTapCreateForPSN(*psn, place.i, options.i, eventsOfInterest.q, callback.i, refcon)
    ...
    If refcon
      ...
      If eType = #NSLeftMouseUp
        ...
        If PressedGadget() >= 0 
          ...
          ;           If LeftDoubleClick
          ;             CallCFunctionFast(refcon, #PB_Event_Gadget, PressedGadget(), #PB_EventType_LeftDoubleClick)
          ;           Else
          ;             If PressedGadget() <> DraggedGadget()
          ;               If PressedGadget() >= 0 And EnteredID = GadgetID(PressedGadget())
          ;                 CallCFunctionFast(refcon, #PB_Event_Gadget, PressedGadget(), #PB_EventType_LeftClick)
          ;               EndIf
          ;             EndIf
          ;           EndIf
        EndIf
  ...
  Procedure ResizeEvent( )
    If EventType() = #PB_EventType_Resize
      CallCFunctionFast(*setcallback, #PB_Event_Gadget, EventGadget(), EventType())
    EndIf
  EndProcedure
  ...
  Procedure.i WaitEvent(event.i, second.i=0)
  	If *setcallback 
  	   If event = #PB_Event_ActivateWindow
  	      CallCFunctionFast(*setcallback, event, -1, -1, 0 )
  	   EndIf
  	   ...
  	   If event = #PB_Event_Gadget
  			...
  			CompilerIf Defined(constants::PB_EventType_Repaint, #PB_Constant) 
  				If EventType = constants::#PB_EventType_Repaint
  					CallCFunctionFast(*setcallback, event, EventGadget, EventType, EventData )
  				EndIf
  			CompilerEndIf
  		...
  		If Not event 
  			CompilerIf Defined(constants::PB_EventType_Repaint, #PB_Constant) 
  			;	CallCFunctionFast(*setcallback, #PB_Event_Gadget, 0, constants::#PB_EventType_Repaint, 0 )
  			CompilerEndIf
  	   EndIf
  	EndIf
