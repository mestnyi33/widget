...
Procedure RemoveItem(*this._s_WIDGET, Index.l)
  If SelectElement(*this\Items(), Index)
    DeleteElement(*this\Items()) ; PB сам очистит вложенный List ColText()
  EndIf
EndProcedure
...
Procedure RemoveColumn(*this._s_WIDGET, Index.l)
  If SelectElement(*this\Columns(), Index)
    ; Сначала удаляем данные этой колонки во всех строках
    ForEach *this\Items()
...
Procedure ClearItems(*this._s_WIDGET)
  ClearList(*this\Items()) ; PB всё вычистит сам
EndProcedure
  ...
  If StartDrawing(CanvasOutput(*this\GadgetID))
    ...
    ForEach *this\Items()
      ...
      ForEach *this\Columns()
        ...
        CurX = X + 5
        ; Смещение для чекбокса и иконки (только в 0 колонке)
...
Procedure.i AddColumn(*this._s_WIDGET, position.l, Text.s, Width.l, img.i = -1, Align.b = #ALIGN_LEFT)
  ...
  If position = -1 Or position >= ListSize(*this\Columns())
    LastElement(*this\Columns()) : AddElement(*this\Columns())
  Else
    SelectElement(*this\Columns(), position) : InsertElement(*this\Columns())
  EndIf
...
Procedure.i AddItem(*this._s_WIDGET, Item.l, Text.s, img.i = -1, Flag.q = 0)
  If Not *this : ProcedureReturn #False : EndIf
  If Item = -1 Or Item >= ListSize(*this\Items())
    LastElement(*this\Items()) : AddElement(*this\Items())
  Else
    SelectElement(*this\Items(), Item) : InsertElement(*this\Items())
  EndIf
...
Procedure DoEvents(*this._s_WIDGET)
  ...
  Protected GID = *this\GadgetID
  Protected mx = GetGadgetAttribute(GID, #PB_Canvas_MouseX)
  Protected my = GetGadgetAttribute(GID, #PB_Canvas_MouseY)
  Protected curX, idx
...
OpenWindow(0, 0, 0, 640, 300, "Object ListIcon Full", #PB_Window_SystemMenu | #PB_Window_ScreenCentered)
