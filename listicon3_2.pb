...
Structure GridRow
   ...
   Array Cells.GridCell(0) ; Список ячеек, лежащих в исходном порядке (DataID)
EndStructure
      ...
      ForEach *MyGrid\Rows()
         ...
         If Y >= RowHeight And Y <= H - 30
            ...
            ForEach *MyGrid\Columns() ; Бежим строго по текущему порядку колонок в шапке
               ...
               Define *Cell.GridCell = *MyGrid\Rows()\Cells(*MyGrid\Columns()\DataID)
...
Procedure CanvasCallback()
   ...
   HoverCol = HowerColumnn(*MyGrid, X)
...
Procedure GetCell( *MyGrid.PureGrid, *row.GridRow, col.l )
   ...
   If *column
      ProcedureReturn *row\Cells(*column\DataID)
   EndIf
EndProcedure
...
Procedure RemoveColumn(*MyGrid.PureGrid, col.l)
   ...
   If *column
      ...
      ForEach *MyGrid\Rows()
         *Cell = @*MyGrid\Rows()\Cells(*column\DataID)
