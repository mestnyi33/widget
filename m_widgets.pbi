...
Module DD
  ...
  Procedure.i Text(Object.i, Text.S, Actions.i=#PB_Drag_Copy)
    If Not *Drag : *Drag = AllocateStructure(Drop_S) : EndIf
  ...
  Procedure.i DropAction(Object.i)
    If Not Object : *Drag = 0 : EndIf
    
    If *Drag And FindMapElement(*Drop(), Hex(Object))
      ProcedureReturn Bool(*Drop()\Format = *Drag\Format And (*Drop()\PrivateType = *Drag\PrivateType)) * *Drop()\Actions  
    EndIf
  EndProcedure
  ...
  Procedure.s DropText(Object.i)
    If DropAction(Object)
      ...
      ProcedureReturn *Drag\Text
    EndIf
  EndProcedure
  
  Procedure.i DropPrivate(Object.i)
    If DropAction(Object)
      Debug "Drop type - "+*Drag\PrivateType
      ProcedureReturn *Drag\PrivateType
    EndIf
  EndProcedure
  ...
  Procedure.i DropImage(Object.i, Image.i=-1, Depth.i=24)
    ...
    With *Drag
      If DropAction(Object) And \ImageID
        Debug "Drop image - "+*Drag\ImageID
        
        If Image =- 1
          Result = CreateImage(#PB_Any, \ImageWidth, \ImageHeight) : Image = Result
        Else
...
Module Widget
  ...
  Macro Event_Widgets(_this_, _event_type_, _event_item_=-1, _event_data_=0)
      If _this_
        Events(_this_, _event_item_, _event_type_, _this_\Mouse\x , _this_\Mouse\y, _event_data_)
        
        ; Scrollbar
        If _this_\Parent And 
           _this_\Parent\Scroll 
          Select _this_ 
            Case _this_\Parent\Scroll\v, 
                 _this_\Parent\Scroll\h 
              _this_ = _this_\Parent
          EndSelect
        EndIf
        
        *Value\This = _this_
        _this_\Event = _event_type_
        
        
        ; PostEvent(#PB_Event_Widget, \Root\Window, \Root\Parent, _event_type_, _event_data_)
        If _this_\Function[1] And 
           CallCFunctionFast(_this_\Function[1], _this_, _event_type_, _event_item_, _event_data_)
          _this_\Repaint = 1
        ElseIf _this_\Function[2] And 
               CallCFunctionFast(_this_\Window\Function[1], _this_, _event_type_, _event_item_, _event_data_)
          _this_\Repaint = 1
        ElseIf _this_\Function[3] And 
               CallCFunctionFast(_this_\Root\Function[1], _this_, _event_type_, _event_item_, _event_data_)
          _this_\Repaint = 1
        EndIf
        
        ;         Send(_this_, _event_type_, _event_item_, _event_data_)
      EndIf
  EndMacro
      ...
      If *Value\Last <> *This
        ...
        If *Value\Last And *Value\Last <> Parent And *Value\Last <> Window And *Value\Last <> Root() 
          If *Value\Last\Mouse\Buttons
            ;             Debug "selected out"
          Else
            If *Value\Last\Root\Drag > 0
              ; Если курсор был над 
              ; "принимающим" виджетом
              SetCursor(*Value\Last\Root\Drag, #PB_Cursor_Default)
              *Value\Last\Root\Drag =- 1
            EndIf
          ...
          If (Not *Value\Last Or (*Value\Last And *Value\Last\Parent <> *This))
            ...
            Else
              If \Root\Drag =- 1 And
                ...
                \Root\Drag = *This
              EndIf
              Event_Widgets(*This, #PB_EventType_MouseEnter, \at)
            EndIf
          EndIf
      ...
      Select EventType 
        ...
        Case #PB_EventType_LeftButtonDown, #PB_EventType_MiddleButtonDown, #PB_EventType_RightButtonDown 
          ...
          If EventType <> #PB_EventType_MiddleButtonDown
            ...
            If *This And *This = *Value\Last
              ...
              If \Deactive
                If \Deactive <> *This
                  Event_Widgets(\Deactive, #PB_EventType_LostFocus, \Deactive\at)
                EndIf
                
                Event_Widgets(*This, #PB_EventType_Focus, \at)
                \Deactive = 0
              EndIf
              
              Event_Widgets(*This, EventType, \at)
            EndIf
          EndIf
          ...
          If EventType <> #PB_EventType_MiddleButtonDown
            If *Value\Focus And
              ...
              *Value\Focus\State = 1 
              ...
              Event_Widgets(*Value\Focus, EventType, *Value\Focus\at)
              If *Value\Focus\Root\Drag > 0
                Event_Widgets(*Value\Focus\Root\Drag, #PB_EventType_Drop, *Value\Focus\index[2], *Value\Focus)
              EndIf
              ...
              If Bool(MouseScreenX>=*Value\Focus\Clip\X And MouseScreenX<*Value\Focus\Clip\X+*Value\Focus\Clip\Width And 
                ...
                If *Value\Focus = *This       
                  If EventType = #PB_EventType_LeftButtonUp
                    Event_Widgets(*Value\Focus, #PB_EventType_LeftClick, *Value\Focus\at)
                  EndIf
                  If EventType = #PB_EventType_RightClick
                    Event_Widgets(*Value\Focus, #PB_EventType_RightClick, *Value\Focus\at)
                  EndIf
                EndIf
              ...
              Else
                *Value\Focus\State = 0
                If *Value\Focus\Root\Drag > 0
                  ; Если курсор был над 
                  ; "принимающим" виджетом
                  SetCursor(*Value\Focus\Root\Drag, #PB_Cursor_Default)
                  *Value\Focus\Root\Drag =- 1
                EndIf
        ...
        Case #PB_EventType_Input, 
          ...
          If *This And (*Value\Focus = *This Or *This = *Value\Active)
            ...
            Event_Widgets(*This, EventType, 0)
          EndIf
