...
DeclareModule Structures
  ...
  *canvasaddress = AllocateStructure(Canvas)
...
;     
    ...
    With *This
        ...
        Else
          Widget = Canvas
        EndIf
        If Canvas <> *canvasaddress\Gadget Or 
           \Type <> #PB_GadgetType_String
          ProcedureReturn
        EndIf
        
        ; Get at point widget
        *canvasaddress\Mouse\From = From(*This)
        ...
        Select EventType 
          ...
          Case #PB_EventType_LeftClick 
            ; Debug ""+*canvasaddress\Mouse\Buttons+" Last - "+*Last +" Widget - "+*Widget +" Focus - "+*Focus +" This - "+*This
            ...
            If Not *canvasaddress\Mouse\From 
              ProcedureReturn 0
            EndIf
        EndSelect
        ...
        If Not \Hide And Not \Disable And \Interact And Widget <> Canvas And CanvasModifiers 
          Select EventType 
            ...
            Case #PB_EventType_MouseMove, #PB_EventType_LeftButtonUp
              If Not *canvasaddress\Mouse\Buttons 
              ...
              If (*Last = *This)
                ...
                ForEach List()
                  If List()\Widget\Focus = List()\Widget And List()\Widget <> *This 
                    ...
                    List()\Widget\Focus = 0
                    *Last = List()\Widget
                    Events(List()\Widget, #PB_EventType_LostFocus, *canvasaddress\Gadget, 0)
                    *Last = *Widget 
                    
                    PostEvent(#PB_Event_Gadget, *canvasaddress\Window, *canvasaddress\Gadget, #PB_EventType_Repaint)
                    Break 
                  EndIf
                Next
                PopListPosition(List())
          ...
          Select EventType
            Case #PB_EventType_MouseLeave
              If CanvasModifiers 
                ...
                PushListPosition(List())
                ForEach List()
                  If *canvasaddress\Gadget = Canvas And List()\Widget\Focus <> List()\Widget And List()\Widget <> *This
              ...
              If \Cursor[1] <> GetGadgetAttribute(*canvasaddress\Gadget, #PB_Canvas_Cursor)
                SetGadgetAttribute(*canvasaddress\Gadget, #PB_Canvas_Cursor, \Cursor[1])
                \Cursor[1] = 0
              EndIf
              
            Case #PB_EventType_MouseEnter    
              If Not \Cursor[1] 
                \Cursor[1] = GetGadgetAttribute(*canvasaddress\Gadget, #PB_Canvas_Cursor)
              EndIf
              SetGadgetAttribute(*canvasaddress\Gadget, #PB_Canvas_Cursor, \Cursor)
          ...
          Case #PB_Shortcut_Tab
            ForEach List()
              If List()\Widget = List()\Widget\Focus
                Result | Events(List()\Widget, #PB_EventType_LostFocus);, Canvas) 
                NextElement(List())
                Debug List()\Widget
                Result | Events(List()\Widget, #PB_EventType_Focus);, Canvas) 
                Break
              EndIf
            Next
        EndSelect
