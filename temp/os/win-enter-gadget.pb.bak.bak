...
CompilerIf #PB_Compiler_IsMainFile ;= 100
  ...
  CompilerIf #PB_Compiler_OS = #PB_OS_Windows
    Procedure SpinGadget_( gadget, x,y,width,height, minimum,maximum, flag )
      Protected r = SpinGadget( gadget, x,y,width,height, minimum,maximum, flag )
      Protected g 
      ...
      ; spin-buttons
      Protected spin = GetWindow_( g, #GW_HWNDNEXT )
      CompilerIf #PB_Compiler_OS = #PB_OS_Windows
        ; SetWindowLongPtr_( spin, #GWL_STYLE, GetWindowLongPtr_( spin, #GWL_STYLE ) | #WS_CLIPSIBLINGS )
        ; SetParent_( spin, GetParent_(g))
        ;SetWindowPos_( spin, #GW_HWNDFIRST, 10,0,0,0, #SWP_NOSIZE )
      CompilerEndIf
      
      ProcedureReturn r
    EndProcedure
    Macro SpinGadget( gadget, x,y,width,height, minimum,maximum, flag=0 )
      SpinGadget_( gadget, x,y,width,height, minimum,maximum, flag )
    EndMacro
  CompilerEndIf
  ...
  If OpenWindow(#PB_Any, 0, 0, 995, 605, "", #PB_Window_SystemMenu | #PB_Window_ScreenCentered)
    ...
    Define handle, gadget
    Repeat
      Define  Event = WaitWindowEvent()
      handle = GetUMGadget( GetUMWindow( ) )
      If handle
        gadget = IDGadget( handle )
        Debug ""+ gadget +" "+ handle ;+" "+ GetClassName( handle )
      EndIf
