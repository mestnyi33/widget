...
Procedure Tool_Gadgets_Event( )
  Static sSteps
  Select EventGadget()
    Case #Tool_Align_To_Grid
      Select EventType()
        Case #PB_EventType_LeftClick
           Protected State1 = Bool(GetGadgetState(#Tool_Align_To_Line) = #PB_Checkbox_Checked)
           Protected State2 = Bool(GetGadgetState(#Tool_Align_To_Grid) = #PB_Checkbox_Checked)
...
Procedure Tool_Gadgets( Window, Width, Height )
  ContainerGadget(#Tool_Container_Mode, 5, 5, Width-10, Height-10)
    ...
    OptionGadget(#Tool_Align_To_Grid, GadgetX(#Tool_Grid_Container)+6, GadgetY(#Tool_Grid_Container)+2, 152-12, 16, "Выровнять по сетке")
    OptionGadget(#Tool_Align_To_Line, GadgetX(#Tool_Line_Container)+6, GadgetY(#Tool_Line_Container)+2, 152-12, 16, "Выровнять по линии")
   CloseGadgetList()
   CloseGadgetList()
...
;   EndIf
  ...
  BindEvent(#PB_Event_Gadget, @Tool_Gadgets_Event(),Window)
EndProcedure
 ...
 Static Window = #Tool
  ...
  Else
    OpenWindow( Window, 245, 144, Width, Height, "Tool", #PB_Window_Invisible|#PB_Window_SystemMenu, ParentID )
     Tool_Gadgets( Window, Width, Height )
