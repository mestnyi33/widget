...
Module Widget
  ...
  Procedure  _draw_window_(*this._s_widget)
    With *this 
      If \__height
        If \resize
          \caption\text\x = \caption\x + \caption\text\_padding
          \caption\text\y = \caption\y + (\caption\height-TextHeight("A"))/2
        EndIf
        ...
        If \fs
          ...
          Else
            For i=1 To \fs
              Line(\x[#__c_1]+i-1,\y[#__c_1]+\caption\height-Bool(\round)*(\fs-i+1),1,\height[#__c_1]-\caption\height,\caption\color\back[\color\state])
              Line(\x[#__c_1]+\width[#__c_1]+i-\fs-1,\y[#__c_1]+\caption\height-Bool(\round)*i,1,\height[#__c_1]-\caption\height,\caption\color\back[\color\state])
              Line(\x[#__c_1]+Bool(\round)*\fs-Bool(\round)*i,\y[#__c_1]+\height[#__c_1]-i,\width[#__c_1]-Bool(\round)*((\fs-i+1)*2),1,\caption\color\back[\color\state])
              
              ;             ; inner frame round
              ;             RoundBox(\x[#__c_1]+i-1, \y[#__c_1]+i-1, \width[#__c_1]-i*2+2, \height[#__c_1]-i*2+2, \round, \round, \caption\color\back[\color\state])
              ;             RoundBox(\x[#__c_1]+i-1, \y[#__c_1]+\caption\height-\fs+i-1, \width[#__c_1]-i*2+2, \height[#__c_1]-\caption\height+\fs-i*2+2, \round, \round, \caption\color\back[\color\state])
              ;             RoundBox(\x[#__c_1]+i-1, \y[#__c_1]+\caption\height-\fs+i-2, \width[#__c_1]-i*2+2, \height[#__c_1]-\caption\height+\fs-i*2+4, \round, \round, \caption\color\back[\color\state])
            Next
          EndIf
        EndIf 
        ...
        If \caption\color\back 
          DrawingMode(#PB_2DDrawing_Gradient|#PB_2DDrawing_AlphaBlend)
          _box_gradient_( 0, \caption\x, \caption\y, \caption\width, \caption\height, \caption\color\fore[\color\state], \caption\color\back[\color\state], \round, \caption\color\alpha)
          ;_box_gradient_( 0, \x[#__c_1], \y[#__c_1], \width[#__c_1], \height[#__c_1]-\height[#__c_2]-\fs, \caption\color\fore[\color\state], \caption\color\back[\color\state], \round, \caption\color\alpha)
        EndIf
        
;         ; 
;         ClipOutput(\x[#__c_4],\y[#__c_4],\width[#__c_4],\caption\height+\round*2)
;         DrawingMode(#PB_2DDrawing_Outlined|#PB_2DDrawing_AlphaBlend)
;         RoundBox(\x[#__c_1],\y[#__c_1],\width[#__c_1],\height[#__c_1]+\round*2,\round,\round,\color\frame[\color\state])
;         ClipOutput(\x[#__c_4],\y[#__c_4],\width[#__c_4],\height[#__c_4])
        ...
        DrawingMode(#PB_2DDrawing_Default|#PB_2DDrawing_AlphaBlend)
        ForEach \columns()
          If \columns()\text\string = "close"
            RoundBox(\columns()\x, \columns()\y, \columns()\width, \columns()\height, \caption\round, \caption\round, $FF0000FF&$FFFFFF|\color[1]\alpha<<24)
          EndIf
          If \columns()\text\string = "minimize"
            RoundBox(\columns()\x, \columns()\y, \columns()\width, \columns()\height, \caption\round, \caption\round, $FFFF0000&$FFFFFF|\color[2]\alpha<<24)
          EndIf
          If \columns()\text\string = "maximize"
            RoundBox(\columns()\x, \columns()\y, \columns()\width, \columns()\height, \caption\round, \caption\round, $FF00FF00&$FFFFFF|\color[3]\alpha<<24)
          EndIf
        Next
     EndIf
    EndWith
  ...
  Procedure.i Draw(*this._s_widget, Childrens=0)
    ...
    With *this
      ...
      If \text And (\text\change Or (\resize And *this\text\multiline =- 1)) : *this\text\change = 1
      ...
      If \height > 0 And \width > 0 And Not \hide And Not (*this = \root And \width[#__c_4] > 0 And \height[#__c_4] > 0)
        ...
        If \color\back[\interact * \color\state]
          DrawingMode(#PB_2DDrawing_Default|#PB_2DDrawing_AlphaBlend)
          RoundBox(\x[#__c_2],\y[#__c_2],\width[#__c_2],\height[#__c_2],\round,\round,\color\back[\interact * \color\state])
        EndIf
        
        
        ; draw widgets
        Select \type
          Case #__Type_Window       : _draw_window_(*this)
...
;         EndIf
        ...
        If \fs
          DrawingMode(#PB_2DDrawing_Outlined|#PB_2DDrawing_AlphaBlend)
          ; draw clip frame
          RoundBox(\x[#__c_4],\y[#__c_4],\width[#__c_4],\height[#__c_4],\round,\round, $FF000000)
          
          ; draw out frame
          RoundBox(\x[#__c_1],\y[#__c_1],\width[#__c_1],\height[#__c_1],\round,\round,\color\frame[\color\state])
          
          If \fs = 1
            Line(\x[#__c_2],\y[#__c_2]-1,\width[#__c_2],1,\color\frame[\color\state])
          Else
            ; draw inner frame 
           Box(\x[#__c_2]-1, \y[#__c_2]-1, Bool(\width[#__c_1]-\fs*2>-2)*(\width[#__c_1]-\fs*2+2), 
                     Bool(\height[#__c_1]-\fs*2-\__height>-2)*(\height[#__c_1]-\fs*2-\__height+2),\color\frame[\color\state])
          EndIf
        EndIf
...
;         EndIf
  ...
  Procedure.i ClearItems(*this._s_widget) 
    With *this
      ...
      ;        PostEvent(#PB_Event_gadget, \root\window, \root\canvas, #__Event_repaint)
      ;       EndIf
    EndWith
  EndProcedure
  ...
  Procedure.i SetActive(*this._s_widget)
    ...
    Macro _set_active_state_(_active_, _state_)
      ...
      If Not(_active_ = _active_\root And _active_\root\type =- 5)
        ...
        PostEvent(#PB_Event_Gadget, _active_\root\window, _active_\root\canvas, #__Event_repaint)
      EndIf
      ...
      With *this
        ...
        Select \type
          ...
          Case #__Type_Window
            ...
            \caption\button[0]\width = Bool(\caption\width>0) * (\caption\width+2)
            \caption\button[0]\height = Bool(\caption\height>0) * (\caption\height+2)
            
            
            ForEach \columns()
;               If \columns()\text\string = "close"
              ;Debug  \columns()\text\string
                ; caption close button
                ;\columns()\width = \caption\len
                \columns()\height = \caption\len
                \columns()\x = \x[#__c_2]+\width[#__c_2]-\columns()\index * (\columns()\width+5)
                \columns()\y = \y[#__c_1]+(\caption\height-\columns()\height)/2
;               EndIf
;               
;               If \columns()\text\string = "minimize"
;                 ; caption minimize button
;                 \columns()\width = \caption\button[1]\width
;                 \columns()\height = \caption\button[1]\height
;                 \columns()\x = \caption\button[1]\x-\columns()\index * (\columns()\width+5)
;                 \columns()\y = \caption\button[1]\y
;               EndIf
;                 
;               If \columns()\text\string = "maximize"
...
;                 \columns()\width = \caption\button[1]\width
;                 \columns()\height = \caption\button[1]\height
;                 \columns()\x = \caption\button[2]\x-\columns()\index * (\columns()\width+5)
;                 \columns()\y = \caption\button[1]\y
;               EndIf
            Next
        ...
        If Not IsRoot(*this) And \parent 
          ...
          ;clip_y = \parent\y[#__c_4] + Bool(\parent\y[#__c_4] < \parent\y[#__c_2]) * (\parent\y[#__c_2]-\parent\y) 
          ;clip_y = \parent\y[#__c_4] + Bool(\parent\y[#__c_4] < \parent\y[#__c_1]) * (\parent\y[#__c_2]-\parent\y) 
          ...
          ;; clip_height = ((\parent\y[#__c_6]+\parent\height[#__c_6]))-clip_h 
        EndIf
        ...
        If clip_x And \x[#__c_1] < clip_x : \x[#__c_5] = clip_x : Else : \x[#__c_5] = \x[#__c_1] : EndIf
        If clip_y And \y[#__c_1] < clip_y : \y[#__c_5] = clip_y : Else : \y[#__c_5] = \y[#__c_1] : EndIf
        If clip_width And (\x[#__c_1]+\width[#__c_1]) > clip_width : \width[#__c_5] = clip_width - \x[#__c_5] : Else : \width[#__c_5] = \width[#__c_1] - (\x[#__c_5]-\x[#__c_1]) : EndIf
        If clip_height And (\y[#__c_1]+\height[#__c_1]) > clip_height : \height[#__c_5] = clip_height - \y[#__c_5] : Else : \height[#__c_5] = \height[#__c_1] - (\y[#__c_5]-\y[#__c_1]) : EndIf
        
        If clip_x And \x[#__c_2] < clip_x : \x[#__c_6] = clip_x : Else : \x[#__c_6] = \x[#__c_2] : EndIf
        If clip_y And \y[#__c_2] < clip_y : \y[#__c_6] = clip_y : Else : \y[#__c_6] = \y[#__c_2] : EndIf
        If clip_width And (\x[#__c_2]+\width[#__c_2]) > clip_width : \width[#__c_6] = clip_width - \x[#__c_6] : Else : \width[#__c_6] = \width[#__c_2] - (\x[#__c_6]-\x[#__c_2]) : EndIf
        
        If clip_height And clip_height < (\y[#__c_2]+\height[#__c_2])  
          \height[#__c_6] = clip_height - \y[#__c_6] 
        Else 
          \height[#__c_6] = (\y[#__c_2]+\height[#__c_2]) - \y[#__c_6]  ; \height[#__c_2] - (\y[#__c_6]-\y[#__c_2]) 
        EndIf
  ...
  Procedure.i Form(X.l,Y.l,Width.l,Height.l, Text.s, Flag.i=0, *parent._s_widget=0)
    ...
    With *this
      ...
      \caption\button[1]\hide = constants::_check_(flag, #__Window_SystemMenu, #False)
      \caption\button[2]\hide = constants::_check_(flag, #__Window_MinimizeGadget, #False)
      \caption\button[3]\hide = constants::_check_(flag, #__Window_MaximizeGadget, #False)
      
      If constants::_check_(flag, #__Window_SystemMenu)
        AddColumn(*this, 1, "close", 20)
      EndIf
      
      If constants::_check_(flag, #__Window_MinimizeGadget)
        If constants::_check_(flag, #__Window_SystemMenu, #False)
          AddColumn(*this, 1, "close", 20)
        EndIf
        AddColumn(*this, 2, "minimize", 20)
      EndIf
      
      If constants::_check_(flag, #__Window_MaximizeGadget)
        If constants::_check_(flag, #__Window_MinimizeGadget)
          AddColumn(*this, 3, "maximize", 20)
        Else
          If constants::_check_(flag, #__Window_SystemMenu, #False)
            AddColumn(*this, 1, "close", 20)
          EndIf
          AddColumn(*this, 2, "maximize", 20)
        EndIf
      EndIf
      ...
      If Not \caption\button[2]\hide Or Not \caption\button[3]\hide Or \fs
        \caption\button[1]\hide = 0
        
      EndIf
      
      \__height = constants::_check_(flag, #__flag_borderless, #False) * 23;+\bs;+\fs
      ;\round = 7
...
;                #PB_Ignore, #PB_Ignore)
    ...
    *f_1 = form(10, 10, 180,  120, "form_1")
    button(10, -10, 80, 40, "Butt_1", #__flag_anchorsgadget)
    closelist()
    
    *f_2 = form(10, 10, 180,  120, "form_2")
