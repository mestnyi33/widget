...
Structure _s_EDIT
   ...
   StructureUnion
      stop.l
      End.l     ; Индекс конца выделения (символ)
   EndStructureUnion
   ...
   StructureUnion
      ...
      caret_x.l
   EndStructureUnion
   w.l       ; Ширина выделения в пикселях
EndStructure
...
Procedure.i edit_reset_selection(*this._s_WIDGET, direction = 0) ; -1 - Left, 1 - Right
   ...
   If *first_s = *last_s And *first_s\edit\start = *first_s\edit\stop : ProcedureReturn #False : EndIf
      ...
      If *first_s <> *last_s
         ...
         Repeat
            If @*this\__rows() = *this\row\active[0]
               ...
               EndIf
            EndIf
            *this\__rows()\edit\start = 0
            *this\__rows()\edit\stop = 0
   ...
   Else
      ...
      Protected head.s = Left(*first_s\Str(0), *first_s\edit\start)
      Protected tail.s = Mid(*last_s\Str(0), *last_s\edit\stop + 1)
      ...
      *this\row\edit\start = Len(head)
      *this\row\active[0] = *first_s
      *this\mask | (#__mask_update)
   EndIf
   
   ; Сброс масок (только для выжившей, остальные и так удалены)
   *this\row\active[0]\edit\start = 0
   *this\row\active[0]\edit\stop = 0
   ...
   *this\row\edit\stop = *this\row\edit\start
      ...
      Case #PB_EventType_KeyDown ; --- ГОРЯЧИЕ КЛАВИШИ ---
         Select keyboard()\key
               ...
               If Not edit_reset_selection(*this) 
                  ...
                  If pos = 0
                     ...
                     If PreviousElement(*this\__rows())
                        ...
                        *this\row\edit\start = Len(*row\Str(0))
            ...
            Case #PB_Shortcut_Return 
               ...
               *row = *this\row\active[0]
               If *row
                  ...
                  *this\row\active[1] = *row
                  ;
                  *this\row\edit\start = 0
                  *this\row\edit\stop = 0
            ...
            Case #PB_Shortcut_Left
               If *row
                  If *this\row\edit\start > Len(*row\Str(0))
                     *this\row\edit\start = Len(*row\Str(0))
                  EndIf
                  If *this\row\edit\start = 0
            ...
            Case #PB_Shortcut_End
               If *row
                  ...
                  *this\row\edit\stop = Len(*row\Str(0))
                  
                  ; 3. Если SHIFT не зажат — сбрасываем выделение
                  If (keyboard()\key[1] & #PB_Canvas_Shift) = 0
                     *this\row\edit\start = *this\row\edit\stop
            ...
            Case #PB_Shortcut_PageUp, #PB_Shortcut_PageDown
               If *row
                  ...
                  *this\row\active[0] = *row
                  *this\row\edit\start = Min(*this\row\edit\start, Len(*row\Str(0)))
            ...
            Case #PB_Shortcut_A ; --- SELECT ALL ---
               If keyboard()\key[1] & #PB_Canvas_Control
                  ...
                  ForEach *this\__rows()
                     *this\__rows()\edit\start = 0
                     *this\__rows()\edit\stop = Len(*this\__rows()\Str(0))
                     *this\__rows()\mask &~ #__mask_active
                     *this\__rows()\mask | (#__mask_edit | #__mask_update)
                  Next
                  PopListPosition(*this\__rows())
                  If LastElement(*this\__rows())
                     *this\row\edit\start = Len(*this\__rows()\Str(0))
               ...
               If keyboard()\key[1] & #PB_Canvas_Control
                  ...
                  ForEach *this\__rows()
                     If *this\__rows()\mask & #__mask_edit
                        Clip + Mid(*this\__rows()\Str(0), *this\__rows()\edit\start + 1, *this\__rows()\edit\stop - *this\__rows()\edit\start) + #LF$
                     EndIf
                  Next
                  PopListPosition(*this\__rows())
                     ...
                     If i = 1
                        ...
                        *row\Str(0) = head + current_line
                        If count = 1
                           ...
                           *row\Str(0) + tail
                           *this\row\edit\start = Len(head + current_line)
                        EndIf
                     Else
                        ...
                        *row\mask | (#__mask_active | #__mask_edit)
                        ...
                        If i = count
                           ...
                           *this\row\edit\start = Len(current_line)
                        Else
...
Procedure update_edit(*this._s_WIDGET, *row._s_ROWS)
   ...
   If *row\mask & #__mask_update
      ...
      Protected caret_pos = *this\row\edit\start
      
      ; Debug ""+ *this\row\edit\stop +" "+ *this\row\edit\start +" "+ *start_r +" "+ *end_r
      ...
      If *start_r And *end_r
         ...
         If *row = *start_r And *row = *end_r
            *row\edit\start = Min(*this\row\edit\stop, *this\row\edit\start)
            *row\edit\stop   = Max(*this\row\edit\stop, *this\row\edit\start)
         ElseIf *row\y < min_y Or *row\y > max_y
            *row\edit\start = 0 : *row\edit\stop = 0
         ElseIf *row\y > min_y And *row\y < max_y
            *row\edit\start = 0 : *row\edit\stop = MemoryStringLength(*txt_ptr)
         ElseIf *row = *start_r
            If is_down : *row\edit\start = *this\row\edit\stop : *row\edit\stop = MemoryStringLength(*txt_ptr)
               Else       : *row\edit\start = 0 : *row\edit\stop = *this\row\edit\stop : EndIf
         ElseIf *row = *end_r
            If is_down : *row\edit\start = 0 : *row\edit\stop = *this\row\edit\start
               Else       : *row\edit\start = *this\row\edit\start : *row\edit\stop = MemoryStringLength(*txt_ptr) : EndIf
         EndIf
         
         ; --- 2. РАСЧЕТ ПИКСЕЛЕЙ ВЫДЕЛЕНИЯ (ДЛЯ ТЕКУЩЕЙ СТРОКИ) ---
         ;          If *rw\edit\start > 0
         ;             *rw\edit\x = TextWidth(Left(txt, *rw\edit\start))
         ;          Else
         ;             *rw\edit\x = 0
         ;          EndIf
         ;          If *rw\edit\start = *rw\edit\stop
         ;             *rw\edit\w = 0
         ;          Else
         ;             *rw\edit\w = TextWidth(Mid(txt, *rw\edit\start + 1, *rw\edit\stop - *rw\edit\start))
         ;          EndIf
         
         If *row\edit\start = *row\edit\stop And *row\edit\start = 0
            *row\edit\x = 0 : *row\edit\w = 0
         Else
            If ListSize(*row\tokens()) = 0
               DrawingFont(Font_Editor_Normal)
               *row\edit\x = TextWidth(Mid(txt, 1, *row\edit\start))
               *row\edit\w = TextWidth(Mid(txt, *row\edit\start + 1, *row\edit\stop - *row\edit\start))
            Else
               Protected x1 = -1, x2 = -1
               PushListPosition(*row\tokens())
               ForEach *row\tokens()
                  ...
                  If x1 = -1 And *row\edit\start >= ts - 1 And *row\edit\start < ts + tl
                     If *row\tokens()\font : DrawingFont(*row\tokens()\font) : Else : DrawingFont(Font_Editor_Normal) : EndIf
                     x1 = *row\tokens()\x + TextWidth(Mid(txt, ts, *row\edit\start - ts + 1))
                  EndIf
                  
                  ; Ищем X для конца выделения
                  If x2 = -1 And *row\edit\stop >= ts - 1 And *row\edit\stop < ts + tl
                     If *row\tokens()\font : DrawingFont(*row\tokens()\font) : Else : DrawingFont(Font_Editor_Normal) : EndIf
                     x2 = *row\tokens()\x + TextWidth(Mid(txt, ts, *row\edit\stop - ts + 1))
                  EndIf
         ...
         If *row = *active And Not *this\mask & #__mask_drag 
            ...
            Else
               If ListSize(*active\tokens()) = 0
                  DrawingFont(Font_Editor_Normal)
                  *this\row\edit\caret_x = TextWidth(Mid(txt_active, 1, caret_pos))
               Else
                  ...
                  ForEach *active\tokens()
                     ...
                     If caret_pos >= ts - 1 And caret_pos < ts + tl
                        ...
                        *this\row\edit\caret_x = *active\tokens()\x + TextWidth(Mid(txt_active, ts, caret_pos - ts + 1))
                        Break
                     EndIf
                  Next
                  ...
                  If ListIndex(*active\tokens()) = ListSize(*active\tokens()) - 1 And caret_pos >= ts + tl - 1
                     *this\row\edit\caret_x = *active\width
                  EndIf
...
Procedure _update_tab(*this._s_WIDGET)
   Protected cur_x = 0
   PushListPosition(*this\__tabs())
   ForEach *this\__tabs()
      *this\__tabs( )\x = cur_x
      ; Замеряем текст и добавляем отступы (по 10px с каждой стороны)
      *this\__tabs()\width = TextWidth(*this\__tabs()\title) + *this\padding\x * 2
...
Procedure _draw_tab(*this._s_WIDGET, rx.l, ry.l)
   ; 1. ВЫЧИСЛЯЕМ СТАРТ (с учетом выравнивания и скролла самого таббара)
   Protected start_x = rx + GetTabStartX(*this) - *this\scroll\x
   ...
   ForEach *this\__tabs()
      *tab = @*this\__tabs()
      ...
      ; РИСУЕМ ТЕЛО ТАБА
      Box(start_x, ry, *tab\width, *this\Height, color)
      
      ; Вычисляем координаты текста для центровки
      tw = TextWidth( *tab\title)
      If *this\text\align & #__align_center
         tx = start_x + (*tab\Width - tw) / 2
      ElseIf *this\text\align & #__align_right
         tx = start_x + (*tab\Width - tw) - *this\padding\X
      Else
         tx = start_x + *this\padding\X 
      EndIf
      DrawText(tx, ry + (*this\Height - th)/2, *tab\title, txtColor, color)
      
      ; РАМКА (Верх, Лево, Право)
      Line(start_x, ry, *tab\width, 1, $CCCCCC) 
      Line(start_x, ry, 1, *this\Height, $CCCCCC)                   
      Line(start_x + *tab\width, ry, 1, *this\Height, $CCCCCC) 
      
      If *tab\mask & #__mask_active
         active_x = start_x : active_w = *tab\width
      EndIf
      
      start_x + *tab\width + *this\tab\spacing 
      i + 1
   Next
   PopListPosition(*this\__tabs())
...
Procedure draw_columns(*this._s_WIDGET, rx.l, ry.l)
   ...
   Line(rx, ry + *this\column\height, *this\Width, 1, #COL_COLOR_BORDER)
EndProcedure
               ...
               If data_idx <= ArraySize(*row\Str())
                  ...
                  If *this\row\active[1] And data_idx = 0
                     If *row\mask & #__mask_edit
                        Protected s_x = col_x + offset + *row\edit\x
                        If *this\row\active[1]\y > *this\row\active[0]\y And *row <> *this\row\active[1] Or 
                           (Len(txt) = *row\edit\stop And *row\y < *this\row\active[0]\y)
                           Box(s_x, dy + 2, *row\edit\w + 7, *row\height - 4, #COLOR_BACK_SELECTED)
                        Else
                           Box(s_x, dy + 2, *row\edit\w, *row\height - 4, #COLOR_BACK_SELECTED)
                        EndIf
                     EndIf
                  EndIf
...
Procedure add_row(*this._s_WIDGET, Text.s = "", Index.i = -1, Level.i = 0, *start = 0, len.i = -1)
   ...
   *row\sublevel = Level
   ...
   ;*row\edit = AllocateStructure(_s_SEL)
   *row\mask | #__mask_change
         ...
         If *pressed_row
            If *pressed_row\mask & #__mask_edit
               If *this\mask & #__mask_drag
                  ...
                  If *this\row\edit\start <> caret 
                     *this\row\edit\start = caret 
                     *this\mask | (#__mask_redraw)
                  EndIf
               EndIf
            Else
            ...
            *hover_row\mask | #__mask_press
            ...
            If MouseClick() = 1
               *this\row\active[1] = *hover_row
               ...
               If *this\row\edit
                  ...
                  PopListPosition(*this\__rows( ))
                  ;
                  *this\row\edit\start = edit_make_caret(*this)
                  *this\row\edit\stop = *this\row\edit\start
                  ;
            ...
            If MouseClick() > 1
               If *hover_row\mask & #__mask_edit
                  ...
                  If MouseClick() = 2
                     Protected start.i = *this\row\edit\stop
                     Protected stop.i = *this\row\edit\start + 1 ; Начинаем со следующего символа
                     ...
                     *this\row\edit\stop = start
                     *this\row\edit\start = stop - 1
                  EndIf
                  If MouseClick() = 3
                     *this\row\edit\stop = 0
                     *this\row\edit\start = Len
                  EndIf
                  If *this\row\edit\start <> *this\row\edit\stop
                     *this\mask | (#__mask_redraw)
                  EndIf
               EndIf
            EndIf
