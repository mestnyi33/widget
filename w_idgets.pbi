...
;            *this\type = #PB_GadgetType_HyperLink
       If  *this\color\_state & #__s_back
          If \color\fore > 0
            DrawingMode(#PB_2DDrawing_Gradient)
            _box_gradient_(\Vertical,\X[1],\Y[1],\Width[1],\Height[1],\color\Fore[\color\state],\color\Back[\color\state], \round)
          Else
...
;                 
                ...
                DrawingMode(#PB_2DDrawing_Transparent)
                  If \Vertical
                    If \text\rotate = 270
                      DrawRotatedText(\X[2]+\Text\x+(Height-Text_Y) + Bool(Not \Text\Align\horizontal), \Y[2]+\Text\y+Text_X, String4.s, 270, \Color\Front[Bool(*this\color\_state & #__s_front) * \color\state])
                    EndIf
                    If \text\rotate = 90
                      DrawRotatedText(\X[2]+\Text\Y+Text_Y - 1, \Y[2]+\Text\X+(width-Text_X), String4.s, 90, \Color\Front[Bool(*this\color\_state & #__s_front) * \color\state])
                    EndIf
                  Else
                    If \text\rotate = 0
                      DrawRotatedText(\X[2]+\Text\X+Text_X, \Y[2]+\Text\Y+Text_Y-1, String4.s, 0, \Color\Front[Bool(*this\color\_state & #__s_front) * \color\state])
                    EndIf
                    If \text\rotate = 180
                      DrawRotatedText(\X[2]+\Text\X+(Width-Text_X), \Y[2]+\Text\Y+(Height-Text_Y) + Bool(Not \Text\Align\vertical), String4.s, 180, \Color\Front[Bool(*this\color\_state & #__s_front) * \color\state])
                    EndIf
                  EndIf
...
;                 EndIf
        ...
        If  *this\color\_state & #__s_frame
        If \fs
          DrawingMode(#PB_2DDrawing_Outlined)
          RoundBox(\X[1],\Y[1],\Width[1],\Height[1], \round,\round, \Color\Frame[\color\state])
        EndIf
    ...
    Procedure   Hyperlink_Draw(*this._s_widget)
      ...
      If *this\text\string
        ...
        If *this\flag\lines
          Line(*this\x+*this\scroll\x+1, *this\y+*this\scroll\y+*this\scroll\height-2, *this\scroll\width, 1, *this\color\front[*this\color\state]&$FFFFFF|*this\color\alpha<<24)
        EndIf
      EndIf
    EndProcedure
...
;       Box(*this\x, *this\y, *this\width, *this\height, $FFFF0000)
    ...
    Procedure.i Option(X.l,Y.l,Width.l,Height.l, Text.s, Flag.i=0)
      ...
      *this\y =- 2147483648
        ...
        *this\fs = 1;Bool(Not constants::_check_(Flag, #__flag_borderless))
        *this\bs = *this\fs
    ...
    Procedure.i HyperLink(X.l,Y.l,Width.l,Height.l, Text.s, Color.i, Flag.i=0)
      ...
      *this\y =- 2147483648
        ...
        *this\class = #PB_Compiler_Procedure
        \color = _get_colors_()
        \color\_state = #__s_front
;         \color\alpha = 255
         \color\fore[0] =- 1
         \color\fore[1] =- 1
         \color\fore[2] =- 1
         \color\back[1] = \color\back
         \color\back[2] = \color\back
;         \color\frame = $FF7E7E7E
        \interact = 1
        
        \fs = 1;Bool(Not constants::_check_(Flag, #__flag_borderless))
        \bs = \fs
    ...
    Procedure Events(*this._s_widget, event_type.l, mouse_x.l, mouse_y.l, _wheel_x_.b=0, _wheel_y_.b=0)
      ...
      If *this\type = #PB_GadgetType_HyperLink
        Select event_type
          ...
          Case #PB_EventType_LeftButtonDown : *this\color\state = #__s_2 : Repaint = 1
          Case #PB_EventType_MouseEnter     
           ; Debug ""+*this\scroll\x +" "+ Str(mouse_x-*this\x)
...
CompilerIf #PB_Compiler_IsMainFile
  ...
  If Open(OpenWindow(#PB_Any, 0, 0, 605+30, 140+200+140+140, "ScrollBarGadget", #PB_Window_SystemMenu | #PB_Window_ScreenCentered))
    ...
    *w = widget::Progress  (300+270+30, 140+200,  25, 120 ,0, 300, #__bar_vertical|#__bar_inverted)
    ...
    Option(10, 10, 100, 20, "option_0")
    Option(10, 32, 100, 20, "option_1")
    CheckBox(10, 54, 100, 20, "checkbox_0")
