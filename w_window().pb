...
Module Widget
  ...
  Procedure  _draw_window_(*this._s_widget)
    With *this 
         ; Draw caption back
        If \__height And \caption\color\back 
          DrawingMode(#PB_2DDrawing_Gradient|#PB_2DDrawing_AlphaBlend)
        ...
        EndIf
         ...
         If \fs
          ...
          Else
            For i=1 To \fs
              If Not \__height
               ; Line(\x[#__c_1]-Bool(\round)*((\fs-i+1)*2),\y[#__c_1]+i,\width[#__c_1]+Bool(\round)*\fs-(Bool(\round)*i)*2,1,\caption\color\back[\color\state])
              EndIf
...
;         ClipOutput(\x[#__c_4],\y[#__c_4],\width[#__c_4],\height[#__c_4])
        ...
        If \caption\text\string
          ...
          ;         Box(\caption\x[#__c_4], \caption\y[#__c_4], \caption\width[#__c_4], \caption\height[#__c_4], $FF000000)
          ;         
          ;         ; Draw string
          If \resize
    ...
    With *this
      ...
      If \height > 0 And \width > 0 And Not \hide And Not (*this = \root And \width[#__c_4] > 0 And \height[#__c_4] > 0)
        ...
        If \color\back[\interact * \color\state]
          DrawingMode(#PB_2DDrawing_Default|#PB_2DDrawing_AlphaBlend)
          RoundBox(\x[#__c_1],\y[#__c_1],\width[#__c_1],\height[#__c_1],\round,\round,\color\back[\interact * \color\state])
        EndIf
    ...
    If *this > 0
      With *this
        ...
        If \type = #__Type_Window
          ...
          ; 
          \caption\x[#__c_2] = \x[#__c_1] + \fs - 1
          \caption\y[#__c_2] = \y[#__c_1] + \fs - 1
          \caption\height[#__c_2] = \__height - \fs + 2
          ...
          EndIf
            ...
            If Not \caption\button[3]\hide
              \caption\width[#__c_2] = \caption\button[3]\x - \x[#__c_2] - \caption\_padding + 2
            ElseIf Not \caption\button[2]\hide
              \caption\width[#__c_2] = \caption\button[2]\x - \x[#__c_2] - \caption\_padding + 2
            ElseIf Not \caption\button[1]\hide
              \caption\width[#__c_2] = \caption\button[1]\x - \x[#__c_2] - \caption\_padding + 2
            Else
              \caption\width[#__c_2] = \width[#__c_1] - \fs*2 + 2
            EndIf
...
;       \flag\window\borderless = constants::_check_(flag, #__Window_BorderLess)
      ...
      \caption\button[2]\hide = constants::_check_(flag, #__Window_MaximizeGadget, #False)
      \caption\button[3]\hide = constants::_check_(flag, #__Window_MinimizeGadget, #False)
      ...
      If \caption\button[2]\hide = 0 Or 
         \caption\button[3]\hide = 0
        \caption\button[1]\hide = 0
      EndIf
      
      If \caption\button[1]\hide = 0
        \caption\hide = 0
      EndIf
