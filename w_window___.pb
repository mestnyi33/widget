  ...
  Procedure  _draw_window_(*this._s_widget)
    With *this 
      If \__height
        ...
        If \fs
          If \round
          ;  ClipOutput(\x[#__c_4], \caption\y[#__c_4] - \fs, \width[#__c_4],\caption\height)
          EndIf
         DrawingMode(#PB_2DDrawing_Outlined|#PB_2DDrawing_AlphaBlend)
         RoundBox(\caption\x, \caption\y, \caption\width, \caption\height-\fs,\round,\round,\color\frame[\color\state])
        ;  RoundBox(\x[#__c_1],\y[#__c_1],\width[#__c_1],\height[#__c_1],\round,\round,\color\frame[\color\state])
         ; Line(\x[#__c_2],\y[#__c_2]-1,\width[#__c_2],1,\color\frame[\color\state])
; ;         DrawingMode(#PB_2DDrawing_Default|#PB_2DDrawing_AlphaBlend)
;           RoundBox(\x[#__c_2]-\fs+1, \y[#__c_2]-\round, \fs, \round,\round,\round,\caption\color\back[\color\state])
;            DrawingMode(#PB_2DDrawing_Gradient|#PB_2DDrawing_AlphaBlend)
;           _box_gradient_( 0, \x[#__c_2]-\fs+1, \y[#__c_1]+\round, \fs, \height[#__c_1]-\round*2, \caption\color\fore[\color\state], \caption\color\back[\color\state], \round, \caption\color\alpha)
        EndIf
         ; Draw caption back
        If \caption\color\back 
          DrawingMode(#PB_2DDrawing_Gradient|#PB_2DDrawing_AlphaBlend)
          _box_gradient_( 0, \caption\x+1, \caption\y+1, \caption\width-2, \caption\height-\fs, \caption\color\fore[\color\state], \caption\color\back[\color\state], \round, \caption\color\alpha)
        EndIf
        ...
        For i=0 To \round
          Line(\x[#__c_1]+i+1,\y[#__c_1]+\caption\height-3,1,Bool(\round)*(i-\round),\caption\color\back[\color\state])
          Line(\x[#__c_1]+\width[#__c_1]+i-\round-2,\y[#__c_1]+\caption\height-3,1,-Bool(\round)*(i),\caption\color\back[\color\state])
        Next
        
        DrawingMode(#PB_2DDrawing_Outlined|#PB_2DDrawing_AlphaBlend)
        Line(\x[#__c_1],\y[#__c_2]-\round-\fs+1,1,\round,\color\frame[\color\state])
        Line(\x[#__c_1]+\width[#__c_1]-1,\y[#__c_2]-\round-\fs+1,1,\round,\color\frame[\color\state])
...
;       Height + \__height + \fs*2
  ...
  Procedure window_set_state_(*this._s_widget, button.l, state.l)
    Protected result.b
    
    If state
      If *this\caption\button[button]\color\state <> state
        *this\caption\button[button]\color\state = state
        result = #True
      EndIf
    Else
      If *this\caption\button[button]\color\state <> 0
        *this\caption\button[button]\color\state = 0
        result = #True
      EndIf
    EndIf
