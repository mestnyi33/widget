...
Procedure Send( eventtype, *entered.s_WIDGET, *pressed.s_WIDGET, *focussed.s_WIDGET, *leaved.s_WIDGET )
  If *leaved <> *entered
    If *leaved
      *leaved\state\enter = #False
      *leaved\state\repaint = #True
      DoWidgetEvents( *leaved, #PB_EventType_MouseLeave )
    EndIf
    
    *leaved = *entered
    
    If *entered And
       *entered\state\enter = #False
      *entered\state\enter = #True 
      *entered\state\repaint = #True
      DoWidgetEvents( *entered, #PB_EventType_MouseEnter )
    EndIf
  EndIf
  ...
  If eventtype = #PB_EventType_MouseMove
    If *pressed
      If *pressed\state\drag = #False 
        *pressed\state\drag = #True
        *pressed\state\repaint = #True
        DoWidgetEvents( *pressed, #PB_EventType_DragStart )
      EndIf
      
      DoWidgetEvents( *pressed, eventtype )
    ElseIf *entered
      DoWidgetEvents( *entered, eventtype )
    EndIf
  EndIf
  ...
  If eventtype = #PB_EventType_LeftButtonDown Or 
    ...
    If *entered
      If *focussed <> *entered
        If *focussed
          *focussed\state\focus = #False
          *focussed\state\repaint = #True
          DoWidgetEvents( *focussed, #PB_EventType_LostFocus )
        EndIf
        
        *focussed = *entered
        *focussed\state\focus = #True
        *focussed\state\repaint = #True
        DoWidgetEvents( *focussed, #PB_EventType_Focus )
      EndIf
      
      *pressed = *entered
      *pressed\state\press = #True
      *pressed\state\repaint = #True
      DoWidgetEvents( *pressed, eventtype )
    EndIf 
  EndIf
      ...
      *pressed\state\repaint = #True
      ...
      If *pressed = *entered 
        ...
        If eventtype = #PB_EventType_LeftButtonUp
          DoWidgetEvents( *pressed, #PB_EventType_LeftClick )
        EndIf
        If eventtype = #PB_EventType_RightButtonUp
          DoWidgetEvents( *pressed, #PB_EventType_RightClick )
        EndIf
        ...
        If time_click And 
          ...
          If eventtype = #PB_EventType_LeftButtonUp
            DoWidgetEvents( *pressed, #PB_EventType_LeftDoubleClick )
          EndIf
          If eventtype = #PB_EventType_RightButtonUp
            DoWidgetEvents( *pressed, #PB_EventType_RightDoubleClick )
          EndIf
          time_click = 0
        Else
      ...
      Else
        ;             ; do enter events
        ;             Repaint | DoWidgetEvents( *entered, #PB_EventType_MouseEnter, WidgetMouse( )\x, WidgetMouse( )\y )
      EndIf
      
      If *pressed\state\drag = #True
        *pressed\state\drag = #False
        If *entered
          *entered\state\repaint = #True
          DoWidgetEvents( *entered, #PB_EventType_Drop )
        EndIf
      EndIf
      *pressed = #Null
    EndIf
...
;   EnteredWidget() = *entered
;   PressedWidget() = *pressed
;   FocusedWidget() = *focussed
;   LeavedWidget() = *leaved
EndProcedure
;-
...
Procedure DoWidgetEvents( *this.s_WIDGET, eventtype )
  ...
  If *this\count\items
    If Atbox( *this\draw\x+*this\fs, *this\draw\y+*this\fs, *this\draw\width-*this\fs*2, *this\draw\height-*this\fs*2, WidgetMouse( )\x, WidgetMouse( )\y )
      If eventtype = #PB_EventType_MouseMove
        If PressedItem( *this )
          If PressedItem( *this )\state\drag = #False
            PressedItem( *this )\state\drag = #True
            PressedItem( *this )\state\repaint = #True
            DoItemEvents( *this, PressedItem( *this ), #PB_EventType_DragStart )
          EndIf
          
          DoItemEvents( *this, PressedItem( *this ), eventtype )
        ElseIf EnteredItem( *this )
          DoItemEvents( *this, EnteredItem( *this ), eventtype )
        EndIf
      EndIf
      ...
      If LeavedItem( *this ) <> EnteredItem( *this )
        If LeavedItem( *this )
          LeavedItem( *this )\state\enter = #False
          LeavedItem( *this )\state\repaint = #True
      ...
      If eventtype = #PB_EventType_LeftButtonDown Or 
        ...
        If EnteredItem( *this )
          If FocusedItem( *this ) <> EnteredItem( *this )
            If FocusedItem( *this )
              FocusedItem( *this )\state\focus = #False
              FocusedItem( *this )\state\repaint = #True
              DoItemEvents( *this, FocusedItem( *this ), #PB_EventType_LostFocus )
            EndIf
            
            FocusedItem( *this ) = EnteredItem( *this )
            FocusedItem( *this )\state\focus = #True
            FocusedItem( *this )\state\repaint = #True
            DoItemEvents( *this, FocusedItem( *this ), #PB_EventType_Focus )
          EndIf
          
          PressedItem( *this ) = EnteredItem( *this )
          PressedItem( *this )\state\press = #True
          PressedItem( *this )\state\repaint = #True
          DoItemEvents( *this, PressedItem( *this ), eventtype )
        EndIf 
      EndIf
      ...
      If eventtype = #PB_EventType_LeftButtonUp Or 
        ...
        If PressedItem( *this )
          PressedItem( *this )\state\press = #False
          PressedItem( *this )\state\repaint = #True
          DoItemEvents( *this, PressedItem( *this ), eventtype )
          If PressedItem( *this )\state\drag
            PressedItem( *this )\state\drag = 0
            If EnteredItem( *this )
              EnteredItem( *this )\state\repaint = #True
              DoItemEvents( *this, EnteredItem( *this ), #PB_EventType_Drop )
            EndIf
          EndIf
          PressedItem( *this ) = #Null
        EndIf
      EndIf
...
Procedure WidgetEvents( Canvas, eventtype )
  ...
  If eventtype = #PB_EventType_MouseMove Or
    ...
    If Root( )
      ...
      Send( eventtype, EnteredWidget( ), PressedWidget( ), FocusedWidget( ), LeavedWidget( ) )
    EndIf
  EndIf
