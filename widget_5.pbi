...
Procedure DoWidgetEvents( *this.s_WIDGET, eventtype )
  ...
  If *this\state\enter
    If *this\state\press;WidgetMouse( )\buttons
  ...
  If *this\canvas\event
    ...
    Else
      PostWidgetEvent( *this, eventtype )
    EndIf
  EndIf
...
Procedure WidgetEvents( Canvas, eventtype )
  ...
  If eventtype = #PB_EventType_MouseMove Or
    ...
    If RootWidget( )
      ...
      ; for the item mouse enter&leave state
      If EnteredWidget( )
        If EnteredWidget( )\count\items
          If Atbox( EnteredWidget( )\draw\x+EnteredWidget( )\fs, EnteredWidget( )\draw\y+EnteredWidget( )\fs, EnteredWidget( )\draw\width-EnteredWidget( )\fs*2, EnteredWidget( )\draw\height-EnteredWidget( )\fs*2, WidgetMouse( )\x, WidgetMouse( )\y )
            EnteredWidget( )\row\entered = GetAtPointitems( EnteredWidget( )\items( ), WidgetMouse( )\x-EnteredWidget( )\x, WidgetMouse( )\y-EnteredWidget( )\y )
            
            If EnteredWidget( )\row\entered And
               EnteredWidget( )\row\entered\state\enter = #False
              
              If EnteredWidget( )\row\leaved <> EnteredWidget( )\row\entered
                If EnteredWidget( )\row\leaved
                  EnteredWidget( )\row\leaved\state\enter = #False
                  EnteredWidget( )\row\leaved\state\repaint = #True
                  DoItemEvents( EnteredWidget( ), EnteredWidget( )\row\leaved, #PB_EventType_MouseLeave )
                EndIf
                
                EnteredWidget( )\row\leaved = EnteredWidget( )\row\entered
              EndIf
              
              EnteredWidget( )\row\entered\state\enter = #True 
              EnteredWidget( )\row\entered\state\repaint = #True
              DoItemEvents( EnteredWidget( ), EnteredWidget( )\row\entered, #PB_EventType_MouseEnter )
            EndIf
          EndIf  
        EndIf
      EndIf
  ...
  If eventtype = #PB_EventType_MouseMove
    ;
    If PressedWidget( )
      If PressedWidget( )\row\pressed
        If PressedWidget( )\row\pressed\state\drag = #False
          PressedWidget( )\row\pressed\state\drag = #True
          PressedWidget( )\row\pressed\state\repaint = #True
          DoItemEvents( PressedWidget( ), PressedWidget( )\row\pressed, #PB_EventType_DragStart )
        EndIf
        
        DoItemEvents( PressedWidget( ), PressedWidget( )\row\pressed, eventtype )
      EndIf
    ...
    ElseIf EnteredWidget( )
      If EnteredWidget( )\count\items And EnteredWidget( )\row\entered
        DoItemEvents( EnteredWidget( ), EnteredWidget( )\row\entered, eventtype )
      EndIf
  ...
  If eventtype = #PB_EventType_LeftButtonDown Or 
    ...
    If EnteredWidget( )
      If EnteredWidget( )\count\items
        If EnteredWidget( )\row\entered
          If EnteredWidget( )\row\focused <> EnteredWidget( )\row\entered
            If EnteredWidget( )\row\focused
              EnteredWidget( )\row\focused\state\focus = #False
              EnteredWidget( )\row\focused\state\repaint = #True
              DoItemEvents( EnteredWidget( ), EnteredWidget( )\row\focused, #PB_EventType_LostFocus )
            EndIf
            
            EnteredWidget( )\row\focused = EnteredWidget( )\row\entered
            EnteredWidget( )\row\focused\state\focus = #True
            EnteredWidget( )\row\focused\state\repaint = #True
            DoItemEvents( EnteredWidget( ), EnteredWidget( )\row\focused, #PB_EventType_Focus )
          EndIf
          
          EnteredWidget( )\row\pressed = EnteredWidget( )\row\entered
          EnteredWidget( )\row\pressed\state\press = #True
          EnteredWidget( )\row\pressed\state\repaint = #True
          DoItemEvents( EnteredWidget( ), EnteredWidget( )\row\pressed, eventtype )
        EndIf 
      EndIf
  ...
  If eventtype = #PB_EventType_LeftButtonUp Or 
    ...
    If PressedWidget( )
      ...
      If PressedWidget( )\count\items
        If PressedWidget( )\row\pressed
          PressedWidget( )\row\pressed\state\press = #False
          PressedWidget( )\row\pressed\state\repaint = #True
          DoItemEvents( PressedWidget( ), PressedWidget( )\row\pressed, eventtype )
          
          If PressedWidget( )\row\pressed\state\drag
            PressedWidget( )\row\pressed\state\drag = 0
            
            If EnteredWidget( )\row\entered
              EnteredWidget( )\row\entered\state\repaint = #True
              DoItemEvents( EnteredWidget( ), EnteredWidget( )\row\entered, #PB_EventType_Drop )
            EndIf
          EndIf
          PressedWidget( )\row\pressed = #Null
        EndIf
      EndIf
...
AddWidget(*root3, 20,20, 100,260-20+300)
...
AddWidget(*root3, 140,20, 100,260-20+300)
;Debug "3 - "+*root3\address +" "+ Str(widget( )) +" "+ Str(*root3\child( ))
Define i
For i=1 To 10;00000
  AddItem(*root3\child( ), i, "text-"+Str(i))
