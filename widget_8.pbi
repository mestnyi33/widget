...
Macro SetCanvasData( _gadget_, _data_ )
  If Not FindMapElement( WidgetCanvas( ), Str( GadgetID( _gadget_ ) ) )
    ...
    *widget\canvas.allocate( CANVAS, ( ) )
    ...
    ;WidgetCanvas( )\window = WidgetCanvas( )\widget\canvas\window
  EndIf
EndMacro
...
Procedure.i PostWidgetEvent( *this.s_WIDGET, eventtype.l, *button = #PB_All, *data = #Null )
  ...
  CallCFunctionFast( *this\canvas\widget\event\data )
EndProcedure

Procedure.i BindWidgetEvent( *this.s_WIDGET, *callback, eventtype.l = #PB_All, item.l = #PB_All )
  If *this = *this\canvas\widget
    If *callback
      If Not RootWidget( )\canvas\widget\event
        RootWidget( )\canvas\widget\event.allocate( EVENT )
      EndIf
      RootWidget( )\canvas\widget\event\data = *callback
      RootWidget( )\canvas\widget\event\type = eventtype
      RootWidget( )\canvas\widget\event\item = item
    EndIf
  Else
    ;
...
If OpenWindow(#PB_Any, 0, 0, 800, 600, "PanelGadget", #PB_Window_SystemMenu | #PB_Window_ScreenCentered)
  ...
  Define *root4.s_WIDGET = OpenWidget(0,-1, 590, 10, 200, 600-20)
