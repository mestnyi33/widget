...
Macro RootWidget( )
  WidgetCanvas( )\canvas\widget ;*widget\root
EndMacro                        ; Returns first created widget 

Macro OpenedWidget( )
  WidgetCanvas( )\canvas\parent
...
Macro GetCanvasData( _address_ )
  *widget\root( Str( _address_ ) )\canvas\widget
EndMacro
...
Macro SetCanvasData( _gadget_, _data_ )
  If Not FindMapElement( WidgetCanvas( ), Str( GadgetID( _gadget_ ) ) )
    ...
    *widget\root.allocate( ROOT, ( ) )
    WidgetCanvas( )\canvas\widget = _data_
    WidgetCanvas( )\canvas\gadget = _gadget_
    WidgetCanvas( )\canvas\window = WidgetCanvas( )\canvas\widget\window
  EndIf
EndMacro
