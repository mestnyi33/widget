...
CompilerIf Not Defined(Bar, #PB_Module)
  ...
  DeclareModule bar
    ...
    Macro _move_childrens_(_parent_, _change_x_, _change_y_)
      If _parent_\container And _parent_\count\childrens ; And ListSize(_parent_\childrens())
        PushListPosition(GetEvent()\childrens())
        ForEach GetEvent()\childrens()
          If GetEvent()\childrens()\parent = _parent_ 
            ; Debug GetEvent()\childrens()\class +" - "+ GetEvent()\childrens()\parent\class +" - "+ _parent_\class +" - "+ _parent_\parent\class; Bool(GetEvent()\childrens()\type = #PB_GadgetType_Button)
            
            Resize(GetEvent()\childrens(), 
                   GetEvent()\childrens()\x[#__c_3] + (_change_x_),
                   GetEvent()\childrens()\y[#__c_3] + (_change_y_), 
                   #PB_Ignore, #PB_Ignore)
          EndIf
        Next
        PopListPosition(GetEvent()\childrens())
      EndIf
    EndMacro
  ...
  Module bar
    ...
    Procedure SetParent(*this._s_widget, *Parent._s_widget, parent_item.l=0)
      ...
      CompilerElse
        GetEvent()\widget = *this
        *this\parent = *Parent
        
        If *this\parent
          ;Debug Bool(*this\parent\type = #PB_GadgetType_ScrollArea)
          ;Debug *this\class +" "+ *this\parent\window
        ...
        AddElement(GetEvent()\childrens()) : GetEvent()\childrens() = *this
      CompilerEndIf
    EndProcedure
            ...
            If *this\bar\state <> *this\bar\from
              If *this\bar\button[*this\bar\state]\color\state = #__s_1
                ...
                *this\bar\button[*this\bar\state]\color\state = #__s_0
                ...
                If *this\cursor And 
                   *this\bar\state = #__b_3 And 
                   *this\root\canvas\gadget_set_cursor_widget = *this
                  *this\root\canvas\gadget_set_cursor_widget = 0
    ...
    Procedure.i Spin(x.l, y.l, width.l, height.l, Min.l,Max.l, Flag.i=0, round.l=0, Increment.f=1.0)
      Protected *this._s_widget = Create(#PB_GadgetType_Spin, Root()\opened, 16, min,max,0, flag, round, Increment)
    ...
    Procedure.i Scroll(x.l, y.l, width.l, height.l, Min.l,Max.l,PageLength.l, Flag.i=0, round.l=0)
      Protected *this._s_widget = Create(#PB_GadgetType_ScrollBar, Root()\opened, #__scroll_buttonsize, min,max,pagelength, flag, round, 1.0)
      *this\class = #PB_Compiler_Procedure
      CompilerIf Defined(widget, #PB_Module)
        widget::_set_last_parameters_(*this, *this\type, Flag, Root()\opened)
      CompilerElse
    ...
    Procedure.i ScrollArea(x.l, y.l, width.l, height.l, Scroll_AreaWidth.l, Scroll_AreaHeight.l, scroll_step.l=1, Flag.i=0)
      ...
      *this\class = #PB_Compiler_Procedure
      ...
      CompilerElse
        ;Debug Root()\opened\class
        ;Debug Root()\opened\window
        SetParent(*this, Root()\opened)
      CompilerEndIf
    ...
    Procedure CallBack()
      ...
      If change
        ...
        ForEach GetEvent()\childrens()
          If _from_point_(mouse_x, mouse_y, GetEvent()\childrens(), [#__c_4])
            *this = GetEvent()\childrens()
            Break
          EndIf
        Next
...
CompilerIf #PB_Compiler_IsMainFile
  ...
  Procedure v_GadgetCallBack()
    ...
    ;ProcedureReturn
    ForEach GetEvent()\childrens()
      If GetEvent()\childrens()\bar\vertical And GetEvent()\childrens()\type = GadgetType(EventGadget())
        Repaint | SetState(GetEvent()\childrens(), state)
      EndIf
    Next
  ...
  Procedure h_GadgetCallBack()
    ...
    ;ProcedureReturn
    ForEach GetEvent()\childrens()
      If Not GetEvent()\childrens()\bar\vertical And GetEvent()\childrens()\type = GadgetType(EventGadget())
        Repaint | SetState(GetEvent()\childrens(), state)
      EndIf
    Next
