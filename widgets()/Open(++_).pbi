    ...
    Procedure _events_(*this._s_widget, _event_type_.l, _mouse_x_.l, _mouse_y_.l, _wheel_x_.b=0, _wheel_y_.b=0)
      ...
      If *this\splitter And *this\bar\from <> #__b_3
        If *this\splitter And *this\splitter\first And Not *this\splitter\g_first
          If _events_(*this\splitter\first, _event_type_, _mouse_x_, _mouse_y_)
            ProcedureReturn 1
          EndIf
        EndIf
        If *this\splitter And *this\splitter\second And Not *this\splitter\g_second
          If _events_(*this\splitter\second, _event_type_, _mouse_x_, _mouse_y_)
            ProcedureReturn 1
          EndIf
        EndIf
      EndIf
              ...
              LastElement(Events()\childrens()) : Repeat                                 
                ...
                If _is_child_(Events()\childrens(), Root()\entered)
                  If Events()\childrens()\state = #__s_1
                    ...
                    Repaint | _events_(Events()\childrens(), #__Event_MouseLeave, mouse_x, mouse_y)
                  EndIf
            ...
            ForEach Events()\childrens()
              If _is_child_(Events()\childrens(), Root()\entered)
                If Events()\childrens()\state = #__s_0
                  ...
                  Repaint | _events_(Events()\childrens(), #__Event_MouseEnter, mouse_x, mouse_y)
                EndIf
...
;         EndIf
      ...
      ElseIf eventtype = #__Event_Input Or
        ...
        If GetActive() 
          If GetActive()\gadget
            Repaint | _events_(GetActive()\gadget, eventtype, mouse_x, mouse_y)
          Else
            Repaint | _events_(GetActive(), eventtype, mouse_x, mouse_y)
          EndIf
        EndIf
        
      Else
        If Root()\entered And change
          Repaint | _events_(Root()\entered, eventtype, mouse_x, mouse_y)
        EndIf
        If Root()\selected And Root()\entered <> Root()\selected And change 
          Repaint | _events_(Root()\selected, eventtype, mouse_x, mouse_y)
        EndIf
      EndIf
      ...
      If Root()\mouse\buttons
        ...
        If Not Root()\mouse\buttons
          ...
          ; ;           _events_(Root()\entered, #__Event_drop, mouse_x, mouse_y)
          ; ;         EndIf
          ...
          If GetActive() 
            If GetActive()\state
              If Not Root()\mouse\drag
                Repaint | _events_(GetActive(), #__Event_LeftClick, mouse_x, mouse_y)
              EndIf
            Else
              Repaint | _events_(GetActive(), #__Event_MouseLeave, mouse_x, mouse_y)
              Repaint | _events_(Root()\entered, #__Event_MouseEnter, mouse_x, mouse_y)
            EndIf
            ...
            If GetActive()\gadget
              If GetActive()\gadget\state
                If Not Root()\mouse\drag
                  Repaint | _events_(GetActive()\gadget, #__Event_LeftClick, mouse_x, mouse_y)
                EndIf
              Else
                Repaint | _events_(GetActive()\gadget, #__Event_MouseLeave, mouse_x, mouse_y)
                Repaint | _events_(Root()\entered, #__Event_MouseEnter, mouse_x, mouse_y)
              EndIf
            EndIf
          EndIf
