        ...
        If Not \hide And \color\alpha
          ...
          If \type = #PB_GadgetType_TrackBar And \bar\thumb\len
            ...
            If \bar\vertical
              ...
              Line(\bar\button[3]\x+Bool(\bar\inverted)*(\bar\button[3]\width-3-2)+1,\bar\area\pos + _thumb_,3, 1,\bar\button[#__b_3]\color\Frame)
              Line(\bar\button[3]\x+Bool(\bar\inverted)*(\bar\button[3]\width-3-2)+1,\bar\area\pos + *this\bar\area\len - *this\bar\thumb\len + _thumb_,3, 1,\bar\button[#__b_3]\color\Frame)
            ...
            Else
              ...
              Line(\bar\area\pos + _thumb_, \bar\button[3]\y+Bool(Not \bar\inverted)*(\bar\button[3]\height-3-2)+1,1,3,\bar\button[#__b_3]\color\Frame)
              Line(\bar\area\pos + *this\bar\area\len - *this\bar\thumb\len + _thumb_, \bar\button[3]\y+Bool(Not \bar\inverted)*(\bar\button[3]\height-3-2)+1,1,3,\bar\button[#__b_3]\color\Frame)
            EndIf
  ...
  Procedure.i _Create(type.l, size.l, min.l, max.l, pagelength.l, flag.i=0, round.l=7, parent.i=0, scroll_step.f=1.0)
      ...
      With *this
        ...
        If \Type = #PB_GadgetType_TrackBar
          ...
          If Bool(Flag & #__bar_Inverted = #__bar_Inverted)
            \bar\inverted = #True
          EndIf
        ...
        If \Type = #PB_GadgetType_Spin
          ...
          If Not Bool(Flag&#__bar_nobuttons)
            ; min thumb size
            ;\bar\button[#__b_3]\len = 16
          EndIf
          
          Size = #__spin_buttonsize
      ...
      With *this
        *event\widget = *this
        ...
        If \Type = #PB_GadgetType_Spin
          ...
          Protected Size = #__spin_buttonsize
          ...
          \bar\button[#__b_3]\len = #__spin_buttonsize
...
CompilerIf #PB_Compiler_IsMainFile
  ...
  Procedure ReDraw_CanvasWindow(Canvas)
    If StartDrawing(CanvasOutput(Canvas))
      FillMemory( DrawingBuffer(), DrawingBufferPitch() * OutputHeight(), $F6)
