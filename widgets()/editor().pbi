...
;     Else
      ...
      Else
        If _this_\scroll\width < _this_\row\_s()\text\width+_this_\text\x*2 + 1 ; *this\text\caret\width
          _this_\scroll\width = _this_\row\_s()\text\width+_this_\text\x*2 + 1 ; *this\text\caret\width
        EndIf
      EndIf
;     EndIf
  EndMacro
        ...
        If \scroll And \text\change
          ...
          If \scroll\x <> \scroll\h\bar\min
            ...
            If Not \vertical
              If \text\align\right
                ForEach \row\_s()
                  \row\_s()\text\x = \x[2] + \scroll\width - \row\_s()\text\width - \text\x - 1
              ...
              If \text\align\horizontal
                ForEach \row\_s()
                  \row\_s()\text\x = \x[2] + (\scroll\width - \row\_s()\text\width)/2
                 ; \row\_s()\x =  \row\_s()\text\x
                Next
              EndIf
            EndIf
...
CompilerIf #PB_Compiler_IsMainFile
  ...
  If OpenWindow(0, 0, 0, 615, (height+5)*8+20+110, "String on the canvas", #PB_Window_SystemMenu | #PB_Window_ScreenCentered)
    ...
    *S_1 = GetGadgetData(Gadget(1, 8,  (height+5)*1+10, 140, height, text_h))
    ...
    *S_1 = GetGadgetData(Gadget(-1, 8+150,  (height+5)*1+10, 140, height, text_h, #__string_right))
    ...
    *S_11 = GetGadgetData(Gadget(11, 305+8,  (height+5)*1+10, 140, height, text_h, #__text_invert))
    ...
    *S_11 = GetGadgetData(Gadget(-1, 305+8+150,  (height+5)*1+10, 140, height, text_h, #__string_right|#__text_invert))
