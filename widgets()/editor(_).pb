...
;     ProcedureReturn 3
        ...
        If (_line_ = *this\row\_s()\index Or *this\index[2] = *this\row\_s()\index) Or    ; линия
          ...
          If _line_ = *this\index[2]  ; And *this\index[2] = *this\row\_s()\index
            If *this\row\caret\pos[1] > *this\row\caret\pos[2]
              _edit_sel_(*this, *this\row\caret\pos[2], *this\row\caret\pos[1] - *this\row\caret\pos[2])
            Else
              _edit_sel_(*this, *this\row\caret\pos[1], *this\row\caret\pos[2] - *this\row\caret\pos[1])
            EndIf
          ...
          ElseIf _line_ = *this\row\_s()\index 
            If _line_ > *this\index[2] 
              _edit_sel_(*this, 0, *this\row\caret\pos[1])
            Else
              _edit_sel_(*this, *this\row\caret\pos[1], *this\row\_s()\text\len - *this\row\caret\pos[1])
            EndIf
          ...
          ElseIf *this\index[2] = *this\row\_s()\index
            ...
            If *this\countitems = 1 And 
              ...
              *this\index[1] = 0
              ...
              If *this\row\caret\pos[2] > *this\row\caret\pos[1]
                _edit_sel_(*this, 0, *this\row\caret\pos[2])
              Else
                *this\row\caret\pos[1] = *this\row\_s()\text\len
                _edit_sel_(*this, *this\row\caret\pos[2], Bool(_line_ <> *this\index[2]) * (*this\row\_s()\text\len - *this\row\caret\pos[2]))
              EndIf
            ...
            Else
              ...
              If *this\index[2] > _line_ 
                _edit_sel_(*this, 0, *this\row\caret\pos[2])
              Else
                _edit_sel_(*this, *this\row\caret\pos[2], (*this\row\_s()\text\len - *this\row\caret\pos[2]))
              EndIf
            EndIf
  ...
  Procedure   _edit_sel_update_(*this._struct_)
    ...
    ;         If *this\index[2] = *this\index[1]
    ;           If *this\row\caret\pos[1]<>*this\row\caret\pos[2]
    ;             _edit_sel_(*this, 0, *this\row\caret\pos[1]-*this\row\selected\text\len)
    ;           Else
    ;             _edit_sel_(*this, *this\row\caret\pos[1]-*this\row\selected\text\len, 0)
    ;           EndIf
    ;         EndIf
    ;         
    ;       EndIf
      ...
      Else
        ...
        ForEach \row\_s()
          If Not \row\_s()\hide
            ...
            ;               ; \row\selected = \row\_s()
            ;                 _edit_sel_(*this, \row\caret\pos[1]-\row\selected\text\len , 0)
            ;               _text_scroll_x_(*this)
            ;             
            ;             EndIf
        ...
        If \countitems
          ...
          ForEach \row\_s()
            ...
            If \row\_s()\draw 
              ...
              Protected text_x_sel = \row\_s()\text[2]\x-*this\scroll\h\page\pos
              ...
              If *this\text\editable And \row\_s()\text[2]\width And *this\color\front <> *this\row\color\front[2]
                ...
                CompilerIf #PB_Compiler_OS = #PB_OS_MacOS
                  ...
                  If (*this\index[1] > *this\index[2] Or 
                    ...
                    DrawRotatedText(Text_X, Text_Y, \row\_s()\text\string.s, *this\text\rotate, *this\row\color\front[*this\row\color\state])
                    
                    If *this\row\color\fore[2]
                      DrawingMode(#PB_2DDrawing_Gradient|#PB_2DDrawing_AlphaBlend)
                      _box_gradient_(*this\vertical,text_x_sel, Y, \row\_s()\text[2]\width, \row\_s()\height, *this\row\color\fore[2], *this\row\color\back[2], \row\_s()\round)
                    Else
                      DrawingMode(#PB_2DDrawing_Default|#PB_2DDrawing_AlphaBlend)
                      Box(text_x_sel, Y, \row\_s()\text[2]\width, \row\_s()\height, *this\row\color\back[2])
                    EndIf
                    
                    If \row\_s()\text[2]\string.s
                      DrawingMode(#PB_2DDrawing_Transparent|#PB_2DDrawing_AlphaBlend)
                      DrawRotatedText(text_x_sel, Text_Y, \row\_s()\text[2]\string.s, *this\text\rotate, *this\row\color\front[2])
                    EndIf
                  ...
                  Else
                    ...
                    If \row\_s()\text[3]\string.s
                      DrawingMode(#PB_2DDrawing_Transparent|#PB_2DDrawing_AlphaBlend)
                      DrawRotatedText(\row\_s()\text[3]\x-*this\scroll\h\page\pos, Text_Y, \row\_s()\text[3]\string.s, *this\text\rotate, *this\row\color\front[*this\row\color\state])
                    EndIf
                    
                    If \row\_s()\text[2]\string.s
                      DrawingMode(#PB_2DDrawing_Transparent|#PB_2DDrawing_AlphaBlend)
                      DrawRotatedText(Text_X, Text_Y, \row\_s()\text[1]\string.s+\row\_s()\text[2]\string.s, *this\text\rotate, *this\row\color\front[2])
                    EndIf
                    
                    If \row\_s()\text[1]\string.s
                      DrawingMode(#PB_2DDrawing_Transparent|#PB_2DDrawing_AlphaBlend)
                      DrawRotatedText(Text_X, Text_Y, \row\_s()\text[1]\string.s, *this\text\rotate, *this\row\color\front[*this\row\color\state])
                    EndIf
                  EndIf
                ...
                CompilerElse
                  ...
                  If \row\_s()\text[1]\string.s
                    DrawRotatedText(\row\_s()\text[1]\x-*this\scroll\h\page\pos, Text_Y, \row\_s()\text[1]\string.s, *this\text\rotate, *this\row\color\front[*this\row\color\state])
                  EndIf
                  If \row\_s()\text[2]\string.s
                    DrawRotatedText(text_x_sel, Text_Y, \row\_s()\text[2]\string.s, *this\text\rotate, *this\row\color\front[2])
                  EndIf
              ...
              Else
                If \row\_s()\text[2]\width
                  DrawingMode(#PB_2DDrawing_Default|#PB_2DDrawing_AlphaBlend)
                  Box(text_x_sel, Y, \row\_s()\text[2]\width, \row\_s()\height, $FFFBD9B7);*this\row\color\back[2])
                EndIf
                
                If \color\state = 2
                  DrawingMode(#PB_2DDrawing_Transparent)
                  DrawRotatedText(Text_X, Text_Y, \row\_s()\text\string.s, *this\text\rotate, *this\row\color\front[2])
                Else
                  DrawingMode(#PB_2DDrawing_Transparent)
                  DrawRotatedText(Text_X, Text_Y, \row\_s()\text\string.s, *this\text\rotate, *this\row\color\front[*this\row\color\state])
                EndIf
      ...
      If *this\row\caret\pos <> State
        *this\row\caret\pos = State
        ...
        While *end\c 
          If *end\c = #LF 
            ...
            If len + Item >= state
              ...
              *this\row\caret\pos[1] = state - (len-(*end-*str)/#__sOC) - Item
        ...
        If *this\index[1] <> Item 
          ...
          *this\row\caret\pos[1] = (state - len - Item) 
          *this\row\caret\pos[2] = *this\row\caret\pos[1]
        EndIf
...
;           EndIf
  ...
  Procedure.i events_key_editor(*this._struct_, eventtype.l, mouse_x.l, mouse_y.l)
    ...
    With *this
        ...
        Case #PB_EventType_KeyDown
          Select *this\root\keyboard\key
            Case #PB_Shortcut_Home : *this\row\caret\pos[2] = 0
              ...
              Repaint = _edit_sel_draw_(*this, *this\index[2], *this\row\caret\pos[2])
            ...
            Case #PB_Shortcut_A        ; Ok
              If _key_control_ And
                ...
                Repaint = _edit_sel_draw_(*this, \countitems - 1, \text\len)
              EndIf
            ...
            Case #PB_Shortcut_Up       ; Ok
              If *this\index[1] > _line_first_
                ...
                If _key_shift_
                  ...
                  Else
                    Repaint = _edit_sel_draw_(*this, *this\index[1] - _step_, *this\row\caret\pos[1])  
                  EndIf
              ...
              ElseIf *this\index[1] = _line_first_
                ...
                If *this\row\caret\pos[1] <> _caret_min_ : *this\row\caret\pos[2] = _caret_min_ : _caret_last_pos_ = *this\row\caret\pos[1]
                  Repaint = _edit_sel_draw_(*this, _line_first_, *this\row\caret\pos[2])  
                EndIf
            ...
            Case #PB_Shortcut_Down     ; Ok
              If *this\index[1] < _line_last_
                ...
                If _key_shift_
                  If _key_control_
                    Repaint = _edit_sel_draw_(*this, *this\index[2], *this\row\caret\pos[2])  
                    Repaint = _edit_sel_draw_(*this, \countitems - 1, *this\text\len)  
                  Else
                    Repaint = _edit_sel_draw_(*this, *this\index[1] + _step_, *this\row\caret\pos[1])  
                  EndIf
                ElseIf *this\root\keyboard\key[1] & #PB_Canvas_Alt 
              ...
              ElseIf *this\index[1] = _line_last_
                ...
                If *this\row\caret\pos[1] <> _caret_max_ : *this\row\caret\pos[2] = _caret_max_ : _caret_last_pos_ = *this\row\caret\pos[1]
                  Repaint = _edit_sel_draw_(*this, _line_last_, *this\row\caret\pos[2])  
                EndIf
              ...
              If _key_shift_        
                ...
                Else
                  ...
                  ElseIf *this\row\caret\pos[1] > _caret_min_
                    Repaint = _edit_sel_draw_(*this, _line_, *this\row\caret\pos[1] - _step_)  
                  EndIf
                EndIf
                
              ElseIf *this\index[1] > _line_first_
                If *this\root\keyboard\key[1] & #PB_Canvas_Alt 
                  *this\row\caret\pos[2] = _edit_sel_start_(*this)
            ...
            Case #PB_Shortcut_Right    ; Ok
              ...
              ElseIf *this\index[1] < _line_last_
                If *this\root\keyboard\key[1] & #PB_Canvas_Alt 
                  *this\row\caret\pos[2] = _edit_sel_stop_(*this)
      ...
      ;           _edit_sel_(*this, \row\caret\pos[1] , \row\caret\pos[2]-\row\caret\pos[1] )
      ;         Else
      ;           ; Debug \row\caret\pos[1] -\row\caret\pos[2]
      ;           _edit_sel_(*this, \row\caret\pos[2], \row\caret\pos[1]-\row\caret\pos[2])
      ;         EndIf
      ;         
      ;         StopDrawing() 
      ;       EndIf                                                  
                ...
                If *this\text\editable And *this\row\box\checked
                  ...
                  If *this\row\caret\pos[2] = *this\row\caret\pos[1] 
                    ...
                    *this\row\selected = #Null
                    ...
                    Repaint = _edit_sel_(*this, *this\row\caret\pos[2], *this\row\caret\pos[1])
