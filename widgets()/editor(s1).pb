    ...
    If Not *this\hide
      ...
      With *this
        ...
        If (\text\change Or \resize)
          ...
          If \scroll\v And \scroll\v\max <> (\scroll\height - \flag\gridlines) And 
...
;           If \index[2] >= 0 And \index[2] < ListSize(\row\_s()) And
;              SelectElement(\row\_s(), \index[2]) And (\row\_s()\y+\row\_s()\height >= \height[2])
;             
;             Bar::SetState(\scroll\v, ((\row\_s()\y+\row\_s()\height)-(\height[2]+\text\y)))
;           EndIf
          
          If \row\selected And (\row\selected\y+\row\selected\height) >= \height[2]
            Debug  "get "+\row\selected\index +" "+ \row\_s()\index
            Bar::SetState(\scroll\v, ((\row\selected\y+\row\selected\height)-(\height[2]+\text\y)))
          EndIf
           Debug  "get "+\row\selected\index +" "+ \row\_s()\index
         EndIf 
