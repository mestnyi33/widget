...
;     EndIf
  ...
  Macro _make_scroll_height_(_this_)
    If _this_\vertical
      _this_\scroll\width + _this_\row\_s()\text\width + _this_\flag\gridlines
    Else
      _this_\scroll\height + _this_\row\_s()\text\height + _this_\flag\gridlines
    EndIf
...
;                 \row\_s()\text\pos = pos+ListSize(\row\_s()) : pos + \row\_s()\text\Len
;             \row\_s()\text\Len = (*End-*str)>>#PB_Compiler_Unicode
;             \row\_s()\text\string.s = PeekS (*str, \row\_s()\text\Len)
;             \row\_s()\text\width = TextWidth(\row\_s()\text\string.s)
           ; \row\_s()\Text\Height = \Text\Height
            
            ; Debug ""+\row\_s()\text\pos +" "+ \row\_s()\text\string.s
            ;                     
            ;\row\_s()\y = Text_Y 
            ...
            ;Else
            If \row\_s()\text\rotate = 0
              ;                   If _y2_
              ;                     \row\_s()\text\y = _y_ + (_height_-\row\_s()\text\height)/2
              ;                   Else
              \row\_s()\text\y = _y_ + _y2_ ; - Bool(\row\_s()\text\align\bottom)*\row\_s()\text\padding
                                           ;                   EndIf
            ...
            ElseIf \row\_s()\text\rotate = 180
              \row\_s()\text\y = _y_ + (_height_-_y2_); + Bool(\row\_s()\text\align\bottom)*\row\_s()\text\padding)
