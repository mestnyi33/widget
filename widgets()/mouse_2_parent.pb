...
CompilerIf Not Defined(Bar, #PB_Module)
  ...
  Module bar
    ...
    ;-
    ;- PUBLICs
    Procedure _is_child(*parent._s_widget, *this._s_widget)
        ...
        If *this\parent <> *Parent
          If *this\parent
            ...
            *this\parent\count\childrens - 1
            ...
            ChangeCurrentElement(Events()\childrens(), *this\adress)
            MoveElement(Events()\childrens(), #PB_List_After, *Parent\adress)
            
            While PreviousElement(Events()\childrens()) 
              If _is_child(*this, Events()\childrens())
                MoveElement(Events()\childrens(), #PB_List_After, *this\adress)
              EndIf
            Wend
          EndIf
          ...
          *this\parent = *Parent
          ...
          If Not (*this\parent And *this\parent\type = #PB_GadgetType_Splitter)
            ...
            *this\adress = AddElement(Events()\childrens()) 
            Events()\childrens() = *this
          EndIf
          
          ;  Debug ListIndex(Events()\childrens())  
        EndIf
      CompilerEndIf
    EndProcedure
    ...
    Procedure.i Scroll(x.l, y.l, width.l, height.l, Min.l,Max.l,PageLength.l, Flag.i=0, round.l=0)
      ...
      *this\bar\inverted = Bool(Flag & #__bar_Inverted = #__bar_Inverted)
      ...
      If Flag & #__bar_child = #__bar_child
        *this\parent = Root()\opened
        *this\root = Root()\opened\root
        *this\window = Root()\opened\window
        ; 
        *this\index = Root()\opened\index
        *this\adress = Root()\opened\adress
      Else
        CompilerIf Defined(widget, #PB_Module)
          widget::_set_last_parameters_(*this, *this\type, Flag, Root()\opened)
        CompilerElse
...
;           \scroll\v = Scroll(0,0,0,0,0,Scroll_AreaHeight,Height, #__bar_child|#__bar_vertical, 7)
;           \scroll\h = Scroll(0,0,0,0,0,Scroll_AreaWidth,Width, #__bar_child, 7)
        ...
        ; 
;         \scroll\v\index = *this\index
;         \scroll\h\index = *this\index
;         \scroll\v\adress = *this\adress
;         \scroll\h\adress = *this\adress
;         
    ...
    Procedure CallBack()
      ...
      If change
        ...
        ; set mouse state (entered&leaved)
        If Root()\entered <> *this
          If Root()\entered And Not (#__from_mouse_state And _is_child(Root()\entered, *this))
...
;                 If Root()\entered\parent And Root()\entered\parent\scroll And 
;                    Root()\entered\parent\scroll\v And Root()\entered\parent\scroll\h  
;                   ChangeCurrentElement(Events()\childrens(), Root()\entered\parent\adress)
;                 Else
;                   ChangeCurrentElement(Events()\childrens(), Root()\entered\adress)
;                 EndIf
                
                  ChangeCurrentElement(Events()\childrens(), Root()\entered\adress)
                ; Debug ""+Root()\entered\index +" "+ Root()\entered\adress +" "+ Events()\childrens()
                
                Repeat                 
                  If _is_child(Events()\childrens(), Root()\entered)
                    If Events()\childrens()\state = #__s_1
                      Events()\childrens()\state = #__s_0
                      
                      Repaint | _events(Events()\childrens(), #__Event_MouseLeave, mouse_x, mouse_y)
                    EndIf
                  EndIf
                Until PreviousElement(Events()\childrens()) = #False 
              EndIf
              ...
              ForEach Events()\childrens()
                ...
                If _is_child(Events()\childrens(), Root()\entered)
                  If Events()\childrens()\state = #__s_0
                    ...
                    Repaint | _events(Events()\childrens(), #__Event_MouseEnter, mouse_x, mouse_y)
                  EndIf
                EndIf
              Next
            EndIf
      ...
      If (eventtype = #__Event_LeftButtonDown Or
          eventtype = #__Event_RightButtonDown) And
         _is_widget_(Root()\entered) 
...
CompilerIf #PB_Compiler_IsMainFile
  ...
  If OpenWindow(0, 0, 0, 650, 550, "enter", #PB_Window_SystemMenu | #PB_Window_ScreenCentered)
    ...
    c_1_1 = Container(10, 10, 130, 130, #__Flag_NoGadget)
    CloseList()
    
    c_1_2 = Container(10, 175, 130, 130, #__Flag_NoGadget)
    CloseList()
    
    s_0 = Splitter(235, 10, 330, 330, c_0, c_1, #PB_Splitter_Vertical)
