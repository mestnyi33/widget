...
Module Editor
  ...
  Macro _set_content_X_(_this_)
    If _this_\vertical
      ...
      If *this\text\align\bottom
        *this\scroll\y = ((*this\height - *this\bs*2 - *this\text\y*2)-(*this\text\height**this\count\items)) 
        
      ElseIf *this\text\align\vertical
        *this\scroll\y = (((*this\height - *this\bs*2 -*this\text\y*2)-(*this\text\height**this\count\items))/2)
    ...
    Else
      ...
      If *this\text\align\right
        *this\scroll\x = ((*this\width - *this\bs*2 - *this\text\x*2)-*this\row\_s()\text\width)
        
      ElseIf *this\text\align\horizontal
        *this\scroll\x = ((*this\width - *this\bs*2 - *this\text\x*2 + *this\row\margin\width)-*this\row\_s()\text\width-Bool(*this\row\_s()\text\width % 2))/2 
    ...
    ElseIf _this_\text\rotate = 180
      ;_this_\scroll\y = (_this_\height - _this_\bs*2 - _this_\text\y*2)
      
      If _this_\row\_s()\text\align\right
         _this_\scroll\x = _this_\row\_s()\text\width
      ; _this_\row\_s()\text\x = _x_ + _this_\row\_s()\text\width + _this_\row\_s()\text\padding 
      ElseIf _this_\row\_s()\text\align\horizontal
       ; _this_\row\_s()\text\x = _x_ + (_width_+_this_\row\_s()\text\width)/2 
       _this_\scroll\x = (_this_\row\_s()\text\width-Bool(_this_\row\_s()\text\width % 2))/2 
       
     Else
        _this_\scroll\x = (_this_\width - _this_\bs*2 - _this_\text\x*2) ; _x_ + _width_-_this_\row\_s()\text\padding 
      EndIf
  ...
  EndIf
    ...
    ;     ;If _this_\text\vertical
    ;     If _this_\row\_s()\text\rotate = 90
    ;       ;                   If _y2_ < 0
    ;       ;                     _this_\row\_s()\text\x = _x_ + (_width_-_this_\row\_s()\text\height)/2
;       ;                   Else
;       _this_\row\_s()\text\x = _x_ + _y2_
;       ;                   EndIf
;       
;       If _this_\row\_s()\text\align\right
;         _this_\row\_s()\text\y = _y_ + _this_\row\_s()\text\align\height+_this_\row\_s()\text\width + _this_\row\_s()\text\padding
;       ElseIf _this_\row\_s()\text\align\horizontal
;         _this_\row\_s()\text\y = _y_ + (_height_+_this_\row\_s()\text\align\height+_this_\row\_s()\text\width)/2
;       Else
;         _this_\row\_s()\text\y = _y_ + _height_-_this_\row\_s()\text\padding
;       EndIf
;       
;     ElseIf _this_\row\_s()\text\rotate = 270
;       _this_\row\_s()\text\x = _x_ + (_width_-_y2_)
;       
;       If _this_\row\_s()\text\align\right
;         _this_\row\_s()\text\y = _y_ + (_height_-_this_\row\_s()\text\width-_this_\row\_s()\text\padding) 
;       ElseIf _this_\row\_s()\text\align\horizontal
;         _this_\row\_s()\text\y = _y_ + (_height_-_this_\row\_s()\text\width)/2 
;       Else
;         _this_\row\_s()\text\y = _y_ + _this_\row\_s()\text\padding 
;       EndIf
;       
;     EndIf
;     
;     ;Else
;     If _this_\row\_s()\text\rotate = 0
;       ;                   If _y2_
;       ;                     _this_\row\_s()\text\y = _y_ + (_height_-_this_\row\_s()\text\height)/2
;       ;                   Else
;       _this_\row\_s()\text\y = _y_ + _y2_ ; - Bool(_this_\row\_s()\text\align\bottom)*_this_\row\_s()\text\padding
;                                    ;                   EndIf
;       
;       If _this_\row\_s()\text\align\right
;         _this_\row\_s()\text\x = _x_ + (_width_-_this_\row\_s()\text\align\width-_this_\row\_s()\text\width - _this_\row\_s()\text\padding) 
;       ElseIf _this_\row\_s()\text\align\horizontal
;         _this_\row\_s()\text\x = _x_ + (_width_-_this_\row\_s()\text\align\width-_this_\row\_s()\text\width)/2
;       Else
;         _this_\row\_s()\text\x = _x_ + _this_\row\_s()\text\padding
;       EndIf
;       
;     ElseIf _this_\row\_s()\text\rotate = 180
;       _this_\row\_s()\text\y = _y_ + (_height_-_y2_); + Bool(_this_\row\_s()\text\align\bottom)*_this_\row\_s()\text\padding)
;       
;       If _this_\row\_s()\text\align\right
;         _this_\row\_s()\text\x = _x_ + _this_\row\_s()\text\width + _this_\row\_s()\text\padding 
;       ElseIf _this_\row\_s()\text\align\horizontal
;         _this_\row\_s()\text\x = _x_ + (_width_+_this_\row\_s()\text\width)/2 
;       Else
;         _this_\row\_s()\text\x = _x_ + _width_-_this_\row\_s()\text\padding 
;       EndIf
;       
;     EndIf
;     ;EndIf
...
CompilerIf #PB_Compiler_IsMainFile
  ...
  Define height=80, Text1.s = " Vertical & Horizontal" + #LF$ + "   Centered   Text in   " + #LF$ + "Multiline StringGadget"
  ...
  If OpenWindow(0, 0, 0, 615, (height+5)*8+20+110, "String on the canvas", #PB_Window_SystemMenu | #PB_Window_ScreenCentered)
    *S_0 = GetGadgetData(Gadget(0, 8,  10, 290, height, Text1, #__string_readonly|#__text_top))
    *S_1 = GetGadgetData(Gadget(1, 8,  (height+5)*1+10, 290, height, Text1, #__string_numeric))
    *S_2 = GetGadgetData(Gadget(2, 8,  (height+5)*2+10, 290, height, Text1, #__text_bottom))
    *S_3 = GetGadgetData(Gadget(3, 8,  (height+5)*3+10, 290, height, Text1, #__string_uppercase|#__string_center))
    *S_4 = GetGadgetData(Gadget(4, 8, (height+5)*4+10, 290, height, Text1, #__flag_vertical|#__string_lowercase|#__text_top))
    *S_5 = GetGadgetData(Gadget(5, 8, (height+5)*5+10, 290, height, Text1, #__flag_vertical|#__flag_borderless))
    *S_6 = GetGadgetData(Gadget(6, 8, (height+5)*6+10, 290, height, Text1, #__flag_vertical|#__text_bottom))
    ...
    *S_10 = GetGadgetData(Gadget(10, 305+8,  10, 290, height, Text1, #__string_readonly|#__text_top|#__string_right|#__text_invert))
    *S_11 = GetGadgetData(Gadget(11, 305+8,  (height+5)*1+10, 290, height, Text1, #__string_numeric|#__string_right|#__text_invert))
    *S_12 = GetGadgetData(Gadget(12, 305+8,  (height+5)*2+10, 290, height, Text1, #__string_right|#__text_bottom|#__text_invert))
    *S_13 = GetGadgetData(Gadget(13, 305+8,  (height+5)*3+10, 290, height, Text1, #__flag_vertical|#__string_lowercase|#__string_center|#__text_invert))
    *S_14 = GetGadgetData(Gadget(14, 305+8, (height+5)*4+10, 290, height, Text1, #__flag_vertical|#__text_top|#__string_right|#__text_invert))
    *S_15 = GetGadgetData(Gadget(15, 305+8, (height+5)*5+10, 290, height, Text1, #__flag_vertical|#__flag_borderless|#__string_right|#__text_invert))
    *S_16 = GetGadgetData(Gadget(16, 305+8, (height+5)*6+10, 290, height, Text1, #__flag_vertical|#__text_bottom|#__string_right|#__text_invert))
