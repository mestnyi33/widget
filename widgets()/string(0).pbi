...
Module Editor
  ...
  Macro _make_scroll_width_(_this_)
    If _this_\vertical
      If _this_\text\multiline < 0 And (_this_\row\_s()\text\width+_this_\text\x*2 + *this\text\caret\width) > _this_\height[2]
        _this_\scroll\height = _this_\height[2] - _this_\row\margin\width - Bool(_this_\scroll\width > _this_\width[2]) * _this_\scroll\v\height
      Else
  ...
  Procedure.i text_multiline_make(*this._s_widget)
    ...
    Protected _x_, _y_, _width_, _height_
    
    With *this
      _x_=*this\x[2] + *this\text\x
      ...
      If string_out <> String.s+Str(*this)     ;And (Not \text\multiline And Not ListSize(\row\_s()))
        ...
        If Not \row\margin\hide
          If \vertical
            \row\margin\width =  \width[2]
            \row\margin\height = TextWidth(Str(\count\items))+11
            
            \text\align\height = \row\margin\height
          Else
            \row\margin\width = TextWidth(Str(\count\items))+11
            \row\margin\height = \height[2]
            
            \text\align\width = \row\margin\width
          EndIf
          
          If \text\align\right
            \row\margin\x = \x[2] + \width[2] - \row\margin\width
          Else
            \row\margin\x = \x[2]
          EndIf
          If \text\align\bottom
            \row\margin\y = \y[2] + \height[2] - \row\margin\height
          Else
            \row\margin\y = \y[2]
          EndIf
          ...
          While *end\c : If *end\c = #LF : len = (*end-*str)/#__sOC : String = PeekS (*str, len)
              ...
              If AddElement(\row\_s())
                ...
                *this\row\_s()\margin\string = Str(\row\_s()\index)
                ...
                If \vertical
                 *this\row\_s()\margin\x = \row\_s()\text\x
                  If \text\align\bottom
                    *this\row\_s()\margin\y = *this\row\margin\y + 3
                  Else
                    *this\row\_s()\margin\y = (*this\row\margin\y + *this\row\margin\height) - TextWidth(*this\row\_s()\margin\string) - 3
                  EndIf
                Else
                 *this\row\_s()\margin\y = \row\_s()\text\y
                  If \text\align\right
                    *this\row\_s()\margin\x = *this\row\margin\x + 3
                  Else
                    *this\row\_s()\margin\x = (*this\row\margin\x + *this\row\margin\width) - TextWidth(*this\row\_s()\margin\string) - 3
                  EndIf
                EndIf
