...
;     #PB_Flag_Single
  ...
  Structure _S_widget ; Extends _S_bar
    ...
    *widget._S_widget ; active gadget
    *scroll._S_scroll 
  ...
  Macro _init_default_value_(_this_, _event_type_)
    Select _event_type_
      ...
      Case #PB_EventType_MouseWheel
        _this_\root\mouse\wheel\y = GetGadgetAttribute(_this_\root\_gadget, #PB_Canvas_WheelDelta)
      Case #PB_EventType_Input 
        _this_\root\input = GetGadgetAttribute(_this_\root\_gadget, #PB_Canvas_Input)
      Case #PB_EventType_KeyDown, #PB_EventType_KeyUp
        _this_\root\Key = GetGadgetAttribute(_this_\root\_gadget, #PB_Canvas_Key)
        _this_\root\Key[1] = GetGadgetAttribute(_this_\root\_gadget, #PB_Canvas_Modifiers)
      Case #PB_EventType_MouseEnter, #PB_EventType_MouseMove, #PB_EventType_MouseLeave
        _this_\root\mouse\x = GetGadgetAttribute(_this_\root\_gadget, #PB_Canvas_MouseX)
        _this_\root\mouse\y = GetGadgetAttribute(_this_\root\_gadget, #PB_Canvas_MouseY)
...
Module Widget
  ...
  Macro set_cursor(_this_, _cursor_)
    SetGadgetAttribute(_this_\root\_gadget, #PB_Canvas_Cursor, _cursor_)
  EndMacro
  
  Macro Get_Cursor(_this_)
    GetGadgetAttribute(_this_\root\_gadget, #PB_Canvas_Cursor)
  EndMacro
  ...
  Procedure CallBack_Popup()
    ...
    ;             Protected *Widget._S_widget = GetGadgetData(\root\_gadget)
    ;             
    ;             If CallBack(\childrens(), #PB_EventType_LeftButtonDown, WindowMouseX(\root\_window), WindowMouseY(\root\_window))
    ...
    ;             EndIf
    ;             
    ;             SetActiveGadget(*Widget\root\_gadget)
    ;             *Widget\color\state = 0
    ;             *Widget\box\checked = 0
    ;             SetActive(*Widget)
    ;             ReDraw(*Widget\root)
    ;             HideWindow(\root\_window, 1)
    ;             
    ;           Case #PB_Event_Gadget
    ;             mouse_x = GetGadgetAttribute(\root\_gadget, #PB_Canvas_MouseX)
    ;             mouse_y= GetGadgetAttribute(\root\_gadget, #PB_Canvas_MouseY)
    ;             
  ...
  Procedure.i Display_Popup(*this._S_widget, *Widget._S_widget, x.i=#PB_Ignore,y.i=#PB_Ignore)
    ;     With *this
    ;       If X=#PB_Ignore 
    ;         X = \x+GadgetX(\root\_gadget, #PB_Gadget_ScreenCoordinate)
    ;       EndIf
    ;       If Y=#PB_Ignore 
    ;         Y = \y+\height+GadgetY(\root\_gadget, #PB_Gadget_ScreenCoordinate)
    ;       EndIf
    ;       
    ;       If StartDrawing(CanvasOutput(\root\_gadget))
    ;         
    ...
    ;       If *Widget\resize
    ;         ResizeWindow(*Widget\root\_window, x, y, width, Height)
    ;         ResizeGadget(*Widget\root\_gadget, #PB_Ignore, #PB_Ignore, width, Height)
    ;       EndIf
    ;     EndWith
    ;     
    ;     ReDraw(*Widget)
    ;     
    ;     HideWindow(*Widget\root\_window, 0, #PB_Window_NoActivate)
  EndProcedure
  ...
  Procedure.i Popup(*Widget._S_widget, X.l,Y.l,Width.l,Height.l, Flag.i=0)
    ...
    ;         
    ;         If X=#PB_Ignore 
    ;           X = *Widget\x+GadgetX(*Widget\root\_gadget, #PB_Gadget_ScreenCoordinate)
    ;         EndIf
    ;         If Y=#PB_Ignore 
    ;           Y = *Widget\y+*Widget\height+GadgetY(*Widget\root\_gadget, #PB_Gadget_ScreenCoordinate)
    ;         EndIf
    ;         If Width=#PB_Ignore
    ;           Width = *Widget\width
    ;         EndIf
    ...
    ;         EndIf
    ;         
    ;         If IsWindow(*Widget\root\_window)
    ;           Protected WindowID = WindowID(*Widget\root\_window)
    ;         EndIf
    ;         
    ;         \root\parent = *Widget
    ;         \root\_window = OpenWindow(#PB_Any, X,Y,Width,Height, "", #PB_Window_BorderLess|#PB_Window_NoActivate|(Bool(#PB_Compiler_OS<>#PB_OS_Windows)*#PB_Window_Tool), WindowID) ;|#PB_Window_NoGadgets
    ;         \root\_gadget = CanvasGadget(#PB_Any,0,0,Width,Height)
    ;         Resize(\root, 1,1, width, Height)
    ;         
    ;         SetWindowData(\root\_window, *this)
    ;         SetGadgetData(\root\_gadget, *Widget)
    ;         
    ;         BindEvent(#PB_Event_ActivateWindow, @CallBack_Popup(), \root\_window);, \_gadget )
    ;         BindGadgetEvent(\root\_gadget, @CallBack_Popup())
    ;       EndIf
    ;     EndWith  
    ;     
    ...
    If Not *this\hide
      ...
      With *this
        ...
        If \resize
          ...
          PostEvent(#PB_Event_Widget, \root\_window, *this, #PB_EventType_Resize, \resize)
        ...
        If \text\editable And ListSize(\items())
          If \text\change =- 1
            ...
            PostEvent(#PB_Event_Widget, \root\_window, *this, #PB_EventType_Change)
          EndIf
  ...
  Procedure String_Caret(*this._S_widget, Line.i = 0)
    ...
    With *this
      ...
      If \root\_gadget And StartDrawing(CanvasOutput(\root\_gadget)) 
        If FontID : DrawingFont(FontID) : EndIf
    ...
    If *this
      *this\index[#s_1] = 0
      ...
      With *this
        Select EventType
          Case #PB_EventType_LeftButtonUp
            If \root\_gadget And #PB_Cursor_Default = GetGadgetAttribute(\root\_gadget, #PB_Canvas_Cursor)
              SetGadgetAttribute(\root\_gadget, #PB_Canvas_Cursor, *this\cursor)
            EndIf
            ...
            Else
              ...
              If \text[2] And \text[2]\len
                ...
                If *this\text\caret[1] < Caret And Caret < *this\text\caret[1] + \text[2]\len
                  SetGadgetAttribute(\root\_gadget, #PB_Canvas_Cursor, #PB_Cursor_Default)
          ...
          Case #PB_EventType_MouseMove
            If *this\root\mouse\buttons & #PB_Canvas_LeftButton 
              ...
              If *this\text\caret <> Caret
                ...
                If \text\caret[2] ; *this\cursor <> GetGadgetAttribute(\root\_gadget, #PB_Canvas_Cursor)
    ...
    With *this
      ...
      If \height>0 And \width>0 And Not \hide And \color\alpha 
        ...
        If \width[#c_4] > 0 And \height[#c_4] > 0
          ...
          If *this\focus And (*this = Root()\active\widget Or *this = Root()\active)  ; Demo default coordinate
            DrawingMode(#PB_2DDrawing_Outlined)
  ...
  Procedure.i ClearItems(*this._S_widget) 
    With *this
      ...
      ;        PostEvent(#PB_Event_Gadget, \root\_window, \root\_gadget, #PB_EventType_Repaint)
      ;       EndIf
    EndWith
  EndProcedure
  ...
  Procedure GetFocus()
    ProcedureReturn Root()\active\widget
  EndProcedure
  ...
  Procedure.i GetWindow(*this._S_widget)
    If *this = *this\root
      ProcedureReturn *this\root\_window
    Else
  ...
  Procedure.i GetGadget(*this._S_widget)
    ProcedureReturn *this\root\_gadget
  EndProcedure
  ...
  Procedure.i SetActive(*this._S_widget)
    ...
    Macro _set_active_state_(_state_)
      ...
      If Not(Root()\active = Root()\active\root And Root()\active\root\type =- 5)
        ...
        PostEvent(#PB_Event_Gadget, Root()\active\root\_window, Root()\active\root\_gadget, #PB_EventType_Repaint)
      EndIf
      
      If Root()\active\widget
        Root()\active\widget\focus = _state_
        If _state_
          Events(Root()\active\widget, Root()\active\widget\from, #PB_EventType_Focus, -1, -1)
        Else
          Events(Root()\active\widget, Root()\active\widget\from, #PB_EventType_LostFocus, -1, -1)
        EndIf
      EndIf
    EndMacro
      ...
      If \type > 0
        If Root()\active\widget <> *this
          ...
          If Root()\active <> \window
            ...
            Root()\active = \window
            Root()\active\widget = *this
            
            _set_active_state_(1)
          Else
            If Root()\active\widget
              Root()\active\widget\focus = 0
              Events(Root()\active\widget, Root()\active\widget\from, #PB_EventType_LostFocus, -1, -1)
            EndIf
            
            Root()\active\widget = *this
            Root()\active\widget\focus = 1
            Events(Root()\active\widget, Root()\active\widget\from, #PB_EventType_Focus, -1, -1)
          EndIf
  ...
  Procedure.i SetCursor(*this._S_widget, Cursor.i, CursorType.i=#PB_Canvas_Cursor)
    ...
    With *this
      If \cursor <> Cursor
        ...
        SetGadgetAttribute(\root\_gadget, CursorType, Cursor)
...
;       EndIf
  ...
  Procedure.i Form(X.l,Y.l,Width.l,Height.l, Text.s, Flag.i=0, *parent._S_widget=0)
    ...
    Else
      ;OpenList(Root()\parent)
      
      _set_last_parameters_(*this, #PB_GadgetType_Window, Flag, Root()\parent) 
    EndIf
  ...
  Procedure.i Events(*this._S_widget, at.i, EventType.i, mouse_x.i, mouse_y.i, WheelDelta.i = 0)
    ...
    If *this > 0
      ...
      With *this
        Protected canvas = \root\_gadget
        Protected window = \root\_window
            ...
            Select \type 
              Case #PB_GadgetType_Window
                If at = 1
                  ...
                  If *this = \root
                    PostEvent(#PB_Event_CloseWindow, \root\_window, *this)
              ...
              Case #PB_GadgetType_ComboBox
                ...
                ;                 Else
                ;                   HideWindow(\popup\root\_window, 1)
                ;                 EndIf
      ...
      Select EventType 
        Case #PB_EventType_MouseMove,
          ...
          If Root()\mouse\buttons
            ...
            If Root()\active\widget 
              repaint | Events(Root()\active\widget, Root()\active\widget\from, #PB_EventType_MouseMove, mouse_x, mouse_y)
            EndIf  
        ...
        Case #PB_EventType_LeftButtonUp, 
          ...
          If Root()\active\widget And
             Root()\active\widget\state = 2 
            Root()\active\widget\state = 1 
            
            repaint | Events(Root()\active\widget, Root()\active\widget\from, EventType, mouse_x, mouse_y)
            
            If _from_point_(mouse_x, mouse_y, Root()\active\widget, [#c_4])
              
              If Root()\active\widget = *this       
                If EventType = #PB_EventType_LeftButtonUp
                  repaint | Events(Root()\active\widget, Root()\active\widget\from, #PB_EventType_LeftClick, mouse_x, mouse_y)
                EndIf
                If EventType = #PB_EventType_RightClick
                  repaint | Events(Root()\active\widget, Root()\active\widget\from, #PB_EventType_RightClick, mouse_x, mouse_y)
                EndIf
              EndIf
              
            Else
              Root()\active\widget\state = 0
              repaint | Events(Root()\active\widget, Root()\active\widget\from, #PB_EventType_MouseLeave, mouse_x, mouse_y)
            EndIf
            
            Root()\active\widget\root\mouse\buttons = 0   
            If Root()\active\widget\root\mouse\delta
              FreeStructure(Root()\active\widget\root\mouse\delta)
              Root()\active\widget\root\mouse\delta = 0
              Root()\active\widget\drag = 0
            EndIf
        ...
        Case #PB_EventType_Input, 
          ...
          If *this And (Root()\active\widget = *this Or *this = Root()\active)
            
            \root\keyboard\input = GetGadgetAttribute(\root\_gadget, #PB_Canvas_Input)
            \root\keyboard\key = GetGadgetAttribute(\root\_gadget, #PB_Canvas_Key)
            \root\keyboard\key[1] = GetGadgetAttribute(\root\_gadget, #PB_Canvas_Modifiers)
...
CompilerIf #PB_Compiler_IsMainFile
  ...
  If OpenWindow(3, 0, 0, 655, 405, "hide/show widgets", #PB_Window_SystemMenu | #PB_Window_ScreenCentered)
    If Open(3, 0, 0, 655, 605, "Root", #PB_Flag_AnchorsGadget)
      Define *w,*w1,*w2
