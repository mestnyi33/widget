...
Module Widget
  ...
  Macro _set_text_(_this_, _flag_)
    ...
    If _this_\text
      ...
      If Not _this_\scroll
       _this_\scroll = AllocateStructure(_s_scroll)
      EndIf
...
;     EndIf
  ...
  Procedure.s _make_multiline(*this._S_widget, String.s)
  ...
  Procedure.s make_multiline(*this._S_widget, String.s)
    ...
    With *This
      ...
      If \count\items
          \scroll\x = 0;\x[#__c_2]
          \scroll\y = 0;\y[#__c_2]
          \scroll\width = 0;\width[#__c_2]
          \scroll\height = 0;\height[#__c_2]
        
        ClearList(\Items())
        
        If \Vertical
          If \Text\Align\Bottom
            \scroll\x = \x[#__c_2] + (\Width[#__c_2]-(\Text\Height*\count\items)) - \Text\padding
          ElseIf \Text\Align\Vertical
            \scroll\x = \x[#__c_2] + (\Width[#__c_2]-(\Text\Height*\count\items))/2
          Else
            \scroll\x = \x[#__c_2] 
          EndIf
        Else
          If \Text\Align\Bottom
            \scroll\y = \y[#__c_2] + (\Height[#__c_2]-(\Text\Height*\count\items)) - \Text\padding
          ElseIf \Text\Align\Vertical
            \scroll\y = \y[#__c_2] + (\Height[#__c_2]-(\Text\Height*\count\items))/2                     
          Else
            \scroll\y = \y[#__c_2]- \scroll\v\bar\page\pos
          EndIf
        EndIf
        ...
        Protected pos, *Sta.Character = @String, *End.Character = @String 
         Protected _x_=*this\x, _y_=*this\y, _width_=*this\width, _height_=*this\height, _y2_=Text_Y
             If \image And \image\width
               _width_ + \image\width ;+ 20
             EndIf
...
;             \Items()\x = \x[#__c_2]
;             \Items()\y = \y[#__c_2]+Text_Y
            
            
          
            If \Vertical
              If \scroll\height < \items()\text\width
                \scroll\height = \items()\text\width
              EndIf
              
              \Items()\y = \scroll\width + (\scroll\x - \x[#__c_2])
              \scroll\width + \Text\Height
            Else
              If \scroll\width < \items()\text\width
                \scroll\width = \items()\text\width
              EndIf
              
              \Items()\y = \scroll\height + (\scroll\y - \y[#__c_2])
              \scroll\height + \Text\Height
            EndIf
            
            _y2_=\Items()\y;Text_Y
            
            ;\scroll\x = \x[#__c_2] + (\width[#__c_2]-\scroll\width)
...
;                   EndIf
                  ...
                  If \Items()\text\align\right
                    \Items()\text\x = _x_ + (_width_-\Items()\text\align\width-\Items()\text\width - \Items()\text\padding-1) 
                  ElseIf \Items()\text\align\horizontal
                    \Items()\text\x = _x_ + (_width_-\Items()\text\align\width-\Items()\text\width)/2
                  Else
                    \Items()\text\x = _x_ + \Items()\text\padding
                ...
                ElseIf \Items()\text\rotate = 180
                  \Items()\text\y = _y_ + (_height_-_y2_  -  Bool(\text\rotate = 180)*2); + Bool(\Items()\text\align\bottom)*\Items()\text\padding)
...
;         If \vertical
          If \Text\Align\bottom
            \scroll\y = \y[#__c_2] + (\height[#__c_2]-\scroll\height) - \Text\padding
          ElseIf \Text\Align\vertical
            \scroll\y = \y[#__c_2] + (\height[#__c_2]-\scroll\height)/2   
          Else
            \scroll\y = \y[#__c_2] + \Text\padding
          EndIf
;         Else
;           
          If \Text\Align\right
            \scroll\x = \x[#__c_2] + (\width[#__c_2]-\scroll\width) - \Text\padding
          ElseIf \Text\Align\horizontal
            \scroll\x = \x[#__c_2] + (\width[#__c_2]-\scroll\width)/2
          Else
            \scroll\x = \x[#__c_2] + \Text\padding
          EndIf
;         EndIf    
...
; ; ;         Wend
        ...
        If \image
          \image\x = \x[#__c_2]+ (\width[#__c_2] - \scroll\width - \image\width)/2
         \image\y = \y[#__c_2]+(\height[#__c_2]-\image\height)/2
       EndIf 
...
;         EndIf
        ...
        ElseIf \type = #PB_GadgetType_Button 
          \text\string.s = make_multiline(*this._S_widget, \text\string.s[1])
...
CompilerIf #PB_Compiler_IsMainFile
  ...
  If Open(11, 0, 0, 325+80, 160, "Button on the canvas", #PB_Window_SystemMenu | #PB_Window_SizeGadget | #PB_Window_ScreenCentered)
    ...
    redraw(root())
    ResizeWindow(11, 200, WindowY(0)+WindowHeight(0, #PB_Window_FrameCoordinate)+10, #PB_Ignore, #PB_Ignore)
...
;     
   ...
   Global c2, *editor_0._S_widget, *editor_1._S_widget
   
  Procedure ResizeCallBack_111()
    Protected Width = WindowWidth(EventWindow(), #PB_Window_InnerCoordinate) 
    Protected Height = WindowHeight(EventWindow(), #PB_Window_InnerCoordinate)
    
    Resize(*editor_0, Width-90, #PB_Ignore, #PB_Ignore, Height-40)
    Resize(*editor_1, #PB_Ignore, #PB_Ignore, Width-110, Height-40)
    ResizeGadget(c2, 10, 10, Width-20, Height-20)
    SetWindowTitle(EventWindow(), Str(*editor_1\width))
  EndProcedure
  
  If Open(111, 0, 0, 325+80, 160, "Button on the canvas", #PB_Window_SystemMenu | #PB_Window_SizeGadget | #PB_Window_ScreenCentered)
    c2 = GetGadget(Root())
    ...
    With *editor_1
      ResizeImage(0, 32,32)
      *editor_1 = Editor(10, 10, 180,  120);, #__text_MultiLine)
      SetText(*editor_1, Text)
      ;\Cursor = #PB_Cursor_Hand
      ;SetColor(*editor_1, #PB_Gadget_FrontColor, $4919D5)
      ;SetFont(*editor_1, FontID(0))
    EndWith
    
    With *editor_0
      *editor_0 = Editor(200, 10,  180, 120, #__text_MultiLine | #__flag_Vertical)
      SetText(*editor_0, Text)
      ;       SetColor(*editor_0, #PB_Gadget_BackColor, $CCBFB4)
      ;SetColor(*editor_0, #PB_Gadget_FrontColor, $D56F1A)
      ;SetFont(*editor_0, FontID(0))
    EndWith
    
    
;     With *editor_1
;       ResizeImage(0, 32,32)
;       *editor_1 = Button(10, 42, 250,  60, "Button (Horisontal)", #__text_MultiLine,-1)
;       ;       SetColor(*editor_1, #PB_Gadget_BackColor, $D58119)
;       \Cursor = #PB_Cursor_Hand
;       SetColor(*editor_1, #PB_Gadget_FrontColor, $4919D5)
;       ;SetFont(*editor_1, FontID(0))
;     EndWith
...
;     With *editor_0
;       *editor_0 = Button(270, 10,  60, 120, "Button (Vertical)", #__text_MultiLine | #__flag_Vertical)
;       ;       SetColor(*editor_0, #PB_Gadget_BackColor, $CCBFB4)
;       SetColor(*editor_0, #PB_Gadget_FrontColor, $D56F1A)
;       ;SetFont(*editor_0, FontID(0))
;     EndWith
    ...
    redraw(root())
    ResizeWindow(111, WindowX(11)+WindowWidth(11, #PB_Window_FrameCoordinate)+10, WindowY(0)+WindowHeight(0, #PB_Window_FrameCoordinate)+10, #PB_Ignore, #PB_Ignore)
    
    BindEvent(#PB_Event_SizeWindow, @ResizeCallBack_111(), 111)
    PostEvent(#PB_Event_SizeWindow, 111, #PB_Ignore)
    
    ;     BindGadgetEvent(g, @CallBacks())
    ;     PostEvent(#PB_Event_Gadget, 111,111, #PB_EventType_Resize)
    ;     
