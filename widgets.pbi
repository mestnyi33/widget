  ...
  *Bar\Type =- 1
  ...
  Procedure.i __Draw_Panel(*This.Widget_S, scroll_x,scroll_y)
    ...
    With *This 
      ...
      If \Color\Frame<>-1
        DrawingMode(#PB_2DDrawing_Outlined|#PB_2DDrawing_AlphaBlend)
        RoundBox( scroll_x+\X, scroll_y+\Y+\Tabheight/2, \Width, \Height-\Tabheight/2, \Radius, \Radius, \Color\Frame&$FFFFFF|Alpha)
      EndIf
      
      ForEach \Rows()
        Protected State_3,px=8,py=0
        
        State_3 = \Rows()\Color\State
        
        If \Rows()\Text\Change
          \Rows()\Text\width = TextWidth(\Rows()\Text\String)
          \Rows()\Text\height = TextHeight("A")
          
          \Rows()\x = x+px
          \Rows()\y = \y+py
          \Rows()\width = \Rows()\Text\width+10
          x + \Rows()\width - 1
        EndIf
        
        \Rows()\Text\x = \Rows()\x+5
        \Rows()\Text\y = \Rows()\y+((\Rows()\height-py-1)-\Rows()\Text\height)/2
        ...
        If \Color\back[State_3]<>-1
          ...
          EndIf
          BoxGradient( \Vertical, scroll_x+\Rows()\X, scroll_y+\Rows()\Y, \Rows()\Width, \Rows()\Height-py, \Color\Fore[State_3], \Color\Back[State_3], \Radius, \color\alpha)
        EndIf
        
        ; Draw thumb frame
        If \Color\Frame[State_3] 
          DrawingMode(#PB_2DDrawing_Outlined|#PB_2DDrawing_AlphaBlend)
          RoundBox( scroll_x+\Rows()\X, scroll_y+\Rows()\Y, \Rows()\Width, \Rows()\Height-py, \Radius, \Radius, \Color\Frame[State_3]&$FFFFFF|Alpha)
        EndIf
        
        ; Draw string
        If \Rows()\Text\String
          DrawingMode(#PB_2DDrawing_Transparent|#PB_2DDrawing_AlphaBlend)
          DrawText(scroll_x+\Rows()\Text\x, scroll_y+\Rows()\Text\y, \Rows()\Text\String.s, \Color\Front[State_3]&$FFFFFF|Alpha)
        EndIf
        
        ; Draw image
        If \Rows()\Image\handle
          DrawingMode(#PB_2DDrawing_Transparent|#PB_2DDrawing_AlphaBlend)
          DrawAlphaImage(\Rows()\Image\handle, \Rows()\Image\x, \Rows()\Image\y, \color\alpha)
        EndIf
        
        \Rows()\Text\Change = 0
      Next
      
      If \handle[2] And SelectElement(\Rows(), \index[2])
        State_3 = 2
        ...
        If \Color\Frame[State_3] 
          ...
          ;           If State_3 = 2
          ;             DrawingMode(#PB_2DDrawing_Outlined|#PB_2DDrawing_AlphaBlend)
          ;             RoundBox( scroll_x+\Rows()\X+1, scroll_y+\Rows()\Y+1, \Rows()\Width-2, \Rows()\Height-py-2, \Radius, \Radius, \Color\Front[State_3]&$FFFFFF|Alpha)
          ;           EndIf
        EndIf
...
;         RoundBox( scroll_x+\X, scroll_y+\Y, \Width, \Height, \Radius, \Radius, \Color\Frame&$FFFFFF|Alpha)
        ...
        ; Line(scroll_x+\Text\x+5, scroll_y+\Y+\Tabheight, \Text\width+6, 1)
      EndIf
      
      ForEach \Rows()
        Protected State_3,px=2,py
        
        If \handle[2] = \Rows() ; \index[2] = ListIndex(\Rows()) ; (\Index=*This\Index[1] Or \Index=\focus Or \Index=\Index[1])
          State_3 = 2
          ;px=0
          py=0
        Else
        ...
        \Rows()\Text\x = \Rows()\x+5
        \Rows()\Text\y = \Rows()\y+((\Rows()\height-py-1+Bool(State_3 = 2)*4)-\Rows()\Text\height)/2
        ...
        If \Color\back[State_3]<>-1
          If \Color\Fore[State_3]
            DrawingMode( #PB_2DDrawing_Gradient|#PB_2DDrawing_AlphaBlend)
          EndIf
          BoxGradient( \Vertical, scroll_x+\Rows()\X, scroll_y+\Rows()\Y+Bool(State_3 = 2)*2, \Rows()\Width, \Rows()\Height-py-1-Bool(State_3 = 2)*20, \Color\Fore[State_3], \Color\Back[State_3], \Radius, \color\alpha)
        EndIf
        
        ; Draw thumb frame
        If \Color\Frame[State_3] 
          DrawingMode(#PB_2DDrawing_Outlined|#PB_2DDrawing_AlphaBlend)
          RoundBox( scroll_x+\Rows()\X, scroll_y+\Rows()\Y+Bool(State_3 = 2)*2, \Rows()\Width, \Rows()\Height-py-1, \Radius, \Radius, \Color\Frame[State_3]&$FFFFFF|Alpha)
          
          If State_3 = 2
            DrawingMode(#PB_2DDrawing_Outlined|#PB_2DDrawing_AlphaBlend)
            Line( scroll_x+\Rows()\X+1, scroll_y+\Rows()\Y+1+\Rows()\Height-py-1, \Rows()\Width-2, 1, $FFFFFFFF)
          EndIf
        EndIf
        ...
        If \Rows()\Text\String
          DrawingMode(#PB_2DDrawing_Transparent|#PB_2DDrawing_AlphaBlend)
          DrawText(scroll_x+\Rows()\Text\x, scroll_y+\Rows()\Text\y, \Rows()\Text\String.s, \Color\Front[0]&$FFFFFF|Alpha)
        EndIf
  ...
  Procedure.i _Draw_Panel(*This.Widget_S, scroll_x,scroll_y)
    ...
    With *This 
      ...
      ForEach \Rows()
        ...
        \Rows()\Text\x = \Rows()\x+5
        \Rows()\Text\y = \Rows()\y+((\Rows()\height-py-1+Bool(State_3 = 2)*3)-\Rows()\Text\height)/2
        ...
        If \Color\back[State_3]<>-1
          If \Color\Fore[State_3]
            DrawingMode( #PB_2DDrawing_Gradient|#PB_2DDrawing_AlphaBlend)
          EndIf
          BoxGradient( \Vertical, scroll_x+\Rows()\X, scroll_y+\Rows()\Y+Bool(State_3 = 2)*2, \Rows()\Width, \Rows()\Height-py-1, \Color\Fore[State_3], \Color\Back[State_3], \Radius, \color\alpha)
        EndIf
...
;         RoundBox( scroll_x+\X, scroll_y+\Y, \Width, \Height, \Radius, \Radius, \Color\Frame&$FFFFFF|Alpha)
  ...
  Procedure.i SetParent(*This.Widget_S, *Parent.Widget_S, Item.i=-1)
    ...
    With *This
      If *This > 0
        ...
        If Item >= 0 And SelectElement(\p\Rows(), Item)
          \pi = \p\Rows()
        EndIf
  ...
  Procedure.i AddItem(*This.Widget_S, Item.i, Text.s, Image.i=-1, Flag.i=0)
    With *This
      ...
      \Rows()\Focus =- 1
      \Rows()\Text\String = Text.s
      \Rows()\Text\Change = 1
      \Rows()\height = \TabHeight
        ...
        Select EventType
          ...
          Case #PB_EventType_LeftButtonDown
            ...
            ElseIf ListSize(\Rows())
              If \handle[1] 
                ;ChangeCurrentElement(\Rows(), \handle[1])
                ;\index[1] >= 0 And
  ...
  Procedure.i CallBack_Items(*This.Widget_S, Item.i, buttons.i, Control.i)
    ...
    With *This
      If \index[1] <> Item And Item =< ListSize(\Rows())
        If IsList(\index[1], \Rows()) 
        ...
        If buttons And SelectList(Item, \Rows())
        ...
        If buttons
          If (\Flag\MultiSelect And Not Control)
            ...
            ForEach \Rows()
              If Not \Rows()\Hide
                If ((\Index[2] =< \index[1] And \Index[2] =< \Rows()\index And \index[1] >= \Rows()\index) Or
      ...
      If *This > 0 And Not \hide And \color\alpha
        ...
        ElseIf (MouseScreenX>=x+\X And MouseScreenX<x+\X+\Width And MouseScreenY>y+\Y And MouseScreenY=<y+\Y+\Height) 
          ...
          If \at =- 1
            ...
            ; items at point
            ForEach \Rows()
              If (MouseScreenX>\Rows()\X And MouseScreenX=<\Rows()\X+\Rows()\Width And 
                  MouseScreenY>\Rows()\Y And MouseScreenY=<\Rows()\Y+\Rows()\Height)
                
                If \index[1] <> ListIndex(\Rows())
                  \Rows()\Color\State = 1
                  \index[1] = ListIndex(\Rows())
                  \handle[1] = \Rows()
                  repaint=1
                EndIf
                
              ElseIf \Rows()\Color\State = 1
                \Rows()\Color\State = 0
                \index[1] =- 1
                \handle[1] = 0
                repaint=1
  ...
  Procedure.i Bar(Type.i, X.i,Y.i,Width.i,Height.i, Min.i, Max.i, PageLength.i, Flag.i=0, Radius.i=7, SliderLen.i=7)
    ...
    ; Set parent
    If LastElement(*openedlist())
      If LastElement(*openedlist()\Rows())
        SetParent(*This, *openedlist(), ListIndex(*openedlist()\Rows()))
      Else
...
CompilerIf #PB_Compiler_IsMainFile
  ...
  If OpenWindow(3, 0, 0, 665, 605, "Position de la souris sur la fenêtre", #PB_Window_SystemMenu | #PB_Window_ScreenCentered)
    CanvasGadget(100, 0, 0, 665, 605, #PB_Canvas_Keyboard )
    ...
    AddItem(Widgets(Str(#PB_GadgetType_Panel)), -1, "Panel_0"+Str(#PB_GadgetType_Panel)) 
    Widgets(Str(250)) = Button(5, 10, 90,20, "Button_0" )
    ; Widgets(Str(250)) = Splitter(5, 55, 160,50, Button(0, 0, 100,20, "ButtonGadget"), Button(0, 0, 0,20, "StringGadget")) 
    
    AddItem(Widgets(Str(#PB_GadgetType_Panel)), -1, "Panel_1") 
    ...
    ;AddItem(Widgets(Str(#PB_GadgetType_Panel)), -1, "Panel_2") 
