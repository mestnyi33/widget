...
Module DD
  ...
  Procedure Set()
    Protected x,y
    *Drag\img = CreateImage(#PB_Any, 16,16, 32, #PB_Image_Transparent)
    
    If Not *Drag\Type
      If StartDrawing(ImageOutput(*Drag\img))
        If *Drag\Text
      ...
      ResizeImage(*Drag\img, *Drag\Width, *Drag\Height)
    EndIf
    ...
    If StartDrawing(ImageOutput(*Drag\img))
      ...
      EndIf
      If *Drag\ImageID
        DrawAlphaImage(*Drag\ImageID, 0,0, 230)
      ;  DrawAlphaImage(ImageID(d_cursor), 0,0)
...
;         Line(2, 0, 1, 5, $FF000000)
    ...
    SetCursor(EventGadget(), ImageID(*Drag\img), 0,0);x,y)
        ...
        Case #PB_EventType_MouseEnter
          ...
          If DropAction(*this)
            *Drag\widget = *this
            ...
            Protected img = CopyImage(*Drag\img, -1)
...
;               ;               Else
;                               DrawingMode(#PB_2DDrawing_Outlined|#PB_2DDrawing_AlphaBlend)
;                             Box(0, 4, 12, 4, $FFFFFFFF)
;                             Box(4, 0, 4, 12, $FFFFFFFF)
;                             
;                             DrawingMode(#PB_2DDrawing_Default|#PB_2DDrawing_AlphaBlend)
;                             Box(1, 4+1, 10, 2, $FF000000)
;                             Box(4+1, 1, 2, 10, $FF000000)
              ;             EndIf
              ;             
...
;               Box(0, 0, *Drag\Width, *Drag\Height, $FF000000)
              ...
              StopDrawing()
            EndIf  
            SetCursor(EventGadget(), ImageID(img), 0,0);(*Drag\Width)/2, (*Drag\Height)/2)
            
            
          Else
            Debug "set denied cursor"
            ; SetGadgetAttribute(EventGadget(), #PB_Canvas_Cursor, #PB_Cursor_Default)
            SetCursor(EventGadget(), ImageID(*Drag\img), 0,0);, *Drag\Width/2, *Drag\Height/2)
            *Drag\widget = 0
          EndIf
