  ...
  If *this
    ...
    *this\height = height
    ...
    If *this\parent
      ...
      *this\visible = 1;Interrect( 0, 0, *this\parent\width, *this\parent\height, *this\x, *this\y, *this\width, *this\height )
...
Procedure DrawWidget( *this._s_WIDGET, x, y )
  If *this\address
    If *this\visible 
      ;SetOrigin( *this\parent\x,*this\parent\y )
      ;
      DrawFirst( *this, x, y )
    ...
    If StartDrawing( CanvasOutput( *this\root\canvas\gadget ) )
      ...
      ForEach widget( )
        If widget( )\root\canvas\gadget = *this\root\canvas\gadget
          DrawWidget( widget( ), 0,0);widget( )\parent\x,widget( )\parent\y )
        EndIf
      Next
      StopDrawing( )
    EndIf
...
Procedure AddObject( *this._s_WIDGET, type, x,y,width,height, frameSize = 1)
  ...
  If *this
    LastElement( widget( ) )
    result = AddElement( widget( ) )
    If result
      widget( ) = AllocateStructure( _s_WIDGET )
      
      widget( )\fs = frameSize
      widget( )\address = result
      widget( )\color\back = $F3F3F3
      
      SetParentWidget( widget( ), *this )
      ResizeWidget( widget( ), x,y,width,height )
      
      DoWidgetEvents( widget( ), #PB_EventType_Create )
      PostRepaint( widget( ), #PB_EventType_Repaint )
  ...
  If RootWidget( )
    ...
    If eventtype = #PB_EventType_MouseMove Or
      ...
      If RootWidget( )\count\childrens
        ...
        EnteredWidget( ) = GetAtPointWidget( Widget( ), WidgetMouse( )\x, WidgetMouse( )\y )
