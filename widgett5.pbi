...
Structure _s_ROOT Extends _s_WIDGET
  canvas._s_CANVAS
  List *list._s_WIDGET( ) ; Widget( )\ - current element
EndStructure
;--     STRUCT
Structure _s_STRUCT
  Map *canvas._s_ROOT( )   ;
...
;-
Macro Widget( ) 
  RootWidget( )\list( ) ;RootWidget( )\canvas\last
...
Declare SetParentWidget( *this._s_WIDGET, *parent._s_WIDGET )
;}
...
Procedure AddWidget( *this._s_WIDGET, *parent._s_WIDGET )
  If *parent\root
    LastElement( *parent\root\list( ) )
    AddElement( *parent\root\list( ) ) 
    *this\index = ListIndex( *parent\root\list( )) 
    *this\address[1] = @*parent\root\list( ) ;??????????
    *parent\root\list( ) = *this
  EndIf
EndProcedure
...
Procedure AddObject( *this._s_WIDGET, type, x,y,width,height, frameSize = 1)
  ...
  If *this
    ...
    If AddElement( *this\child( ) )
      *this\child( ) = AllocateStructure( _s_WIDGET )
      
      If *this\last\item
        *this\last\item\last\widget = *this\child( )
      EndIf
      *this\child( )\parent\item = *this\last\item
      
      *this\child( )\fs = frameSize
      *this\child( )\color\back = $F3F3F3
      *this\child( )\address = @*this\child( ) ;???????????
      AddWidget( *this\child( ), *this )
    ...
    *parent\count\childrens + 1
    ...
    If *this\parent\widget
      ...
      *this\parent\widget\count\childrens - 1
      ...
      AddElement( *parent\child( ) )
      *parent\child( ) = *this 
      *this\address = @*parent\child( )
      ;       If *this\parent\widget = *this\root
      ;         Debug " add "
;            ; If RootWidget( ) <> *parent\root
;               ChangeCurrentRootWidget( *parent\root\canvas\address )
;            ; EndIf
;       
