...
; Ctrl -  (PageUP & PageDown) прокрутка на один итем верх и вниз коретка остаетс€ на том же итеме
; Shift - (PageUP & PageDown) выделени€ на один итем верх и вниз если доходим на первый или последный выдимий итем то прокручиваем
;         (Left&Right) 
;
;
;
...
;  оличество прочитанных байтов: 1107522
; 18 - Read time
; 261 - add gadget time
; 173 - add widget time
...
CompilerIf #PB_Compiler_IsMainFile
   ...
   ;File$ = OpenFileRequester("¬ыберите файл", GetCurrentDirectory() + "AkelPad.ini", "¬се файлы (*.*)|*.*", 0)
   
   If Asc(File$)
     If ReadFile(0, File$, Format)
       length = Lof(0)                                         ; „итает размер файла в байтах
       FileSeek(0, 0)                                          ; length-100000)                         ; set the file pointer 100000 chars from end of file
       *mem = AllocateMemory(length)                           ; ¬ыдел€ет блок пам€ти с размером файла
       If *mem
         bytes = ReadData(0, *mem, length)                    ; „итает данные из файла и помещает их в блок пам€ти
         Debug " оличество прочитанных байтов: " + Str(bytes)
